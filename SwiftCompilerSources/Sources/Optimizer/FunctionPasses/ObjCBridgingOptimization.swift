...
/// ```
let objCBridgingOptimization = FunctionPass(
  name: "objc-bridging-opt",
  {
...
private func optimizeOptionalBridging(
  forArgumentOf block: BasicBlock,
  ...
  // copy + makeAvailable.
  let replacement = originalObjCValue.copy(
    at: originalObjCValueSwitch,
    andMakeAvailableIn: block,
    context
  )
...
/// Returns true if the pass should continue running.
private func optimizeNonOptionalBridging(
  _ apply: ApplyInst,
  _ context: FunctionPassContext
) -> Bool {
    ...
    let originalObjCValueSwitch = isOptionalBridging(of: se.enumOp, isBridging: isBridgeToSwiftCall)
  {
    ...
    let originalObjCValue = originalObjCValueSwitch.enumOp
    let optionalReplacement = originalObjCValue.copy(
      at: originalObjCValueSwitch,
    ...
    let replacement = builder.createUncheckedEnumData(
      enum: optionalReplacement,
      caseIndex: someCase,
  ...
  let emptySwiftValue = noneBuilder.createApply(
    ...
    bridgeToSwiftCall.substitutionMap,
    arguments: Array(bridgeToSwiftCall.arguments)
  )
...
private func isOptionalBridging(of value: Value, isBridging: (Value) -> ApplyInst?) -> SwitchEnumInst? {
  ...
  for incomingVal in arg.incomingPhiValues {
    ...
    guard let enumInst = incomingVal as? EnumInst,
      ...
      singleEnumUse.instruction is BranchInst
    else {
      return nil
    }
    if let payload = enumInst.payload {
      ...
      if callArgument.type.isEnum {
        guard let sourceEnum = callArgument as? EnumInst,
          ...
          sourceEnum.type == se.enumOp.type
        else {
          return nil
        }
        someSwitch = se
      ...
      guard let singlePred = enumInst.parentBlock.singlePredecessor,
        ...
        se.getUniqueSuccessor(forCaseIndex: enumInst.caseIndex) === enumInst.parentBlock
      else {
        return nil
      }
      noneSwitch = se
...
func isBridgeToSwiftCall(_ value: Value) -> ApplyInst? {
  ...
  guard
    bridgingFunc.hasSemanticsAttribute("bridgeFromObjectiveC")
      ...
      || funcName == "$sSa10FoundationE36_unconditionallyBridgeFromObjectiveCySayxGSo7NSArrayCSgFZ"
  else {
    return nil
  }
  guard bridgingCall.arguments.count == 2,
    bridgingCall.getArgumentConvention(calleeArgIndex: 0) == .directGuaranteed
  else {
    return nil
  }
  return bridgingCall
