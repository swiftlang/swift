...
private struct OverflowCheckRemover {
  ...
  mutating func run() {
    ...
    for block in reversePostOrder {
      for inst in block.instructions {
        switch inst {
        case let condBranch as CondBranchInst:
          registerBranchFormula(condBranch)
        case let condFail as CondFailInst:
  ...
  private mutating func registerCondFailFormula(_ condFail: CondFailInst) {
    ...
    // e.g. `cond_fail %cmp_ult(x, y)` proves `x >= y` afterwards. The relation only holds in blocks
    // dominated by the `cond_fail`, not by the (possibly earlier) comparison.
    if let comparison = condFail.condition as? BuiltinInst {
      addComparisonRelation(comparison, trueBlock: nil, falseBlock: condFail.parentBlock)
    }
  }
  ...
  private mutating func registerBranchFormula(_ condBranch: CondBranchInst) {
    ...
    addComparisonRelation(comparison, trueBlock: condBranch.trueBlock,
                          falseBlock: condBranch.falseBlock)
  }
