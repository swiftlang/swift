...
The module may set the following variables if `swiftDarwin_DIR` is not set and targeting the swiftDarwin overlay.

 ``swiftDarwin_DIR_FOUND``
   true if the overlay was found

 ``swiftDarwin_INCLUDE_DIR``
   the directory containing the corresponding overlay swift module folder

The module may set the following variables if `swiftGlibc_DIR` is not set and targeting the GlibC overlay.

 ``swiftGlibc_DIR_FOUND``
   true if the overlay was found

 ``swiftGlibc_INCLUDE_DIR``
   the directory containing the corresponding overlay swift module folder

The module may set the following variables if `swiftWinSDK_DIR` is not set and targeting the WinSDK overlay.

 ``swiftWinSDK_DIR_FOUND``
   true if the overlay was found

 ``swiftWinSDK_INCLUDE_DIR``
   the directory containing the corresponding overlay swift module folder

The module may set the following variables if `swiftCRT_DIR` is not set and targeting the CRT overlay.

 ``swiftCRT_DIR_FOUND``
   true if the overlay was found

 ``swiftCRT_INCLUDE_DIR``
   the directory containing the corresponding overlay swift module folder

The module may set the following variables if `swiftAndroid_DIR` is not set and targeting the Android overlay.

 ``swiftAndroid_DIR_FOUND``
   true if the overlay was found

 ``swiftAndroid_INCLUDE_DIR``
   the directory containing the corresponding overlay swift module folder
...
elseif(WIN32)
  ...
  list(APPEND swiftCRT_MODULE_NAME "CRT.swiftmodule")
elseif(ANDROID)
  ...
  list(APPEND swiftAndroid_MODULE_NAME "Android.swiftmodule")
else()
