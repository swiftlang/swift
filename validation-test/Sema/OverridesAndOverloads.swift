...
#if ERRORS
  // FIXME: https://github.com/apple/swift/issues/43348
  Derived().foo() as P1 // expected-error {{ambiguous use of 'foo()'; cannot select between potential result types 'any P1', 'P1ImplS1'}}
...
#if ERRORS
  Base().foo(P1ImplC1x())
  // expected-error @-1 {{ambiguous use of 'foo'; cannot select between potential parameter types '(any P1)', '(C1)'}}
#endif
}
