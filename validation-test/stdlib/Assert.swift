...
func testTrapsAreNoreturn(i: Int) -> Int {
  ...
  case 5:
    _invariantFailure("cannot happen")
...
Assert.test("_invariant")
  ...
  .code {
  var x = 2
  _invariant(x * 21 == 42, "should not fail")
  expectCrashLater()
  _invariant(x == 42, "this should fail")
}

Assert.test("_invariantFailure")
  .skip(.custom(
    { !_isStdlibInternalChecksEnabled() },
    reason: "optimizer assumes that the code path is unreachable"))
  .crashOutputMatches("this should fail")
  .code {
  expectCrashLater()
  _invariantFailure("this should fail")
}
