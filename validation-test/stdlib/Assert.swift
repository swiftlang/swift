...
func testTrapsAreNoreturn(i: Int) -> Int {
  ...
  case 5:
    _correctnessCheckFailure("cannot happen")
...
Assert.test("_correctnessCheck")
  ...
  .code {
  var x = 2
  _correctnessCheck(x * 21 == 42, "should not fail")
  expectCrashLater()
  _correctnessCheck(x == 42, "this should fail")
}

Assert.test("_correctnessCheckFailure")
  .skip(.custom(
    { !_isStdlibInternalChecksEnabled() },
    reason: "optimizer assumes that the code path is unreachable"))
  .crashOutputMatches("this should fail")
  .code {
  expectCrashLater()
  _correctnessCheckFailure("this should fail")
}
