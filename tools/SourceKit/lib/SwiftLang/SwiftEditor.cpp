...
void EditorDiagConsumer::handleDiagnostic(
  ...
  if (BufferIDOpt && !isInputBufferID(*BufferIDOpt)) {
    if (Info.ID == diag::error_from_clang.ID ||
        ...
        Info.ID == diag::note_from_clang.ID || !IsNote) {
  ...
  auto Pos = std::lower_bound(
      Diagnostics.begin(), Diagnostics.end(), SKInfo.Offset,
...
void mergeSplitRanges(unsigned Off1, unsigned Len1, unsigned Off2,
                      unsigned Len2,
...
template <class Iter> struct TokenMismatch {
...
struct SwiftSyntaxMap {
  ...
  bool forEachChanged(const SwiftSyntaxMap &Prev,
    ...
    TokenMismatch<ForwardIt> Forward(Tokens.begin(), Tokens.end(),
                                     Prev.Tokens.begin(), Prev.Tokens.end());
    ...
    TokenMismatch<ReverseIt> Backward(Tokens.rbegin(), Tokens.rend(),
                                      Prev.Tokens.rbegin(), Prev.Tokens.rend());
    assert(Backward.foundMismatch());

    // Set or extend the affected range to include the  mismatched range
    SwiftEditorCharRange MismatchRange = {Forward.mismatchStart(),
                                          Backward.mismatchEnd()};
    if (!Affected) {
    ...
    while (To != Tokens.rbegin() &&
           (To - 1)->endOffset() <= Affected->EndOffset)
      --To;
...
struct SwiftSemanticToken {
  ...
  SwiftSemanticToken(CodeCompletionDeclKind Kind, unsigned ByteOffset,
                     unsigned Length, bool IsRef, bool IsSystem)
...
public:
  ...
  SwiftInvocationRef getInvocation() const { return InvokRef; }
...
private:
  ...
  takeSemanticTokens(ImmutableTextSnapshotRef NewSnapshot);
...
public:
  SwiftDocumentSyntaxInfo(const CompilerInvocation &CompInv,
                          ImmutableTextSnapshotRef Snapshot,
                          std::vector<std::string> &Args, StringRef FilePath)
  ...
  SourceFile &getSourceFile() { return Parser->getSourceFile(); }

  unsigned getBufferID() { return BufferID; }

  const LangOptions &getLangOptions() { return Parser->getLangOptions(); }
...
std::vector<SwiftSemanticToken> SwiftDocumentSemanticInfo::takeSemanticTokens(
    ImmutableTextSnapshotRef NewSnapshot) {
  ...
  TokSnapshot->foreachReplaceUntil(
      NewSnapshot, [&](ReplaceImmutableTextUpdateRef Upd) -> bool {
        if (SemaToks.empty())
          return false;

        auto ReplaceBegin = std::lower_bound(
            SemaToks.begin(), SemaToks.end(), Upd->getByteOffset(),
        ...
        if (Upd->getLength() == 0) {
          ...
          ReplaceEnd = std::upper_bound(
              ReplaceBegin, SemaToks.end(),
        ...
        if (Delta != 0) {
          for (std::vector<SwiftSemanticToken>::iterator I = ReplaceEnd,
                                                         E = SemaToks.end();
               I != E; ++I)
            I->ByteOffset += Delta;
        }
...
SwiftDocumentSemanticInfo::getSemanticDiagnostics(
  ...
  for (const auto &curDE : curSemaDiags) {
    bool existsAsParserDiag =
        std::binary_search(sortedParserDiags.begin(), sortedParserDiags.end(),
  ...
  finalDiags.insert(finalDiags.end(), sortedParserDiags.begin(),
...
void SwiftDocumentSemanticInfo::updateSemanticInfo(
    ...
    std::vector<DiagnosticEntryInfo> Diags, ImmutableTextSnapshotRef Snapshot,
    uint64_t ASTGeneration) {
...
public:
  ...
  void handlePrimaryAST(ASTUnitRef AstUnit) override {
    ...
    if (Generation == SemaInfoRef->getASTGeneration()) {
      ...
      if (DocSnapshot->getStamp() !=
          EditableBuffer->getSnapshot()->getStamp()) {
    ...
    SemaInfoRef->updateSemanticInfo(
        std::move(SemaToks),
        std::move(Consumer.getDiagnosticsForBuffer(BufferID)), DocSnapshot,
        Generation);
...
void SwiftDocumentSemanticInfo::processLatestSnapshotAsync(
  ...
  if (Invok) {
    ...
    auto Consumer =
        std::make_shared<AnnotAndDiagASTConsumer>(EditableBuffer, SemaInfoRef);
  ...
  struct SwiftEditorDocument::Implementation {
    ...
    Implementation(StringRef FilePath, SwiftLangSupport &LangSupport,
      ...
      // cursor_info is made.
      SemanticInfo = new SwiftDocumentSemanticInfo(FilePath, ASTMgr,
                                                   NotificationCtr, fileSystem);
    }
  };
  ...
  static UIdent getAccessLevelUID(AccessLevel Access) {
    ...
    static UIdent AccessFilePrivate(
  ...
  public:
    SwiftDocumentStructureWalker(SourceManager &SrcManager, unsigned BufferID,
                                 EditorConsumer &Consumer)
    ...
    bool walkToSubStructurePre(SyntaxStructureNode Node) override {
      unsigned StartOffset =
          SrcManager.getLocOffsetInBuffer(Node.Range.getStart(), BufferID);
      unsigned EndOffset =
          SrcManager.getLocOffsetInBuffer(Node.Range.getEnd(), BufferID);
      unsigned NameStart;
      unsigned NameEnd;
      if (Node.NameRange.isValid()) {
        NameStart = SrcManager.getLocOffsetInBuffer(Node.NameRange.getStart(),
                                                    BufferID);
        NameEnd =
            SrcManager.getLocOffsetInBuffer(Node.NameRange.getEnd(), BufferID);
      } else {
        NameStart = NameEnd = 0;
      }
      ...
      if (Node.BodyRange.isValid()) {
        BodyOffset = SrcManager.getLocOffsetInBuffer(Node.BodyRange.getStart(),
                                                     BufferID);
        BodyEnd =
            SrcManager.getLocOffsetInBuffer(Node.BodyRange.getEnd(), BufferID);
      } else {
      ...
      if (Node.DocRange.isValid()) {
        DocOffset =
            SrcManager.getLocOffsetInBuffer(Node.DocRange.getStart(), BufferID);
        DocEnd =
            SrcManager.getLocOffsetInBuffer(Node.DocRange.getEnd(), BufferID);
      }
      ...
      for (auto Attr : Node.Attrs) {
        if (auto AttrUID = SwiftLangSupport::getUIDForDeclAttribute(Attr)) {
          ...
          if (AttrRange.isValid()) {
            auto CharRange =
                Lexer::getCharSourceRangeFromSourceRange(SrcManager, AttrRange);
            AttrOffset =
                SrcManager.getLocOffsetInBuffer(CharRange.getStart(), BufferID);
            AttrEnd =
                SrcManager.getLocOffsetInBuffer(CharRange.getEnd(), BufferID);
          }
      ...
      Consumer.beginDocumentSubStructure(
          StartOffset, EndOffset - StartOffset, Kind, AccessLevel,
          SetterAccessLevel, NameStart, NameEnd - NameStart, BodyOffset,
          BodyEnd - BodyOffset, DocOffset, DocEnd - DocOffset, DisplayName,
          TypeName, RuntimeName, SelectorName, InheritedNames, Attrs);
      ...
      for (const auto &Elem : Node.Elements) {
        ...
        UIdent Kind =
            SwiftLangSupport::getUIDForSyntaxStructureElementKind(Elem.Kind);
        unsigned Offset =
            SrcManager.getLocOffsetInBuffer(Elem.Range.getStart(), BufferID);
    ...
    StringRef getObjCSelectorName(const Decl *D, SmallString<64> &Buf) {
      // We only vend the selector name for @IBAction and @IBSegueAction
      // methods.
    ...
    bool walkToNodePre(SyntaxNode Node) override {
      ...
      Consumer.beginDocumentSubStructure(StartOffset, EndOffset - StartOffset,
                                         Kind, UIdent(), UIdent(), 0, 0, 0, 0,
                                         0, 0, StringRef(), StringRef(),
                                         StringRef(), StringRef(), {}, {});
      return true;
    }
  ...
  private:
    ...
    public:
      PlaceholderFinder(SourceLoc PlaceholderLoc, EditorPlaceholderExpr *&Found)
          : PlaceholderLoc(PlaceholderLoc), Found(Found) {}

      std::pair<bool, Expr *> walkToExprPre(Expr *E) override {
        if (isa<EditorPlaceholderExpr>(E) &&
            E->getStartLoc() == PlaceholderLoc) {
    ...
    public:
      ...
      explicit ClosureTypeWalker(SourceManager &SM, ClosureInfo &Info)
          : SM(SM), Info(Info) {}
      ...
      bool walkToTypeReprPre(TypeRepr *T) override {
        if (auto *FTR = dyn_cast<FunctionTypeRepr>(T)) {
          ...
          for (auto &ArgElt : FTR->getArgsTypeRepr()->getElements()) {
            ...
            if (!name.empty()) {
              NR = CharSourceRange(ArgElt.NameLoc, name.getLength());
            }
            SourceLoc SRE =
                Lexer::getLocForEndOfToken(SM, ArgElt.Type->getEndLoc());
    ...
    /// For example, if the CallExpr is enclosed in another expression or
    /// statement such as "outer(inner(<#closure#>))", or "if
    /// inner(<#closure#>)", then trailing closure should not be applied to the
    /// inner call.
    std::pair<Expr *, bool> enclosingCallExprArg(SourceFile &SF, SourceLoc SL) {
      ...
      public:
          ...
          if (SR.isValid() && SM.rangeContainsTokenLoc(SR, TargetLoc)) {
            if (auto closure = dyn_cast<ClosureExpr>(E)) {
              if (closure->hasSingleExpressionBody()) {
                // Treat a single-expression body like a brace statement and
                // reset the enclosing context. Note: when the placeholder is
                // the whole body it is handled specially as wrapped in braces
                // by shouldUseTrailingClosureInTuple().
        ...
        bool walkToStmtPre(Stmt *S) override {
          ...
          if (SR.isValid() && SM.rangeContainsTokenLoc(SR, TargetLoc)) {
            // A statement inside an expression - e.g. `foo({ if ... })` -
            // resets the enclosing context.
            OuterExpr = nullptr;
            EnclosingCallAndArg = {nullptr, nullptr};
  ...
  public:
    ...
    bool scan(SourceFile &SF, unsigned BufID, unsigned Offset, unsigned Length,
              std::function<void(Expr *Args, bool UseTrailingClosure,
                                 bool isWrappedWithBraces, ArrayRef<Param>,
                                 CharSourceRange)>
                  Callback,
      ...
      if (Args && ECE.second) {
        if (isa<ParenExpr>(Args)) {
          ...
          UseTrailingClosure = shouldUseTrailingClosureInTuple(
              TE, PlaceholderStartLoc, isWrappedWithBraces);
        }
      }

      Callback(Args, UseTrailingClosure, isWrappedWithBraces,
               TargetClosureInfo.Params, TargetClosureInfo.ReturnTypeRange);
      return true;
    }
  };
  ...
  SwiftEditorDocument::~SwiftEditorDocument() { delete &Impl; }
  ...
  ImmutableTextSnapshotRef SwiftEditorDocument::replaceText(
    ...
    RefPtr<SwiftDocumentSemanticInfo> SemanticInfo;
    {
      ...
      // multi-line tokens at line boundaries, and ensure all parts of these
      // tokens will be cleared.
      Impl.AffectedRange =
          Impl.SyntaxMap.adjustForReplacement(Offset, Length, Str.size());

      // We need to release `AccessMtx` before calling into the ASTManager,
      // since it may call back to the editor for document state.
    }
    ...
    if (ProvideSemanticInfo) {
      ...
      if (Length != 0 || Buf->getBufferSize() != 0) {
        ...
        // FIXME: we should also update any "interesting" ASTs that depend on
        // this document here, e.g. any ASTs for files visible in an editor.
        // However, because our API conflates this with any file with unsaved
        // changes we do not update all open documents, since there could be too
        // many of them.
      }
    }
  ...
  void SwiftEditorDocument::parse(ImmutableTextSnapshotRef Snapshot,
    ...
    if (Impl.SemanticInfo->getInvocation()) {
      ...
      Lang.getASTManager()->initCompilerInvocation(CompInv, Args, StringRef(),
                                                   Error);
    }
  ...
  void SwiftEditorDocument::readSyntaxInfo(EditorConsumer & Consumer) {
    ...
    if (Consumer.syntaxTreeEnabled()) {
      ...
      if (Consumer.syntaxMapEnabled()) {
        Consumer.handleRequestError("Retrieving both a syntax map and a syntax "
                                    "tree at the same time is "
                                    "not supported. Use the SyntaxClassifier "
                                    "in swiftSyntax to generate "
      ...
      if (Impl.Edited) {
        // We're ansering an edit request. Report all highlighted token ranges
        // not in the previous syntax map to the Consumer and extend the
        // AffectedRange to contain all added/removed token ranges.
        SawChanges =
  ...
  void SwiftEditorDocument::readSemanticInfo(ImmutableTextSnapshotRef Snapshot,
    ...
    // If there's no value returned for diagnostics it means they are
    // out-of-date (based on a different snapshot).
  ...
  void SwiftEditorDocument::applyFormatOptions(OptionsDictionary & FmtOptions) {
    ...
    FmtOptions.valueForOption(KeyIndentSwitchCase,
                              Impl.FormatOptions.IndentSwitchCase);
  }
  ...
  void SwiftEditorDocument::formatText(unsigned Line, unsigned Length,
    ...
    Consumer.recordAffectedLineRange(LineRange.startLine(),
                                     LineRange.lineCount());
  }
  ...
  void SwiftEditorDocument::expandPlaceholder(unsigned Offset, unsigned Length,
    ...
    Scanner.scan(
        SF, BufID, Offset, Length,
        [&](Expr *Args, bool UseTrailingClosure, bool isWrappedWithBraces,
          ...
          llvm::SmallString<128> ExpansionStr;
          {
            ...
            if (UseTrailingClosure) {
              ...
              if (isa<ParenExpr>(Args)) {
                // There appears to be no other parameters in this call, so
                // we'll expand replacement for trailing closure and cover call
                // parens. For example: foo.bar(<#closure#>) turns into foo.bar
                // <#closure#>.
                EffectiveOffset =
                    SM.getLocOffsetInBuffer(Args->getStartLoc(), BufID);
                OS << " ";
              } else {
                ...
                if (Elems.size() == 1) {
                  ...
                  assert(Elems.size() > 1);
                  SourceLoc BeforeLoc = Lexer::getLocForEndOfToken(
                      SM, Elems[Elems.size() - 2]->getEndLoc());
            ...
            for (auto &Param : ClosureParams) {
              ...
              if (Param.NameRange.isValid()) {
                // If we have a parameter name, just output the name as is and
                // skip the type. For example:
  ...
  void verifyIncrementalParse(SwiftEditorDocumentRef EditorDoc,
    ...
    // If the serialized format of the two trees doesn't match incremental
    // parsing we have found an error.
  ...
  void SwiftLangSupport::editorReplaceText(
      StringRef Name, llvm::MemoryBuffer * Buf, unsigned Offset,
      unsigned Length, EditorConsumer &Consumer) {
    bool LogReuseRegions = ::getenv("SOURCEKIT_LOG_INCREMENTAL_REUSE_REGIONS");
    bool ValidateSyntaxTree =
        ::getenv("SOURCEKIT_INCREMENTAL_PARSE_VALIDATION");
  ...
  // EditorFormatText
  //===----------------------------------------------------------------------===//
  void SwiftLangSupport::editorApplyFormatOptions(
      StringRef Name, OptionsDictionary & FmtOptions) {
    auto EditorDoc = EditorDocuments->getByUnresolvedName(Name);
    if (EditorDoc)
  ...
  void SwiftLangSupport::editorExtractTextFromComment(
      StringRef Source, EditorConsumer & Consumer) {
  ...
  //===----------------------------------------------------------------------===//
  void SwiftLangSupport::editorExpandPlaceholder(
      StringRef Name, unsigned Offset, unsigned Length,
      EditorConsumer &Consumer) {
    auto EditorDoc = EditorDocuments->getByUnresolvedName(Name);
    if (!EditorDoc) {
      Consumer.handleRequestError("No associated Editor Document");
