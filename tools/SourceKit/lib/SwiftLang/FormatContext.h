...
// This is basically the opposite of what the SourceManager Can Do.
// Given a buffer and an Offset the SourceManager can tell us the line,column
// Could we leverage that to get us the line number faster
// maybe via a binary search
...
public:
  ...
  std::pair<LineRange, std::string> indent(unsigned LineIndex, FormatContext &FC, StringRef Text) {
    ...
    size_t ExpandedIndent = swift::ide::getExpandedIndentForLine(LineAndColumn.first, FmtOptions, Text);
    auto AddIndentFunc = [&] () {
