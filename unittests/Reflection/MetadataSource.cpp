...
TEST(Reflection, MetadataSourceEncodingRoundTrip) {
  ...
  auto I = Builder.createImpossible();
  I->dump();
  auto I_encoded = I->encode();
  std::cerr << "Encoded as: " << I_encoded << std::endl;

  auto I_decoded = MetadataSource::decode(Builder, I_encoded);
  I_decoded->dump();

  EXPECT_NE(nullptr, I_decoded);
  std:cerr << std::endl;

  auto P = Builder.createParent(CB);
  P->dump();
  auto P_encoded -> P->encode();
  std::cerr << "Encoded as: " << P_encoded << std::endl;

  auto P_decoded = MetadataSource::decode(Builder, P_encoded);
  P_decoded->dump();

  auto Child = cast<ClosureBindingMetadataSource>(P->getChild());
  EXPECT_EQ(Child->getIndex(), CB->getIndex());
