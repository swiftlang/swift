...
TEST(ModuleDepGraph, ChainedNoncascadingDependents) {
  ...
  EXPECT_EQ(simulateLoad(graph, &job0, {{providesNominal, {"a", "b", "c"}}}), LoadResult::AffectsDownstream);
  EXPECT_EQ(simulateLoad(graph, &job1, {{dependsNominal,  {"x", "b"}}, {providesNominal, {"_z"}}}), LoadResult::AffectsDownstream);
  EXPECT_EQ(simulateLoad(graph, &job2, {{dependsNominal,  {"_z"}}}), LoadResult::AffectsDownstream);
