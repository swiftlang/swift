...
class WASIStdlib(cmake_product.CMakeProduct):
    ...
    def _test(self, host_target, target_triple,
              variant='wasi-wasm32', embedded_variant='wasi'):
        ...
        if not os.path.exists(wasmkit_bin_path) or not self.should_test_executable():
            test_target = "check-swift-only_non_executable-%s-custom" % variant
        else:
            test_target = "check-swift-%s-custom" % variant
        ...
        if target_triple == 'wasm32-wasip1-threads':
            ...
            test_targets = [test_target,
                            'check-swift-embedded-%s' % embedded_variant]
...
class WASIP2Stdlib(WASIStdlib):
    def build(self, host_target):
        target_triple = 'wasm32-wasip2'
        wasmstdlibhelpers.build_stdlib(
            args=self.args, toolchain=self.toolchain,
            source_dir=self.source_dir, build_dir=self.build_dir,
            host_target=host_target, short_triple='wasip2-wasm32',
            append_platform_cmake_options=lambda opts:
                self._append_platform_cmake_options(opts, target_triple))

    def _append_platform_cmake_options(self, cmake_options, target_triple):
        # CMake has no wasip2 system name; SWIFT_SDKS=WASIP2 (below) selects the triple/subdir.
        cmake_options.define('CMAKE_SYSTEM_NAME:STRING', 'WASI')
        cmake_options.define('SWIFT_WASIP2_SYSROOT_PATH:STRING',
                             self._wasi_sysroot_path(target_triple))
        cmake_options.define('SWIFT_PRIMARY_VARIANT_SDK:STRING', 'WASIP2')
        cmake_options.define('SWIFT_SDKS:STRING', 'WASIP2')
        # Embedded core stdlib resolves its sysroot via the bare-arch key, so set both
        # it and the module key. Only WASIP2 is configured here, so no wasip1 collision.
        cmake_options.define('SWIFT_SDK_embedded_ARCH_wasm32_PATH:PATH',
                             self._wasi_sysroot_path('wasm32-wasip2'))
        cmake_options.define(
            'SWIFT_SDK_embedded_ARCH_wasm32-unknown-wasip2_PATH:PATH',
            self._wasi_sysroot_path('wasm32-wasip2'))

        lit_test_paths = [
            'IRGen', 'stdlib', 'Concurrency/Runtime', 'embedded', 'AutoDiff',
            'DebugInfo', 'SILOptimizer',
            # TODO(katei): Enable all interpreter tests
            'Interpreter/enum.swift',
        ]
        lit_test_paths = [os.path.join(
            self.build_dir, 'test-wasip2-wasm32', path)
            for path in lit_test_paths]
        cmake_options.define('SWIFT_LIT_TEST_PATHS:STRING',
                             ';'.join(lit_test_paths))
    ...
    def test(self, host_target):
        self._test(host_target, 'wasm32-wasip2',
                   variant='wasip2-wasm32', embedded_variant='wasip2')
