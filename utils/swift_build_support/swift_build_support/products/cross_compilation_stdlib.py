...
class CrossCompilationStdlib(product.Product):
    ...
    def build(self, host_target):
        if not self.args.cross_compilation_target:
            raise ValueError("Cross-compilation target must be specified with --cross-compilation-target")
    ...
    def _get_sysroot(self):
        if not self.args.cross_compilation_sysroot:
            raise ValueError("Cross-compilation sysroot must be specified with --cross-compilation-sysroot")
    ...
    def _build(self, host_target, target_name, target_triple):
        ...
        self._build_stdlib(host_target, target_name, target_triple, llvm_cmake_dir, sysroot)
        toolchain_file = self._create_toolchain_file(target_name, target_triple, sysroot)
        self._build_dispatch(target_name, target_triple, toolchain_file)
        if self.args.build_swift_static_stdlib:
            self._build_dispatch(target_name, target_triple=target_triple, toolchain_file=toolchain_file, static=True)
        self._build_foundation(target_name, target_triple, toolchain_file)
        if self.args.build_swift_static_stdlib:
            self._build_foundation(target_name, target_triple=target_triple, toolchain_file=toolchain_file, static=True)
    ...
    def _build_stdlib(self, host_target, target_name, target_triple, llvm_cmake_dir, sysroot):
        ...
        stdlib.cmake_options.define('CMAKE_SYSTEM_NAME:STRING', self._get_system_name(platform))
        stdlib.cmake_options.define('CMAKE_SYSTEM_PROCESSOR:STRING', self._get_system_processor(arch))
        stdlib.cmake_options.define('CMAKE_SYSROOT:PATH', sysroot)

        target_c_flags=self._target_c_flags()
        # Add -target and --sysroot since libdispatch sub-dependency of Swift doesn't pick up on
        # CMAKE_SYSTEM_PROCESSOR or CMAKE_SYSROOT. This can be fixed in the future I'm sure.
        ...
        stdlib.cmake_options.define('SWIFT_STDLIB_BUILD_TYPE:STRING', self._build_variant)
        ...
        if self.args.build_runtime_with_host_compiler:
            stdlib.cmake_options.define('SWIFT_BUILD_RUNTIME_WITH_HOST_COMPILER:BOOL', "TRUE")
        ...
        stdlib.cmake_options.define(f'SWIFT_SDK_{platform.upper()}_ARCH_{arch.lower()}_PATH',
                                    sysroot)
    
        # Disable overlay that's hardcoded to point to /usr
        if platform == 'linux':
            stdlib.cmake_options.define('SWIFT_SDK_LINUX_CXX_OVERLAY_SWIFT_COMPILE_FLAGS:STRING', '')

        # Library features
        stdlib.cmake_options.define('SWIFT_BUILD_DYNAMIC_STDLIB:BOOL', 'TRUE')
        stdlib.cmake_options.define('SWIFT_BUILD_STATIC_STDLIB:BOOL', 'TRUE' if self.args.build_swift_static_stdlib else 'FALSE')
        stdlib.cmake_options.define('SWIFT_BUILD_STATIC_SDK_OVERLAY:BOOL', 'TRUE')
        stdlib.cmake_options.define('SWIFT_ENABLE_EXPERIMENTAL_CONCURRENCY:BOOL', 'TRUE')
        stdlib.cmake_options.define('SWIFT_ENABLE_EXPERIMENTAL_DISTRIBUTED:BOOL', 'TRUE')
        ...
        stdlib.cmake_options.define('SWIFT_ENABLE_EXPERIMENTAL_OBSERVATION:BOOL', 'TRUE')
        stdlib.cmake_options.define('SWIFT_SHOULD_BUILD_EMBEDDED_STDLIB:BOOL', 'FALSE')

        # Source directories
        source_dir = os.path.dirname(self.source_dir)
        stdlib.cmake_options.define('SWIFT_PATH_TO_LIBDISPATCH_SOURCE:PATH', os.path.join(
            source_dir, 'swift-corelibs-libdispatch'))
        stdlib.cmake_options.define(
            'SWIFT_ENABLE_EXPERIMENTAL_STRING_PROCESSING:BOOL', 'TRUE')
        stdlib.cmake_options.define('SWIFT_PATH_TO_STRING_PROCESSING_SOURCE:PATH',
        ...
        stdlib.build_with_cmake([], self._build_variant, [],
                                prefer_native_toolchain=not self.args.build_runtime_with_host_compiler)

        # Copy swiftrt.o to the sysroot. This is required since -sdk looks for swiftrt.o at
        # <sysroot>/usr/lib/swift/linux/<arch> instead of the -resource-dir, which may be a bug?
        arch_dir = os.path.join(sysroot, 'usr', 'lib', 'swift', platform, arch)
        shell.makedirs(arch_dir)
        shutil.copy(os.path.join(stdlib.build_dir, 'lib', 'swift', platform, arch, 'swiftrt.o'),
                    arch_dir)
    ...
    def _append_platform_cmake_options(self, target_name, target_triple, toolchain_file, cmake_options):
    ...
    def _build_dispatch(self, target_name, target_triple, toolchain_file, static=False):
        dispatch = CMakeProduct(
            ...
            build_dir=self._build_dir(target_name, 'libdispatch-static' if static else 'libdispatch'))
        self._append_platform_cmake_options(target_name, target_triple, toolchain_file, dispatch.cmake_options)
        dispatch.cmake_options.define('BUILD_SHARED_LIBS:BOOL', 'FALSE' if static else 'TRUE')
        dispatch.cmake_options.define('ENABLE_SWIFT:BOOL', 'TRUE')

        dispatch.build_with_cmake([], self.args.build_variant, [],
                                    prefer_native_toolchain=not self.args.build_runtime_with_host_compiler,
    ...
    def _build_foundation(self, target_name, target_triple, toolchain_file, static=False):
        foundation = CMakeProduct(
            args=self.args,
            toolchain=self.toolchain,
            source_dir=os.path.join(
                os.path.dirname(self.source_dir), 'swift-corelibs-foundation'),
            build_dir=self._build_dir(target_name, 'foundation-static' if static else 'foundation'))
        self._append_platform_cmake_options(target_name, target_triple, toolchain_file, foundation.cmake_options)
        foundation.cmake_options.define('BUILD_SHARED_LIBS:BOOL', 'FALSE' if static else 'TRUE')
        foundation.cmake_options.define('FOUNDATION_BUILD_TOOLS', 'FALSE')

        # Build directories
        foundation.cmake_options.define('dispatch_DIR',
            os.path.join(self._build_dir(target_name, 'libdispatch'), 'cmake', 'modules'))
    ...
    def _build_swift_testing(self, target_name, target_triple, toolchain_file):
        ...
        self._append_platform_cmake_options(target_name, target_triple, toolchain_file, swift_testing.cmake_options)
        swift_testing.cmake_options.define('BUILD_SHARED_LIBS', 'TRUE')

        # Build directories
        swift_testing.cmake_options.define('dispatch_DIR',
            os.path.join(self._build_dir(target_name, 'libdispatch'), 'cmake', 'modules'))
        swift_testing.cmake_options.define('Foundation_DIR',
            os.path.join(self._build_dir(target_name, 'foundation'), 'cmake', 'modules'))

        host_toolchain = self._host_toolchain_path()
        swift_testing.cmake_options.define('SwiftTesting_MACRO', 
            os.path.join(host_toolchain, 'lib', 'swift', 'host', 'plugins', 'libTestingMacros.so'))
    ...
    def _build_xctest(self, target_name, target_triple, toolchain_file):
        ...
        self._append_platform_cmake_options(target_name, target_triple, toolchain_file, xctest.cmake_options)
        xctest.cmake_options.define('BUILD_SHARED_LIBS', 'TRUE')
    
        # Build directories
        xctest.cmake_options.define('dispatch_DIR',
            os.path.join(self._build_dir(target_name, 'libdispatch'), 'cmake', 'modules'))
        xctest.cmake_options.define('Foundation_DIR',
            os.path.join(self._build_dir(target_name, 'foundation'), 'cmake', 'modules'))
    ...
    def _create_toolchain_file(self, target_name, target_triple, sysroot):
        platform, arch = target_name.split('-')
        toolchain_file_path = self._build_dir(target_name, 'cross-compilation-toolchain.cmake')
