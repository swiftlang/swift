...
for line in sys.stdin:
    ...
    if match:
        ...
        git_mv_cmd = 'git mv %s %s' % (shlex.quote(from_filename), shlex.quote(to_filename))
        ...
        sed_remove_requires_asserts_cmd = \
            'sed -e "s|^//.*REQUIRES: asserts.*$||g" -i "" %s' % (shlex.quote(to_filename))
        execute_cmd(sed_remove_requires_asserts_cmd)

        # "git add" the result.
        git_add_cmd = 'git add %s' % (shlex.quote(to_filename))
