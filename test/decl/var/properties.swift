...
// Disambiguated as stored property with a trailing closure in the initializer.
var disambiguateImplicitGet1: Int = 0 { // expected-error {{cannot convert the expression's type '$T2' to type 'Int'}}
  return 42
}
...
var x20: X {
  ...
  set {
    y = newValue // expected-error{{'X' is not convertible to 'Y'}}
  }
}
...
var x21: X {
  ...
  set(v) {
    y = v // expected-error{{'X' is not convertible to 'Y'}}
  }
}
...
extension S {
  ...
  func test(inout other_x: X) {
    x = other_x
    x2 = other_x
    x3 = other_x // expected-error{{cannot assign}}
...
@NSCopying  // expected-error {{'NSCopying' attribute may only be used on properties in classes}}
func copyFunction() {}
