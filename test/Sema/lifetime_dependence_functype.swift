...
@_lifetime(copy ne)
func applyCorrectlyAnnotatedTransfer(ne: NE, transfer: @_lifetime(copy 0) (NE) -> NE) -> NE { // OK
  transfer(ne)
}
...
typealias DependOnEscapableType1 = @_lifetime(copy k) (_ k: inout Klass) -> NE // expected-error{{cannot copy the lifetime of an Escapable type, use '@_lifetime(&k)' instead}}

typealias DependOnEscapableType2 = @_lifetime(copy k) (_ k: borrowing Klass) -> NE // expected-error{{cannot copy the lifetime of an Escapable type, use '@_lifetime(borrow k)' instead}}

typealias DependOnEscapableType3 = @_lifetime(copy k) (_ k: consuming Klass) -> NE // expected-error{{invalid lifetime dependence on an Escapable value with consuming ownership}}
