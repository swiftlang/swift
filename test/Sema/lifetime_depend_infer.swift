...
struct NonEscapableSelf: ~Escapable {
  ...
  // expected-error@-1{{a mutating method with a ~Escapable 'self' requires '@_lifetime(self: ...)'}}
  // expected-error@-2{{a mutating method with a ~Escapable 'inout' parameter requires '@_lifetime(a: ...)'}}
  // expected-note@-3{{use '@_lifetime(a: copy a) to forward the inout dependency}}
}
...
/* DEFAULT: @_lifetime(ne: copy ne) */
@_lifetime(&ne)
func inoutNEParam_NEResult(ne: inout NE) -> NE { ne }
...
extension EscapableNonTrivialSelf {
  ...
  /* DEFAULT: @_lifetime(ne: copy ne) */
  mutating func mutating_inoutNEParam_void(ne: inout NE) { NEImmortal() }

  /* DEFAULT: @_lifetime(ne: copy NE) */
  @_lifetime(&self)
  func inoutNEParam_NEResult(ne: inout NE) -> NE { NEImmortal() }
}
...
struct NonEscapableMutableSelf: ~Escapable {
  ...
  mutating func mutating_noParam_NEResult_noLifetime() -> NE { NEImmortal() } // expected-expected{{!!!}}

  /* DEFAULT: @_lifetime(self: copy Self) */
  @_lifetime(&self)
  mutating func mutating_noParam_NEResult() -> NE { NEImmortal() }

  /* DEFAULT: @_lifetime(self: copy Self) */
  /* DEFAULT: @_lifetime(ne: copy NE) */
  @_lifetime(&self)
  mutating func mutating_inoutNEParam_NEResult(ne: inout NE) -> NE { NEImmortal() }
}
