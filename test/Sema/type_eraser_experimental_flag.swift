// RUN: %target-swift-frontend -typecheck -target %target-swift-5.1-abi-triple -dump-ast -enable-experimental-opaque-type-erasure %s | %FileCheck %s
...
// RUN: %target-swift-emit-module-interface(%t/test1/erasure.swiftinterface) %s -module-name erasure -enable-experimental-opaque-type-erasure -enable-library-evolution -target %target-swift-5.1-abi-triple
...
// RUN: %target-swift-frontend -target %target-swift-5.1-abi-triple -I %t/test1/ -emit-sil %S/Inputs/import_with_opaque_type_erasure.swift | %FileCheck %s --check-prefix CHECK-UNDERLYING-TYPE
...
// RUN: %target-swift-emit-module-interface(%t/test2/erasure.swiftinterface) %s -module-name erasure -enable-experimental-feature OpaqueTypeErasure -enable-library-evolution -target %target-swift-5.1-abi-triple
...
// RUN: %target-swift-frontend -target %target-swift-5.1-abi-triple -I %t/test2/ -emit-sil %S/Inputs/import_with_opaque_type_erasure.swift | %FileCheck %s --check-prefix CHECK-UNDERLYING-TYPE2
