...
// expected-error@-1 {{variable must be declared private or fileprivate because its type 'Generic<Int>.Dependent' (aka 'Int') uses a fileprivate type}}
...
public var failFn: (ActuallyPrivate) -> Void = { _ in }
public var failFn2: (_ x: ActuallyPrivate) -> Void = { _ in }
public var failFn3: (main.ActuallyPrivate) -> Void = { _ in }
public var failFn4: (_ x: main.ActuallyPrivate) -> Void = { _ in }
public var failNested1: (_ x: (ActuallyPrivate) -> Void) -> Void = { _ in }
public var failNested2: (_ x: (main.ActuallyPrivate) -> Void) -> Void = { _ in }
// expected-error@-6 {{cannot be declared public}}
// expected-error@-6 {{cannot be declared public}}
// expected-error@-6 {{cannot be declared public}}
// expected-error@-6 {{cannot be declared public}}
// expected-error@-6 {{cannot be declared public}}
