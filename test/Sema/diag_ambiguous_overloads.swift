...
DefaultValue.foo(1.0, 1)
// expected-error@-1 {{no exact matches in call to static method 'foo'}}
...
do {
  ...
  f2(f1(1 as Double))
  // expected-error@-1 {{no exact matches in call to local function 'f1'}}
  // expected-error@-2 {{ambiguous use of 'f1'}}
  // expected-note@-3 {{result type 'String' of 'f1' does not conform to 'Int'}}
  // expected-note@-4 {{result type 'Double' of 'f1' does not conform to 'Int'}}
...
do {
  ...
  // expected-note@-2 {{result type 'Int' of 'test()' does not conform to 'Int32'}}
  // expected-note@-3 {{result type 'String' of 'test()' does not conform to 'Int32'}}
}
...
do {
  ...
  // expected-note@-1 {{candidate expects value of type 'Int' for parameter #1 (got 'Double')}}
  func test(_: String) {}
  // expected-note@-1 {{candidate expects value of type 'String' for parameter #1 (got 'Double')}}
