...
// RUN: %swift -emit-module -target x86_64-apple-darwin13 -o %t %S/Inputs/foo_swift_module.swift
//
// This file should not have any syntax or type checker errors.
// RUN: %swift -parse -I %t %clang-importer-sdk -F %S/Inputs/mock-sdk -target x86_64-apple-darwin13 -module-cache-path %t/clang-module-cache %s
//
// RUN: %swift-ide-test -print-ast-typechecked -source-filename %s -I %t %clang-importer-sdk -target x86_64-apple-darwin13 -F %S/Inputs/mock-sdk -module-cache-path %t/clang-module-cache -function-definitions=false -prefer-type-repr=false -print-implicit-attrs=true > %t.printed.txt
...
//
// RUN: %swift-ide-test -print-ast-typechecked -source-filename %s -I %t %clang-importer-sdk -target x86_64-apple-darwin13 -F %S/Inputs/mock-sdk -module-cache-path %t/clang-module-cache -function-definitions=false -prefer-type-repr=true -print-implicit-attrs=true > %t.printed.txt
// RUN: FileCheck %s -check-prefix=PASS_COMMON -strict-whitespace < %t.printed.txt
// RUN: FileCheck %s -check-prefix=PASS_PRINT_AST -strict-whitespace < %t.printed.txt
// RUN: FileCheck %s -check-prefix=PASS_RW_PROP_GET_SET -strict-whitespace < %t.printed.txt
// RUN: FileCheck %s -check-prefix=PASS_2200 -strict-whitespace < %t.printed.txt
...
// RUN: FileCheck %s -check-prefix=PASS_QUAL_UNQUAL -strict-whitespace < %t.printed.txt
//
// RUN: %swift -emit-module -o %t -I %t %clang-importer-sdk -target x86_64-apple-darwin13 -F %S/Inputs/mock-sdk %s
// RUN: %swift-ide-test -print-module -source-filename %s %clang-importer-sdk -target x86_64-apple-darwin13 -F %S/Inputs/mock-sdk -module-cache-path %t/clang-module-cache -I %t -module-to-print=print_ast_tc_decls -print-implicit-attrs=true > %t.printed.txt
// RUN: FileCheck %s -check-prefix=PASS_COMMON -strict-whitespace < %t.printed.txt
...
//
// RUN: %swift-ide-test -print-module -source-filename %s %clang-importer-sdk -target x86_64-apple-darwin13 -F %S/Inputs/mock-sdk -module-cache-path %t/clang-module-cache -I %t -module-to-print=print_ast_tc_decls -synthesize-sugar-on-types=true -print-implicit-attrs=true > %t.printed.txt
// RUN: FileCheck %s -check-prefix=PASS_PRINT_MODULE_INTERFACE -strict-whitespace < %t.printed.txt
// RUN: FileCheck %s -check-prefix=PASS_QUAL_UNQUAL -strict-whitespace < %t.printed.txt
// RUN: FileCheck %s -check-prefix=SYNTHESIZE_SUGAR_ON_TYPES -strict-whitespace < %t.printed.txt
// RUN: FileCheck %s -check-prefix=PASS_EXPLODE_PATTERN -strict-whitespace < %t.printed.txt

// RUN: %swift-ide-test -print-module -source-filename %s %clang-importer-sdk -target x86_64-apple-darwin13 -F %S/Inputs/mock-sdk -module-cache-path %t/clang-module-cache -I %t -module-to-print=print_ast_tc_decls -synthesize-sugar-on-types=true -fully-qualified-types-if-ambiguous=true -print-implicit-attrs=true > %t.printed.txt
// RUN: FileCheck %s -check-prefix=PASS_PRINT_MODULE_INTERFACE -strict-whitespace < %t.printed.txt
...
// PASS_EXPLODE_PATTERN-NEXT: {{^}}  @objc @IBOutlet var anOutlet: d0181_TestIBAttrs!{{$}}
