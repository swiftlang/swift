...
// RUN: %swift-ide-test -print-module -source-filename %s -sdk %sdk -module-to-print=Foundation.NSArray -function-definitions=false -module-cache-path=%t/mcp > %t/Foundation.NSArray.printed.txt
// RUN: FileCheck -input-file %t/Foundation.NSArray.printed.txt -check-prefix=CHECK1 %s

// RUN: %swift-ide-test -print-module -source-filename %s -sdk %sdk -module-to-print=Foundation.NSArray -function-definitions=false -module-cache-path=%t/mcp > %t/Foundation.NSArray-failable-inits.printed.txt
...
// RUN: %swift-ide-test -print-module -source-filename %s -sdk %sdk -module-to-print=Foundation.NSKeyValueCoding -function-definitions=false -print-regular-comments -module-cache-path=%t/mcp > %t/Foundation.NSKeyValueCoding.printed.txt
// RUN: FileCheck -input-file %t/Foundation.NSKeyValueCoding.printed.txt -check-prefix=CHECK2 %s

// CHECK2: extension NSObject

// RUN: %swift-ide-test -print-module -source-filename %s -sdk %sdk -module-to-print=Foundation.NSString -function-definitions=false -module-cache-path=%t/mcp > %t/Foundation.NSString.printed.txt
