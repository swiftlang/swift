...
// RUN: %swift-ide-test -sdk %sdk -target %target-triple -code-completion -source-filename %s -code-completion-token=PLAIN_TOP_LEVEL_1 > %t.toplevel.txt
// RUN: FileCheck %s -check-prefix=PLAIN_TOP_LEVEL < %t.toplevel.txt
// RUN: FileCheck %s -check-prefix=NO_STDLIB_PRIVATE < %t.toplevel.txt

// RUN: %swift-ide-test -sdk %sdk -target %target-triple -code-completion -source-filename %s -code-completion-token=PRIVATE_NOMINAL_MEMBERS_1 > %t.members.txt
...
// PLAIN_TOP_LEVEL-DAG: Decl[GlobalVar]/OtherModule: NSJapaneseEUCStringEncoding[#UInt#]{{$}}
