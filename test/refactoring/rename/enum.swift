...
// RUN: diff -u %S/Outputs/enum/first.swift.expected %t.result/first_ref_base.swift
...
// RUN: %refactor -rename -source-filename %s -pos=7:21 -new-name 'newParamName' > %t.result/third_ref.swift
// RUN: diff -u %S/Outputs/enum/third.swift.expected %t.result/third_ref.swift
