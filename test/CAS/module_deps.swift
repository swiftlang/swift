...
/// make sure the number of CASFS is correct. 9 entries with 2 line each + 1 extra bracket
// RUN: NUM_CMDS=$(%S/Inputs/SwiftDepsExtractor.py %t/deps.json deps commandLine | wc -l)
// RUN: if [ ! $NUM_CMDS -eq 19 ]; then echo "wrong number of CASFS from scanning"; exit 1; fi
