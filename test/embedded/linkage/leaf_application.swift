...
// SIL checking
// RUN: %target-swift-frontend %t/Library.swift -parse-as-library -entry-point-function-name Library_main -enable-experimental-feature EmbeddedLinkageModel -enable-experimental-feature Embedded -enable-experimental-feature IntermediateLibrary -emit-sil -emit-module-path %t/Modules/Library.swiftmodule -o - | %FileCheck -check-prefix LIBRARY-SIL %s

// IR checking to ensure we get the right weak symbols.
// RUN: %target-swift-frontend %t/Library.swift -parse-as-library -entry-point-function-name Library_main -enable-experimental-feature EmbeddedLinkageModel -enable-experimental-feature Embedded -enable-experimental-feature IntermediateLibrary -emit-ir -o - | %FileCheck -check-prefix LIBRARY-IR --dump-input-filter all %s
...
// LIBRARY-IR: @"$es23_swiftEmptyArrayStorageSi_S3itvp" = linkonce_odr {{(protected |dllexport )?}}global
