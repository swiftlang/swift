...
something = 42 // expected-error {{'@lvalue $T2' is not identical to '(LocalType) -> LocalType'}}
...
scopedFunction = 42 // expected-error {{'@lvalue $T2' is not identical to '(Bool) -> Bool'}}

// FIXME: Should be an error -- a type name and a function cannot overload.
var _ : Int = TypeNameWins(42)

TypeNameWins = 42 // expected-error {{'@lvalue $T2' is not identical to '(Int) -> Int'}}
