...
// RUN: %swifti_driver -driver-print-jobs -sdk %S/Inputs/nonexistent-sdk 2>&1 | FileCheck %s --check-prefix=SDKWARNING
...
// RUN: env PATH=%t/usr/bin %swifti_driver_plain -### %s | FileCheck -check-prefix=NOSDK %s
...
// RUN: env PATH=%t/usr/bin %swifti_driver_plain -### %s | FileCheck -check-prefix=XCRUN-SDK %s
...
// RUN: env PATH=%t/usr/bin %swifti_driver_plain -### %s | FileCheck -check-prefix=ROOT-SDK %s
