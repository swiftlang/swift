...
// RUN: %swiftc_driver -driver-print-bindings %t/source.swift -incremental -output-file-map %t.json 2>&1 | FileCheck %s -check-prefix=MUST-EXEC

// RUN: touch -t 201401240006 %t/source.o
// RUN: %swiftc_driver -driver-print-bindings %t/source.swift -incremental -output-file-map %t.json 2>&1 | FileCheck %s -check-prefix=MAY-EXEC
