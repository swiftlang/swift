...
/*
 ...
 This file (together with defer_and_silgen) tests all four of those sends, ensuring that when silgen is not
 ...
 Except for the expected diagnostics and the RUN arguments, this file and defer_and_silgen.swift are identical.
...
func callActorFuncsFromNonisolated(a : A, ns : NonSendable) async {
    ...
    //deferred warning @-1{{non-sendable type 'NonSendable' passed in call to actor-isolated instance method 'actorTakesNS' cannot cross actor boundary}}

    _ = await a.actorRetsNS()
    //deferred warning @-1{{on-sendable type 'NonSendable' returned by call to actor-isolated instance method 'actorRetsNS()' cannot cross actor boundary}}
}
...
func readActorState(a : A) {
    ...
    print(a.actorNS);
    //deferred warning @-1{{non-sendable type 'NonSendable' in asynchronous access to actor-isolated property 'actorNS' cannot cross actor boundary}}
}
...
@available(SwiftStdlib 5.1, *)
    ...
    func callNonisolatedFuncsFromActor(ns: NonSendable) async {
        ...
        await takesNS(ns)
        //deferred warning @-1{{non-sendable type 'NonSendable' exiting actor-isolated context in call to non-isolated global function 'takesNS' cannot cross actor boundary}}

        _ = await retsNS()
        //deferred warning @-1{{non-sendable type 'NonSendable' returned by call from actor-isolated context to non-isolated global function 'retsNS()' cannot cross actor boundary}}
    }
    ...
    func callActorFuncsFromDiffActor(ns : NonSendable, a : A) async {
        ...
        _ = await a.actorRetsNS()
        //deferred warning @-1{{non-sendable type 'NonSendable' returned by call to actor-isolated instance method 'actorRetsNS()' cannot cross actor boundary}}
    }
    ...
    func captureNonSendableInReturnedClosure(ns : NonSendable) -> @Sendable () -> () {
        ...
        return {print(ns)}
        //deferred warning @-1{{capture of 'ns' with non-sendable type 'NonSendable' in a `@Sendable` closure}}
    }
    ...
    func usesGlobalFromActor() {
        ...
        print(globalNS)
        //deferred warning @-1{{non-sendable type 'NonSendable' in asynchronous access to main actor-isolated let 'globalNS' cannot cross actor boundary}}
    }
}
