...
do {
  struct S {
    ...
    // expected-warning@+2:30 {{feature 'AsyncCallerExecution' will cause nonisolated async getter for property 'executionGetAsyncS' to run on the caller's actor; use @execution(concurrent) to preserve behavior}}{{-1:5-5=@execution(concurrent) }}{{none}}
    var executionGetAsyncS: Int {
      @execution(concurrent) get async {}
    }
    // expected-warning@+2:30 {{feature 'AsyncCallerExecution' will cause nonisolated async getter for subscript 'subscript' to run on the caller's actor; use @execution(concurrent) to preserve behavior}}{{-1:5-5=@execution(concurrent) }}{{none}}
    subscript(executionGetAsyncS _: Int) -> Int {
      @execution(concurrent) get async {}
    }
  }
