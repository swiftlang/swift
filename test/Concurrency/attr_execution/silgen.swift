// RUN: %target-swift-emit-silgen %s -module-name attr_execution_silgen -target %target-swift-5.1-abi-triple -enable-experimental-feature ExecutionAttribute | %FileCheck %s
...
// CHECK:   [[Z:%.*]] = move_value [lexical] [var_decl] [[TTFI]]
...
// CHECK:   [[TTFI:%.*]] = thin_to_thick_function [[GLOBAL_CALLER_FUNC]]
// CHECK:   [[THUNK:%.*]] = function_ref @$sScA_pSgIegHg_IegH_TRScMTU : $@convention(thin) @async (@guaranteed @async @callee_guaranteed (@sil_isolated @sil_implicit_leading_param @guaranteed Optional<any Actor>) -> ()) -> ()
// CHECK:   [[PA:%.*]] = partial_apply [callee_guaranteed] [[THUNK]]([[TTFI]]) : $@convention(thin) @async (@guaranteed @async @callee_guaranteed (@sil_isolated @sil_implicit_leading_param @guaranteed Optional<any Actor>) -> ()) -> ()
...
// CHECK:   apply [[V1_B_C_B]]() : $@async @callee_guaranteed () -> ()
...
// CHECK:   [[V2:%.*]] = move_value [lexical] [var_decl] [[TTFI]]
...
// CHECK:   [[V3:%.*]] = move_value [lexical] [var_decl] [[X_C]]
// CHECK:   [[V3_B:%.*]] = begin_borrow [[V3]]
// CHECK:   [[V3_B_C:%.*]] = copy_value [[V3_B]]
// CHECK:   [[V3_B_C_B:%.*]] = begin_borrow [[V3_B_C]]
// CHECK:   apply [[V3_B_C_B]]()

// CHECK:   [[Y_C:%.*]] = copy_value [[Y]]
// CHECK:   [[THUNK:%.*]] = function_ref @$sScA_pSgIegHg_IegH_TRScMTU : $@convention(thin) @async (@guaranteed @async @callee_guaranteed (@sil_isolated @sil_implicit_leading_param @guaranteed Optional<any Actor>) -> ()) -> ()
// CHECK:   [[PA:%.*]] = partial_apply [callee_guaranteed] [[THUNK]]([[Y_C]])
// CHECK:   [[V4:%.*]] = move_value [lexical] [var_decl] [[PA]]
// CHECK:   [[V4_B:%.*]] = begin_borrow [[V4]]
// CHECK:   [[V4_B_C:%.*]] = copy_value [[V4_B]]
// CHECK:   [[V4_B_C_B:%.*]] = begin_borrow [[V4_B_C]]
// CHECK:   apply [[V4_B_C_B]]()
...
// CHECK:   [[TTFI:%.*]] = thin_to_thick_function [[GLOBAL_CALLER_FUNC]]
// CHECK:   [[THUNK:%.*]] = function_ref @$sScA_pSg21attr_execution_silgen13SendableKlassCIegHgg_ADIegHg_TRScMTU : $@convention(thin) @async (@guaranteed SendableKlass, @guaranteed @async @callee_guaranteed (@sil_isolated @sil_implicit_leading_param @guaranteed Optional<any Actor>, @guaranteed SendableKlass) -> ()) -> ()
// CHECK:   [[PA:%.*]] = partial_apply [callee_guaranteed] [[THUNK]]([[TTFI]])
// CHECK:   [[V1:%.*]] = move_value [lexical] [var_decl] [[PA]]
// CHECK:   [[V1_B:%.*]] = begin_borrow [[V1]]
// CHECK:   [[V1_B_C:%.*]] = copy_value [[V1_B]]
// CHECK:   [[V1_B_C_B:%.*]] = begin_borrow [[V1_B_C]]
// CHECK:   apply [[V1_B_C_B]]({{%.*}}) : $@async @callee_guaranteed (@guaranteed SendableKlass) -> ()
