...
// We should only squelch ns2. All other elements in the ns region should result
// in errors.
func transferLetNonTransferrableSquelchedAddressOnly<T>(_ ns: T) async {
  nonisolated(unsafe) let ns2 = ns
  let ns3 = ns2
  let ns4 = ns
...
func useAfterTransferLetSquelchedIndirectAddressOnly<T : ProvidesStaticValue>(_ meta: T.Type) async {
  let ns = T.value
  nonisolated(unsafe) let ns2 = ns
  let ns3 = ns2
  let ns4 = ns
...
class NonIsolatedUnsafeFieldGenericKlass<T> {
