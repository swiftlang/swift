...
// RUN: %target-swift-frontend -target %target-swift-5.1-abi-triple -strict-concurrency=minimal -I %t %s -verify -verify-ignore-unrelated -emit-sil -o /dev/null
// RUN: %target-swift-frontend -target %target-swift-5.1-abi-triple -strict-concurrency=targeted -verify-additional-prefix targeted-complete- -I %t %s -verify -verify-ignore-unrelated -emit-sil -o /dev/null

// RUN: %target-swift-frontend -target %target-swift-5.1-abi-triple -strict-concurrency=complete -verify-additional-prefix targeted-complete- -verify-additional-prefix complete- -I %t %s -verify -verify-ignore-unrelated -emit-sil -o /dev/null -verify-additional-prefix tns-
