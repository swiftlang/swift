...
// RUN: %target-swift-frontend -target %target-swift-5.1-abi-triple -emit-module -swift-version 5 -emit-module-path %t/SerializedStruct.swiftmodule -module-name SerializedStruct %S/Inputs/SerializedStruct.swift

// RUN: %target-swift-frontend -target %target-swift-5.1-abi-triple %s -emit-sil -o /dev/null -verify -verify-ignore-unrelated -swift-version 6 -I %t
