...
// synchronous-start marker rather than inheriting an executor.
func immediateFromNonisolatedCaller(
  order: Recorder, naming executor: UnrelatedExecutor
) async -> Int {
...
@main struct Main {
  static func main() async {
    ...
    tests.test("immediate task from a nonisolated caller enqueues when the offer isn't the default") {
      let order = Recorder()
      let value = await immediateFromNonisolatedCaller(
        order: order, naming: UnrelatedExecutor())
      expectEqual(20, value)
