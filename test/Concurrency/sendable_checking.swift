...
struct DowngradeForPreconcurrency {
  func capture(completion: @escaping @MainActor () -> Void) {
    preconcurrencyContext {
        Task { // expected-ni-warning {{passing closure as a 'sending' parameter risks causing data races between code in the current task and concurrent execution of the closure}}
            completion() // expected-ni-note {{closure captures 'completion' which is accessible to code in the current task}}
