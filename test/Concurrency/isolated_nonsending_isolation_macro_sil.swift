...
// RUN: %target-swift-frontend -parse-as-library -emit-silgen %s | %FileCheck -check-prefix=CHECK -check-prefix=TAG %s
...
// CHECK-LABEL: sil hidden [ossa] @$s39isolated_nonsending_isolation_macro_sil46nonisolatedNonsendingUsePoundIsolationDirectlyyyYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor) -> () {
// CHECK: bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitIsolationActor):
//
// #isolation without default arg
// CHECK:   hop_to_executor [[ACTOR]]
// CHECK:   [[CAST_TUPLE:%.*]] = unchecked_value_cast [[ACTOR]] to $(Builtin.Word, Builtin.Word)
...
// CHECK:   [[CAST_BACK:%.*]] = unchecked_value_cast [[REFORMED_TUPLE]] to $Optional<any Actor>
// CHECK:   [[MARK_DEP:%.*]] = mark_dependence [nonescaping] [[CAST_BACK]] on [[ACTOR]]
// CHECK:   [[ISO:%.*]] = move_value [lexical] [var_decl] [[MARK_DEP]]
...
//
// CHECK-LABEL: sil hidden [ossa] @$s39isolated_nonsending_isolation_macro_sil45nonisolatedNonsendingPoundIsolationDefaultArgyyYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor) -> () {
// CHECK: bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitIsolationActor):
// CHECK:   [[CAST_PTR:%.*]] = unchecked_value_cast [[ACTOR]] to $(Builtin.Word, Builtin.Word)
