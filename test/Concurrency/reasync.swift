...
func callReasyncRethrowsFunction() async throws {
  ...
  // expected-warning@-1 {{no calls to throwing functions occur within 'try' expression}}{{3-7=}}
  try await reasyncRethrowsFunction { }
  // expected-warning@-1 {{no 'async' operations occur within 'await' expression}}{{7-13=}}
  // expected-warning@-2 {{no calls to throwing functions occur within 'try' expression}}{{3-7=}}
