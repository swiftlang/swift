...
func tuple_patterns() {
  ...
  // CHECK: [[BADDR:%[0-9]+]] = alloc_var stack $Float32
  ...
  // CHECK: [[DADDR:%[0-9]+]] = alloc_var stack $Float32
  // CHECK: [[A:%[0-9]+]] = load [[AADDR]]
  // CHECK: store [[A]] to [[CADDR]]
  // CHECK: [[B:%[0-9]+]] = load [[BADDR]]
  // CHECK: store [[B]] to [[DADDR]]

  // CHECK: [[EADDR:%[0-9]+]] = alloc_var stack $Int64
  // CHECK: [[FADDR:%[0-9]+]] = alloc_var stack $Float32
  // CHECK: [[GADDR:%[0-9]+]] = alloc_var stack $()
  // CHECK: [[HADDR:%[0-9]+]] = alloc_var stack $Float64
  ...
  // CHECK-NOT: alloc_box $Float32
  // CHECK: [[KADDR:%[0-9]+]] = alloc_var stack $()
...
// CHECK: [[X:%[0-9]+]] = alloc_var stack $Int64
// CHECK-NEXT: [[Y:%[0-9]+]] = alloc_var stack $Int64
...
// CHECK: [[X:%[0-9]+]] = alloc_var stack $Int64
// CHECK-NEXT: store %0 to [[X]]
// CHECK: [[Y:%[0-9]+]] = alloc_var stack $Int64
// CHECK-NEXT: store %1 to [[Y]]
...
// CHECK: [[XADDR:%[0-9]+]] = alloc_var stack $(Int64, Float32, ())
