...
func physical_tuple_lvalue(c:Int) {
  // CHECK: [[CADDR:%[0-9]+]] = alloc_var stack $Int64
  var x : (Int, Int)
  // CHECK: [[XADDR:%[0-9]+]] = alloc_var stack $(Int64, Int64)
  x.1 = c
...
func tuple_assignment_2(a:[byref] Int, b:[byref] Int, xy:(Int, Int)) {
  ...
  // CHECK: [[XY_ADDR:%[0-9]+]] = alloc_var stack $(Int64, Int64)
  (a, b) = xy
...
func physical_struct_lvalue(c:Int) {
  // CHECK: [[CADDR:%[0-9]+]] = alloc_var stack $Int64
  var v : Val
  // CHECK: [[VADDR:%[0-9]+]] = alloc_var stack $Val
  v.y = c
  // CHECK: [[C:%[0-9]+]] = load [[CADDR]]
  // CHECK: [[X_1:%[0-9]+]] = struct_element_addr [[VADDR]], @y
  // CHECK: store [[C]] to [[X_1]]
}

// CHECK: sil @_T10properties21physical_class_lvalueFT1rCS_3Ref1aSi_T_
func physical_class_lvalue(r:Ref, a:Int) {
  // CHECK: [[RADDR:%[0-9]+]] = alloc_var stack $Ref
  // CHECK: [[AADDR:%[0-9]+]] = alloc_var stack $Int64
...
func physical_subclass_lvalue(r:RefSubclass, a:Int) {
  // CHECK: [[RADDR:%[0-9]+]] = alloc_var stack $RefSubclass
  // CHECK: [[AADDR:%[0-9]+]] = alloc_var stack $Int64
...
func physical_struct_rvalue() -> Int {
  ...
  // CHECK: [[STRUCT_TEMP:%[0-9]+]] = alloc_var stack $Val
  // CHECK: store [[STRUCT]] to [[STRUCT_TEMP]]
  // CHECK: [[RET_ADDR:%[0-9]+]] = struct_element_addr [[STRUCT_TEMP]], @y
  // CHECK: [[RET:%[0-9]+]] = load [[RET_ADDR]]
  // CHECK: dealloc_var stack [[STRUCT_TEMP]]
  // CHECK: return [[RET]]
}
...
func logical_struct_get() -> Int {
  ...
  // CHECK: [[VALUE:%[0-9]+]] = apply [[GET_METHOD]]([[STRUCT_TEMP]])
  // CHECK: [[VALUE_TEMP:%[0-9]+]] = alloc_var stack $Int64
  // CHECK: store [[VALUE]] to [[VALUE_TEMP]]
  // CHECK: [[VALUE1:%[0-9]+]] = load [[VALUE_TEMP]]
  // CHECK: dealloc_var stack [[VALUE_TEMP]]
  // CHECK: dealloc_var stack [[STRUCT_TEMP]]
  // CHECK: return [[VALUE1]]
}
...
func logical_struct_set(val:[byref] Val, z:Int) {
  ...
  val.z = z
  // CHECK: [[ZADDR:%[0-9]+]] = alloc_var stack $Int64
...
func logical_struct_in_reftype_set(val:[byref] Val, z1:Int) {
  ...
  // CHECK: [[Z1ADDR:%[0-9]+]] = alloc_var stack $Int64
  // CHECK: [[Z1:%[0-9]+]] = load [[Z1ADDR]]
  // -- val.ref
  // CHECK: [[VAL_REF_ADDR:%[0-9]+]] = struct_element_addr [[VAL]], @ref
  ...
  // CHECK: [[VAL_REF_VAL_PROP_MAT:%[0-9]+]] = alloc_var stack $Val
  // CHECK: store [[VAL_REF_VAL_PROP]] to [[VAL_REF_VAL_PROP_MAT]]
  // -- val.ref.val_prop.z_tuple
  // CHECK: [[GET_Z_TUPLE_METHOD:%[0-9]+]] = function_ref @_TV10properties3Val7z_tupleTSiSi_g
  // CHECK: [[V_R_VP_Z_TUPLE:%[0-9]+]] = apply [[GET_Z_TUPLE_METHOD]]([[VAL_REF_VAL_PROP_MAT]])
  // CHECK: [[V_R_VP_Z_TUPLE_MAT:%[0-9]+]] = alloc_var stack $(Int64, Int64)
  ...
  // -- cleanup
  // CHECK: dealloc_var stack [[V_R_VP_Z_TUPLE_MAT]]
  // CHECK: dealloc_var stack [[VAL_REF_VAL_PROP_MAT]]
...
func reftype_rvalue_set(val:Val) {
  // CHECK: [[VALADDR:%[0-9]+]] = alloc_var stack $Val
...
func tuple_in_logical_struct_set(val:[byref] Val, z1:Int) {
  ...
  // CHECK: [[Z_TUPLE_MATERIALIZED:%[0-9]+]] = alloc_var stack $(Int64, Int64)
  ...
  // CHECK: dealloc_var stack [[Z_TUPLE_MATERIALIZED]]
  // CHECK: return
}
...
func physical_byref(x:Int) {
  ...
  // CHECK: apply [[BYREF_ARG]]([[XADDR]])
}

// FIXME Byref writebacks currently broken -Joe
// XXX: sil @logical_byref
func logical_byref() {
  byref_arg(&global_prop)
  // XXX: [[BYREF_ARG:%[0-9]+]] = function_ref @byref_arg
  // XXX: [[GETTER:%[0-9]+]] = function_ref @global_prop.getter
  // XXX: [[SETTER:%[0-9]+]] = function_ref @global_prop.setter
  // XXX: [[PROP0:%[0-9]+]] = apply [[GETTER]]()
  // XXX: [[PROP_MATERIALIZED:%[0-9]+]] = alloc_var stack $Int64
  // XXX: store [[PROP0]] to [[PROP_MATERIALIZED]]
  // XXX: apply [[BYREF_ARG]]([[PROP_MATERIALIZED]])
  // XXX: [[PROP1:%[0-9]+]] = load [[PROP_MATERIALIZED]]
  // XXX: apply [[SETTER]]([[PROP1]])
  // XXX: dealloc_var stack [[PROP_MATERIALIZED]]
}
...
func val_subscript_get(v:Val, i:Int) -> Float {
  ...
  // CHECK: [[ELT:%[0-9]+]] = apply [[SUBSCRIPT_GET_METHOD]]([[I]], [[VADDR]])
  // CHECK: [[ELT_MAT:%[0-9]+]] = alloc_var stack $Float
  // CHECK: store [[ELT]] to [[ELT_MAT]]
  // CHECK: [[RET:%[0-9]+]] = load [[ELT_MAT]]
  // CHECK: dealloc_var stack [[ELT_MAT]]
  // CHECK: return [[RET]]
}
