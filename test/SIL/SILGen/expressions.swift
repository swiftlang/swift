...
// CHECK: float_literal $Builtin.FP_IEEE64, 1.1
...
func call_default_args_1(x:Int) {
  ...
  // CHECK: integer_literal ${{.*}}, 219
  // CHECK-NEXT: [[C219:%[0-9]+]] = apply
  // CHECK-NEXT: function_ref
  // CHECK: integer_literal ${{.*}}, 20721
  // CHECK-NEXT: [[C20721:%[0-9]+]] = apply
  // CHECK: apply [[FUNC]]([[X]], [[C219]], [[C20721]])
...
func generic_member_ref<T>(x:Generic<T>) -> Int {
  ...
  // CHECK: [[MEMBER_ADDR:%[0-9]+]] = struct_element_addr [[XADDR]], @mono_member
...
// CHECK: sil @_T11expressions18erasure_from_protoFT1xPS_8RuncibleS_8Bendable__PS0__
func erasure_from_proto(x:protocol<Runcible, Bendable>) -> Runcible {
  return x
...
func tuple_element(x:(Int, Float)) {
  // CHECK: [[XADDR:%.*]] = alloc_var stack $(Int64, Float32)

  int(x.0)
  // CHECK: tuple_element_addr [[XADDR]], 0
  // CHECK: apply

  float(x.1)
  // CHECK: tuple_element_addr [[XADDR]], 1
  // CHECK: apply
...
func if_expr(a:Bool, b:Bool, x:Int, y:Int, z:Int) -> Int {
  // CHECK: bb0({{.*}}):
  // CHECK: [[A:%[0-9]+]] = alloc_var stack $Bool
  // CHECK: [[B:%[0-9]+]] = alloc_var stack $Bool
  // CHECK: [[X:%[0-9]+]] = alloc_var stack $Int64
  // CHECK: [[Y:%[0-9]+]] = alloc_var stack $Int64
  // CHECK: [[Z:%[0-9]+]] = alloc_var stack $Int64
