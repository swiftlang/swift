...
// RUN: %target-swift-frontend -enable-implicit-dynamic -target %target-swift-5.1-abi-triple -emit-module-path %t/OpaqueCrossFileB.swiftmodule -module-name OpaqueCrossFileB %S/Inputs/OpaqueCrossFileB.swift
// RUN: %target-swift-frontend -enable-implicit-dynamic -target %target-swift-5.1-abi-triple -I %t -emit-ir -verify %s | %FileCheck %s
