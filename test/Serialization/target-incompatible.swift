...
// RUN: %swift -target x86_64-unknown-darwin14 -o %t/mips-template.swiftmodule -emit-module -module-name mips %S/../Inputs/empty.swift
...
// RUN: not %target-swift-frontend -I %t -parse %s -DMIPS 2>&1 | FileCheck -check-prefix=CHECK-MIPS %s

// RUN: %swift -target x86_64-unknown-darwin14 -o %t/solaris-template.swiftmodule -emit-module -module-name solaris %S/../Inputs/empty.swift
...
// RUN: not %target-swift-frontend -I %t -parse %s -DSOLARIS 2>&1 | FileCheck -check-prefix=CHECK-SOLARIS %s
