...
public struct Array<T> {
  ...
  // CHECK: sil [serialized] [_semantics "array.get_capacity"] [canonical] [stage=canonical] [ossa] @$sSa12_getCapacitySiyF : $@convention(method) <T> (Array<T>) -> Int
  @inlinable
  @usableFromInline
...
// CHECK: sil shared [serialized] [_semantics "array.get_capacity"] [canonical] [stage=canonical] [ossa] @$sSa12_getCapacitySiyFSi_Tgq5 : $@convention(method) (Array<Int>) -> Int

// Check that a call of a @_semantics function was not inlined if early-serialization is enabled.
// CHECK: sil [serialized] [canonical] [stage=canonical] [ossa] @$ss28userOfSemanticsAnnotatedFuncySiSaySiGF
// CHECK: function_ref
// CHECK: apply
@inlinable
