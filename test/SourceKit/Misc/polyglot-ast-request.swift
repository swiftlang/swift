/// Test SourceKit request polyglot-ast

// RUN: %empty-directory(%t)
// RUN: split-file %s %t --leading-lines

/// Parse header
// RUN: %sourcekitd-test -req=polyglot-ast %t/MyClass.h \
// RUN:   -- %t/empty.swift -target %target-triple \
// RUN:   -sdk %clang-importer-sdk-path \
// RUN:   > %t/ast.json

// RUN: %FileCheck --check-prefix=CHECK %t/MyClass.h --input-file %t/ast.json

/// Parse implementation file
// RUN: %sourcekitd-test -req=polyglot-ast %t/MyClass.m \
// RUN:   -- %t/empty.swift -target %target-triple \
// RUN:   -sdk %clang-importer-sdk-path \
// RUN:   > %t/ast.json
// RUN: %validate-json %t/ast.json > /dev/null
// RUN: %FileCheck --check-prefix=CHECK %t/MyClass.m --input-file %t/ast.json
