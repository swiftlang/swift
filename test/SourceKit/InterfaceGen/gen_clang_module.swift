...
// RUN:         %mcp_opt -target %target-triple %clang-importer-sdk > %t.response
// RUN: diff -u %s.response %t.response

// RUN: %sourcekitd-test -req=interface-gen -module Foo.FooSub -- -I %t.overlays -F %S/../Inputs/libIDE-mock-sdk \
// RUN:         %mcp_opt -target %target-triple %clang-importer-sdk > %t.sub.response
...
// RUN:         %mcp_opt -target %target-triple %clang-importer-sdk > %t.helper.response
...
// RUN:         -F %S/../Inputs/libIDE-mock-sdk  %mcp_opt -target %target-triple %clang-importer-sdk > %t.helper.explicit.response
...
// RUN:         %mcp_opt -target %target-triple %clang-importer-sdk \
...
// RUN:         %mcp_opt -target %target-triple %clang-importer-sdk | %FileCheck -check-prefix=CHECK1 %s
...
// RUN:         %mcp_opt -target %target-triple %clang-importer-sdk | %FileCheck -check-prefix=CHECK-IFACE %s
...
// RUN:         %mcp_opt -target %target-triple %clang-importer-sdk > %t.apinotes_swift3.response
...
// RUN:         %mcp_opt -target %target-triple %clang-importer-sdk > %t.apinotes_swift4.response
