...
// RUN: %target-swift-frontend -fine-grained-dependency-include-intrafile -typecheck -disable-experimental-private-intransitive-dependencies -primary-file %t/main.swift %S/../Inputs/reference-dependencies-helper.swift -emit-reference-dependencies-path - > %t.swiftdeps
// Check that the output is deterministic.
// RUN: %target-swift-frontend  -fine-grained-dependency-include-intrafile -typecheck -disable-experimental-private-intransitive-dependencies -primary-file %t/main.swift %S/../Inputs/reference-dependencies-helper.swift -emit-reference-dependencies-path - > %t-2.swiftdeps
