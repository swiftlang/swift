...
extension PlainClass {
  ...
  // expected-error@-2{{missing ':' after selector piece in 'objc' attribute}}{{27-27=:}}
  // expected-error@-3{{missing ':' after selector piece in 'objc' attribute}}{{38-38=:}}
...
objc(Class:) // expected-error{{'objc' class must have a simple name}}{{11-12=}}
class BadClass1 { }

objc(Protocol:) // expected-error{{'objc' protocol must have a simple name}}{{14-15=}}
protocol BadProto1 { }

class BadClass2 {
  objc(badprop:foo:wibble:) // expected-error{{'objc' property must have a simple name}}{{15-27=}}
  var badprop: Int = 5
