...
func testExhaustiveWithDefault(enum e: ReasonableEnum) -> Bool {
  switch e { // expected-error {{switch covers known cases, but 'ReasonableEnum' may have additional unknown values, a default case will mask those added cases, replace with "@unknown default"}} {{7:3-3=@unknown }}
  case .zero, .one: return true
...
func testGuardedWildcardWithDefault(enum e: ReasonableEnum) -> Bool {
  switch e { // expected-error {{switch covers known cases, but 'ReasonableEnum' may have additional unknown values, a default case will mask those added cases, replace with "@unknown default"}}
    case .zero, .one:
      return true
    case _ where e == .one:
      return true
