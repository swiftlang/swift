...
func patterns(gir: GoodRange<Int>, gtr: GoodTupleGeneratorType) {
  ...
  for (i, f) in gtr {
    sum = sum + i
    sumf = sumf + f
    sum = sum + f  // expected-error{{binary operator '+' cannot be applied operands of type 'Int' and 'Float'}} expected-note{{Overloads for '+' exist with these partially matching parameter lists:}}
  }
