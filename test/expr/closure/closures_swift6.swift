...
class ExplicitSelfRequiredTest_WeakSelf {
  var x = 42
  ...
  func weakSelfError() {
    ...
    doStuff({ [weak self] in method() }) // expected-error {{explicit use of 'self' is required when 'self' is optional, to make control flow explicit}} expected-note{{reference 'self?.' explicitly}} expected-error {{cannot convert value of type '()' to closure result type 'Int'}}
  }
}
