...
class C {
  convenience init() { // expected-note 11 {{selected non-required initializer 'init()'}}
    self.init()
    let _ = self.init()
    let _: C = self.init() // expected-error{{cannot convert initial value of type '()' to specified type 'C'}}
...
class D: C {
  override init(x: Int) {
    super.init(x: x)
    let _ = super.init()
    let _: C = super.init() // expected-error{{cannot convert initial value of type '()' to specified type 'C'}}
