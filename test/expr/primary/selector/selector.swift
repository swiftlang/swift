...
func testAmbiguity() {
  _ = #selector(C1.method3) // expected-error{{ambiguous use of 'method3'; cannot select between potential parameter types '(a: A, b: B)', '(A, b: B)'}}
  _ = #selector(C1.ambiguous) // expected-error{{ambiguous use of 'ambiguous(a:b:)'; cannot select between potential parameter types '(a: A, b: B)', '(a: A, b: A)'}}
}
