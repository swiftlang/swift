...
// RUN: %{python} %S/Inputs/build-modules.py %t %S/Inputs/testcases

// TODO: replace $(cat %t/link.txt) and change back to RUN
// DISABLE: %target-build-swift -g -I%t -o %t/round-trip %s %t/all-tests.swift -L%t %target-cxx-lib $(cat %t/link.txt) -lm -lRoundTrip -L%swift-lib-dir/swift/%target-sdk-name/%target-arch -lswiftRemoteInspection


// TODO: change back to RUN
// DISABLE: %target-codesign %t/round-trip
// DISABLE: %target-run %t/round-trip | %FileCheck %s
