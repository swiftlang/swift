...
RUN: swift-demangle -remangle-new < %S/Inputs/bigtype.txt 2>&1 | sed -E 's/\(([0-9]+):[0-9]+\)/(\1:xxx)/g' > %t.check 2>&1 || true
RUN: %diff -u %S/Inputs/bigtype-remangle.txt %t.check

RUN: swift-demangle -remangle-objc-rt < %S/Inputs/bigtype.txt 2>&1 | sed -E 's/\(([0-9]+):[0-9]+\)/(\1:xxx)/g' > %t.check || true
