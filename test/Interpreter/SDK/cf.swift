...
CFTestSuite.test("Set<CFString>") {
  ...
  expectTrue(s.contains(String("abcxxxxxxxxxxx") as NSString))
  expectFalse(s.contains(String("efxxxxxxxxxxx") as NSString))
}
...
CFTestSuite.test("Dictionary/round-trip")
    .xfail(.always("bridging back is failing"))
    .code {
  let orig: [CFString: Any] = [
    .from("abcxxxxxxxxxxx"): "abc",
    .from("defxxxxxxxxxxx"): "def"
