...
public func classSimpleChainArgTest(_ x2: borrowing Klass) {
...
public func classMultipleNonConsumingUseArgTest(_ x2: borrowing Klass) {
    borrowVal(x2)
    borrowVal(x2)
...
public func classUseAfterConsumeArg(_ x2: borrowing Klass) {
    borrowVal(x2)
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
}
...
public func classDoubleConsumeArg(_ x2: borrowing Klass) {
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
}
...
public func classLoopConsumeArg(_ x2: borrowing Klass) {
    for _ in 0..<1024 {
        consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    }
}
...
public func classDiamondArg(_ x2: borrowing Klass) {
    if boolValue {
        consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    } else {
        consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
...
public func classDiamondInLoopArg(_ x2: borrowing Klass) {
    for _ in 0..<1024 {
      if boolValue {
          consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
      } else {
...
public func classAssignToVar1Arg(_ x: borrowing Klass, _ x2: borrowing Klass) {
...
public func classAssignToVar2Arg(_ x2: borrowing Klass) {
    var x3 = x2 // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    x3 = x2 // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    borrowVal(x3)
}
...
public func classAssignToVar3Arg(_ x: borrowing Klass, _ x2: borrowing Klass) {
    var x3 = x2 // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    x3 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    consumeVal(x3)
}

public func classAssignToVar3OwnedArg(_ x: borrowing Klass, _ x2: __owned Klass) {
    var x3 = x2
    x3 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    consumeVal(x3)
}

public func classAssignToVar3OwnedArg2(_ x: borrowing Klass, _ x2: consuming Klass) {
    var x3 = x2
    x3 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    consumeVal(x3)
}
...
public func classAssignToVar4Arg(_ x2: borrowing Klass) {
...
public func classAssignToVar5(_ x: borrowing Klass) {
...
public func classAssignToVar5Arg(_ x: borrowing Klass, _ x2: borrowing Klass) {
    var x3 = x2 // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    borrowVal(x2)
    x3 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    consumeVal(x3)
...
public func finalClassSimpleChainTest(_ x: borrowing FinalKlass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    let y2 = x2
    let k2 = y2
    borrowVal(k2)
...
public func finalClassSimpleChainTestArg(_ x2: borrowing FinalKlass) {
    let y2 = x2 // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    let k2 = y2
    borrowVal(k2)
}
...
public func finalClassSimpleNonConsumingUseTest(_ x: borrowing FinalKlass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    borrowVal(x2)
}
...
public func finalClassMultipleNonConsumingUseTest(_ x: borrowing FinalKlass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    borrowVal(x2)
    borrowVal(x2)
    consumeVal(x2)
...
public func finalClassMultipleNonConsumingUseTestArg(_ x2: borrowing FinalKlass) {
    borrowVal(x2)
    borrowVal(x2)
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
}
...
public func finalClassUseAfterConsume(_ x: borrowing FinalKlass) {
    let x2 = x // expected-error {{'x2' consumed more than once}}
               // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    borrowVal(x2)
    consumeVal(x2) // expected-note {{consumed here}}
...
public func finalClassUseAfterConsumeArg(_ x2: borrowing FinalKlass) {
    borrowVal(x2)
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
}
...
public func finalClassDoubleConsume(_ x: borrowing FinalKlass) {
    let x2 = x  // expected-error {{'x2' consumed more than once}}
                // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    consumeVal(x2) // expected-note {{consumed here}}
    consumeVal(x2) // expected-note {{consumed again here}}
...
public func finalClassDoubleConsumeArg(_ x2: borrowing FinalKlass) {
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
}
...
public func finalClassLoopConsume(_ x: borrowing FinalKlass) {
    let x2 = x // expected-error {{'x2' consumed in a loop}}
               // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    for _ in 0..<1024 {
        consumeVal(x2) // expected-note {{consumed here}}
...
public func finalClassLoopConsumeArg(_ x2: borrowing FinalKlass) {
    for _ in 0..<1024 {
        consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    }
}
...
public func finalClassDiamond(_ x: borrowing FinalKlass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    if boolValue {
        consumeVal(x2)
    } else {
...
public func finalClassDiamondArg(_ x2: borrowing FinalKlass) {
    if boolValue {
        consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    } else {
        consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
...
public func finalClassDiamondInLoop(_ x: borrowing FinalKlass) {
    let x2 = x // expected-error {{'x2' consumed more than once}}
    // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
...
public func finalClassDiamondInLoopArg(_ x2: borrowing FinalKlass) {
    for _ in 0..<1024 {
      if boolValue {
          consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
      } else {
...
public func finalClassAssignToVar1(_ x: borrowing FinalKlass) {
    let x2 = x // expected-error {{'x2' consumed more than once}}
               // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    var x3 = x2 // expected-note {{consumed here}}
    x3 = x2 // expected-note {{consumed again here}}
...
public func finalClassAssignToVar1Arg(_ x: borrowing FinalKlass, _ x2: borrowing FinalKlass) {
    var x3 = x2 // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    x3 = x2 // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    x3 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    consumeVal(x3)
...
public func finalClassAssignToVar2(_ x: borrowing FinalKlass) {
    let x2 = x // expected-error {{'x2' consumed more than once}}
               // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    var x3 = x2 // expected-note {{consumed here}}
    x3 = x2 // expected-note {{consumed again here}}
...
public func finalClassAssignToVar2Arg(_ x2: borrowing FinalKlass) {
    var x3 = x2 // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    x3 = x2 // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    borrowVal(x3)
}
...
public func finalClassAssignToVar3(_ x: borrowing FinalKlass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    var x3 = x2
    x3 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    consumeVal(x3)
...
public func finalClassAssignToVar3Arg(_ x: borrowing FinalKlass, _ x2: borrowing FinalKlass) {
    var x3 = x2 // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    x3 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    consumeVal(x3)
}

public func finalClassAssignToVar3OwnedArg(_ x: borrowing FinalKlass, _ x2: __owned FinalKlass) {
    var x3 = x2
    x3 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    consumeVal(x3)
}

public func finalClassAssignToVar3OwnedArg2(_ x: borrowing FinalKlass, _ x2: consuming FinalKlass) {
    var x3 = x2
    x3 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    consumeVal(x3)
}

public func finalClassAssignToVar4(_ x: borrowing FinalKlass) {
    let x2 = x // expected-error {{'x2' consumed more than once}}
               // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    let x3 = x2 // expected-note {{consumed here}}
    consumeVal(x2) // expected-note {{consumed again here}}
...
public func finalClassAssignToVar4Arg(_ x2: borrowing FinalKlass) {
    let x3 = x2 // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    consumeVal(x3)
}
...
public func finalClassAssignToVar5(_ x: borrowing FinalKlass) {
    let x2 = x // expected-error {{'x2' used after consume}}
               // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    var x3 = x2 // expected-note {{consumed here}}
    borrowVal(x2) // expected-note {{used here}}
...
public func finalClassAssignToVar5Arg(_ x: borrowing FinalKlass, _ x2: borrowing FinalKlass) {
    var x3 = x2 // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    borrowVal(x2)
    x3 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    consumeVal(x3)
...
public func finalClassAccessField(_ x: borrowing FinalKlass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    borrowVal(x2.k)
    for _ in 0..<1024 {
        borrowVal(x2.k)
...
public func finalClassConsumeField(_ x: borrowing FinalKlass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    consumeVal(x2.k) // expected-error {{cannot consume noncopyable stored property 'x2.k' of a class}}
    for _ in 0..<1024 {
        consumeVal(x2.k) // expected-error {{cannot consume noncopyable stored property 'x2.k' of a class}}
...
public func aggGenericStructSimpleChainTest(_ x: borrowing AggGenericStruct<String>) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    let y2 = x2
    let k2 = y2
    borrowVal(k2)
...
public func aggGenericStructSimpleChainTestArg(_ x2: borrowing AggGenericStruct<String>) {
    let y2 = x2 // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    let k2 = y2
    borrowVal(k2)
}
...
public func aggGenericStructSimpleNonConsumingUseTest(_ x: borrowing AggGenericStruct<String>) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    borrowVal(x2)
}
...
public func aggGenericStructMultipleNonConsumingUseTest(_ x: borrowing AggGenericStruct<String>) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    borrowVal(x2)
    borrowVal(x2)
    consumeVal(x2)
...
public func aggGenericStructMultipleNonConsumingUseTestArg(_ x2: borrowing AggGenericStruct<String>) {
    borrowVal(x2)
    borrowVal(x2)
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
}
...
public func aggGenericStructUseAfterConsume(_ x: borrowing AggGenericStruct<String>) {
    let x2 = x // expected-error {{'x2' consumed more than once}}
               // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    borrowVal(x2)
    consumeVal(x2) // expected-note {{consumed here}}
...
public func aggGenericStructUseAfterConsumeArg(_ x2: borrowing AggGenericStruct<String>) {
    borrowVal(x2)
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
}
...
public func aggGenericStructDoubleConsume(_ x: borrowing AggGenericStruct<String>) {
    let x2 = x  // expected-error {{'x2' consumed more than once}}
                // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    consumeVal(x2) // expected-note {{consumed here}}
    consumeVal(x2) // expected-note {{consumed again here}}
...
public func aggGenericStructDoubleConsumeArg(_ x2: borrowing AggGenericStruct<String>) {
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
}
...
public func aggGenericStructLoopConsume(_ x: borrowing AggGenericStruct<String>) {
    let x2 = x // expected-error {{'x2' consumed in a loop}}
               // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    for _ in 0..<1024 {
        consumeVal(x2) // expected-note {{consumed here}}
...
public func aggGenericStructLoopConsumeArg(_ x2: borrowing AggGenericStruct<String>) {
    for _ in 0..<1024 {
        consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    }
}
...
public func aggGenericStructDiamond(_ x: borrowing AggGenericStruct<String>) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    if boolValue {
        consumeVal(x2)
    } else {
...
public func aggGenericStructDiamondArg(_ x2: borrowing AggGenericStruct<String>) {
    if boolValue {
        consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
    } else {
        consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
...
public func aggGenericStructDiamondInLoop(_ x: borrowing AggGenericStruct<String>) {
    let x2 = x // expected-error {{'x2' consumed more than once}}
    // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    // expected-error @-2 {{'x2' consumed in a loop}}
    for _ in 0..<1024 {
...
public func aggGenericStructDiamondInLoopArg(_ x2: borrowing AggGenericStruct<String>) {
    for _ in 0..<1024 {
      if boolValue {
          consumeVal(x2) // expected-error{{'x2' is borrowed, so it cannot be consumed here}}
      } else {
...
public func aggGenericStructAccessField(_ x: borrowing AggGenericStruct<String>) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    borrowVal(x2.lhs)
    for _ in 0..<1024 {
        borrowVal(x2.lhs)
...
public func aggGenericStructConsumeField(_ x: borrowing AggGenericStruct<String>) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    // expected-error @-1 {{'x2' consumed in a loop}}
    // expected-error @-2 {{'x2' consumed more than once}}
    consumeVal(x2.lhs) // expected-note {{consumed here}}
...
public func aggGenericStructAccessGrandField(_ x: borrowing AggGenericStruct<String>) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    borrowVal(x2.pair.lhs)
    for _ in 0..<1024 {
        borrowVal(x2.pair.lhs)
...
public func aggGenericStructConsumeGrandField(_ x: borrowing AggGenericStruct<String>) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    // expected-error @-1 {{'x2' consumed in a loop}}
    // expected-error @-2 {{'x2' consumed more than once}}
    consumeVal(x2.pair.lhs) // expected-note {{consumed here}}
...
public func closureLetCaptureClassUseAfterConsume1(_ x: borrowing Klass) {
...
public func closureLetCaptureClassUseAfterConsumeError(_ x: borrowing Klass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    let f = {
        borrowVal(x2)
        consumeVal(x2) // expected-error{{'x2' is borrowed by this closure, so it cannot be consumed here}}
...
public func closureVarCaptureClassUseAfterConsume(_ x: borrowing Klass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    var f = {}
    f = { 
        borrowVal(x2)
...
public func closureVarCaptureClassUseAfterConsume1(_ x: borrowing Klass) {
    var x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    x2 = x  // expected-error{{'x' is borrowed, so it cannot be consumed here}}
...
public func closureVarCaptureClassUseAfterConsumeError(_ x: borrowing Klass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    var f = {}
    f = { 
        borrowVal(x2)
...
public func deferCaptureClassUseAfterConsume(_ x: borrowing Klass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    defer {
        borrowVal(x2)
...
public func deferCaptureClassUseAfterConsume2(_ x: borrowing Klass) {
    let x2 = x // expected-error {{'x2' used after consume}}
    // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
...
public func deferCaptureClassOwnedArgUseAfterConsume3(_ x2: __owned Klass) { // expected-error {{'x2' used after consume}}
    defer { // expected-note {{used here}}
        borrowVal(x2)
        consumeVal(x2) // expected-error{{'x2' is borrowed by this deferred block, so it cannot be consumed here}}
        consumeVal(x2) // expected-error{{'x2' is borrowed by this deferred block, so it cannot be consumed here}}
...
public func closureLetAndDeferCaptureClassUseAfterConsume(_ x: borrowing Klass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    let f = {
        defer {
            borrowVal(x2)
...
public func closureLetAndDeferCaptureClassUseAfterConsume2(_ x: borrowing Klass) {
    // TODO: This is wrong
    let x2 = x
    // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    let f = {
...
// TODO: MG
public func closureLetAndDeferCaptureClassUseAfterConsume3(_ x: borrowing Klass) {
    let x2 = x
    // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    let f = {
        consumeVal(x2) // expected-error{{'x2' is borrowed by this closure, so it cannot be consumed here}}
...
public func closureLetAndClosureCaptureClassUseAfterConsume(_ x: borrowing Klass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    let f = {
        let g = {
            borrowVal(x2)
...
public func closureLetAndClosureCaptureClassUseAfterConsume2(_ x: borrowing Klass) {
    let x2 = x // expected-error {{'x2' consumed more than once}}
    // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
...
public func closureLetAndClosureCaptureClassUseAfterConsume3(_ x: borrowing Klass) {
    var x2 = x
    // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    // expected-error @-2 {{'x2' consumed more than once}}
    // expected-error @-3 {{missing reinitialization of closure capture 'x2' after consume}}
...
public func closureVarAndClosureLetCaptureClassUseAfterConsume(_ x: borrowing Klass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    var f = {}
    f = {
        let g = {
...
public func closureVarAndClosureLetCaptureClassUseAfterConsume2(_ x: borrowing Klass) {
    let x2 = x
    // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
...
public func closureVarAndClosureLetCaptureClassUseAfterConsume3(_ x: borrowing Klass) {
    var x2 = x
    // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    x2 = x
    // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
...
public func closureLetAndClosureVarCaptureClassUseAfterConsume(_ x: borrowing Klass) {
    let x2 = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    let f = {
        var g = {}
        g = {
...
public func closureLetAndClosureVarCaptureClassUseAfterConsume2(_ x: borrowing Klass) {
    let x2 = x
    // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
...
public func closureLetAndClosureVarCaptureClassUseAfterConsume3(_ x: borrowing Klass) {
    var x2 = x
    // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
    x2 = x
    // expected-error@-1 {{'x' is borrowed, so it cannot be consumed here}}
...
func testEmptyStruct() {
  ...
  func testGuaranteedArg2(_ x: borrowing EmptyStruct) {
    consume(x) // expected-error{{'x' is borrowed, so it cannot be consumed here}}
  }

  func testGuaranteedArg3(_ x: borrowing EmptyStruct) {
    let _ = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
  }
  ...
  func testGuaranteedArg5(_ x: borrowing EmptyStruct) {
    let y = x // expected-error{{'x' is borrowed, so it cannot be consumed here}}
    _ = y
  }
  ...
  func testGuaranteedArg7(_ x: borrowing EmptyStruct) {
    x.doSomething3() // expected-error{{'x' is borrowed, so it cannot be consumed here}}
  }
...
func testStructContainingEmptyStruct() {
  ...
  func testGuaranteedArg7(_ x: borrowing StructContainingEmptyStruct) {
    x.x.doSomething3() // expected-error{{'x' is borrowed, so it cannot be consumed here}}
  }
...
func testStructContainingTwoEmptyStruct() {
  ...
  func testGuaranteedArg8(_ x: borrowing StructContainingTwoEmptyStruct) {
    x.y.doSomething3() // expected-error{{'x' is borrowed, so it cannot be consumed here}}
  }
}
...
func testMyEnum() {
  func test1(_ x: borrowing MyEnum) {
    if case let .first(y) = consume x { // expected-error{{'x' is borrowed, so it cannot be consumed here}}
      _ = y
    }
  }

  func test2(_ x: borrowing MyEnum) {
    if case let .third(.first(y)) = consume x { // expected-error{{'x' is borrowed, so it cannot be consumed here}}
      _ = y
    }
  }
}
