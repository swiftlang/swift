...
// use(_:)
// CHECK: sil [serialized_for_package] [canonical] [stage=canonical] [ossa] @$s3Lib3useyySiyXEF : $@convention(thin) (@guaranteed @noescape @callee_guaranteed () -> Int) -> () {
...
// CHECK: sil shared [serialized] [canonical] [stage=canonical] [ossa] @$s3Lib9reproduceyyAA9SomethingVFSiyXEfU_ : $@convention(thin) (@in_guaranteed Something) -> Int {
...
// Something.f()
// CHECK: sil [serialized_for_package] [canonical] [stage=canonical] [ossa] @$s3Lib9SomethingV1fSiyF : $@convention(method) (@in_guaranteed Something) -> Int {

// Something.init()
// CHECK: sil [serialized_for_package] [canonical] [stage=canonical] [ossa] @$s3Lib9SomethingVACycfC : $@convention(method) (@thin Something.Type) -> @out Something {

// reproduce(_:)
// CHECK: sil [serialized] [canonical] [stage=canonical] [ossa] @$s3Lib9reproduceyyAA9SomethingVF : $@convention(thin) (@in_guaranteed Something) -> () {
