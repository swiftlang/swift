// RUN: %target-swift-frontend  -parse-as-library -primary-file %s -O -sil-verify-all -Xllvm -sil-print-transform-blocks=false -module-name=test -emit-sil | grep -v debug_value | %FileCheck %s
// RUN: %target-swift-frontend  -parse-as-library -primary-file %s -Osize -sil-verify-all -Xllvm -sil-print-transform-blocks=false -module-name=test -emit-sil | grep -v debug_value | %FileCheck %s
