...
// CHECK: sil package [serialized_for_package] [canonical] [stage=canonical] [ossa] @$s3Lib6libPkgyAA0C6StructVSiF : $@convention(thin) (Int) -> @out PkgStruct {
  // CHECK: struct $PkgStruct
  // CHECK: store {{.*}} to [trivial] %0 : $*PkgStruct

// CHECK: sil [serialized_for_package] [canonical] [stage=canonical] [ossa] @$s3Lib6libPubyAA0C6StructVSiF : $@convention(thin) (Int) -> @out PubStruct {
  // CHECK: struct $PubStruct
  // CHECK: store {{.*}} to [trivial] %0 : $*PubStruct

// CHECK: sil package [serialized_for_package] [canonical] [stage=canonical] [ossa] @$s3Lib9libUfiPkgyAA0cD6StructVSiF : $@convention(thin) (Int) -> @out UfiPkgStruct {
  // CHECK: struct $UfiPkgStruct
  // CHECK: store {{.*}} to [trivial] %0 : $*UfiPkgStruct

/// @inlinable package func inLibUfiPkg(_ arg: Int) -> UfiPkgStruct
// CHECK: sil [serialized] [canonical] [stage=canonical] [ossa] @$s3Lib02inA6UfiPkgyAA0cD6StructVSiF : $@convention(thin) (Int) -> @out UfiPkgStruct {
...
// CHECK: sil [serialized] [canonical] [stage=canonical] [ossa] @$s3Lib02inA6UfiHidyAA0cD6StructVSiF : $@convention(thin) (Int) -> @out UfiHidStruct {
...
// CHECK: sil [serialized] [canonical] [stage=canonical] [ossa] @$s3Lib02inA3PubyAA0C6StructVSiF : $@convention(thin) (Int) -> @out PubStruct {
