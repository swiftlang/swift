...
// Sequences read from parameters, including ones that cannot be implicitly
// copied, are borrowed without copying them.
...
// Iterating an Iterable never copies it, even when it is copyable: the loop
// borrows the sequence's storage in place, so reassigning the sequence while
// iterating over it is an exclusivity violation.

func reassignDuringLoop(_ array: [Int]) {
  var span = array.span
  for _ in span { // expected-note {{conflicting access is here}}
    span = array.span // expected-error {{overlapping accesses to 'span', but modification requires exclusive access; consider copying to a local variable}}
  }
}
...
func reassignStoredPropertyDuringLoop(_ array: [Int]) {
  ...
  for _ in holder.span { // expected-note {{conflicting access is here}}
    holder.span = array.span // expected-error {{overlapping accesses to 'holder.span', but modification requires exclusive access; consider copying to a local variable}}
  }
}
...
// The same holds for a tuple element of a local variable: the loop reads the
// element in place, so the read access covers just that element.
func reassignTupleElementDuringLoop(_ array: [Int]) {
  var pair = (array.span, array.span)
  for _ in pair.0 { // expected-note {{conflicting access is here}}
    pair.0 = array.span // expected-error {{overlapping accesses to 'pair.0', but modification requires exclusive access; consider copying to a local variable}}
  }
}

func reassignTupleDuringLoop(_ array: [Int]) {
  var pair = (array.span, array.span)
  for _ in pair.0 { // expected-note {{conflicting access is here}}
    pair = (array.span, array.span) // expected-error {{overlapping accesses to 'pair', but modification requires exclusive access; consider copying to a local variable}}
  }
}

func reassignOtherTupleElementDuringLoop(_ array: [Int]) {
  var pair = (array.span, array.span)
  for _ in pair.0 {
    pair.1 = array.span
  }
...
func reassignInlineArrayDuringLoop() {
  ...
  for x in array { // expected-note {{conflicting access is here}}
    array = [x, x, x] // expected-error {{overlapping accesses to 'array', but modification requires exclusive access; consider copying to a local variable}}
  }
}
...
// Noncopyable sequences are borrowed the same way.
