...
// RUN: %target-swift-frontend -module-name access_enforcement_noescape -enable-sil-ownership-verifier -enforce-exclusivity=checked -Onone -emit-sil -swift-version 4 -verify -parse-as-library %s
// RUN: %target-swift-frontend -module-name access_enforcement_noescape -enable-sil-ownership-verifier -enforce-exclusivity=checked -Onone -emit-sil -swift-version 3 -parse-as-library %s | %FileCheck %s
// REQUIRES: asserts
