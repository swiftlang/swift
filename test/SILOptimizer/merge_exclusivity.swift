...
// Large, runnable, test that produces a conflict on out of scope access
// run_MergeTest10 Tests that we detect that conflict and avoid merging
...
// TESTSIL-LABEL: sil [noinline] @$S17merge_exclusivity15run_MergeTest10yySiF : $@convention(thin)
// TESTSIL: [[REFADDR:%.*]] = ref_element_addr {{.*}} : $StreamClass, #StreamClass.buffer
// TESTSIL-NEXT: [[B1:%.*]] = begin_access [modify] [static] [no_nested_conflict] [[REFADDR]]
// TESTSIL: end_access [[B1]]
// TESTSIL-NOT: begin_access [modify] [dynamic] [no_nested_conflict] [[REFADDR]]
// TESTSIL-NOT: begin_access [modify] [dynamic] [[REFADDR]]
// TESTSIL-LABEL: } // end sil function '$S17merge_exclusivity15run_MergeTest10yySiF'
@inline(never)
public func run_MergeTest10(_ N: Int) {
  struct Thing {
    var value: String
