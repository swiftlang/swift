...
// CHECK-INPKG: #Pub.pubVar!modify: (Pub) -> @yield_once () yields (inout Int) -> () : @$s3Lib3PubC6pubVarSivM  // Pub.pubVar.modify
...
// CHECK-INPKG: #Pub.pkgVar!modify: (Pub) -> @yield_once () yields (inout Int) -> () : @$s3Lib3PubC6pkgVarSivM  // Pub.pkgVar.modify
...
// CHECK-INPKG:  method #PubProto.pubVar!modify: <Self where Self : PubProto> (inout Self) -> @yield_once () yields (inout Int) -> () : @$s3Lib3PubCAA0B5ProtoA2aDP6pubVarSivMTW  // protocol witness for PubProto.pubVar.modify in conformance Pub
...
// CHECK-LIB: sil [serialized] [exact_self_class] [canonical] [stage=canonical] [ossa] @$s3Lib3PubCyACSicfC : $@convention(method) (Int, @thick Pub.Type) -> @owned Pub {
// CHECK-LIB:  function_ref @$s3Lib3PubCyACSicfc : $@convention(method) (Int, @owned Pub) -> @owned Pub

// Pub.pkgVar.getter
// CHECK-LIB: sil package [serialized_for_package] [canonical] [stage=canonical] [ossa] @$s3Lib3PubC6pkgVarSivg : $@convention(method) (@guaranteed Pub) -> Int {
...
// CHECK-LIB:    #Pub.pubVar!modify: (Pub) -> @yield_once () yields (inout Int) -> () : @$s3Lib3PubC6pubVarSivM  // Pub.pubVar.modify
...
// CHECK-LIB:    #Pub.pkgVar!modify: (Pub) -> @yield_once () yields (inout Int) -> () : @$s3Lib3PubC6pkgVarSivM  // Pub.pkgVar.modify
...
// CHECK-LIB:    method #PubProto.pubVar!modify: <Self where Self : PubProto> (inout Self) -> @yield_once () yields (inout Int) -> () : @$s3Lib3PubCAA0B5ProtoA2aDP6pubVarSivMTW  // protocol witness for PubProto.pubVar.modify in conformance Pub
