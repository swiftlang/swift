...
if #os( { // expected-error {{expected platform name}} expected-error {{expected ')'}} expected-note {{to match this opening '('}}
}

if #os() { // expected-error {{expected platform name}}
}

if #os(OSX { // expected-error {{expected version comparison}} expected-error {{expected ')'}} expected-note {{to match this opening '('}}
}

if #os(OSX) { // expected-error {{expected version comparison}}
}

if #os(OSX >=  { // expected-error {{expected version number}} expected-error {{expected ')'}} expected-note {{to match this opening '('}} 
}

if #os(OSX >= ) { // expected-error {{expected version number}}
}
