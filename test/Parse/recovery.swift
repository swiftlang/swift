...
func garbage() -> () {
  ...
  return a + "a" // expected-error{{binary operator '+' cannot be applied to operands of type 'Int' and 'String'}} expected-note {{overloads for '+' exist with these partially matching parameter lists: (Int, Int), (String, String), (Int, UnsafeMutablePointer<Memory>), (Int, UnsafePointer<Memory>)}}
}
...
enum Rank: Int {
  case Ace = 1
  case Two = 2.1  // expected-error {{cannot convert initial value of type 'Double' to raw type 'Int'}}
}
