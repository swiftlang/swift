...
func nonexistentMember() {
  ...
  let prop = Foo.nonexistent // expected-error{{value of type 'Foo.Type' has no member 'nonexistent'}}
  let meth = Foo.nonexistent() // expected-error{{value of type 'Foo.Type' has no member 'nonexistent'}}
}
