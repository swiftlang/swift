...
//--- test.swift
@_SwiftifyImport(.countedByOrNull(pointer: .return, count: "len"))
...
@_SwiftifyImport(.countedByOrNull(pointer: .return, count: "len"), .nonescaping(pointer: .return))
...
@_SwiftifyImport(.countedByOrNull(pointer: .return, count: "len2"), .countedByOrNull(pointer: .param(1), count: "len1"), .lifetimeDependence(dependsOn: .param(1), pointer: .return, type: .copy))
public func lifetimeDependentCopy(_ p: UnsafePointer<CInt>, _ len1: CInt, _ len2: CInt) -> UnsafePointer<CInt> {
// expected-error@+1{{missing return in global function expected to return 'UnsafePointer<CInt>' (aka 'UnsafePointer<Int32>')}}
}

@_SwiftifyImport(.countedByOrNull(pointer: .return, count: "len2"), .countedByOrNull(pointer: .param(1), count: "len1"), .lifetimeDependence(dependsOn: .param(1), pointer: .return, type: .borrow))
