...
@_SwiftifyImport(.sizedByOrNull(pointer: .param(1), size: "len"), .nonescaping(pointer: .param(1)))
public func myFunc2(_ ptr: UnsafeMutableRawPointer?, _ len: CInt) {
}

@_SwiftifyImport(.sizedByOrNull(pointer: .param(1), size: "len"), .nonescaping(pointer: .param(1)), .sizedByOrNull(pointer: .param(3), size: "len2"), .nonescaping(pointer: .param(3)))
public func myFunc3(_ ptr: UnsafeMutableRawPointer?, _ len: CInt, _ ptr2: UnsafeMutableRawPointer?, _ len2: CInt) {
}

@_SwiftifyImport(.sizedByOrNull(pointer: .param(1), size: "len"), .sizedByOrNull(pointer: .return, size: "len"), .lifetimeDependence(dependsOn: .param(1), pointer: .return, type: .copy), consumingLifetimebound: true)
