...
//--- test.swift
@_SwiftifyImport(.sizedByOrNull(pointer: .return, size: "len"))
...
@_SwiftifyImport(.sizedByOrNull(pointer: .return, size: "len"), .nonescaping(pointer: .return))
...
@_SwiftifyImport(.sizedByOrNull(pointer: .return, size: "len2"), .sizedByOrNull(pointer: .param(1), size: "len1"), .lifetimeDependence(dependsOn: .param(1), pointer: .return, type: .copy))
public func lifetimeDependentCopy(_ p: UnsafeRawPointer, _ len1: CInt, _ len2: CInt) -> UnsafeRawPointer {
// expected-error@+1{{missing return in global function expected to return 'UnsafeRawPointer'}}
}

@_SwiftifyImport(.sizedByOrNull(pointer: .return, size: "len2"), .sizedByOrNull(pointer: .param(1), size: "len1"), .lifetimeDependence(dependsOn: .param(1), pointer: .return, type: .borrow))
public func lifetimeDependentBorrow(_ p: borrowing UnsafeRawPointer, _ len1: CInt, _ len2: CInt) -> UnsafeRawPointer {
// expected-error@+1{{missing return in global function expected to return 'UnsafeRawPointer'}}
}

@_SwiftifyImport(.sizedByOrNull(pointer: .return, size: "len2"), .sizedByOrNull(pointer: .param(1), size: "len1"), .lifetimeDependence(dependsOn: .param(1), pointer: .return, type: .copy), consumingLifetimebound: true)
