// RUN: %target-swift-frontend -parse-as-library -emit-ir -disable-incremental-llvm-codegen %s | FileCheck %s
// RUN: %target-swift-frontend -parse-as-library -O -emit-ir -disable-incremental-llvm-codegen %s | FileCheck --check-prefix=OPT-CHECK %s
