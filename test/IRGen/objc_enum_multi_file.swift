// RUN: rm -rf %t && mkdir %t
// RUN: %target-swift-frontend -module-name main -primary-file %s %S/Inputs/objc_enum_multi_file_helper.swift -emit-ir -disable-incremental-llvm-codegen | FileCheck %s
...
// RUN: %target-swift-frontend -module-name main -primary-file %s -I %t -DIMPORT -emit-ir -disable-incremental-llvm-codegen | FileCheck %s
