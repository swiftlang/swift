// RUN: %target-swift-frontend -parse-stdlib -primary-file %s -emit-ir -disable-incremental-llvm-codegen -o - -disable-objc-attr-requires-foundation-module | FileCheck %s --check-prefix=CHECK --check-prefix=CHECK-%target-runtime
