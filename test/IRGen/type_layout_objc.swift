// RUN: %target-swift-frontend(mock-sdk: %clang-importer-sdk) -emit-ir -disable-incremental-llvm-codegen %s | FileCheck %s --check-prefix=CHECK --check-prefix=CHECK-%target-ptrsize
