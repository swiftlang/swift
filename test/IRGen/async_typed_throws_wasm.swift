...
// Witness thunk: trailing pair [ind_error, swiftself], then Self + WT.
...
// Thin closure literal: ind_result, swiftasync, ind_error. No swiftself.
// CHECK-LABEL: define internal {{(swifttailcc|swiftcc)}} void @"$s{{[^"]+}}fU_"
// CHECK-SAME: (ptr noalias{{[^,]*}} %{{[0-9]+}}, ptr swiftasync %{{[0-9]+}}, ptr %{{[0-9]+}})

// The witness thunk must NOT use the old `[..., swiftself, ind_error, Self, WT]`
...
// evolution. The entry carries [ind_error, swiftself] and the thunk must
