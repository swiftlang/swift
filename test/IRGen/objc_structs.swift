...
func getFrame(_ g: Gizmo) -> NSRect {
  ...
  // CHECK: call void @objc_msgSend_stret(ptr noalias sret({{.*}}) captures(none) {{.*}}, ptr {{.*}}, ptr {{.*}})
  return g.frame()
}
// CHECK: }
...
func makeRect(_ a: Double, b: Double, c: Double, d: Double) -> NSRect {
  // CHECK: call void @NSMakeRect(ptr noalias sret({{.*}}) captures(none) {{.*}}, double {{.*}}, double {{.*}}, double {{.*}}, double {{.*}})
...
func insetRect(_ r: NSRect, x: Double, y: Double) -> NSRect {
  // CHECK: call void @NSInsetRect(ptr noalias sret({{.*}}) captures(none) {{.*}}, ptr byval({{.*}}) align 8 {{.*}}, double {{.*}}, double {{.*}})
...
func convertRectFromBase(_ v: NSView, r: NSRect) -> NSRect {
  ...
  // CHECK: call void @objc_msgSend_stret(ptr noalias sret({{.*}}) captures(none) {{.*}}, ptr {{.*}}, ptr {{.*}}, ptr byval({{.*}}) align 8 {{.*}})
...
// CHECK:   call void @useStructOfNSStringsInObjC(ptr noalias sret({{.*}}) captures(none) {{%.*}}, ptr byval({{.*}}) align 8 {{%.*}})
