// RUN: %target-swift-frontend -enable-reflection-metadata -enable-reflection-names -emit-ir -disable-incremental-llvm-codegen %S/Inputs/reflection_metadata.swift | FileCheck %s
// RUN: %target-swift-frontend -enable-reflection-metadata -emit-ir -disable-incremental-llvm-codegen %S/Inputs/reflection_metadata.swift | FileCheck %s --check-prefix=STRIP_REFLECTION_NAMES
// RUN: %target-swift-frontend -emit-ir -disable-incremental-llvm-codegen %S/Inputs/reflection_metadata.swift | FileCheck %s --check-prefix=STRIP_REFLECTION_METADATA
