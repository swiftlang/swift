...
public func libraryWrapperSize() -> Int {
  MemoryLayout<TrivialHiddenCStructWrapper>.size
}

public func libraryWrapperAlignment() -> Int {
  MemoryLayout<TrivialHiddenCStructWrapper>.alignment
}

public func libraryWrapperStride() -> Int {
  MemoryLayout<TrivialHiddenCStructWrapper>.stride
}

public func libraryWrapperIsPOD() -> Bool {
  _isPOD(TrivialHiddenCStructWrapper.self)
}

public func libraryWrapperIsBitwiseTakable() -> Bool {
  _isBitwiseTakable(TrivialHiddenCStructWrapper.self)
}
...
public func clientWrapperAlignment() -> Int {
  MemoryLayout<TrivialHiddenCStructWrapper>.alignment
}

public func clientWrapperStride() -> Int {
  MemoryLayout<TrivialHiddenCStructWrapper>.stride
}

public func clientWrapperIsPOD() -> Bool {
  _isPOD(TrivialHiddenCStructWrapper.self)
}

public func clientWrapperIsBitwiseTakable() -> Bool {
  _isBitwiseTakable(TrivialHiddenCStructWrapper.self)
}
...
assert(clientWrapperIsPOD() == libraryWrapperIsPOD())
assert(clientWrapperIsBitwiseTakable() == libraryWrapperIsBitwiseTakable())
...
// We also check that the client and library agree on the layout they derive for TrivialHiddenCStructWrapper. They generated
// IR may not be literally the same, but structurally the LLVM types must be identical.

// LIBRARY-SIDE-DAG: %T7Library27TrivialHiddenCStructWrapperV = type <{ %TSo20TrivialHiddenCStructa, %Ts5Int64V }>
// LIBRARY-SIDE-DAG: %TSo20TrivialHiddenCStructa = type <{ %Ts5Int32V, [4 x i8], %TSd }>
// LIBRARY-SIDE-DAG: %Ts5Int32V = type <{ i32 }>
// LIBRARY-SIDE-DAG: %TSd = type <{ double }>
// LIBRARY-SIDE-DAG: %Ts5Int64V = type <{ i64 }>
