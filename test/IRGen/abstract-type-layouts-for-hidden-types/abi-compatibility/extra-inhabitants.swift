...
public func libraryWrapperIsPOD() -> Bool {
  _isPOD(BoolHiddenCStructWrapper.self)
}

public func libraryWrapperIsBitwiseTakable() -> Bool {
  _isBitwiseTakable(BoolHiddenCStructWrapper.self)
}
...
// The hidden _Bool field provides extra inhabitants (it only needs one bit to store its valid values), allowing Optional to use
...
// LIBRARY-SIDE-DAG: %T7Library24BoolHiddenCStructWrapperVSg = type <{ [1 x i8] }>
// CLIENT-SIDE-DAG: %T7Library24BoolHiddenCStructWrapperVSg = type <{ [1 x i8] }>
