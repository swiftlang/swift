// RUN: %target-swift-frontend -I %S/../Inputs -enable-source-import -emit-ir -disable-incremental-llvm-codegen -enable-resilience %s | FileCheck %s
// RUN: %target-swift-frontend -I %S/../Inputs -enable-source-import -emit-ir -disable-incremental-llvm-codegen -enable-resilience -O %s
