// RUN: %target-swift-frontend -DOPTIN -O -enable-opt-in-reflection-metadata -emit-ir %s | %FileCheck %s --check-prefix=CHECK-OPT-IN
// RUN: %target-swift-frontend -DFULL -O -enable-opt-in-reflection-metadata -emit-ir %s | %FileCheck %s --check-prefix=CHECK-FULL
