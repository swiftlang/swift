// RUN: %target-swift-frontend -primary-file %s -emit-ir -disable-incremental-llvm-codegen -enable-resilience -enable-source-import -I %S/../Inputs | FileCheck %s
// RUN: %target-swift-frontend %s -emit-ir -disable-incremental-llvm-codegen -num-threads 8 -enable-resilience -enable-source-import -I %S/../Inputs | FileCheck %s
