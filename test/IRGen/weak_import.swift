...
// RUN: %swift -module-cache-path %t/clang-module-cache -target x86_64-apple-macosx10.9 -sdk %S/Inputs -I %S/Inputs -enable-source-import -primary-file %s -emit-ir | FileCheck -check-prefix=CHECK-10_9 %s
// RUN: %swift -module-cache-path %t/clang-module-cache -target x86_64-apple-macosx10.10 -sdk %S/Inputs -I %S/Inputs -enable-source-import -primary-file %s -emit-ir | FileCheck -check-prefix=CHECK-10_10 %s
