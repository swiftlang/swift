...
// RUN: %target-swift-frontend -I %t -O -primary-file %s -emit-ir -disable-incremental-llvm-codegen | FileCheck -check-prefix=CHECK -check-prefix=CHECK-NORMAL %s
// RUN: %target-swift-frontend -I %t -O -primary-file %s -enable-testing -emit-ir -disable-incremental-llvm-codegen | FileCheck -check-prefix=CHECK -check-prefix=CHECK-TESTABLE %s
