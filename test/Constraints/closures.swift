...
class SR1069_W<T> {
  func append<Key: AnyObject where Key: Hashable>(_ value: T, forKey key: Key) {}
}
class SR1069_C<T> { let w: SR1069_W<(AnyObject, T) -> ()> = SR1069_W() }
struct S<T> {
  let cs: [SR1069_C<T>] = []

  func subscribe<Object: AnyObject where Object: Hashable>(_ object: Object?, method: (Object, T) -> ()) {
