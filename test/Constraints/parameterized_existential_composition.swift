...
struct Bar<Value> { // expected-note {{arguments to generic parameter 'Value' ('any C' (aka 'any A & B') and 'any D<Int>' (aka 'any A<Int> & B')) are expected to be equal}}
  let value: Value
}
...
func run() {
  ...
  _ = Baz<Int>(bar: bar)
  // expected-error@-1 {{cannot convert value of type 'Bar<any C>' (aka 'Bar<any A & B>') to expected argument type 'Bar<any D<Int>>' (aka 'Bar<any A<Int> & B>')}}
}