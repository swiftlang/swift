...
// RUN: %target-typecheck-verify-swift -verify-ignore-unrelated -swift-version 4 -I %t -verify-ignore-unknown
// RUN: %target-typecheck-verify-swift -verify-ignore-unrelated -swift-version 5 -I %t -verify-ignore-unknown
...
// classes.
var ovl2ResultA = obj.ovl2!() // expected-error{{ambiguous use of 'ovl2()', cannot select between potential result types 'A', 'C'}}
...
var ovl5Result = obj.ovl5!() // expected-error{{ambiguous use of 'ovl5()', cannot select between potential result types 'B', 'C'}}
...
func testAnyObjectAmbiguity(_ x: AnyObject) {
  _ = x.ambiguousProperty // expected-error {{ambiguous use of 'ambiguousProperty', cannot select between potential types 'Int', 'String'}}
  _ = x.unambiguousProperty

  _ = x.ambiguousMethod() // expected-error {{ambiguous use of 'ambiguousMethod()', cannot select between potential result types 'Int', 'String'}}
  ...
  _ = x[ambiguousSubscript: 0] // expected-error {{ambiguous use of 'subscript(ambiguousSubscript:)', cannot select between potential result types 'Int', 'String'}}
  ...
  _ = x[differentSelectors: 0] // expected-error {{ambiguous use of 'subscript(differentSelectors:)', cannot select between potential types '(Int) -> Int', '<Self where Self : P3> (differentSelectors: Int) -> Int'}}
}
