...
f2(f2xy)
f2(f2ab)
f2(f2yx) // expected-error{{function signature '(y: Int, x: Int) -> Int' is not compatible with expected type '(x: Int, y: Int) -> Int'}} expected-note {{use a closure to safely wrap calls to the function}}
