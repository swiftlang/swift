...
>(t: T, u: U) -> (X, Y) {
  return (t.foo, u.foo) // expected-error{{cannot convert return expression of type '(X, X)' to expected result type '(X, Y)'}}
}
...
>(t: T) -> X {
  return t.bar // expected-error{{cannot convert return expression of type 'T.Bar' to expected result type 'X'}}
}
...
>(t: T) -> (Y, Z) {
  return (t.bar, t.bar.foo) // expected-error{{cannot convert return expression of type '(Y, X)' to expected result type '(Y, Z)'}}
}
