...
var d_as_b : B = D()
var b_as_d = B() as! D
var bad_b_as_d : D = B()  // expected-error{{cannot convert initializer of type 'B' to specified type 'D'}}
...
func base_class_archetype_casts<T : B>(t: T) {
  var _ : B = t
  _ = B() as! T
  var _ : T = B() // expected-error{{cannot convert initializer of type 'B' to specified type 'T'}}
