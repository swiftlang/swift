...
struct S1 {
  ...
  func testInitRanking() {
    _ = S1() // Okay
    _ = S1.init() // expected-error {{ambiguous use of 'init', cannot select between potential parameter types '(Int, Int)', '(x: Int, y: Int)'}}
  }
}
...
struct S3 {
  ...
  func testInitRanking() {
    _ = S3() // expected-error {{ambiguous use of 'init', cannot select between potential parameter types '(String...)', '(x: Int...)'}}
  }
}
...
struct S4 {
  ...
  func testInitRanking() {
    _ = S4() // expected-error {{ambiguous use of 'init', cannot select between potential parameter types '(Int)', '(x: Int)'}}
  }
}
...
struct S5 {
  ...
  func testFavoring() {
    let x = 0
    _ = S5((x, 0) ^^^ 0) // expected-error {{ambiguous use of operator '^^^', cannot select between potential result types 'Int', 'String'}}
  }
}
