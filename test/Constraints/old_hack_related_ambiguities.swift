...
do {
  ...
  func performLocal(v: Int, block: @escaping (Int) throws -> Void) async throws {
    try await test(over: v, block: block) // expected-error {{ambiguous use of 'test', cannot select between potential parameter types '(over: Int, other: String, block: @escaping (Int) throws -> Void)', '(over: Int, that: String, block: @escaping (Int) throws -> Void)'}}
  }
