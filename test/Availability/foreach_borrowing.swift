...
func testForLoopOverSpanWithoutAvailability(_ seq: Span<Int>) {
  for x in seq { // expected-error {{for-in loop requires 'Span<Int>' to conform to 'Iterable', which is only available in macOS 9999 or newer}}
...
extension BorrowingFallbackNoSequence: Iterable {
  typealias Element = Int
  typealias IterableIterator = IterableIteratorAdapter<IndexingIterator<[Int]>>
  func makeIterableIterator() -> IterableIteratorAdapter<IndexingIterator<[Int]>> {
    IterableIteratorAdapter(iterator: [1, 2, 3].makeIterator())
  }
}

func testInvalidBorrowingNoSequence(seq: BorrowingFallbackNoSequence) {
  for x in seq { // expected-error {{for-in loop requires 'BorrowingFallbackNoSequence' to conform to 'Iterable', which is only available in macOS 9999 or newer}}
  // expected-note@-2 {{add '@available' attribute to enclosing global function}}
  // expected-note@-2 {{add 'if #available' version check}}
    _ = x
  }
