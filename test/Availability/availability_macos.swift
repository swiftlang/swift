...
func testFarFuture(_ s: S) {
  ambiguousInFarFuture(.init()) // expected-error {{ambiguous use of 'init()', cannot select between potential result types 'A', 'B'}}
  s.ambiguousInFarFuture(.init()) // expected-error {{ambiguous use of 'init()', cannot select between potential result types 'A', 'B'}}
}
