...
// RUN: %target-build-swift-dylib(%t/%target-library-name(MultiModuleProtocol2)) %S/Inputs/MultiModuleProtocol/file2.swift \
// RUN:   -emit-module -emit-module-path %t/MultiModuleProtocol2.swiftmodule -module-name MultiModuleProtocol2 -I%t %target-rpath(%t)
// RUN: %target-build-swift-dylib(%t/%target-library-name(MultiModuleProtocol3)) %S/Inputs/MultiModuleProtocol/file3.swift \
// RUN:   -emit-module -emit-module-path %t/MultiModuleProtocol3.swiftmodule -module-name MultiModuleProtocol3 -I%t %target-rpath(%t)
...
// RUN:   -I%t -L%t %s -lm -lMultiModuleProtocol1 -lMultiModuleProtocol2 -lMultiModuleProtocol3 -o %t/a.out %target-rpath(%t)
