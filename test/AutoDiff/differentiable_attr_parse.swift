...
@differentiable(3) // expected-error {{expected a configuration, e.g. 'wrt:', 'primal:', 'adjoint:', 'jvp:', or 'vjp:'}}
func bar(_ x: Float, _: Float) -> Float {
  return 1 + x
}

@differentiable(foo(_:_:)) // expected-error {{expected a configuration, e.g. 'wrt:', 'primal:', 'adjoint:', 'jvp:', or 'vjp:'}}
func bar(_ x: Float, _: Float) -> Float {
  return 1 + x
}

@differentiable(wrt: (.0), foo(_:_:)) // expected-error {{expected a configuration, e.g. 'wrt:', 'primal:', 'adjoint:', 'jvp:', or 'vjp:'}}
func bar(_ x: Float, _: Float) -> Float {
  return 1 + x
}
