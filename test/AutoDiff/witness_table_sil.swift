...
struct S : Proto, VectorProtocol {
  ...
  // CHECK-LABEL: sil {{.*}} @AD__{{.*}}function1{{.*}}_jvp_SSU : $@convention(witness_method: Proto) (Float, Double, @in_guaranteed S) -> (@out Float, @owned @callee_guaranteed (@in_guaranteed Float, @in_guaranteed Double) -> @out Float)
  ...
  // CHECK-LABEL: sil {{.*}} @AD__{{.*}}function1{{.*}}_vjp_SSU : $@convention(witness_method: Proto) (Float, Double, @in_guaranteed S) -> (@out Float, @owned @callee_guaranteed (@in_guaranteed Float) -> (@out Float, @out Double))
  ...
  // CHECK-LABEL: sil {{.*}} @AD__{{.*}}function2{{.*}}_jvp_SSS : $@convention(witness_method: Proto) (Float, Double, @in_guaranteed S) -> (@out Float, @owned @callee_guaranteed (@in_guaranteed Float, @in_guaranteed Double, @in_guaranteed S) -> @out Float)
  ...
  // CHECK-LABEL: sil {{.*}} @AD__{{.*}}function2{{.*}}_vjp_SSS : $@convention(witness_method: Proto) (Float, Double, @in_guaranteed S) -> (@out Float, @owned @callee_guaranteed (@in_guaranteed Float) -> (@out Float, @out Double, @out S))
  ...
  // CHECK-LABEL: sil {{.*}} @AD__{{.*}}function3{{.*}}_jvp_USU : $@convention(witness_method: Proto) (Float, Double, @in_guaranteed S) -> (@out Double, @owned @callee_guaranteed (@in_guaranteed Double) -> @out Double)
  ...
  // CHECK-LABEL: sil {{.*}} @AD__{{.*}}function3{{.*}}_vjp_USU : $@convention(witness_method: Proto) (Float, Double, @in_guaranteed S) -> (@out Double, @owned @callee_guaranteed (@in_guaranteed Double) -> @out Double)
