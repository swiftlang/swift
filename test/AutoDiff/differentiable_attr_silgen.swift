...
// CHECK-LABEL: sil [differentiable wrt 0, 1 adjoint @dfoo] @foo
...
// CHECK-LABEL: sil [differentiable wrt 0, 1 adjoint @dfoo_indir_ret] @foo_indir_ret : $@convention(thin) <T> (Float, @in T) -> @out T {
...
// CHECK-LABEL: sil [differentiable wrt 0, 1, 2, 3, 4, 5 adjoint @dfoo_tuple] @foo_tuple : $@convention(thin) (Float, Float, Float, Float, Float, Float) -> Float
