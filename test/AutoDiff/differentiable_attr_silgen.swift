...
// CHECK-LABEL: sil [reverse_differentiable wrt 0, 1 primal @foo adjoint @dfoo] @foo
...
// CHECK-LABEL: sil [reverse_differentiable wrt 0, 1 primal @foo_indir_ret adjoint @dfoo_indir_ret] @foo_indir_ret : $@convention(thin) <T> (Float, @in T) -> @out T {
...
// CHECK-LABEL: sil [reverse_differentiable wrt 0, 1, 2, 3, 4, 5 primal @foo_tuple adjoint @dfoo_tuple] @foo_tuple : $@convention(thin) (Float, Float, Float, Float, Float, Float) -> Float
