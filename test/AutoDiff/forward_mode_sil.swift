...
// CHECK-SIL-LABEL: sil hidden [ossa] @AD__unary__jvp_src_0_wrt_0 : $@convention(thin) (Float) -> (@out Float, @owned @callee_guaranteed (@in_guaranteed Float) -> @out Float) {
// CHECK-SIL: bb0([[JVP_ORIG_RESULT:%.*]] : $*Float, [[X_ARG:%.*]] : $Float):
// CHECK-SIL:   [[FUNC_1:%.*]] = function_ref @$sSf1moiyS2f_SftFZ : $@convention(method) (Float, Float, @thin Float.Type) -> Float
// CHECK-SIL:   [[JVP_1:%.*]] = function_ref @AD__$sSf1moiyS2f_SftFZ__jvp_src_0_wrt_0_1 : $@convention(method) (Float, Float, @thin Float.Type) -> (@out Float, @owned @callee_guaranteed (@in_guaranteed Float, @in_guaranteed Float) -> @out Float)
// CHECK-SIL:   [[VJP_1:%.*]] = function_ref @AD__$sSf1moiyS2f_SftFZ__vjp_src_0_wrt_0_1 : $@convention(method) (Float, Float, @thin Float.Type) -> (@out Float, @owned @callee_guaranteed (@in_guaranteed Float) -> (@out Float, @out Float))
...
// CHECK-SIL:   [[DIFF_1:%.*]] = apply [[AUTODIFF_EXTRACT_INST_1]]([[ORIG_RESULT_1_BUF:%.*]], [[X_ARG]], [[X_ARG]], {{.*}})
// CHECK-SIL:   [[ORIG_RESULT_1:%.*]] = load [trivial] [[ORIG_RESULT_1_BUF]]
...
// CHECK-SIL:   [[JVP_2:%.*]] = function_ref @AD__$sSf1moiyS2f_SftFZ__jvp_src_0_wrt_0_1 : $@convention(method) (Float, Float, @thin Float.Type) -> (@out Float, @owned @callee_guaranteed (@in_guaranteed Float, @in_guaranteed Float) -> @out Float)
// CHECK-SIL:   [[VJP_2:%.*]] = function_ref @AD__$sSf1moiyS2f_SftFZ__vjp_src_0_wrt_0_1 : $@convention(method) (Float, Float, @thin Float.Type) -> (@out Float, @owned @callee_guaranteed (@in_guaranteed Float) -> (@out Float, @out Float))
...
// CHECK-SIL:   [[DIFF_2:%.*]] = apply [[AUTODIFF_EXTRACT_INST_1]]([[ORIG_RESULT_2_BUF:%.*]], [[ORIG_RESULT_1]], [[X_ARG]], {{.*}})
// CHECK-SIL:   [[ORIG_RESULT_2:%.*]] = load [trivial] [[ORIG_RESULT_2_BUF]]
...
// CHECK-SIL:   [[DIFF_REF:%.*]] = function_ref @AD__unary__differential_src_0_wrt_0 : $@convention(thin) (@in_guaranteed Float, @owned _AD__unary_bb0__DF__src_0_wrt_0) -> @out Float
// CHECK-SIL:   [[DIFF:%.*]] = partial_apply [callee_guaranteed] [[DIFF_REF]]([[DIFF_STRUCT]])
// CHECK-SIL:   store [[ORIG_RESULT_2]] to [trivial] [[JVP_ORIG_RESULT]]
// CHECK-SIL:   return [[DIFF]]
...
// CHECK-SIL-LABEL: sil hidden [ossa] @AD__binary__jvp_src_0_wrt_0_1 : $@convention(thin) (Float, Float) -> (@out Float, @owned @callee_guaranteed (@in_guaranteed Float, @in_guaranteed Float) -> @out Float) {
// CHECK-SIL: bb0([[JVP_ORIG_RESULT:%.*]] : $*Float, [[X_ARG:%.*]] : $Float, [[Y_ARG:%.*]] : $Float):
// CHECK-SIL:   [[FUNC:%.*]] = function_ref @$sSf1moiyS2f_SftFZ : $@convention(method) (Float, Float, @thin Float.Type) -> Float
// CHECK-SIL:   [[JVP:%.*]] = function_ref @AD__$sSf1moiyS2f_SftFZ__jvp_src_0_wrt_0_1 : $@convention(method) (Float, Float, @thin Float.Type) -> (@out Float, @owned @callee_guaranteed (@in_guaranteed Float, @in_guaranteed Float) -> @out Float)
// CHECK-SIL:   [[VJP:%.*]] = function_ref @AD__$sSf1moiyS2f_SftFZ__vjp_src_0_wrt_0_1 : $@convention(method) (Float, Float, @thin Float.Type) -> (@out Float, @owned @callee_guaranteed (@in_guaranteed Float) -> (@out Float, @out Float))
...
// CHECK-SIL:   [[DIFF_1:%.*]] = apply [[AUTODIFF_EXTRACT_INST]]([[ORIG_RESULT_BUF:%.*]], [[X_ARG]], [[Y_ARG]], {{.*}})
// CHECK-SIL:   [[ORIG_RESULT:%.*]] = load [trivial] [[ORIG_RESULT_BUF]]
// CHECK-SIL:   [[DIFF_1_THUNKED:%.*]] = partial_apply [callee_guaranteed] {{%.*}}([[DIFF_1]])
// CHECK-SIL:   [[DIFF_STRUCT:%.*]] = struct $_AD__binary_bb0__DF__src_0_wrt_0_1 ([[DIFF_1_THUNKED]] : $@callee_guaranteed (Float, Float) -> Float)
// CHECK-SIL:   [[DIFF_REF:%.*]] = function_ref @AD__binary__differential_src_0_wrt_0_1 : $@convention(thin) (@in_guaranteed Float, @in_guaranteed Float, @owned _AD__binary_bb0__DF__src_0_wrt_0_1) -> @out Float
// CHECK-SIL:   [[DIFF:%.*]] = partial_apply [callee_guaranteed] [[DIFF_REF]]([[DIFF_STRUCT]])
// CHECK-SIL:   store [[ORIG_RESULT]] to [trivial] [[JVP_ORIG_RESULT]]
