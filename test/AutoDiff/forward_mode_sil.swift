...
// CHECK-SIL:   [[DIFF_STRUCT:%.*]] = struct $_AD__unary_bb0__DF__src_0_wrt_0 ([[MULT_DIFFERENTIAL_1]] : $@callee_guaranteed (Float, Float) -> Float, [[MULT_DIFFERENTIAL_2]] : $@callee_guaranteed (Float, Float) -> Float)
...
// CHECK-SIL:   [[TUPLE_RESULT:%.*]] = tuple ([[ORIGINAL_VAL_2]] : $Float, [[PARTIAL_APP_DIFFERENTIAL]] : $@callee_guaranteed (Float) -> Float)
// CHECK-SIL:   return [[TUPLE_RESULT]] : $(Float, @callee_guaranteed (Float) -> Float)
...
// CHECK-SIL:   release_value [[TEMP_TAN_1]] : $Float
// CHECK-SIL:   release_value [[X_TAN]] : $Float
...
// CHECK-SIL:   [[MULT_JVP_APPLY_TUPLE:%.*]] = apply [[MULT_FUNC_JVP]]([[X_ARG]], [[Y_ARG]], %4) : $@convention(method) (Float, Float, @thin Float.Type) -> (Float, @owned @callee_guaranteed (Float, Float) -> Float)
// CHECK-SIL:   release_value [[AUTODIFF_INST]] : $@differentiable @convention(method) (Float, Float, @nondiff @thin Float.Type) -> Float
// CHECK-SIL:   [[ORIGINAL_VAL:%.*]] = tuple_extract [[MULT_JVP_APPLY_TUPLE]] : $(Float, @callee_guaranteed (Float, Float) -> Float), 0
// CHECK-SIL:   [[MULT_DIFFERENTIAL:%.*]] = tuple_extract [[MULT_JVP_APPLY_TUPLE]] : $(Float, @callee_guaranteed (Float, Float) -> Float), 1
// CHECK-SIL:   [[DIFF_STRUCT:%.*]] = struct $_AD__binary_bb0__DF__src_0_wrt_0_1 ([[MULT_DIFFERENTIAL]] : $@callee_guaranteed (Float, Float) -> Float)
...
// CHECK-SIL:   [[TUPLE_RESULT:%.*]] = tuple ([[ORIGINAL_VAL]] : $Float, [[PARTIAL_APP_DIFFERENTIAL]] : $@callee_guaranteed (Float, Float) -> Float)
// CHECK-SIL:   return [[TUPLE_RESULT]] : $(Float, @callee_guaranteed (Float, Float) -> Float)
...
// CHECK-SIL:   retain_value [[X_TAN]] : $Float
// CHECK-SIL:   retain_value [[Y_TAN]] : $Float
...
// CHECK-SIL:   release_value [[X_TAN]] : $Float
// CHECK-SIL:   release_value [[Y_TAN]] : $Float
// CHECK-SIL:   return [[TAN_RESULT]] : $Float
