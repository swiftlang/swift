...
// RUN: rm -rf %t
// RUN: mkdir %t
// RUN: %swift %clang-importer-sdk -I %S/Inputs/custom-modules/ -module-cache-path %t/clang-module-cache -emit-module -o %t %s
// RUN: %swift %clang-importer-sdk -I %S/Inputs/custom-modules/ -module-cache-path %t/clang-module-cache -parse-as-library %t/imports.swiftmodule -parse -emit-objc-header-path %t/imports.h -import-objc-header %S/../Inputs/empty.h
