...
// RUN: rm -rf %t
// RUN: mkdir %t
// RUN: %swift %clang-importer-sdk -I %S/Inputs/custom-modules/ -emit-module -o %t %s
// RUN: %swift %clang-importer-sdk -I %S/Inputs/custom-modules/ -parse-as-library %t/imports.swiftmodule -parse -emit-objc-header-path %t/imports.h -import-objc-header %S/../Inputs/empty.h
