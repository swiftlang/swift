...
// `self` is the method's `this`, which the caller owns, so it has no ownership
// modifier.

extension Rejections {
  // expected-error@+2{{instance method cannot be marked '@cxx' because 'self' is 'consuming'; a C++ method receives 'this' from its caller, so the implementation must take 'self' with default ownership}}
  @cxx @implementation
  consuming func consumingMethod() -> Int32 { return value }

  // expected-error@+2{{instance method cannot be marked '@cxx' because 'self' is 'borrowing'; a C++ method receives 'this' from its caller, so the implementation must take 'self' with default ownership}}
  @cxx @implementation
  borrowing func borrowingMethod() -> Int32 { return value }
}
...
extension Polymorphic {
  // expected-error@+2{{instance method 'virtualMethod()' cannot implement C++ function 'virtualMethod' because it is the key function of C++ class 'Polymorphic'}}
  @cxx @implementation
...
extension Widget {
  ...
  // expected-error@+2{{instance method 'describe()' cannot implement C++ function 'describe' because it is the key function of C++ class 'Widget'}}
  @cxx @implementation
