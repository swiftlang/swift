...
// FIXME: %target-build-swift %t/XMLParser.swift -enable-library-evolution -emit-library -emit-module -emit-module-path %t/artifacts/MyFoundationXML.swiftmodule -module-name MyFoundationXML -I %t/include -cxx-interoperability-mode=upcoming-swift -o %t/artifacts/%target-library-name(MyFoundationXML)
// FIXME: %target-build-swift %t/Check.swift -I %t/artifacts -L %t/artifacts -lMyFoundationXML -o %t/artifacts/uses-library -index-store-path %t/index-store  -Xfrontend -index-system-modules -Xfrontend -parse-stdlib
