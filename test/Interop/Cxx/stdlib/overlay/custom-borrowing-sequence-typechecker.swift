...
func checkIterable<S>(_ seq: borrowing S) where S: CxxIterable, S: ~Copyable {
  ...
  // expected-note@-4 {{where 'S' = 'ConstRACButNotIterableIteratorSequence'}}
  var _ = seq.makeIterableIterator()
}
...
// ConstRACButNotIterableIteratorSequence doesn't conform to CxxIterable because operator* 
// (the dereference operator) doesn't return a reference. However, it still conforms to CxxRandomAccessCollection.
checkIterable(ConstRACButNotIterableIteratorSequence()) 
// expected-error@-1 {{global function 'checkIterable' requires that 'ConstRACButNotIterableIteratorSequence' conform to 'CxxIterable'}}
checkRandomAccessCollection(ConstRACButNotIterableIteratorSequence())
