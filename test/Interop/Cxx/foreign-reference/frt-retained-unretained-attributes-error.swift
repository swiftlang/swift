...
let frtLocalVar3 = StructWithAPIsReturningCxxFrt.StaticMethodReturningCxxFrt() // expected-note {{'StaticMethodReturningCxxFrt()' returns a SWIFT_SHARED_REFERENCE type, but isn't annotated with SWIFT_RETURNS_RETAINED or SWIFT_RETURNS_UNRETAINED}}
let frtLocalVar4 = StructWithAPIsReturningCxxFrt.StaticMethodReturningCxxFrtWithAnnotation()
let frtLocalVar5 = global_function_returning_cxx_frt() // expected-note {{'global_function_returning_cxx_frt()' returns a SWIFT_SHARED_REFERENCE type, but isn't annotated with SWIFT_RETURNS_RETAINED or SWIFT_RETURNS_UNRETAINED}}
...
let x = returnFRTOverloadedOperators() // expected-note {{'returnFRTOverloadedOperators()' returns a SWIFT_SHARED_REFERENCE type, but isn't annotated with SWIFT_RETURNS_RETAINED or SWIFT_RETURNS_UNRETAINED}}
let y = returnFRTOverloadedOperators() // expected-note {{'returnFRTOverloadedOperators()' returns a SWIFT_SHARED_REFERENCE type, but isn't annotated with SWIFT_RETURNS_RETAINED or SWIFT_RETURNS_UNRETAINED}}
let z = x + y
let w = x - y
...
let _ = DefaultOwnershipSuppressUnannotatedAPIWarning.returnRefType() // expected-note {{'returnRefType()' returns a SWIFT_SHARED_REFERENCE type, but isn't annotated with SWIFT_RETURNS_RETAINED or SWIFT_RETURNS_UNRETAINED}}
...
let _ = DefaultOwnershipInheritance.createBaseTypeNonDefault() // expected-note {{'createBaseTypeNonDefault()' returns a SWIFT_SHARED_REFERENCE type, but isn't annotated with SWIFT_RETURNS_RETAINED or SWIFT_RETURNS_UNRETAINED}}
let _ = DefaultOwnershipInheritance.createDerivedTypeNonDefault() // expected-note {{'createDerivedTypeNonDefault()' returns a SWIFT_SHARED_REFERENCE type, but isn't annotated with SWIFT_RETURNS_RETAINED or SWIFT_RETURNS_UNRETAINED}}
let _ = DefaultOwnershipInheritance.createDerivedTypeNonDefaultUnretained()
let  _ = SourceLocationCaching.FactoryA.make() // expected-note {{'make()' returns a SWIFT_SHARED_REFERENCE type, but isn't annotated with SWIFT_RETURNS_RETAINED or SWIFT_RETURNS_UNRETAINED}}
let  _ = SourceLocationCaching.FactoryB.make() // expected-note {{'make()' returns a SWIFT_SHARED_REFERENCE type, but isn't annotated with SWIFT_RETURNS_RETAINED or SWIFT_RETURNS_UNRETAINED}}
