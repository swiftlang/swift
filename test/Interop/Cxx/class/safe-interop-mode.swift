...
struct Unannotated { // expected-note 2 {{type 'Unannotated' has unknown escapability because its member 'pointer' is a pointer or reference, and Swift cannot tell whether it owns what it points to}}
    Unannotated();
    int *pointer;
};
...
struct UnknownEscapabilityAggregate { // expected-note {{type 'UnknownEscapabilityAggregate' has unknown escapability because its escapability depends on 'Unannotated', whose escapability is unknown}}
...
struct ConditionalMemberBeforePointer { // expected-note {{type 'ConditionalMemberBeforePointer' has unknown escapability because its member 'pointer' is a pointer or reference, and Swift cannot tell whether it owns what it points to}}
    SafeEscapableIf cond;
    int *pointer;
};

struct ConditionalMemberAfterPointer { // expected-note {{type 'ConditionalMemberAfterPointer' has unknown escapability because its member 'pointer' is a pointer or reference, and Swift cannot tell whether it owns what it points to}}
    int *pointer;
    SafeEscapableIf cond;
};

struct SharedPtrBeforePointer { // expected-note {{type 'SharedPtrBeforePointer' has unknown escapability because its member 'pointer' is a pointer or reference, and Swift cannot tell whether it owns what it points to}}
    std::shared_ptr<int> shared;
    int *pointer;
};

struct MyContainer {
    int begin() const { return 0; } // expected-note {{'begin' is unsafe because 'begin' and 'end' are assumed to return iterators, which do not keep the underlying storage alive}}
...
void unsafeFunc(View v1 [[clang::noescape]], View v2); // expected-note {{'unsafeFunc' is unsafe because its non-escapable parameter 'v2' has no lifetime annotation}}

// expected-warning@+1{{the returned type 'View' is annotated as non-escapable; its lifetime dependencies must be annotated}}
View returnsViewNoAnnotation(const Owner &o); // expected-note {{'returnsViewNoAnnotation' is unsafe because it returns a non-escapable value whose lifetime Swift had to infer; annotate the C++ declaration with SWIFT_LIFETIMEBOUND to say what the result depends on}}

struct InferredNonEscapable {
  View v;
};
InferredNonEscapable returnsInferredNonEscapable(const Owner &o); // expected-note {{'returnsInferredNonEscapable' is unsafe because it returns a non-escapable value whose lifetime Swift had to infer; annotate the C++ declaration with SWIFT_LIFETIMEBOUND to say what the result depends on}}
...
struct HasUnannotatedViewGetter {
  ...
  View getView() const; // expected-note {{'getView' is unsafe because it returns a non-escapable value whose lifetime Swift had to infer; annotate the C++ declaration with SWIFT_LIFETIMEBOUND to say what the result depends on}}
};
...
struct SWIFT_NONESCAPABLE ViewWithUnannotatedCtor {
    ViewWithUnannotatedCtor(const Owner &o); // expected-note {{'ViewWithUnannotatedCtor' is unsafe because it returns a non-escapable value whose lifetime Swift had to infer; annotate the C++ declaration with SWIFT_LIFETIMEBOUND to say what the result depends on}}
private:
    const int *member;
};
...
Owner returnsOwnerLifetimebound(const View &v [[clang::lifetimebound]]); // expected-note {{'returnsOwnerLifetimebound' is unsafe because its lifetime annotation is not enforced: the result is Escapable, so Swift drops the dependency}}
...
class SharedObject {
public:
  View getView() const [[clang::lifetimebound]]; // expected-note {{'getView' is unsafe because Swift cannot represent the lifetime annotation on 'self', which is imported as a class}}
private:
  int *p;
} SWIFT_SHARED_REFERENCE(retainSharedObject, releaseSharedObject);

View getViewFromSharedObject(SharedObject* p [[clang::lifetimebound]]); // expected-note {{'getViewFromSharedObject' is unsafe because Swift cannot represent the lifetime annotation on parameter 'p', which is imported as a class}}
...
template <typename T> struct PassThru { // expected-note {{type 'PassThru' has unknown escapability because its escapability depends on 'IsUnsafe', whose escapability is unknown}}
  T field;
};
struct IsUnsafe { int *p; }; // expected-note 2 {{type 'IsUnsafe' has unknown escapability because its member 'p' is a pointer or reference, and Swift cannot tell whether it owns what it points to}}
struct HasUnsafe : TTake2<PassThru<HasUnsafe>, IsUnsafe> {}; // expected-note {{type 'HasUnsafe' has unknown escapability because its escapability depends on 'IsUnsafe', whose escapability is unknown}}
using AlsoUnsafe = PassThru<HasUnsafe>;

struct SWIFT_UNSAFE ExplicitlyUnsafeStruct {};
struct HasUnsafeMember { // expected-note {{type 'HasUnsafeMember' is unsafe because 'ExplicitlyUnsafeStruct' is annotated unsafe in C++}}
  HasUnsafeMember();
  ExplicitlyUnsafeStruct member;
};

struct HasUnsafeBase : ExplicitlyUnsafeStruct { // expected-note {{type 'HasUnsafeBase' is unsafe because 'ExplicitlyUnsafeStruct' is annotated unsafe in C++}}
  HasUnsafeBase();
};
