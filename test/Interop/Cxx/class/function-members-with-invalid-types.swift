...
func ok() {
  // At the moment, we still eagerly import function members when we analyze
  // the storage of imported StructDecls, so using GoodStruct by value (below)
  // still leads to spurious errors (below). For now, we just check that we can
  // indirectly refer to GoodStruct without triggering instantiation errors.
