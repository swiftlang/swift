...
u[unit] = unit // expected-error {{ambiguous use of 'subscript(_:)', cannot select between potential result types 'Num', 'Overloaded.Unit', 'UInt32'}}
               // FIXME: ^this should complain about 'u' being a 'let' constant
...
let uGetRef: UnsafeMutablePointer<UInt32>? = u[getRef] // expected-error {{cannot use mutating getter on immutable value: 'u' is a 'let' constant}}
                                                       // FIXME: ^this should complain about 'u' being a 'let' constant
u[getRef] = uGetRef // expected-error {{ambiguous use of 'subscript(_:)', cannot select between potential result types 'Num', 'UInt32', 'UnsafeMutablePointer<UInt32>?'}}
                    // FIXME: ^this should complain about 'u' being a 'let' constant
