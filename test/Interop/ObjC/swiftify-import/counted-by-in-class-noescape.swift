...
// RUN: %target-swift-frontend -emit-module -plugin-path %swift-plugin-dir -o %t/CountedByClassSpan.swiftmodule -I %t/Inputs -enable-experimental-feature SafeInteropWrappers -enable-experimental-feature LifetimeDependence %t/counted-by-class-noescape.swift -dump-macro-expansions  -swift-version 6 2>&1 | %FileCheck %s

//--- Inputs/module.modulemap
module CountedByClassNoescapeClang {
    header "counted-by-class-noescape.h"
    export *
}

//--- Inputs/counted-by-class-noescape.h
#pragma once
...
// CHECK-LABEL: extension CountedByClassSpan {
// CHECK-NEXT:      @_alwaysEmitIntoClient @available(macOS 9999, *) @lifetime(p: copy p)
// CHECK-NEXT:      public class func simple(_ p: inout MutableSpan<Int32>) {
...
// CHECK-NEXT:      public class func shared(_ len: Int32, _ p1: inout MutableSpan<Int32>, _ p2: inout MutableSpan<Int32>) {
...
// CHECK-NEXT:      }
// CHECK-NEXT:      @_alwaysEmitIntoClient @available(macOS 9999, *) @lifetime(p: copy p)
// CHECK-NEXT:      public class func complexExpr(_ len: Int32, _ offset: Int32, _ p: inout MutableSpan<Int32>) {
...
// CHECK-NEXT:      }
// CHECK-NEXT:      @_alwaysEmitIntoClient @available(macOS 9999, *) @lifetime(p: copy p)
// CHECK-NEXT:      public class func nullUnspecified(_ p: inout MutableSpan<Int32>) {
// CHECK-NEXT:          return unsafe p.withUnsafeMutableBufferPointer { _pPtr in
...
// CHECK-NEXT:      }
// CHECK-NEXT:      @_alwaysEmitIntoClient @available(macOS 9999, *) @lifetime(p: copy p)
// CHECK-NEXT:      public class func nonnull(_ p: inout MutableSpan<Int32>) {
// CHECK-NEXT:          return unsafe p.withUnsafeMutableBufferPointer { _pPtr in
...
// CHECK-NEXT:      }
// CHECK-NEXT:      @_alwaysEmitIntoClient @available(macOS 9999, *) @lifetime(p: copy p)
// CHECK-NEXT:      public class func nullable(_ p: inout MutableSpan<Int32>?) {
// CHECK-NEXT:          return { () in
// CHECK-NEXT:              return if p == nil {
...
// CHECK-NEXT:      public class func returnPointer(_ len: Int32, _ p: inout MutableSpan<Int32>) -> MutableSpan<Int32> {
...
// CHECK-NEXT:      }
// CHECK-NEXT:      @_alwaysEmitIntoClient @available(macOS 9999, *) @lifetime(p: copy p)
// CHECK-NEXT:      public class func staticMethod(_ p: inout MutableSpan<Int32>) {
// CHECK-NEXT:          return unsafe p.withUnsafeMutableBufferPointer { _pPtr in
...
__attribute__((swift_attr("@_SwiftifyImportProtocol(.method(name: \"open func swiftAttr(_ len: Int32, _ p: UnsafeMutablePointer<Int32>!)\", paramInfo: [.countedBy(pointer: .param(2), count: \"len\"), .nonescaping(pointer: .param(2))]), availability: [\"Span\": \"macOS 9999\", \"MutableSpan\": \"macOS 9999\"])")))
@interface SwiftAttrClass
 - (void)swiftAttr:(int)len :(int *)p;
@end

// CHECK:      extension SwiftAttrClass {
// CHECK-NEXT:     @_alwaysEmitIntoClient @available(macOS 9999, *) @lifetime(p: copy p)
...
//--- counted-by-class-noescape.swift
import CountedByClassNoescapeClang
import Cxx

@available(macOS 9999, *)
@lifetime(p1: copy p1) @lifetime(p2: copy p2)
public func call(p1: inout MutableSpan<CInt>, p2: inout MutableSpan<CInt>, p3: inout MutableSpan<CInt>?, x: CInt, y: CInt, a: CountedByClassSpan, b: SwiftAttrClass) {
  a.simple(&p1)
  a.shared(x, &p1, &p2)
