...
// RUN: %target-swift-frontend -swift-version 5 -enable-library-evolution -emit-module -emit-module-interface-path %t/test.swiftinterface -plugin-path %swift-plugin-dir -o %t/test.swiftmodule %t/test.swift -I %t -enable-experimental-feature StabilizedSafeInteropWrappers -strict-memory-safety \
// RUN:   -verify -verify-additional-file %t%{fs-sep}test.h

// Verify that including the binary module works as expected
// RUN: %target-swift-frontend -swift-version 5 -emit-module -o %t/depender.swiftmodule %t/depender.swift -I %t -enable-experimental-feature StabilizedSafeInteropWrappers -strict-memory-safety \
// RUN:   -verify -verify-additional-file %t%{fs-sep}test.h
