...
// RUN: %target-swift-frontend %t/ConformLib.swift -module-name ConformLib -clang-header-expose-decls=all-public -emit-module -emit-module-path %t/ConformLib.swiftmodule -emit-clang-header-path %t/ConformLib.h -cxx-interoperability-mode=upcoming-swift -parse-as-library
