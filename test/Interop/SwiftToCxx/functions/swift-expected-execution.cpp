...
void checkResult(Operation operation, Validate validate) {
  ...
  auto failure = operation(true);
  assert(!failure.has_value());
  auto error = failure.error();
  assert(error.template as<Functions::NaiveErrors>().get() ==
...
void checkResults() {
  ...
  auto smallValue = Functions::SmallResult::init(21, false).value();
  checkResult([&](bool fail) { return smallValue.doubled(fail); }, small);
  auto refValue = Functions::RefResult::init(false).value();
  ...
  checkResult(
      ...
      ref);
  auto largeValue = Functions::LargeResult::init(false).value();
  checkResult(
