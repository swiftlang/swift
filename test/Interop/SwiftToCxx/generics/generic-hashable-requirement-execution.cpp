...
// RUN: %target-interop-build-swift %t/generics.swift -o %t/swift-generics-execution -Xlinker %t/swift-generics-execution.o -module-name Generics -Xfrontend -entry-point-function-name -Xfrontend swiftMain -enable-experimental-feature GenerateBindingsForHashableRequirementsInCXX
...
// RUN: %target-interop-build-swift %t/generics.swift -o %t/non-hashable-execution -Xlinker %t/non-hashable-execution.o -module-name Generics -Xfrontend -entry-point-function-name -Xfrontend swiftMain -enable-experimental-feature GenerateBindingsForHashableRequirementsInCXX
...
// Inherits the Hashable conformance of its superclass.
@_expose(Cxx)
public final class HashableSubclass: HashableClass {
...
public struct HashableMethodHost {
    ...
    public func hashableOverloadBeforeMarker<Value: Hashable>(
        _ value: Value
    ) -> Int {
        return 2
    }

    public func hashableOverloadBeforeMarker<Value: Sendable>(
        _ value: Value
    ) -> Int {
        return 1
    }
...
// CHECK-NEXT: NOT EQUAL
  {
    // A Swift class, and a subclass that inherits the conformance of its
    // superclass.
    auto lhs = Generics::HashableClass::init(1);
    auto rhs = Generics::HashableClass::init(1);
    assert(Generics::genericAreEqual(lhs, rhs));
    assert(!Generics::genericAreEqual(lhs, Generics::HashableClass::init(2)));
    auto sub = Generics::HashableSubclass::init(3);
    assert(Generics::genericAreEqual(sub, Generics::HashableSubclass::init(3)));
    assert(!Generics::genericAreEqual(sub, Generics::HashableSubclass::init(4)));
  }
  {
  ...
  }
  {
    ...
    // An overload with only a marker protocol requirement wins over a
    // Hashable-constrained one, which would abort for a non-Hashable type.
    assert(host.hashableOverloadBeforeMarker(nonHashable) == 1);
  }
  {
