...
// RUN: %target-swift-frontend %s -target %target-cpu-apple-macos15.0 -module-name EmbeddedGenerics -enable-experimental-feature Embedded -clang-header-expose-decls=all-public -enable-experimental-feature GenerateBindingsForHashableRequirementsInCXX -typecheck -verify -emit-clang-header-path %t/embedded-generics.h
// RUN: %FileCheck %s --implicit-check-not='getConformanceWitnessTable<' < %t/embedded-generics.h
