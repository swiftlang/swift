// RUN: %target-swift-frontend -Xllvm -tf-promote-global-variables -Xllvm -tf-dump-intermediates -Onone -emit-sil -verify %s
// RUN: %target-swift-frontend -Xllvm -tf-promote-global-variables -Xllvm -tf-dump-intermediates -Onone -emit-sil -verify %s | %FileCheck %s
import TensorFlow
