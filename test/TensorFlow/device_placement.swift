...
public func explicitDevicePlacementGPU() {
  let x : Tensor<Float> = #tfop("Const", dtype: Float.self, value$tensor: 1.0, __device: "/job:localhost/replica:0/task:0/device:GPU:0")
  _hostOp(x)
}

// CHECK-LABEL: --- TFPartition Accelerator Result: {{.*}}explicitDevicePlacementGPU{{.*}}
// CHECK: string_literal utf8 "/job:localhost/replica:0/task:0/device:GPU:0"
// CHECK: builtin "__tfop_Const,dtype,value$tensor,__device
...
public func explicitDevicePlacementAll() {
  ...
  let x_cpu : Tensor<Float> = #tfop("Identity", x, __shapes: [TensorShape()], __device: "/job:localhost/replica:0/task:0/device:CPU:0")
  let y_cpu : Tensor<Float> = #tfop("Const", dtype: Float.self, value$tensor: 1.0, __shapes: [TensorShape()], __device: "/job:localhost/replica:0/task:0/device:CPU:0")
  // y is sent from CPU to TPU.
  let z_tpu : Tensor<Float> = #tfop("Add", x_cpu, y_cpu , __shapes: [TensorShape()], __device: "/job:localhost/replica:0/task:0/device:TPU:0")
  _hostOp(z_tpu)
}
...
public func GPUToTPUTransfer_Unsupported() {
  let x : Tensor<Float> = #tfop("Const", dtype: Float.self, value$tensor: 1.0, __shapes: [TensorShape()], __device: "/job:localhost/replica:0/task:0/device:GPU:0")
  // expected-error @+1 {{TPU infeed enqueue cannot run on this device}}
  let _ : Tensor<Float> = #tfop("Identity", x, __device: "/job:localhost/replica:0/task:0/device:TPU:0")
}
