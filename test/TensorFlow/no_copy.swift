...
// graph: that there are no host/accelerator copies generated (other than
// returning tensor values to the host).  This tests a combination of the
// partitioning pass being able to recognize various forms, but also checks that
// certain ops implementations are promotable as well.
...
public func SR8313(_ n: Int) -> Tensor<Double> {
  var t = Tensor(1)
  var i = 0
  while i < n {
    t += 1
