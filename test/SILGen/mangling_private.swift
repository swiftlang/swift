...
// RUN: %target-swift-frontend -enable-sil-ownership-verifier -emit-module -o %t %S/Inputs/mangling_private_helper.swift
// RUN: %target-swift-emit-silgen -enable-sil-ownership-verifier %S/Inputs/mangling_private_helper.swift | %FileCheck %s -check-prefix=CHECK-BASE

// RUN: %target-swift-emit-silgen %s -I %t -enable-sil-ownership-verifier | %FileCheck %s

// RUN: cp %s %t
// RUN: %target-swift-emit-silgen %t/mangling_private.swift -I %t -enable-sil-ownership-verifier | %FileCheck %s

// RUN: cp %s %t/other_name.swift
// RUN: %target-swift-emit-silgen %t/other_name.swift -I %t -enable-sil-ownership-verifier -module-name mangling_private | %FileCheck %s -check-prefix=OTHER-NAME
