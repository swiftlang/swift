...
// RUN: %target-swift-frontend -emit-silgen -parse-stdlib -module-name Swift -enable-experimental-feature Embedded -target x86_64-apple-macosx14 %s | %FileCheck %s
...
// CHECK-LABEL: sil hidden [ossa] @${{[se]}}s1fyyyySbXEXBF : $@convention(thin) (@guaranteed @convention(block) @noescape (@convention(block) @noescape (Bool) -> ()) -> ()) -> ()
func f(_: @convention(block) ((Bool) -> ()) -> ()) {}

// CHECK-LABEL: sil hidden [ossa] @${{[se]}}s1gyyyS2bXEXBF : $@convention(thin) (@guaranteed @convention(block) @noescape (@convention(block) @noescape (Bool) -> Bool) -> ()) -> ()
