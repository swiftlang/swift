...
func ifexpr() -> Int {
  ...
  return x;
  // CHECK-LABEL: sil  @_TF13sil_locations6ifexprFT_Si
...
func ifelseexpr() -> Int {
  ...
  return x;
  // CHECK-LABEL: sil  @_TF13sil_locations10ifelseexprFT_Si
...
func ifexpr_return() -> Int {
  ...
  return 6;
  // CHECK-LABEL: sil  @_TF13sil_locations13ifexpr_returnFT_Si
  // CHECK: apply {{.*}} line:[[@LINE-5]]:6
  // CHECK: cond_br {{%.*}}, [[TRUE_BB:bb[0-9]+]], [[FALSE_BB:bb[0-9]+]] // {{.*}} line:[[@LINE-6]]:6
  // CHECK: [[TRUE_BB]]:
...
func ifexpr_rval() -> Int {
  var x = true ? 5 : 6;
  return x;
  // CHECK-LABEL: sil  @_TF13sil_locations11ifexpr_rvalFT_Si
...
func forstmt_empty_cond(var i: Int) -> Int {
  for i=0;;++i {}
    // CHECK-LABEL: sil  @{{.*}}forstmt_empty_cond{{.*}}
...
func simpleDirectCallTest(i: Int) -> Int {
  ...
  // CHECK-LABEL: sil  @_TF13sil_locations20simpleDirectCallTest
...
func foo(x: Int) -> Int {
  ...
  return bar(1)
  // CHECK-LABEL: sil  @_TF13sil_locations3foo
...
func multipleReturnsImplicitAndExplicit() {
  ...
  x++;
  // CHECK-LABEL: sil  @_TF13sil_locations34multipleReturnsImplicitAndExplicitFT_T_
  // CHECK: cond_br
...
func simplifiedImplicitReturn() -> () {
  var y = 0 
  // CHECK-LABEL: sil  @_TF13sil_locations24simplifiedImplicitReturnFT_T_
...
// CHECK-LABEL:sil @_TF13sil_locations10testSwitchFT_T_
func testSwitch() {
  var x:Int
  x = 0
...
func testIf() {
  ...
  // CHECK-LABEL:sil @_TF13sil_locations6testIfFT_T_
  //
...
func testFor() {
  ...
  // CHECK-LABEL: sil @_TF13sil_locations7testForFT_T_
...
func captures_tuple<T, U>(x: (T, U)) -> () -> (T, U) {
  return {x}
  // CHECK-LABEL: sil @_TF13sil_locations14captures_tuple
...
func interpolated_string(x: Int, y: String) -> String {
  ...
  // CHECK-LABEL: sil @_TF13sil_locations19interpolated_string
...
func tuple_element(x: (Int, Float)) {
  int(tuple().0)
  // CHECK-LABEL: sil @_TF13sil_locations13tuple_element
...
func containers() -> (Int[], Dictionary<String, Int>) {
  ...
  // CHECK: alloc_array $(String, Int),                            {{.*}} line:[[@LINE-11]]:22
...
func test_isa_2(p: P) {
  ...
  // CHECK-LABEL: sil @_TF13sil_locations10test_isa_2
...
func printSinglePayloadAddressOnly(v:SinglePayloadAddressOnly) {
  ...
  // CHECK_LABEL: sil @_TF13sil_locations29printSinglePayloadAddressOnly
  // CHECK: bb0
...
func testStringForEachStmt() {
  ...
  // CHECK-LABEL: sil @_TF13sil_locations21testStringForEachStmtFT_T_
...
func testForStmt() {
  ...
  // CHECK-LABEL: sil @_TF13sil_locations11testForStmtFT_T_
...
func testDoWhile() {
  ...
  // CHECK-LABEL: sil @_TF13sil_locations11testDoWhileFT_T_
...
func testWhile() {
  ...
  // CHECK-LABEL: sil @_TF13sil_locations9testWhileFT_T_
  // CHECK: br         {{.*}} line:[[@LINE-9]]:3
