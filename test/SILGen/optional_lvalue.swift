...
func assign_optional_lvalue(x: inout Int?, _ y: Int) {
  x! = y
}
...
// CHECK:         assign {{%.*}} to [[PAYLOAD]]
func assign_iuo_lvalue(x: inout Int!, _ y: Int) {
  x! = y
}
...
func assign_iuo_lvalue_implicit(s: inout S!, _ y: Int) {
  s.x = y
}
...
func assign_optional_lvalue_reabstracted(x: inout (Int -> Int)?,
                                         _ y: Int -> Int) {
  x! = y
}
...
func assign_optional_lvalue_computed(x: inout S?, _ y: Int) -> Int {
  x!.computed = y
  return x!.computed
}
...
func assign_bound_optional_lvalue(x: inout Int?) {
  x? = generate_int()
}
