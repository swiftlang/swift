...
// CHECK-NEXT: sil hidden [ossa] @$s30nonisolated_inherits_isolation0A5AsyncyyYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor) -> () {
...
// CHECK-NEXT: Isolation: caller_isolation_inheriting
// CHECK-NEXT: sil hidden [ossa] @$s30nonisolated_inherits_isolation16unspecifiedAsyncyyYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor) -> () {
// CHECK: bb0([[ARG:%.*]] : @guaranteed $Builtin.ImplicitIsolationActor):
// CHECK:   hop_to_executor [[ACTOR]]
// CHECK:   [[FUNC_REF:%.*]] = function_ref @$s30nonisolated_inherits_isolation22unspecifiedAsyncCalleeyyYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor) -> ()
...
struct NonisolatedStruct {
  ...
  // CHECK-NEXT: // Isolation: caller_isolation_inheriting
  // CHECK-NEXT: sil hidden [ossa] @$s30nonisolated_inherits_isolation17NonisolatedStructV12asynchronousACyt_tYacfC : $@convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, @thin NonisolatedStruct.Type) -> NonisolatedStruct {
  ...
  // CHECK-NEXT: // Isolation: caller_isolation_inheriting
  // CHECK-NEXT: sil hidden [ossa] @$s30nonisolated_inherits_isolation17NonisolatedStructV11asyncMethodyyYaF : $@convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, NonisolatedStruct) -> () {
...
// CHECK-NEXT: // Isolation: caller_isolation_inheriting
// CHECK-NEXT: sil hidden [ossa] @$s30nonisolated_inherits_isolation20useNonisolatedStructyyYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor) -> () {
// CHECK: bb0([[ACTOR:%.*]] :
// CHECK:   hop_to_executor [[ACTOR]]
...
// CHECK:   [[ASYNC_CALL:%.*]] = function_ref @$s30nonisolated_inherits_isolation17NonisolatedStructV11asyncMethodyyYaF : $@convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, NonisolatedStruct) -> ()
...
// CHECK-NEXT: // Isolation: caller_isolation_inheriting
// CHECK-NEXT: sil hidden [ossa] @$s30nonisolated_inherits_isolation21useNonisolatedStruct2yyYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor) -> () {
// CHECK: bb0([[ACTOR:%.*]] :
// CHECK:   hop_to_executor [[ACTOR]]
// CHECK:   [[VALUE:%.*]] = apply {{%.*}}([[ACTOR]], {{%.*}}) : $@convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, @thin NonisolatedStruct.Type) -> NonisolatedStruct
// CHECK:   hop_to_executor [[ACTOR]]
// CHECK:   [[VAR_DECL:%.*]] = move_value [var_decl] [[VALUE]]
// CHECK:   apply {{%.*}}([[ACTOR]], [[VAR_DECL]]) : $@convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, NonisolatedStruct) -> ()
// CHECK:   hop_to_executor [[ACTOR]]
// CHECK:   extend_lifetime [[VAR_DECL]]
...
class MainActorKlass {
  ...
  // CHECK:   [[B_ACTOR_COPY_EX_OPT_B_CAST:%.*]] = unchecked_value_cast [[B_ACTOR_COPY_EX_OPT_B]] to $Builtin.ImplicitIsolationActor
  // CHECK:   apply {{%.*}}([[B_ACTOR_COPY_EX_OPT_B_CAST]], [[B_VAR_DECL_2]]) : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, @guaranteed NonSendableKlass) -> ()
  // CHECK:   hop_to_executor [[B_ACTOR]]
...
struct TestVarUse {
  var test: Int {
    // CHECK-LABEL: sil hidden [ossa] @$s30nonisolated_inherits_isolation10TestVarUseV4testSivg : $@convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, TestVarUse) -> Int
    get async {
      42
    }
  }
...
// CHECK-LABEL: sil hidden [ossa] @$s30nonisolated_inherits_isolation12testUseOfVar1tyAA04TestgE0V_tYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, TestVarUse) -> ()
// CHECK: bb0([[ISOLATION:%.*]] : @guaranteed $Builtin.ImplicitIsolationActor, [[BASE:%.*]] : $TestVarUse)
// CHECK:   [[GETTER:%.*]] = function_ref @$s30nonisolated_inherits_isolation10TestVarUseV4testSivg : $@convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, TestVarUse) -> Int
