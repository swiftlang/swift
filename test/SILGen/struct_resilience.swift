...
// RUN: %target-swift-frontend -module-name struct_resilience -emit-module -enable-resilience -emit-module-path=%t/resilient_struct.swiftmodule -enable-sil-ownership-verifier -module-name=resilient_struct %S/../Inputs/resilient_struct.swift
// RUN: %target-swift-emit-silgen -module-name struct_resilience -I %t -enable-sil-ownership-verifier -enable-resilience %s | %FileCheck %s
