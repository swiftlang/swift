...
func testNonCopyableIterable(seq: borrowing Span<NoncopyableInt>) {
  ...
  // CHECK: = function_ref @$ss12SpanIteratorVsRi_zrlE04nextA08maxCounts0A0VyxGSi_tF : $@convention(method) <τ_0_0 where τ_0_0 : ~Copyable> (Int, @lifetime(copy 1) @inout SpanIterator<τ_0_0>) -> @lifetime(borrow address_for_deps 1) @owned Span<τ_0_0>
...
func testForEachLocations(seq: borrowing Span<Int>, val: Int) {
  ...
  // CHECK: [[NEXT_SPAN:%.*]] = function_ref @$ss12SpanIteratorVsRi_zrlE04nextA08maxCounts0A0VyxGSi_tF {{.*}}, loc "{{.*}}":[[@LINE+27]]:3
