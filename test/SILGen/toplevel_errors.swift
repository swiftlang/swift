...
// CHECK: sil @main
// CHECK: [[ERR:%.*]] = alloc_existential_box $_ErrorType, $MyError
// CHECK: [[T0:%.*]] = function_ref @_TFO15toplevel_errors7MyError1AFMS0_S0_
// CHECK: [[T1:%.*]] = apply [[T0]](
// CHECK: store [[T1]] to [[ERR]]#1 : $*MyError
// CHECK: builtin "willThrow"([[ERR]]#0 : $_ErrorType)
// CHECK: br bb3([[ERR]]#0 : $_ErrorType)
...
// CHECK: bb3([[T0:%.*]] : $_ErrorType):
// CHECK: builtin "errorInMain"([[T0]] : $_ErrorType)
