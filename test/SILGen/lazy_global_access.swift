// RUN: %target-swift-emit-silgen -enable-sil-ownership-verifier %s | %FileCheck %s -check-prefix=MAIN
// RUN: %target-swift-emit-silgen -parse-as-library -enable-sil-ownership-verifier %s | %FileCheck %s -check-prefix=LIBRARY
