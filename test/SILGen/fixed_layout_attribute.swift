// RUN: %target-swift-emit-silgen -enable-sil-ownership-verifier -parse-as-library %s | %FileCheck %s --check-prefix=FRAGILE --check-prefix=CHECK
// RUN: %target-swift-emit-silgen -enable-resilience -enable-sil-ownership-verifier -parse-as-library %s | %FileCheck %s --check-prefix=RESILIENT --check-prefix=CHECK

// RUN: %target-swift-emit-silgen -enable-sil-ownership-verifier -parse-as-library -enable-testing %s
// RUN: %target-swift-emit-silgen -enable-sil-ownership-verifier -parse-as-library -enable-testing -enable-resilience %s
