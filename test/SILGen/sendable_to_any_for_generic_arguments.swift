...
struct TestGeneral {
  ...
  // CHECK: [[FUNCV_REF:%.*]] = struct_extract {{%.*}} : $TestGeneral, #TestGeneral.funcV
  // CHECK-NEXT: {{%.*}} = unchecked_trivial_bit_cast [[FUNCV_REF]] : $S<(Array<any Sendable>) -> Optional<any Sendable>> to $S<(Array<Any>) -> Optional<Any>>
  // CHECK: [[DATA_REF:%.*]] = struct_extract {{%.*}} : $Test, #<abstract function>Test.data
