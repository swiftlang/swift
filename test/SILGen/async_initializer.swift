...
struct MyStruct {
  ...
  // NI-NS-DAG: sil hidden [ossa] @$s12initializers8MyStructVACyYacfC : $@convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, @thin MyStruct.Type) -> MyStruct {
  init() async {}
}

enum MyEnum {
  // NI-DAG: sil hidden [ossa] @$s12initializers6MyEnumOACyYacfC : $@convention(method) @async (@thin MyEnum.Type) -> MyEnum
  // NI-NS-DAG: sil hidden [ossa] @$s12initializers6MyEnumOACyYacfC : $@convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, @thin MyEnum.Type) -> MyEnum {
  init() async {}
}
...
actor MyActor {
  ...
  // NI-NS-NEXT: [[ISOLATION_1_B_CAST:%.*]] = unchecked_value_cast [[ISOLATION_1_B]] : $Optional<any Actor> to $Builtin.ImplicitIsolationActor
...
class EarthPerson : Person {
  // NI-NS-LABEL: sil hidden [exact_self_class] [ossa] @$s12initializers11EarthPersonCACyYacfC : $@convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, @thick EarthPerson.Type) -> @owned EarthPerson {
  // NI-NS: bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitIsolationActor,
  ...
  // NI-NS-DAG: sil hidden [ossa] @$s12initializers11EarthPersonCACyYacfc : $@convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, @owned EarthPerson) -> @owned EarthPerson {
  required init() async {}
  ...
  // NI-NS-DAG: sil hidden [ossa] @$s12initializers11EarthPersonC4nameACSS_tYacfc : $@convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, @owned String, @owned EarthPerson) -> @owned EarthPerson {
  ...
  // NI-NS-DAG: sil private [transparent] [thunk] [ossa] @$s12initializers11EarthPersonCAA0C0A2aDPxyYacfCTW : $@convention(witness_method: Person) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, @thick EarthPerson.Type) -> @out EarthPerson {
}
...
class NorthAmericaPerson : EarthPerson {
  ...
  // NI-NS-DAG: sil hidden [ossa] @$s12initializers18NorthAmericaPersonCACyYacfc : $@convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, @owned NorthAmericaPerson) -> @owned NorthAmericaPerson {
  ...
  // NI-NS-DAG: sil hidden [ossa] @$s12initializers18NorthAmericaPersonC4nameACSS_tYacfc : $@convention(method) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, @owned String, @owned NorthAmericaPerson) -> @owned NorthAmericaPerson {
...
class Cat {
  ...
  // NI-NS:          {{%[0-9]+}} = apply {{%[0-9]+}}({{%.*}}) : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor) -> ()
...
// NI-NS-LABEL:  sil hidden [ossa] @$s12initializers7makeCatyyYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor) -> () {
// NI-NS:        bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitIsolationActor):
// NI-NS:          hop_to_executor [[ACTOR]]
...
// NI-NS-LABEL: sil hidden [ossa] @$s12initializers7makeDogyyYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor) -> () {
// NI-NS: bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitIsolationActor):
// NI-NS:      hop_to_executor [[ACTOR]]
...
// NI-NS-LABEL:  sil hidden [ossa] @$s12initializers8makeBirbyyYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor) -> () {
// NI-NS:        bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitIsolationActor):
...
// NI-NS-LABEL: sil hidden [ossa] @$s12initializers20makeActorFromGenericAA04SomeC0CyYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor) -> @owned SomeActor {
// NI-NS:      bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitIsolationActor):
// NI-NS:          hop_to_executor [[ACTOR]]
// NI-NS:          apply {{%.*}}({{%.*}})
// NI-NS-NEXT:     hop_to_executor [[ACTOR]]
...
// NI-NS-LABEL: sil hidden [ossa] @$s12initializers26callActorMethodFromGeneric1ayAA04SomeC0C_tYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, @guaranteed SomeActor) -> () {
...
// NI-NS-LABEL: sil hidden{{.*}} [ossa] @$s12initializers15makeActorInTaskyyYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor) -> () {
// NI-NS:       bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitIsolationActor):
// NI-NS:          hop_to_executor [[ACTOR]]
// NI-NS:          apply
...
// NI-NS-LABEL: sil hidden{{.*}} [ossa] @$s12initializers21callActorMethodInTask1ayAA04SomeC0C_tYaF : $@convention(thin) @async (@sil_isolated @sil_implicit_leading_param @guaranteed Builtin.ImplicitIsolationActor, @guaranteed SomeActor) -> () {
// NI-NS: bb0([[ACTOR:%.*]] : @guaranteed $Builtin.ImplicitIsolationActor, [[ARG:%.*]] : @guaranteed $SomeActor):
// NI-NS:   hop_to_executor [[ACTOR]]
