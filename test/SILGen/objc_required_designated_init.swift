...
// RUN: %target-swift-frontend -emit-module -enable-sil-ownership-verifier %S/Inputs/objc_required_designated_init_2.swift -module-name Booms -o %t/Booms.swiftmodule -import-objc-header %S/Inputs/objc_required_designated_init.h
// RUN: %target-swift-emit-silgen -I %t -enable-sil-ownership-verifier -verify %s -import-objc-header %S/Inputs/objc_required_designated_init.h | %FileCheck %s
