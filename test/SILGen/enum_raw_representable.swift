// RUN: %target-swift-emit-silgen -enable-sil-ownership-verifier -emit-sorted-sil %s | %FileCheck %s
// RUN: %target-swift-emit-silgen -enable-sil-ownership-verifier -emit-sorted-sil -enable-resilience %s | %FileCheck -check-prefix=CHECK-RESILIENT %s
