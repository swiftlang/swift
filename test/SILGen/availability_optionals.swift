...
func referenceToPotentiallyUnavailableGlobalVariable() {
  ...
  // CHECK: [[GLOBAL_ADDRESSOR:%.*]] = function_ref @_TF22availability_optionalsau22globalAvailableOn10_10Si : $@convention(thin) () -> Builtin.RawPointer
  // CHECK: [[GLOBAL_POINTER:%.*]] = apply [[GLOBAL_ADDRESSOR]]() : $@convention(thin) () -> Builtin.RawPointer
  // CHECK: [[GLOBAL_ADDR:%.*]] = pointer_to_address [[GLOBAL_POINTER]] : $Builtin.RawPointer to $*Int
...
func referenceToPotentiallyUnavailableGlobalFunction() {
  ...
  // CHECK: [[AVAILABLE]]:
  // CHECK: [[DATA_ADDR:%.*]] = init_enum_data_addr [[OPT_ADDR]]#1 : $*Optional<() -> Int>, #Optional.Some!enumelt.1
  // CHECK: [[FUNC_VAL:%.*]] = function_ref @_TF22availability_optionals20funcAvailableOn10_10FT_Si : $@convention(thin) () -> Int
  // CHECK: [[THICK_FUNC_VAL:%.*]] = thin_to_thick_function [[FUNC_VAL]] : $@convention(thin) () -> Int to $@callee_owned () -> Int
  // CHECK: [[REABSTRACTION_THUNK_HELPER:%.*]] = function_ref @[[HELPER_NAME:.*]] : $@convention(thin) (@out Int, @in (), @owned @callee_owned () -> Int) -> ()
  // CHECK: [[STORED_VAL:%.*]] = partial_apply [[REABSTRACTION_THUNK_HELPER]]([[THICK_FUNC_VAL]]) : $@convention(thin) (@out Int, @in (), @owned @callee_owned () -> Int) -> ()
