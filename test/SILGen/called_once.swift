...
// CHECK-LABEL: sil hidden [ossa] @$s11called_once12testCallOnceyyyyXOnF : $@convention(thin) (@owned @called(once) @callee_guaranteed () -> ()) -> () {
// CHECK: bb0([[F:%.*]] : @owned $@called(once) @callee_guaranteed () -> ()):
// CHECK:   [[LOCAL:%.*]] = alloc_box ${ let @called(once) @callee_guaranteed () -> () }, let, name "local"
