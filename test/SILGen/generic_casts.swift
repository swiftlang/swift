...
// RUN: %target-swift-emit-silgen -module-name generic_casts -Xllvm -sil-full-demangle -enable-sil-ownership-verifier %s | %FileCheck --check-prefix=CHECK --check-prefix=CHECK-%target-runtime %s
