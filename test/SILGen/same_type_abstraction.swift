...
extension MyProtocol where Data == (ReadData, ReadData) {
  // CHECK-LABEL: sil hidden @_T021same_type_abstraction10MyProtocolPAaaBRz8ReadDataQz_AEt0G0RtzlE07currentG0AE_AEtyF : $@convention(method) <Self where Self : MyProtocol, Self.Data == (Self.ReadData, Self.ReadData)> (@in_guaranteed Self) -> (@out Self.ReadData, @out Self.ReadData)
