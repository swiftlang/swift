// RUN: %target-swift-emit-silgen-ossa -enable-sil-opaque-values -Xllvm -sil-print-types -sdk %S/Inputs %s -enable-objc-interop
...
// CHECK:   [[NSANSE:%.*]] = function_ref @$sSo6NSAnseySo7AnsibleCSgADFTo : $@convention(c) (Optional<Ansible>) -> @autoreleased Optional<Ansible>
...
// CHECK:   [[NOPROTO:%.*]] = function_ref @$sSo14hasNoPrototypeyyFTo : $@convention(c) () -> ()
// CHECK:   apply [[NOPROTO]]()
...
// CHECK:   [[FUNC:%.*]] = function_ref @$sSo6NSAnseySo7AnsibleCSgADFTo : $@convention(c) (Optional<Ansible>) -> @autoreleased Optional<Ansible>
