...
// RUN: %swift -emit-silgen -module-cache-path=%t/clang-module-cache -sdk=%S/Inputs %s | FileCheck %s
...
// CHECK:  [[NATIVE_BUF:%.*]] = alloc_stack $String
// CHECK:  initialize_var [[NATIVE_BUF]]#1
// CHECK:  apply [[NSSTRING_TO_STRING]]([[RES_BRIDGED]], [[NATIVE_BUF]]#1)
// CHECK:  [[RES_NATIVE:%.*]] = load [[NATIVE_BUF]]#1
// CHECK:  dealloc_stack [[NATIVE_BUF]]#0 : $*[local_storage] String
...
// CHECK:   [[NATIVE_STR_VALUE:%.*]] = struct_extract [[NATIVE_IN]] : $String, #str_value
// CHECK:   [[NATIVE_STR_OWNER:%.*]] = struct_extract [[NATIVE_STR_VALUE]] : $StringByteData, #owner
// CHECK:   retain [[NATIVE_STR_OWNER]]
// CHECK:   [[STRING_TO_NSSTRING:%.*]] = function_ref @swift_StringToNSString
// CHECK:   [[NATIVE_TMP:%.*]] = alloc_stack $String
// CHECK:   store [[NATIVE_IN]] to [[NATIVE_TMP]]#1
// CHECK:   [[BRIDGED:%.*]] = apply [[STRING_TO_NSSTRING]]([[NATIVE_TMP]]#1)
// CHECK:   apply [[SET_FOO]]([[F]], [[BRIDGED]])
// CHECK:   release [[BRIDGED]]
// CHECK:   dealloc_stack [[NATIVE_TMP]]#0
// CHECK:   [[NATIVE_STR_VALUE:%.*]] = struct_extract [[NATIVE_IN]] : $String, #str_value
// CHECK:   [[NATIVE_STR_OWNER:%.*]] = struct_extract [[NATIVE_STR_VALUE]] : $StringByteData, #owner
...
class Bas : NSObject {
  ...
  // CHECK:   [[STRING_TO_NSSTRING:%.*]] = function_ref @swift_StringToNSString
  // CHECK:   [[PROP_TEMP:%.*]] = alloc_stack $String
  // CHECK:   store [[PROP]] to [[PROP_TEMP]]#1
  // CHECK:   [[NSSTR:%.*]] = apply [[STRING_TO_NSSTRING]]([[PROP_TEMP]]#1)
  ...
  // CHECK:   [[NSSTRING_TO_STRING:%.*]] = function_ref @swift_NSStringToString
  // CHECK:   [[STR_TEMP:%.*]] = alloc_stack $String
  // CHECK:   apply [[NSSTRING_TO_STRING]]([[VALUE]], [[STR_TEMP]]#1)
  // CHECK:   [[STR:%.*]] = load [[STR_TEMP]]#1
  ...
  // CHECK:   [[STRING_TO_NSSTRING:%.*]] = function_ref @swift_StringToNSString
  // CHECK:   [[STR_TEMP:%.*]] = alloc_stack $String
  // CHECK:   store [[STR]] to [[STR_TEMP]]#1
  // CHECK:   [[NSSTR:%.*]] = apply [[STRING_TO_NSSTRING]]([[STR_TEMP]]#1)
  // CHECK:   autorelease_return [[NSSTR]]
  // CHECK: }
  ...
  // CHECK:   [[NSSTRING_TO_STRING:%.*]] = function_ref @swift_NSStringToString
  // CHECK:   [[STR_TEMP:%.*]] = alloc_stack $String
  // CHECK:   apply [[NSSTRING_TO_STRING]]([[NSSTR]], [[STR_TEMP]]#1)
  // CHECK:   [[STR:%.*]] = load [[STR_TEMP]]#1
