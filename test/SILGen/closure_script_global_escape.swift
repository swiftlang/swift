// RUN: %target-swift-emit-silgen -module-name foo -enable-sil-ownership-verifier %s | %FileCheck %s
// RUN: %target-swift-emit-sil -module-name foo -enable-sil-ownership-verifier -verify %s
