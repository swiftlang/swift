...
struct Doit {
  ...
  // CHECK: [[MARK1:%.*]] = mark_unresolved_noncopyable [assignable_but_not_consumable] [[ACCESS1]]
  // CHECK: [[GEP1:%.*]] = struct_element_addr [[MARK1]]
  // CHECK: [[ACCESS2:%.*]] = begin_access [modify] [dynamic] [[ADDR2]]
  // CHECK: [[MARK2:%.*]] = mark_unresolved_noncopyable [assignable_but_not_consumable] [[ACCESS2]]
  ...
  // CHECK: [[MARK3:%.*]] = mark_unresolved_noncopyable [assignable_but_not_consumable] [[ACCESS3]]
  // CHECK: [[GEP3:%.*]] = struct_element_addr [[MARK3]]
  // CHECK: [[ACCESS4:%.*]] = begin_access [modify] [dynamic] [[ADDR4]]
  // CHECK: [[MARK4:%.*]] = mark_unresolved_noncopyable [assignable_but_not_consumable] [[ACCESS4]]
