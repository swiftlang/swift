// RUN: %target-swift-frontend(mock-sdk: %clang-importer-sdk) -Xllvm -sil-print-types -emit-silgen -module-name Test -parse-as-library %s -verify -unavailable-decl-optimization=stub -disable-experimental-serialize-debug-info | %FileCheck %s --check-prefixes=CHECK
