...
func foo(y: consuming String, z: String) -> () -> String {
    ...
    // CHECK:   [[MARK:%.*]] = mark_unresolved_noncopyable [assignable_but_not_consumable] [[YACCESS]]
    // CHECK:   [[UNWRAP:%.*]] = moveonlywrapper_to_copyable_addr [[MARK]]
    // CHECK:   assign [[ZCOPY]] to [[UNWRAP]]
    y = z

    // CHECK:   [[YACCESS:%.*]] = begin_access [read] [unknown] [[Y]]
    // CHECK:   [[MARK:%.*]] = mark_unresolved_noncopyable [no_consume_or_assign] [[YACCESS]]
...
struct Butt {
    ...
    consuming func merged(with other: Butt) -> () -> Butt {
        ...
        // CHECK:   [[MARK:%.*]] = mark_unresolved_noncopyable [assignable_but_not_consumable] [[SELFACCESS]]
        // CHECK:   [[UNWRAP:%.*]] = moveonlywrapper_to_copyable_addr [[MARK]]
        // CHECK:   assign [[OCOPY]] to [[UNWRAP]]
        self = other

        // CHECK:   [[SELFACCESS:%.*]] = begin_access [read] [unknown] [[SELF]]
        // CHECK:   [[MARK:%.*]] = mark_unresolved_noncopyable [no_consume_or_assign] [[SELFACCESS]]
