...
struct LetFieldStruct {
  let letk = Kraken()
  var vark = Kraken()

  // CHECK-LABEL: sil hidden @_TFV15guaranteed_self14LetFieldStruct10letkMethodfS0_FT_T_ : $@convention(method) (@guaranteed LetFieldStruct) -> () {
  // CHECK: bb0([[SELF:%.*]] : $LetFieldStruct):
  // CHECK: [[KRAKEN:%.*]] = struct_extract [[SELF]] : $LetFieldStruct, #LetFieldStruct.letk
  // CHECK-NEXT: [[KRAKEN_FUN:%.*]] = class_method [[KRAKEN]] : $Kraken, #Kraken.enrage!1 : Kraken -> () -> () , $@convention(method) (@guaranteed Kraken) -> ()
  // CHECK-NEXT: apply [[KRAKEN_FUN]]([[KRAKEN]])
  // CHECK-NEXT: [[KRAKEN:%.*]] = struct_extract [[SELF]] : $LetFieldStruct, #LetFieldStruct.letk
  // CHECK-NEXT: strong_retain [[KRAKEN]]
  // CHECK: [[DESTROY_SHIP_FUN:%.*]] = function_ref @_TF15guaranteed_self11destroyShipFCS_6KrakenT_ : $@convention(thin) (@owned Kraken) -> ()
  // CHECK-NEXT: strong_retain [[KRAKEN]]
  // CHECK-NEXT: apply [[DESTROY_SHIP_FUN]]([[KRAKEN]])
  // CHECK-NEXT: [[KRAKEN_BOX:%.*]] = alloc_box $Kraken
  // CHECK-NEXT: [[KRAKEN2:%.*]] = struct_extract [[SELF]]
  // CHECK-NEXT: strong_retain [[KRAKEN2]]
  // CHECK-NEXT: store [[KRAKEN2]] to [[KRAKEN_BOX]]#1
  // CHECK: [[DESTROY_SHIP_FUN:%.*]] = function_ref @_TF15guaranteed_self11destroyShipFCS_6KrakenT_ : $@convention(thin) (@owned Kraken) -> ()
  // CHECK-NEXT: [[KRAKEN_COPY:%.*]] = load [[KRAKEN_BOX]]#1
  ...
  // CHECK-LABEL: sil hidden @_TFV15guaranteed_self14LetFieldStruct10varkMethodfS0_FT_T_ : $@convention(method) (@guaranteed LetFieldStruct) -> () {
  // CHECK: bb0([[SELF:%.*]] : $LetFieldStruct):
  // CHECK: [[KRAKEN:%.*]] = struct_extract [[SELF]] : $LetFieldStruct, #LetFieldStruct.vark
  // CHECK-NEXT: [[KRAKEN_FUN:%.*]] = class_method [[KRAKEN]] : $Kraken, #Kraken.enrage!1 : Kraken -> () -> () , $@convention(method) (@guaranteed Kraken) -> ()
  // CHECK-NEXT: apply [[KRAKEN_FUN]]([[KRAKEN]])
  // CHECK-NEXT: [[KRAKEN:%.*]] = struct_extract [[SELF]] : $LetFieldStruct, #LetFieldStruct.vark
  // CHECK-NEXT: strong_retain [[KRAKEN]]
  // CHECK: [[DESTROY_SHIP_FUN:%.*]] = function_ref @_TF15guaranteed_self11destroyShipFCS_6KrakenT_ : $@convention(thin) (@owned Kraken) -> ()
  // CHECK-NEXT: strong_retain [[KRAKEN]]
  // CHECK-NEXT: apply [[DESTROY_SHIP_FUN]]([[KRAKEN]])
...
struct StructIntTreeNode {
  // Needed to make IntTreeNode non-trivial
  let k = Kraken()

  let value : Int
  let left, right : NodeBox<StructIntTreeNode>

  init() {}

  // CHECK-LABEL: sil hidden @_TFV15guaranteed_self17StructIntTreeNode4findfS0_FSiS0_ : $@convention(method) (Int, @guaranteed StructIntTreeNode) -> @owned StructIntTreeNode {
  // CHECK-NOT: strong_release
  ...
  // CHECK-NOT: strong_release
  // CHECK: return
  func find(v : Int) -> StructIntTreeNode {
    if v == value { return self }
...
struct MutableStructIntTreeNode {
  // Needed to make IntTreeNode non-trivial
  let k = Kraken()

  let value : Int
  var left, right : NodeBox<MutableStructIntTreeNode>

  init() {}

  // CHECK-LABEL: sil hidden @_TFV15guaranteed_self24MutableStructIntTreeNode4findfS0_FSiS0_ : $@convention(method) (Int, @guaranteed MutableStructIntTreeNode) -> @owned MutableStructIntTreeNode {
    // CHECK-NOT: strong_release
  // CHECK-NOT: release_value
  // CHECK: retain_value
  // CHECK-NOT: retain_value
  ...
  // CHECK-NOT: strong_release
  // CHECK: return
  func find(v : Int) -> MutableStructIntTreeNode {
    if v == value { return self }
    if v < value { return left.value.find(v) }
    return right.value.find(v)
  }
