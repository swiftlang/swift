...
// CHECK-LABEL: sil hidden [ossa] @$s8builtins30testTaskAddCancellationHandlerSVyYaF : $@convention(thin) @async () -> UnsafeRawPointer {
// CHECK:   [[CLOSURE_FN:%.*]] = function_ref @$s8builtins30testTaskAddCancellationHandlerSVyYaFyycfU_ : $@convention(thin) () -> ()
// CHECK:   [[CLOSURE:%.*]] = thin_to_thick_function [[CLOSURE_FN]]
// CHECK:   [[RESULT_VALUE:%.*]] = builtin "taskAddCancellationHandler"(%3 : $@callee_guaranteed () -> ()) : $UnsafeRawPointer
// CHECK:   [[RESULT:%.*]] = move_value [var_decl] [[RESULT_VALUE]]
// CHECK:   return [[RESULT]]
// CHECK: } // end sil function '$s8builtins30testTaskAddCancellationHandlerSVyYaF'
...
// CHECK:   [[CLOSURE_FN:%.*]] = function_ref @$s8builtins36testTaskAddPriorityEscalationHandlerSVyYaFys5UInt8V_ADtcfU_ : $@convention(thin) (UInt8, UInt8) -> ()
// CHECK:   [[CLOSURE:%.*]] = thin_to_thick_function [[CLOSURE_FN]]
// CHECK:   [[RESULT_VALUE:%.*]] = builtin "taskAddPriorityEscalationHandler"(%3 : $@callee_guaranteed (UInt8, UInt8) -> ()) : $UnsafeRawPointer
// CHECK:   [[RESULT:%.*]] = move_value [var_decl] [[RESULT_VALUE]]
// CHECK:   return [[RESULT]]
