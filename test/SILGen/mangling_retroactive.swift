...
// RUN: %target-swift-frontend -emit-module -verify-sil-ownership -o %t %S/Inputs/RetroactiveA.swift
// RUN: %target-swift-frontend -emit-module -verify-sil-ownership -o %t %S/Inputs/RetroactiveB.swift
// RUN: %target-swift-emit-silgen -verify-sil-ownership -I %t %s | %FileCheck %s
