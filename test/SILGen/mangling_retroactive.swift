...
// RUN: %target-swift-frontend -emit-module -enable-sil-ownership-verifier -o %t %S/Inputs/RetroactiveA.swift
// RUN: %target-swift-frontend -emit-module -enable-sil-ownership-verifier -o %t %S/Inputs/RetroactiveB.swift
// RUN: %target-swift-emit-silgen -enable-sil-ownership-verifier -I %t %s | %FileCheck %s
