...
public class AddressOnlySetter {
  ...
  init() {
    ...
    // CHECK: assign_or_init #AddressOnlySetter.value, self {{%.*}} : $AddressOnlySetter, value [[E2]] : $*AddressOnlyEnum
...
extension ReabstractedSetter where T == Int {
  init() {
    ...
    // CHECK: assign_or_init #ReabstractedSetter.value, self {{%.*}} : $*ReabstractedSetter<Int>, value [[CF2]]
    self.value = { x in }
  }
}
