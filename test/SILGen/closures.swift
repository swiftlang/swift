// RUN: %target-swift-emit-silgen-ossa -o /dev/null -enable-sil-opaque-values -Xllvm -sil-print-types -module-name closures -parse-stdlib -parse-as-library %s -target %target-swift-6.2-abi-triple
// RUN: %target-swift-emit-silgen -Xllvm -sil-print-types -module-name closures -parse-stdlib -parse-as-library %s -target %target-swift-6.2-abi-triple | %FileCheck %s
// RUN: %target-swift-emit-silgen -Xllvm -sil-print-types -module-name closures -parse-stdlib -parse-as-library %s -target %target-swift-6.2-abi-triple | %FileCheck %s --check-prefix=GUARANTEED
