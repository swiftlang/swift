...
// RUN: %target-swift-emit-silgen -module-name default_arguments_serialized -Xllvm -sil-full-demangle -enable-sil-ownership-verifier -swift-version 4 -I %t %s | %FileCheck %s

// RUN: %target-swift-emit-sil -module-name default_arguments_serialized -Xllvm -sil-full-demangle -enable-sil-ownership-verifier -O -swift-version 4 -I %t %s | %FileCheck %s --check-prefix=OPT
