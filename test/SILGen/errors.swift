...
// CHECK: sil hidden @_TF6errors11dont_return{{.*}} : $@convention(thin) <T> (@out T, @in T) -> @error ErrorProtocol {
// CHECK:      [[BOX:%.*]] = alloc_existential_box $ErrorProtocol, $HomeworkError
// CHECK-NEXT: [[T0:%.*]] = metatype $@thin HomeworkError.Type
...
// CHECK-NEXT: try_apply [[DR_FN]]<Cat>([[RET_TEMP]]#1, [[ARG_TEMP]]#1) : $@convention(thin) <τ_0_0> (@out τ_0_0, @in τ_0_0) -> @error ErrorProtocol, normal [[DR_NORMAL:bb[0-9]+]], error [[DR_ERROR:bb[0-9]+]]
// CHECK:    [[DR_NORMAL]]({{%.*}} : $()):
...
// CHECK-NEXT: checked_cast_addr_br copy_on_success ErrorProtocol in [[SRC_TEMP]]#1 : $*ErrorProtocol to HomeworkError in [[DEST_TEMP]]#1 : $*HomeworkError, [[IS_HWE:bb[0-9]+]], [[NOT_HWE:bb[0-9]+]]
...
// CHECK-LABEL: sil shared [transparent] [reabstraction_thunk] @_TTRXFo__dSizoPs13ErrorProtocol__XFo__iSizoPS___ : $@convention(thin) (@out Int, @owned @callee_owned () -> (Int, @error ErrorProtocol)) -> @error ErrorProtocol
...
// CHECK-NEXT: try_apply [[FN]]<T>([[BOX_DATA]], [[ARG_BOX]]#1) : $@convention(thin) <τ_0_0> (@out τ_0_0, @in τ_0_0) -> @error ErrorProtocol, normal [[SUCCESS:[^ ]+]], error [[CLEANUPS:[^ ]+]]
// CHECK: [[SUCCESS]]({{%.+}} : $()):
