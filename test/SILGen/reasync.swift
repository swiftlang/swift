...
// CHECK: [[FN:%.*]] = function_ref @$s7reasync0A8FunctionyyyyYXEYF : $@convention(thin) @async (@noescape @async @callee_guaranteed () -> ()) -> ()
...
// CHECK-LABEL: sil hidden [ossa] @$s7reasync23throwingReasyncFunctionyyyyYKXEYKF : $@convention(thin) @async (@noescape @async @callee_guaranteed () -> @error Error) -> @error Error {
...
// CHECK: [[FN:%.*]] = function_ref @$s7reasync23throwingReasyncFunctionyyyyYKXEYKF : $@convention(thin) @async (@noescape @async @callee_guaranteed () -> @error Error) -> @error Error
