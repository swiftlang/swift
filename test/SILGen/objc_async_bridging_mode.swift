...
// RUN: %target-swift-frontend(mock-sdk: %clang-importer-sdk) -Xllvm -sil-print-types -emit-silgen -I %S/Inputs/custom-modules -target %target-swift-5.1-abi-triple -enable-upcoming-feature ObjCAsyncBridgeImmediateTask %s | %FileCheck --check-prefix=CHECK-IMMEDIATE %s
...
class MyServer: NSObject {
    // --- Default mode: uses legacy _runTaskForBridgedAsyncMethod
    ...
    // --- ObjCAsyncBridgeImmediateTask feature: uses _runTaskImmediateForBridgedAsyncMethod
