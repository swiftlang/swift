...
// RUN: %target-swift-emit-silgen -enable-sil-ownership-verifier -I %t %s %S/testable-multifile.swift -module-name main | %FileCheck %s
// RUN: %target-swift-emit-silgen -enable-sil-ownership-verifier -I %t %S/testable-multifile.swift %s -module-name main | %FileCheck %s
// RUN: %target-swift-emit-silgen -enable-sil-ownership-verifier -I %t -primary-file %s %S/testable-multifile.swift -module-name main | %FileCheck %s

// Just make sure we don't crash later on.
// RUN: %target-swift-emit-ir -enable-sil-ownership-verifier -I %t -primary-file %s %S/testable-multifile.swift -module-name main -o /dev/null
// RUN: %target-swift-emit-ir -enable-sil-ownership-verifier -I %t -O -primary-file %s %S/testable-multifile.swift -module-name main -o /dev/null
