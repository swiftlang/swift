...
// CHECK-LABEL: sil hidden [ossa] @$s18borrowing_for_loop32testNonCopyableBorrowingSequenceyyF : $@convention(thin) () -> ()
func testNonCopyableBorrowingSequence() {
  let buffer = UnsafeMutableBufferPointer<NoncopyableInt>.allocate(capacity: 4)
  let seq = Span(_unsafeElements: buffer)
...
// CHECK: sil hidden [ossa] @$s18borrowing_for_loop29testCopyableBorrowingSequenceyyF : $@convention(thin) () -> ()
func testCopyableBorrowingSequence() {
  let arr = [0, 1, 2, 3]
  let seq = arr.span
