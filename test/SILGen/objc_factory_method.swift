...
// RUN: %swift %clang-importer-sdk -enable-objc-failable-initializers -target x86_64-apple-macosx10.9 -module-cache-path %t/clang-module-cache %s -emit-silgen | FileCheck %s

// FIXME: The -emit-sil line is there only to check that the generated
// SIL doesn't freak out DI.
// RUN: %swift %clang-importer-sdk -enable-objc-failable-initializers -target x86_64-apple-macosx10.9 -module-cache-path %t/clang-module-cache %s -emit-sil | FileCheck %s
