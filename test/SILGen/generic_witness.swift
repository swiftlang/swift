...
// CHECK-LABEL: sil hidden @_TF15generic_witness3foouR_S_8RunciblerFq_T_ : $@convention(thin) <B where B : Runcible> (@in B) -> () {
...
func bar(var x: Runcible) {
  ...
  // CHECK: [[EXIST:%.*]] = open_existential_addr [[TEMP]]#1 : $*Runcible to $*[[OPENED:@opened(.*) Runcible]]
