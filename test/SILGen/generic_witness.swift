...
func foo<B:Runcible>(x:B) {
  ...
  // CHECK: specialize [[METHOD]] : {{.*}}, $[cc(method)] ((x : Int64), [byref] B) -> (), A = Int
  x.runce(5)
}
...
func bar(x : Runcible) {
  ...
  // CHECK: [[METHOD:%.*]] = protocol_method [[BOX]]#1 : $*Runcible, #Runcible.runce!1 : $[cc(method)] <A> ((x : A), [byref] This) -> ()
  // CHECK: specialize [[METHOD]] : {{.*}}, $[cc(method)] ((x : Int64), [byref] This) -> (), A = Int
  x.runce(5)
}
