...
func generic_nondependent_context<T>(x:T, y:Int) -> Int {
  ...
  // CHECK: [[FOO:%.*]] = function_ref @{{closure[0-9]*}} : $[thin] <T> ((), (Builtin.ObjectPointer, [byref] Int64)) -> Int64
...
func generic_dependent_context<T>(x:T, y:Int) -> T {
  func foo() -> T { return x }
  // CHECK: [[FOO:%.*]] = function_ref @{{closure[0-9]*}} : $[thin] <T> ((), (Builtin.ObjectPointer, [byref] T)) -> T
