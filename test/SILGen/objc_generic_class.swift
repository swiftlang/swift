...
class Generic<T>: NSObject {
  ...
  // CHECK-LABEL: sil @_TFC18objc_generic_class7GenericD : $@cc(method) @thin <T> (@owned Generic<T>) -> () {
  // CHECK-NEXT:  bb0({{%.*}} : $Generic<T>):
  // CHECK-LABEL: sil @_TToFC18objc_generic_class7GenericD : $@cc(objc_method) @thin <T> (Generic<T>) -> () {
...
// CHECK-NOT: sil @_TFC18objc_generic_class7Genericd
// CHECK-NOT: sil @_TFCSo8NSObjectd

// CHECK-LABEL: sil @_TFC18objc_generic_class11SubGeneric1D : $@cc(method) @thin <U, V> (@owned SubGeneric1<U, V>) -> () {
