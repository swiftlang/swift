...
struct ConformsToReturnsMetatype : ReturnsMetatype {
  // CHECK-LABEL: sil hidden [transparent] [thunk] @_TTWV18result_abstraction25ConformsToReturnsMetatypeS_15ReturnsMetatypeS_FS1_16getAssocMetatypeUS1__U__fRQPS1_FT_MQS2_5Assoc : $@cc(witness_method) @thin (@inout ConformsToReturnsMetatype) -> @thick S.Type
...
struct ConformsToReturnsFunction : ReturnsFunction {
  // CHECK-LABEL: sil hidden [transparent] [thunk] @_TTWV18result_abstraction25ConformsToReturnsFunctionS_15ReturnsFunctionS_FS1_7getFuncUS1__U___fQPS1_FT_FQS2_3ArgQS2_6Result : $@cc(witness_method) @thin (@in_guaranteed ConformsToReturnsFunction) -> @owned @callee_owned (@out R, @in S) -> ()
...
struct ConformsToReturnsAssocWithMetatype : ReturnsAssoc {
  ...
  // CHECK-LABEL: sil hidden [transparent] [thunk] @_TTWV18result_abstraction34ConformsToReturnsAssocWithMetatypeS_12ReturnsAssocS_FS1_8getAssocUS1__U__fRQPS1_FT_QS2_5Assoc : $@cc(witness_method) @thin (@out @thick S.Type, @inout ConformsToReturnsAssocWithMetatype) -> ()
...
struct ConformsToReturnsAssocWithFunction : ReturnsAssoc {
  typealias Assoc = S -> R
  // CHECK-LABEL: sil hidden [transparent] [thunk] @_TTWV18result_abstraction34ConformsToReturnsAssocWithFunctionS_12ReturnsAssocS_FS1_8getAssocUS1__U__fRQPS1_FT_QS2_5Assoc : $@cc(witness_method) @thin (@out @callee_owned (@out R, @in S) -> (), @inout ConformsToReturnsAssocWithFunction) -> ()
