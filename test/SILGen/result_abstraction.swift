...
struct ConformsToReturnsMetatype : ReturnsMetatype {
  // CHECK-LABEL: sil hidden [transparent] [thunk] @_TTWV18result_abstraction25ConformsToReturnsMetatypeS_15ReturnsMetatypeS_FS1_16getAssocMetatypeuR_S1_rfRq_FT_Mw_5Assoc : $@convention(witness_method) (@inout ConformsToReturnsMetatype) -> @thick S.Type
...
struct ConformsToReturnsFunction : ReturnsFunction {
  // CHECK-LABEL: sil hidden [transparent] [thunk] @_TTWV18result_abstraction25ConformsToReturnsFunctionS_15ReturnsFunctionS_FS1_7getFuncuR_S1_rfq_FT_Fw_3Argw_6Result : $@convention(witness_method) (@in_guaranteed ConformsToReturnsFunction) -> @owned @callee_owned (@out R, @in S) -> ()
...
struct ConformsToReturnsAssocWithMetatype : ReturnsAssoc {
  ...
  // CHECK-LABEL: sil hidden [transparent] [thunk] @_TTWV18result_abstraction34ConformsToReturnsAssocWithMetatypeS_12ReturnsAssocS_FS1_8getAssocuR_S1_rfRq_FT_w_5Assoc : $@convention(witness_method) (@out @thick S.Type, @inout ConformsToReturnsAssocWithMetatype) -> ()
...
struct ConformsToReturnsAssocWithFunction : ReturnsAssoc {
  typealias Assoc = S -> R
  // CHECK-LABEL: sil hidden [transparent] [thunk] @_TTWV18result_abstraction34ConformsToReturnsAssocWithFunctionS_12ReturnsAssocS_FS1_8getAssocuR_S1_rfRq_FT_w_5Assoc : $@convention(witness_method) (@out @callee_owned (@out R, @in S) -> (), @inout ConformsToReturnsAssocWithFunction) -> ()
