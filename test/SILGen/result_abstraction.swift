...
struct ConformsToReturnsMetatype : ReturnsMetatype {
  // CHECK-LABEL: sil @_TTWV18result_abstraction25ConformsToReturnsMetatypeS_15ReturnsMetatypeFS1_16getAssocMetatypeUS1__U__fRQPS1_FT_MQS2_5Assoc : $@cc(witness_method) @thin (@inout ConformsToReturnsMetatype) -> @thick S.Type
...
struct ConformsToReturnsFunction : ReturnsFunction {
  // CHECK-LABEL: sil @_TTWV18result_abstraction25ConformsToReturnsFunctionS_15ReturnsFunctionFS1_7getFuncUS1__U___fRQPS1_FT_FQS2_3ArgQS2_6Result : $@cc(witness_method) @thin (@inout ConformsToReturnsFunction) -> @owned @callee_owned (@out R, @in S) -> ()
...
struct ConformsToReturnsAssocWithMetatype : ReturnsAssoc {
  ...
  // CHECK-LABEL: sil @_TTWV18result_abstraction34ConformsToReturnsAssocWithMetatypeS_12ReturnsAssocFS1_8getAssocUS1__U__fRQPS1_FT_QS2_5Assoc : $@cc(witness_method) @thin (@out @thick S.Type, @inout ConformsToReturnsAssocWithMetatype) -> ()
...
struct ConformsToReturnsAssocWithFunction : ReturnsAssoc {
  typealias Assoc = S -> R
  // CHECK-LABEL: sil @_TTWV18result_abstraction34ConformsToReturnsAssocWithFunctionS_12ReturnsAssocFS1_8getAssocUS1__U__fRQPS1_FT_QS2_5Assoc : $@cc(witness_method) @thin (@out @callee_owned (@out R, @in S) -> (), @inout ConformsToReturnsAssocWithFunction) -> ()
