...
// RUN: %target-swift-frontend -module-name Mod -emit-module -enable-private-imports -verify-sil-ownership -swift-version 5 -o %t %S/Inputs/private_import_module.swift
// RUN: %target-swift-emit-silgen -verify-sil-ownership -I %t -primary-file %s %S/private_import_other.swift -module-name main -swift-version 5 | %FileCheck %s
// RUN: %target-swift-emit-silgen -verify-sil-ownership -I %t %s %S/private_import_other.swift -module-name main -swift-version 5 | %FileCheck %s
// RUN: %target-swift-emit-silgen -verify-sil-ownership -I %t %S/private_import_other.swift %s -module-name main -swift-version 5 | %FileCheck %s
