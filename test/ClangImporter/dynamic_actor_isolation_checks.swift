...
// RUN: %target-swift-frontend(mock-sdk: %clang-importer-sdk) -target %target-swift-5.1-abi-triple -enable-upcoming-feature DynamicActorIsolation -Xllvm -sil-print-types -emit-silgen -module-name preconcurrency_conformances %t/src/checks.swift | %FileCheck %t/src/checks.swift
// RUN: %target-swift-frontend(mock-sdk: %clang-importer-sdk) -target %target-swift-5.1-abi-triple -swift-version 6 -disable-dynamic-actor-isolation -Xllvm -sil-print-types -emit-silgen -module-name preconcurrency_conformances %t/src/checks_disabled.swift | %FileCheck %t/src/checks_disabled.swift
