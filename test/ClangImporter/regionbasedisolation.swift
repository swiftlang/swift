...
extension ObjCObject {
  ...
  } // expected-ni-error {{task or actor-isolated value cannot be sent}}
...
@MainActor func testMainActorMerging(_ y: NSObject) async {
  let x = ObjCObject()
  await x.useValue(y)
  await useValueConcurrently(x)  // expected-ni-error {{sending 'x' risks causing data races}}
...
func testTaskLocal(_ y: NSObject) async {
  let x = ObjCObject()
  await x.useValue(y)
  await useValueConcurrently(x)

  // This is not safe since we merge x into y's region making x task
  // isolated. We then try to send it to a main actor function.
  await useValueMainActor(x) // expected-ni-error {{sending 'x' risks causing data races}}
  // expected-ni-note @-1 {{sending task-isolated 'x' to main actor-isolated global function 'useValueMainActor' risks causing data races between main actor-isolated and task-isolated uses}}
}
