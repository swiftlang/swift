...
func test(_ value: RegularEnum, _ exhaustiveValue: ExhaustiveEnum) {
  switch value { // expected-error {{switch must be exhaustive}} expected-note {{handle unknown values using "@unknown case _"}}
  case .A: break
  case .B: break
  }
...
func testAttributes(
  ...
  switch retetb { // expected-error {{switch must be exhaustive}} expected-note {{handle unknown values using "@unknown case _"}}
  case .A, .B: break
  }
