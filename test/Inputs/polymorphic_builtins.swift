...
#if CHECK_COMPILETIME_DIAGNOSTICS
  static func nonTransparentAdd(_ lhs: Self, _ rhs: Self) -> Self
#endif
}
...
#if CHECK_COMPILETIME_DIAGNOSTICS
  public static func nonTransparentAdd(_ lhs: Self, _ rhs: Self) -> Self {
    fatalError("In default SIMDStorageWithOps add impl. Only here for never types?!")
  }
#endif
}
...
extension SIMD where Scalar: FixedWidthInteger {
  ...
  public static func &+(lhs: Self, rhs: Self) -> Self {
    ...
    //
    // NOTE: We need to add in a check that this value is no longer in a
    // transparent function. For now, we just call unsafely.
  ...
  public static func &*(lhs: Self, rhs: Self) -> Self {
    ...
    // branch is very likely in a generic context.
    //
    // NOTE: We purposely do not guard this with an ispod to make sure we trap
    // in generic transparent contexts.
    if _fastPath(Self._hasVectorRepresentation) {
...
#if CHECK_COMPILETIME_DIAGNOSTICS
  // In this case, we forget to put transparent here and get a nice warning.
  public static func nonTransparentAdd(_ lhs: Self, _ rhs: Self) -> Self {
    return Self(_vector: Builtin.generic_add(lhs._vector, rhs._vector))
  }
#endif
