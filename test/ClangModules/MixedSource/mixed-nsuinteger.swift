...
var r: Int = a[0].countByEnumerating(p, objects: pp, count: i)

// When working with instances typed as user-defined, NSUInteger comes
// across as UInt.
var rr: UInt = userTypedObj.countByEnumerating(p, objects: pp, count: ui)
...
func gen<T:NSFastEnumeration>(t:T) {
  ...
  t.countByEnumerating(p, objects: pp, count: i)
}
