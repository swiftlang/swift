...
func test_cfunc1(i: Int) {
  cfunc1() // okay
  cfunc1(i) // expected-error{{cannot convert the expression's type 'Int' to type '() -> Void'}}
}
...
func test_cfunc2(i: Int) {
  ...
  cfunc2(b:17, a:i) // expected-error{{cannot convert the expression's type '(b: IntegerLiteralConvertible, a: Int)' to type 'IntegerLiteralConvertible'}}
}
