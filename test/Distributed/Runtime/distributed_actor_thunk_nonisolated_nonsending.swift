...
// Deploying to a runtime with swift_distributed_execute_target_with_isolation
// emits accessors with the isolated parameter, exercise that shape too
// RUN: %target-build-swift -module-name main -target %target-future-triple -j2 -parse-as-library -I %t %s %S/../Inputs/FakeDistributedActorSystems.swift %S/../Inputs/FakeNonsendingActorSystems.swift -o %t/a-isolated.out
// RUN: %target-codesign %t/a-isolated.out
// RUN: %target-run %t/a-isolated.out | %FileCheck %s
