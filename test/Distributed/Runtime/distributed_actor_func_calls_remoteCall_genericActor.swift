...
distributed actor Greeter<A: Codable> {
  distributed func generic<V: Codable>(_ value: V) -> String {
    return "\(value)"
  }

  distributed func generic2<B: Codable>(
...
func test() async throws {
  ...
  let local: Greeter<String> = Greeter(system: system)
  let ref: Greeter<String> = try Greeter.resolve(id: local.id, using: system)
