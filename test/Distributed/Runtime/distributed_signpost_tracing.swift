...
@main struct Main {
  static func main() async throws {
    ...
    // The thunk brackets encoding the invocation, carrying the target's mangled
    // accessor record name and how many arguments are being encoded
    ...
    // Only then is 'remoteCall' invoked
    // CHECK-NEXT: event]{{.*}}distributed_outbound_remote_call: actor=0x{{[0-9a-f]+}} actorType=main.Greeter targetActorId={{.*}} targetFunction=$s4main7GreeterC5greet4nameS2S_tYaKFTE

    // The callee side: 'executeDistributedTarget' splits decoding the
    // invocation from executing the target
    // CHECK-NEXT: event]{{.*}}distributed_inbound_execute_target: actor=0x{{[0-9a-f]+}} actorType=main.Greeter targetActorId={{.*}} targetFunction=$s4main7GreeterC5greet4nameS2S_tYaKFTE
    ...
    // CHECK-NEXT: end]{{.*}}distributed_outbound_encode_arguments: success=true errorType=
    // CHECK-NEXT: event]{{.*}}distributed_outbound_remote_call: {{.*}}targetFunction=$s4main7GreeterC4boomSSyYaKFTE
    // CHECK-NEXT: event]{{.*}}distributed_inbound_execute_target: {{.*}}targetFunction=$s4main7GreeterC4boomSSyYaKFTE
    ...
    // The execution interval reports the type of the error the target threw,
    // and so does the result handler event
