...
distributed actor DA {
  ...
  // === The span ID lives in a 'var' box, so the 'catch' further down reads
  // back the very value the interval was opened with
  // CHECK:       [[SPAN_BOX:%[0-9]+]] = alloc_box ${ var UInt64 }
  // CHECK:       [[SPAN_BORROW:%[0-9]+]] = begin_borrow [var_decl] [[SPAN_BOX]]
  // CHECK:       [[SPAN_ADDR:%[0-9]+]] = project_box [[SPAN_BORROW]], 0

  // The interval opens with the target's mangled accessor record name and the
  // argument count, and is handed 'self' as the actor being called
  // CHECK:       {{%[0-9]+}} = string_literal utf8 "$s25distributed_thunk_tracing2DAC5greet4nameS2S_tYaKFTE"
  // CHECK:       [[BEGIN_FN:%[0-9]+]] = function_ref @$s11Distributed06_traceA20EncodeArgumentsBegin11targetActor0F10Identifier13argumentCounts6UInt64Vx_SSSitAA0aG0RzlF : $@convention(thin) <τ_0_0 where τ_0_0 : DistributedActor> (@guaranteed τ_0_0, @guaranteed String, Int) -> UInt64
  // CHECK:       [[SPAN:%[0-9]+]] = apply [[BEGIN_FN]]<DA>([[SELF]], {{%[0-9]+}}, {{%[0-9]+}}) : $@convention(thin) <τ_0_0 where τ_0_0 : DistributedActor> (@guaranteed τ_0_0, @guaranteed String, Int) -> UInt64
  // CHECK:       store [[SPAN]] to [trivial] [[SPAN_ADDR]]
  ...
  // === Success: the span ID is read back out of the box and the interval is
  // closed with no error, so the trace records it as a success
  // CHECK:       [[SPAN_READ:%[0-9]+]] = begin_access [read] [unknown] [[SPAN_ADDR]]
  // CHECK:       [[SPAN_VAL:%[0-9]+]] = load [trivial] [[SPAN_READ]]
  // CHECK:       [[NO_ERROR:%[0-9]+]] = enum $Optional<any Error>, #Optional.none!enumelt
  // CHECK:       [[FAILED_DFLT:%[0-9]+]] = function_ref @$s11Distributed06_traceA18EncodeArgumentsEnd_5error6failedys6UInt64V_s5Error_pSgSbtFfA1_ : $@convention(thin) () -> Bool
  // CHECK:       [[FAILED:%[0-9]+]] = apply [[FAILED_DFLT]]() : $@convention(thin) () -> Bool
  // CHECK:       [[END_FN:%[0-9]+]] = function_ref @$s11Distributed06_traceA18EncodeArgumentsEnd_5error6failedys6UInt64V_s5Error_pSgSbtF : $@convention(thin) (UInt64, @guaranteed Optional<any Error>, Bool) -> ()
  // CHECK:       {{%[0-9]+}} = apply [[END_FN]]([[SPAN_VAL]], [[NO_ERROR]], [[FAILED]]) : $@convention(thin) (UInt64, @guaranteed Optional<any Error>, Bool) -> ()

  // === Only then the outbound event, carrying the same mangled name...
  // CHECK:       {{%[0-9]+}} = string_literal utf8 "$s25distributed_thunk_tracing2DAC5greet4nameS2S_tYaKFTE"
  // CHECK:       [[EVENT_FN:%[0-9]+]] = function_ref @$s11Distributed06_traceA10RemoteCall11targetActor0E10Identifieryx_SStAA0aF0RzlF : $@convention(thin) <τ_0_0 where τ_0_0 : DistributedActor> (@guaranteed τ_0_0, @guaranteed String) -> ()
  // CHECK:       {{%[0-9]+}} = apply [[EVENT_FN]]<DA>([[SELF]], {{%[0-9]+}}) : $@convention(thin) <τ_0_0 where τ_0_0 : DistributedActor> (@guaranteed τ_0_0, @guaranteed String) -> ()

  // ...and only after that is 'remoteCall' invoked, on the same 'self'
  ...
  // CHECK:       try_apply [[REMOTE_CALL]]<DA, Never, String>({{%[0-9]+}}, [[SELF]], {{%[0-9]+}}, {{%[0-9]+}}, {{%[0-9]+}}, {{%[0-9]+}}, {{%[0-9]+}}) : $@convention(method) @async <τ_0_0, τ_0_1, τ_0_2 where τ_0_0 : DistributedActor, τ_0_1 : Error, τ_0_2 : Decodable, τ_0_2 : Encodable, τ_0_0.ID == LocalTestingActorID> (@guaranteed τ_0_0, @in_guaranteed RemoteCallTarget, @inout LocalTestingInvocationEncoder, @thick τ_0_1.Type, @thick τ_0_2.Type, @guaranteed LocalTestingDistributedActorSystem) -> (@out τ_0_2, @error any Error), normal {{bb[0-9]+}}, error {{bb[0-9]+}}

  // === Failure: one shared catch block closes the interval, reporting the
  // error that was thrown so the trace carries its type
  // CHECK:       [[CATCH_BB:bb[0-9]+]]({{%[0-9]+}} : @owned $any Error):

  // The error is bound to a 'let' so it can be both reported and rethrown
  // CHECK:       {{%[0-9]+}} = move_value [lexical] [var_decl] {{%[0-9]+}}

  // The same span ID as above, read back out of the box
  // CHECK:       [[SPAN_READ2:%[0-9]+]] = begin_access [read] [unknown] [[SPAN_ADDR]]
  // CHECK:       [[SPAN_VAL2:%[0-9]+]] = load [trivial] [[SPAN_READ2]]
  // CHECK:       [[SOME_ERROR:%[0-9]+]] = enum $Optional<any Error>, #Optional.some!enumelt, {{%[0-9]+}}
  // CHECK:       [[FAILED_DFLT2:%[0-9]+]] = function_ref @$s11Distributed06_traceA18EncodeArgumentsEnd_5error6failedys6UInt64V_s5Error_pSgSbtFfA1_ : $@convention(thin) () -> Bool
  // CHECK:       [[FAILED2:%[0-9]+]] = apply [[FAILED_DFLT2]]() : $@convention(thin) () -> Bool
  // CHECK:       [[END_FN2:%[0-9]+]] = function_ref @$s11Distributed06_traceA18EncodeArgumentsEnd_5error6failedys6UInt64V_s5Error_pSgSbtF : $@convention(thin) (UInt64, @guaranteed Optional<any Error>, Bool) -> ()
  // CHECK:       {{%[0-9]+}} = apply [[END_FN2]]([[SPAN_VAL2]], [[SOME_ERROR]], [[FAILED2]]) : $@convention(thin) (UInt64, @guaranteed Optional<any Error>, Bool) -> ()

  // The interval is closed before the error is rethrown, never after
  ...
  // CHECK:       br [[CATCH_BB]]({{%[0-9]+}})
  // CHECK:       [[RET_ERR_BB]]({{%[0-9]+}} : @owned $any Error):
  // CHECK:       br [[CATCH_BB]]({{%[0-9]+}})
  // CHECK:       [[DONE_ERR_BB]]({{%[0-9]+}} : @owned $any Error):
  // CHECK:       br [[CATCH_BB]]({{%[0-9]+}})

  // The rethrow itself is shared with the 'remoteCall' error path
