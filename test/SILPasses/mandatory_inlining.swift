...
// CHECK-LABEL: sil @_TF18mandatory_inlining3foo
...
// CHECK-LABEL: sil [transparent] @_TF18mandatory_inlining3bar
...
// CHECK-LABEL: sil [transparent] @_TF18mandatory_inlining3baz
// CHECK: return
...
// CHECK-LABEL: sil @_TF18mandatory_inlining4spam

@transparent func ham(x: Int) -> Int {
  return spam(x)
}

// CHECK-LABEL: sil [transparent] @_TF18mandatory_inlining3ham
...
// CHECK-LABEL: sil @_TF18mandatory_inlining4eggs
  // CHECK: function_ref @_TF18mandatory_inlining4spam
  // CHECK: apply
  // CHECK: return

@transparent func call_auto_closure(x: @auto_closure () -> Bool) -> Bool {
  return x()
}
...
// CHECK-LABEL: sil @{{.*}}test_auto_closure_with_capture
  // CHECK-NOT: = apply
  // CHECK: return
...
// CHECK-LABEL: sil @_TF18mandatory_inlining33test_auto_closure_without_captureFT_Sb
...
// CHECK-LABEL: sil @_TF18mandatory_inlining14call_uncurried
  // CHECK-NOT: = apply
  // CHECK: return
...
// CHECK-LABEL: sil @_TF18mandatory_inlining12call_curried
  // CHECK: = apply
  // CHECK: = apply
  // CHECK: return
...
@transparent func &&& (lhs: Bool, rhs: @auto_closure ()->Bool) -> Bool {
  if lhs {
    return rhs()
  }
...
// CHECK-LABEL: sil @_TF18mandatory_inlining26test_chained_short_circuit
...
func testInlineUnionElement() -> X {
  return X.onetransp;
  // CHECK-LABEL: sil @_TF18mandatory_inlining22testInlineUnionElementFT_OS_1X
...
@transparent
func call_let_auto_closure(let x: @auto_closure () -> Bool) -> Bool {
  return x()
}

// CHECK: sil @{{.*}}test_let_auto_closure_with_value_capture
