...
struct EnumeratedBorrowingSequence<S: BorrowingSequence> where S.Element: ~Copyable {
  struct Element: ~Copyable, ~Escapable {
    var counter: Int
    var element: Borrow<S.Element>
  }
  
  struct Iterator: ~Copyable, ~Escapable {
    var iterator: S.BorrowingIterator
    var count: Int
    
    mutating func nextSpan(maximumCount _: Int) -> Span<Element> {
      let nextSpan = iterator.nextSpan(maximumCount: 1)
      if nextSpan.isEmpty { return Span() }
