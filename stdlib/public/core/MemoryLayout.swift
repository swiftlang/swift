...
extension MemoryLayout where T: AnyObject {
  ...
  /// the value. Properties are not directly accessible if they trigger any
  /// `didSet` or `willSet` accessors, perform any representation changes such
  /// as bridging or closure reabstraction, or mask the value out of overlapping
  /// storage as for packed bitfields.
  ///
  ...
  ///     }
  ///
  /// When the retun value of this method is non-`nil`, then accessing the value
  /// by key path or via the returned pointer are equivalent. For example:
  ///
  ...
  ///     }
  ///
  /// Note that the returned pointer is only valid until the root object gets
  /// deinitialized. It is the responsibility of the caller to ensure that the
  /// object stays alive while it is using the pointer. (The
  /// `withExtendedLifetime` call above is one example of how this can be done.)
  ///
  /// Additionally, the Law of Exclusivity still applies: the caller must ensure
  /// that any access of the instance variable through the returned pointer will
  /// not overlap with any other access to the same variable, unless both
  /// accesses are reads.
  ///
