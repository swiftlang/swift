...
where Encoding.EncodedScalar : RangeReplaceableCollection {
  ...
  public mutating func parseScalar<I : IteratorProtocol>(
    ...
    _correctnessCheck(scalarBitCount % numericCast(Encoding.CodeUnit.bitWidth) == 0)
    _correctnessCheck(1...4 ~= scalarBitCount / 8)
    _correctnessCheck(scalarBitCount <= _buffer._bitCount)
