...
internal func _float80ToStringImpl2(
  ...
  _ debug: Bool) -> UInt64 {
    ...
    var buffer = unsafe MutableSpan<UTF8.CodeUnit>(_unchecked: textBuffer,
                                                   count: Int(bufferLength))
    let textRange = Float80ToASCII(value: value, buffer: &buffer)
    let textLength = textRange.upperBound - textRange.lowerBound
  ...
  buffer utf8Buffer: inout MutableSpan<UTF8.CodeUnit>) -> Range<Int>
{
    ...
    let lowerMidpointExact = UInt128(_low: isBoundary ? threeQuarterUlp : halfUlp,
                                     _high: significand - 1)

    return _backend_256bit(buffer: &buffer,
                           ...
                           forceExponential: binaryExponent > 65 || (binaryExponent == 65 && !isBoundary))
}
#endif
...
fileprivate func _backend_256bit(
  ...
  binaryExponent: Int,
  forceExponential: Bool) -> Range<Int> {

    // Step 3: Estimate the base 10 exponent
    var base10Exponent = decimalExponentFor2ToThe(binaryExponent)

    // Step 4: Compute a power-of-10 scale factor
    var powerOfTenRoundedDown = UInt256()
    var powerOfTenRoundedUp = UInt256()
    ...
    // Include 8 "0" characters at the beginning of the buffer for finishFormatting to use
    buffer.storeBytes(of: 0x3030303030303030,
                      toByteOffset: 0,
                      as: UInt64.self)
    // Start writing digits just after that
    let firstDigit = 8
    var nextDigit = firstDigit
    buffer.storeBytes(of: 0x30 + UInt8(truncatingIfNeeded: t.extractIntegerPart(integerBits)),
                      toByteOffset: nextDigit,
                      as: UInt8.self)
    nextDigit &+= 1
    ...
    if delta < t {
        ...
        while d0 < t0 {
            let d12 = d1234 / 100
            let d34 = d1234 % 100
            unsafe buffer.storeBytes(of: asciiDigitTable[Int(bitPattern:d12)],
                                     toUncheckedByteOffset: nextDigit,
                                     as: UInt16.self)
    ...
    return finishFormatting(&buffer, sign, firstDigit, nextDigit,
                            forceExponential, base10Exponent)
}
#endif
...
fileprivate struct UInt256 {
    ...
    static func &- (lhs: UInt256, rhs: UInt256) -> UInt256 {
        ...
        let newhigh = UInt128(_low: t._low, _high: t._high &+ lhs.high._high &+ ~rhs.high._high)
        return UInt256(high: newhigh, low: newlow)
    }
...
fileprivate func intervalContainingPowerOf10_Binary128(p: Int, lower: inout UInt256, upper: inout UInt256) -> Int {
    if p >= 0 && p <= 55 {
        ...
        lower = UInt256(high: exactHigh, exactLow, 0, low: 0)
        upper = lower
        return binaryExponentFor10ToThe(p)
    }
    ...
    lower = UInt256(high: powersOf10_Binary128[offset + 3],
    ...
    if extraPower > 0 {
        let extra = UInt128(_low: powersOf10_Exact128[extraPower * 2],
