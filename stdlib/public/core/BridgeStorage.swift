...
> {
  ...
  init(native: Native, bits: Int) {
    _correctnessCheck(_usesNativeSwiftReferenceCounting(NativeClass.self))
    
    // More bits are available on some platforms, but it's not portable
    _correctnessCheck(0...1 ~= bits,
  ...
  var nativeInstance_noSpareBits: Native {
    @inline(__always) get {
      _correctnessCheck(isNative)
      _correctnessCheck(_nonPointerBits(rawValue) == 0)
