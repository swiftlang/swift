...
class _SwiftRawStringStorage : _SwiftNativeNSString {
  ...
  internal init(_doNotCallMe: ()) {
    _correctnessCheckFailure("Use the create method")
  }
  ...
  final var unusedCapacity: Int {
    _correctnessCheck(capacity >= count)
...
where CodeUnit : UnsignedInteger & FixedWidthInteger {
  ...
  internal static func create(
    ...
    _correctnessCheck(count >= 0 && count <= capacity)
    ...
    storage.count = count
    _correctnessCheck(storage.capacity >= capacity)
    return storage
  }
  ...
  convenience init(coder aDecoder: AnyObject) {
    _correctnessCheckFailure("init(coder:) not implemented for _SwiftStringStorage")
  }
...
extension _SwiftStringStorage {
  ...
  where OtherCodeUnit : FixedWidthInteger & UnsignedInteger {
    ...
    _correctnessCheck(self.count + otherCount <= self.capacity)
...
extension _SwiftStringStorage {
  ...
  internal final func _opaqueAppendInPlace(
    ...
    _correctnessCheck(other._isOpaque)
    if other._isSmall {
