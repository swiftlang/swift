...
internal let _tlsKey: __swift_thread_key_t = {
  ...
  _invariant(success == 0, "somehow failed to create TLS key")
  _invariant(key != sentinelValue, "Didn't make a new key")
  return key
}()
...
internal func _initializeThreadLocalStorage()
  -> UnsafeMutablePointer<_ThreadLocalStorage>
{
  _invariant(_stdlib_thread_getspecific(_tlsKey) == nil,
    "already initialized")
  ...
  _invariant(success == 0, "setspecific failed")
  return tlsPtr
}
