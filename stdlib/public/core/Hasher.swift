...
internal struct _HasherTailBuffer {
  ...
  mutating func append(_ bytes: UInt64, count: UInt64) -> UInt64? {
    _correctnessCheck(count >= 0 && count < 8)
    _correctnessCheck(bytes & ~((1 &<< (count &<< 3)) &- 1) == 0)
    let c = byteCount & 7
    let shift = c &<< 3
    if c + count < 8 {
...
internal struct _BufferingHasher<Core: _HasherCore> {
  ...
  internal mutating func combine(bytes: UnsafeRawBufferPointer) {
    ...
    _correctnessCheck(remaining >= 0 && remaining < 8)
    if remaining > 0 {
...
public struct Hasher {
  ...
  internal static func _hash(seed: (UInt64, UInt64), _ value: UInt) -> Int {
    var core = RawCore(seed: seed)
#if arch(i386) || arch(arm)
    _correctnessCheck(UInt.bitWidth < UInt64.bitWidth)
