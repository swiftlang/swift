...
>(
  ...
  _invariant(Source.bitWidth < Target.bitWidth)
  _invariant(count >= 0)
...
struct _UnmanagedString<CodeUnit>
  ...
  init(start: UnsafePointer<CodeUnit>, count: Int) {
    _invariant(CodeUnit.self == UInt8.self || CodeUnit.self == UInt16.self)
    self.start = start
    self.count = count
  }
...
extension _UnmanagedString : RandomAccessCollection {
  ...
  internal subscript(position: Index) -> UTF16.CodeUnit {
    @inline(__always)
    get {
      _invariant(position >= start && position < end)
  ...
  internal subscript(_ bounds: Range<Index>) -> SubSequence {
    _invariant(bounds.lowerBound >= start && bounds.upperBound <= end)
...
extension _UnmanagedString : _StringVariant {
  ...
  internal subscript(offsetRange: Range<Int>) -> _UnmanagedString {
    _invariant(offsetRange.lowerBound >= 0 && offsetRange.upperBound <= count)
  ...
  internal subscript(offsetRange: PartialRangeUpTo<Int>) -> SubSequence {
    _invariant(offsetRange.upperBound <= count)
    return _UnmanagedString(
      start: start, 
  ...
  internal func _copy<TargetCodeUnit>(
    ...
    if CodeUnit.bitWidth == TargetCodeUnit.bitWidth {
      ...
      _invariant(TargetCodeUnit.bitWidth == 16)
      memcpy_zext(
        dst: target.baseAddress._unsafelyUnwrappedUnchecked,
        src: start,
        count: self.count)
    } else {
      _invariant(CodeUnit.bitWidth == 16 && TargetCodeUnit.bitWidth == 8)
      _invariant(self.filter { $0 >= UInt8.max }.isEmpty, "ASCII only")
      memcpy_trunc(
        dst: target.baseAddress._unsafelyUnwrappedUnchecked,
        src: start,
        count: self.count)
