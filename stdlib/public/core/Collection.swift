...
public protocol Collection: Sequence where SubSequence: Collection {
  ...
  ///   starting index.
  subscript(offset offset: Range<IndexDistance>) -> SubSequence { get }
...
extension Collection where SubSequence == Slice<Self> {
  ...
  ///   starting index.
  @_inlineable
  public subscript(offset offset: Range<IndexDistance>) -> SubSequence {
    let start = index(startIndex, offsetBy: offset.lowerBound)
    let end = index(startIndex, offsetBy: offset.upperBound)
  ...
  ///   starting index.
  @_inlineable
  public subscript(offset offset: ClosedRange<IndexDistance>) -> SubSequence {
    let start = index(startIndex, offsetBy: offset.lowerBound)
    let end = index(startIndex, offsetBy: offset.upperBound)
    return self[start..<index(after: end)end]
  }
}
