...
extension _StringGuts {
  ...
  internal func _invariantCheck() {
    ...
    if _object.isNative {
      _correctnessCheck(UInt(_object.nativeRawStorage.count) == self._otherBits)
    } else if _object.isUnmanaged {
    } else if _object.isCocoa {
      if _object.isContiguous {
        _correctnessCheck(_isValidAddress(_otherBits))
      } else {
        _correctnessCheck(_otherBits == 0)
      }
  ...
  _correctnessCheck(MemoryLayout<String>.size == 12, """
    the runtime is depending on this, update Reflection.mm and \
    this if you change it
    """)
#else
  _correctnessCheck(MemoryLayout<String>.size == 16, """
    the runtime is depending on this, update Reflection.mm and \
    this if you change it
    """)
#endif
...
extension _StringGuts {
  ...
  var _nativeCount: Int {
    @inline(__always) get {
      _correctnessCheck(_object.isNative)
...
extension _StringGuts {
  ...
  var _cocoaCount: Int {
    @inline(__always)
    get {
      _correctnessCheck(_object.isCocoa)
      defer { _fixLifetime(self) }
  ...
  var _cocoaRawStart: UnsafeRawPointer {
    @inline(__always)
    get {
      _correctnessCheck(_object.isContiguousCocoa)
      _correctnessCheck(_isValidAddress(_otherBits))
  ...
  where CodeUnit : FixedWidthInteger & UnsignedInteger {
    _correctnessCheck(_object.isContiguousCocoa)
    _correctnessCheck(CodeUnit.bitWidth == _object.bitWidth)
  ...
  init(
    ...
    _correctnessCheck(!_isObjCTaggedPointer(s))
    guard count > 0 else {
      self.init()
      return
    }
    ...
    if start == nil {
      _correctnessCheck(_object.isOpaque)
    } else {
      _correctnessCheck(_object.isContiguous)
    }
  }
}
#else // !_runtime(_ObjC)
...
extension _StringGuts {
  ...
  internal var _unmanagedRawStart: UnsafeRawPointer {
    @inline(__always) get {
      _correctnessCheck(_object.isUnmanaged)
  ...
  where CodeUnit : FixedWidthInteger & UnsignedInteger {
    _correctnessCheck(s.count >= 0)
    self.init(
      object: _StringObject(unmanaged: s.start),
      otherBits: UInt(bitPattern: s.count))
    _correctnessCheck(_object.isUnmanaged)
    _correctnessCheck(_unmanagedRawStart == s.rawStart)
    _correctnessCheck(_unmanagedCount == s.count)
    _invariantCheck()
  }
}
...
extension _StringGuts {
  ...
  var _unmanagedASCIIView: _UnmanagedString<UInt8> {
    @_effects(readonly)
    get {
      _correctnessCheck(_object.isContiguousASCII)
  ...
  var _unmanagedUTF16View: _UnmanagedString<UTF16.CodeUnit> {
    @_effects(readonly)
    get {
      _correctnessCheck(_object.isContiguousUTF16)
      if _object.isUnmanaged {
        return _asUnmanaged()
      } else if _object.isNative {
...
extension _StringGuts {
  ...
  internal func _asOpaque() -> _UnmanagedOpaqueString {
    ...
    _correctnessCheck(_object.isNoncontiguousCocoa)
...
extension _StringGuts {
  ...
  where CodeUnit : FixedWidthInteger & UnsignedInteger {
    _correctnessCheck(unusedCapacity >= 0)
...
extension _StringGuts {
  ...
  internal var count: Int {
    ...
    _correctnessCheck(Int(self._otherBits) >= 0)
  ...
  where CodeUnit : FixedWidthInteger & UnsignedInteger {
    _correctnessCheck(CodeUnit.bitWidth == 8 || CodeUnit.bitWidth == 16)
  ...
  mutating func append(_ other: _StringGuts, range: Range<Int>) {
    _correctnessCheck(range.lowerBound >= 0 && range.upperBound <= other.count)
...
extension _StringGuts {
  ...
  CodeUnit : FixedWidthInteger & UnsignedInteger {
    ...
    if _fastPath(paramsOpt == nil) {
      ...
      unowned(unsafe) let storage = _object.nativeStorage(of: CodeUnit.self)
      _correctnessCheck(storage.unusedCapacity >= deltaCount)
    ...
    _correctnessCheck(dst + suffixRange.count == storage.end)
    self = _StringGuts(_large: storage)
    _invariantCheck()
  }
