//===----------------------------------------------------------------------===//
//
// This source file is part of the Swift.org open source project
//
// Copyright (c) 2014 - 2016 Apple Inc. and the Swift project authors
// Licensed under Apache License v2.0 with Runtime Library Exception
//
// See http://swift.org/LICENSE.txt for license information
// See http://swift.org/CONTRIBUTORS.txt for the list of Swift project authors
//
//===----------------------------------------------------------------------===//

@_exported import Intents
import Foundation

#if os(iOS)
@available(iOS 10.0, *)
extension INSetSeatSettingsInCarIntent {
  @nonobjc
  public convenience init(
    enableHeating: Bool? = nil,
    enableCooling: Bool? = nil,
    enableMassage: Bool? = nil,
    seat: INCarSeat = .Unknown,
    level: Int? = nil,
    relativeLevelSetting: INRelativeSetting = .Unknown
  ) {
    self.init(__enableHeating: enableHeating.map { NSNumber(bool: $0) },
      enableCooling: enableCooling.map { NSNumber(bool: $0) },
      enableMassage: enableMassage.map { NSNumber(bool: $0) },
      seat: seat,
      level: level.map { NSNumber(long: $0) },
      relativeLevelSetting: relativeLevelSetting)
  }

  @nonobjc  
  public final var enableHeating: Bool? {
    return __enableHeating?.boolValue
  }

  @nonobjc  
  public final var enableCooling: Bool? {
    return __enableCooling?.boolValue
  }

  @nonobjc  
  public final var enableMassage: Bool? {
    return __enableMassage?.boolValue
  }

  @nonobjc  
  public final var level: Int? {
    return __level?.longValue
  }
}
#endif
