...
public func os_log(
	...
	dso: UnsafeMutablePointer<Void>? = nil, 
	log: OSLog = .default, 
	...
	_ args: CVarArg...) 
{
	...
	message.withUTF8Buffer { (buf: UnsafeBufferPointer<UInt8>) in
		let str = UnsafePointer<Int8>(buf.baseAddress!)
		var info = Dl_info()
		_ = dladdr(str, &info)

		let valist = getVaList(args)
		_swift_os_log(info.dli_fbase, log, type, str, valist)
	}
}
...
internal func _swift_os_log(_ dso: UnsafeMutablePointer<Void>!, _ log: OSLog, _ type: OSLogType, _ format: UnsafePointer<Int8>!, _ args: CVaListPointer)
