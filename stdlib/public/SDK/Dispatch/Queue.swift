...
public extension DispatchQueue {
	...
	public class func getSpecific<T>(key: DispatchSpecificKey<T>) -> T? {
		let k = UnsafePointer<Void>(OpaquePointer(bitPattern: Unmanaged.passUnretained(key)))
		if let p = __dispatch_get_specific(k) {
			let v = Unmanaged<_DispatchSpecificValue<T>>
				.fromOpaque(OpaquePointer(p))
				.takeUnretainedValue()
			return v.value
		}
		return nil
	...
	public func setSpecific<T>(key: DispatchSpecificKey<T>, value: T) {
		...
		let k = UnsafePointer<Void>(OpaquePointer(bitPattern: Unmanaged.passUnretained(key)))
		let p = UnsafeMutablePointer<Void>(OpaquePointer(bitPattern: Unmanaged.passRetained(v)))
...
private func _destructDispatchSpecificValue(ptr: UnsafeMutablePointer<Void>?) {
	if let p = ptr {
		Unmanaged<AnyObject>.fromOpaque(OpaquePointer(p)).release()		
	}
}
