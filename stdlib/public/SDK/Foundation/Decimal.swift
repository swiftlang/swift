...
extension Decimal {
    ...
    public static let leastNormalMagnitude = Decimal(_exponent: -127, _length: 1, _isNegative: 0, _isCompact: 1, _reserved: 0, _mantissa: (0x0001, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0001))
    ...
    public init(signBit: Bool, exponent: Int, significand: Decimal) {
        self.init(_exponent: Int32(exponent), _length: significand._length, _isNegative: signBit ? 1 : 0, _isCompact: significand._isCompact, _reserved: 0, _mantissa: significand._mantissa)
    }

    public init(magnitudeOf magnitude: Decimal, signOf: Decimal) {
...
public func +(lhs: Decimal, rhs: Decimal) -> Decimal {
    ...
    var leftOp = lhs
    var rightOp = rhs
    NSDecimalAdd(&res, &leftOp, &rightOp, .roundPlain)
    return res
}
...
public func -(lhs: Decimal, rhs: Decimal) -> Decimal {
    ...
    var leftOp = lhs
    var rightOp = rhs
    NSDecimalSubtract(&res, &leftOp, &rightOp, .roundPlain)
    return res
}
...
public func /(lhs: Decimal, rhs: Decimal) -> Decimal {
    ...
    var leftOp = lhs
    var rightOp = rhs
    NSDecimalDivide(&res, &leftOp, &rightOp, .roundPlain)
    return res
}
...
public func *(lhs: Decimal, rhs: Decimal) -> Decimal {
    ...
    var leftOp = lhs
    var rightOp = rhs
    NSDecimalMultiply(&res, &leftOp, &rightOp, .roundPlain)
    return res
}
...
public func pow(_ x: Decimal, _ y: Int) -> Decimal {
    var res = Decimal()
    var num = x
    NSDecimalPower(&res, &num, y, .roundPlain)
    return res
}
...
extension Decimal {
    // This is just here for conformance; however Decimal is not castable to any bit representation formable via int type derivatives in that sizeof(Decimal) > sizeof(UIntMax)
    public typealias _BitsType = Decimal
    
    @warn_unused_result
    public static func _fromBitPattern(_ bits: _BitsType) -> Decimal {
        return bits
    }
