...
public struct IndexSet : ReferenceConvertible, Equatable, BidirectionalCollection, SetAlgebra {
    ...
    public func union(_ other: IndexSet) -> IndexSet {
        ...
        while let i = boundaryIterator.next() {
            if !flag {
                // One of the two sets containment is true by definition of iteration, so start a range.
                flag = true
                start = i
            } else {
                // If both sets containment is false then the range has ended. Otherwise, continue on.
                if !(self.contains(i) && other.contains(i)) {
                    flag = false
                    result.insert(integersIn: start..<(i + 1))
                } else {
                    // Update in case we fall off the edge of this while loop
                    end = i
                }
            }
        }
        
        // Flag may be left set, if we ended on a pair of values that was in both last ranges
        if flag {
...
/// Iterate two index sets on the boundaries of their ranges. This is where all of the interesting stuff happens for union, exclusive or, intersect, etc.
