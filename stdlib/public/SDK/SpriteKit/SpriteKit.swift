...
extension SKWarpGeometryGrid {
  ...
  public convenience init(columns: Int, rows: Int, destinationPositions: [simd.float2] = [float2](), sourcePositions: [simd.float2] = [float2]()) {
  ...
  /// - Parameter positions: the source positions for the new SKWarpGeometryGrid
  public func replacingBySourcePositions(positions sources: [simd.float2]) -> SKWarpGeometryGrid {
    let requiredElementsCount = (self.numberOfColumns + 1) * (self.numberOfRows + 1)
     _precondition(requiredElementsCount == sources.count, "Mismatch found when replacing sourcePositions.")
    return self.__replacingSourcePositions(sources)
  }
  /// - Parameter positions: the destination positions for the new SKWarpGeometryGrid
  public func replacingByDestinationPositions(positions destinations: [simd.float2]) -> SKWarpGeometryGrid {
    let requiredElementsCount = (self.numberOfColumns + 1) * (self.numberOfRows + 1)
    _precondition(requiredElementsCount == destinations.count, "Mismatch found when replacing destinationPositions.")
    return self.__replacingDestPositions(destinations)
  }
}
