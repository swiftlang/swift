...
suspend_other_threads(struct thread *self)
{
  ...
  do {
    ...
    for (;;) {
      ...
      if ((int64_t)tid != self->tid && !seen_thread(tid)) {
        ...
        int retries = 0;
        while (sig_to_use <= 0 && retries < 5) {
          // when we are unable to get a suitable signal, the usual
          // cause is glibc masking signals temporarily, for things like
          // pthread creation/termination, try a few fairly quick checks
          struct timespec shortDelay;
          shortDelay.tv_sec = 0;
          shortDelay.tv_nsec = 10000000;
          nanosleep(&shortDelay, NULL);
          sig_to_use = signal_for_suspend(our_pid, tid);
          ++retries;
        }
    ...
    if (waitingToSignal) {
      timeout = (struct timespec){ 0, 10000000 };
    }
  ...
  if (waitingToSignal) {
    warn("failed to suspend thread while processing a crash; "
