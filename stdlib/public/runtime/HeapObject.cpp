...
// swiftClientFastPaths library which is currently implemented for ARM64 Mach-O.
#if __arm64__ && __LP64__ && defined(__APPLE__) && defined(__MACH__)
asm(R"(
// Define a mask used by swift_retain/releaseClient in the ClientFastPaths
...
// zero when running against older runtimes. ClientFastPaths references it using
...
// that it comes out right in ClientFastPaths.
...
// ClientFastPaths will reference these so that it can fall back to a
