...
struct ClassImpl : ReflectionMirrorImpl {
  ...
  bool hasSuperclassMirror() {
    ...
    return ((description->SuperclassType) && (Clas->Superclass));
  }
...
// func _getChildCount<T>(type: Any.Type) -> Int
SWIFT_CC(swift) SWIFT_RUNTIME_STDLIB_API
intptr_t swift_reflectionMirror_recursiveCount(
                                      const Metadata *type,
                                      const Metadata *T) {
  return call(nullptr, T, type, [](ReflectionMirrorImpl *impl) {
    return impl->recursiveCount();
  });
}

// func _getChildMetadata<T>(
//   type: Any.Type,
//   index: Int,
