...
#if SWIFT_CONCURRENCY_ENABLE_PRIORITY_ESCALATION
  // This balances with the retain in taskEnqueueDirectOrSteal
  // since a reference to a Task may exist long after it finishes
  // executing so it must remain alive until all links are gone.
  swift_release(task);
#endif
}
