...
struct AsyncTask::PrivateStorage {
  ...
  uint8_t LocalStealerExclusionValue;
  // On 64-bit, there are 3 bytes of padding
  // and on 32-bit, there are 3 bytes of padding and 4 available bytes
...
/// Remove the enqueued bit in the ActiveTaskStatus atomically.
/// This must be done when a Task's intrusive link is dequeued
...
static inline void taskRemoveEnqueued(AsyncTask *task) {
  ...
  do {
    // This could be an atomic AND rather than a CAS loop
    newStatus = oldStatus.withoutEnqueued();
  } while (!task->_private()._status().compare_exchange_weak(oldStatus, newStatus,
                                                       /* success */ std::memory_order_relaxed,
                                                       /* failure */ std::memory_order_relaxed));
}
...
struct ThreadPriorityManager {
  ...
  ThreadPriorityManager(AsyncTask const& task) {
    ...
    basePriorityCeil = overrideFloor;
...
static inline uint32_t taskFlagAsRunningWithoutDependency(AsyncTask &task, bool removeEnqueued = false) {
#if SWIFT_CONCURRENCY_ENABLE_PRIORITY_ESCALATION
...
inline std::pair<bool, uint32_t> AsyncTask::flagAsRunning(uint8_t allowedExclusionValue, bool removeEnqueued) {
  ...
  SWIFT_TASK_DEBUG_LOG("%p run by stealer %d, exclusion value %d", this, !removeEnqueued, allowedExclusionValue);

  // Stealers are only enabled with priority escalation
  auto oldStatus = _private()._status().load(std::memory_order_relaxed);
  if (oldStatus.getStealerExclusionValue() != allowedExclusionValue) {
    if (removeEnqueued) taskRemoveEnqueued(this);
    return {false, 0};
  }
#else
  auto oldStatus = _private()._status().load(std::memory_order_relaxed);
...
public:
  ...
  AsyncTaskStealer(AsyncTask *task, JobPriority priority, uint8_t exclusionValue)
      : Job({JobKind::TaskStealer, priority}, &process),
        Task(static_cast<AsyncTask *>(swift_retain(task))),
...
taskEnqueueDirectOrSteal(AsyncTask *task, SerialExecutorRef newExecutor,
  ...
  auto needsStealer = oldStatus.isEnqueued();
  ...
  auto updateStatus = (updateStealerExclusionValue || !needsStealer);
  ...
  do {
    ...
    SWIFT_TASK_DEBUG_LOG(
        ...
        updateStealerExclusionValue, !oldStatus.isEnqueued(), needsStealer,
  ...
  } while ((updateStealerExclusionValue || !oldStatus.isEnqueued()) &&
