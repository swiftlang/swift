...
SWIFT_CC(swiftasync)
static void non_future_adapter(AsyncTask *task, ExecutorRef executor,
  ...
  return asyncContextPrefix->asyncEntryPoint(
      task, executor, _context, asyncContextPrefix->closureContext);
}

SWIFT_CC(swiftasync)
static void future_adapter(AsyncTask *task, ExecutorRef executor,
                           SWIFT_ASYNC_CONTEXT AsyncContext *_context) {
  auto asyncContextPrefix = reinterpret_cast<FutureAsyncContextPrefix *>(
      reinterpret_cast<char *>(_context) - sizeof(FutureAsyncContextPrefix));
  return asyncContextPrefix->asyncEntryPoint(
      asyncContextPrefix->indirectResult, task, executor, _context,
...
SWIFT_CC(swiftasync)
void swift::swift_task_future_wait(OpaqueValue *result, AsyncTask *waitingTask,
...
void swift::swift_task_future_wait_throwing(
    OpaqueValue *result, AsyncTask *waitingTask, ExecutorRef executor,
