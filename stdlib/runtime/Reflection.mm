...
// HACKHACKHACK: try to decide if this descriptor describes a type
// generated by the LLDB expression evaluator; this is a workaround for
// rdar://17986612 such that we can avoid trying to use the
// fieldNames pointer inside such types until the JIT starts emitting
// a validly relocated pointer for that piece of information
static bool isLLDBDefinedType (const NominalTypeDescriptor *descriptor) {
  static std::regex lldb_expr_prefix("^[CV][0-9]+__lldb_expr_[0-9]+",
                                     std::regex::extended | std::regex::optimize);
  static std::regex lldb_repl_prefix("^[CV][0-9]+__lldb_repl_[0-9]+",
                                     std::regex::extended | std::regex::optimize);
  static std::regex lldb_playground_prefix("^[CV][0-9]+__lldb_playground_[0-9]+",
                                           std::regex::extended | std::regex::optimize);
  ...
  const bool is_playground = std::regex_search(Name, lldb_playground_prefix);
  
  if (is_playground)
    return true;
  
  const bool is_repl = std::regex_search(Name, lldb_repl_prefix);
  
  if (is_repl)
    return true;
  
  const bool is_expr = std::regex_search(Name, lldb_expr_prefix);
...
StringMirrorTuple swift_StructMirror_subscript(intptr_t i,
  ...
  if (!isLLDBDefinedType(Struct->Description)) {
...
StringMirrorTuple swift_ClassMirror_subscript(intptr_t i,
  ...
  if (!isLLDBDefinedType(Clas->getDescription())) {
    // Get the field name from the doubly-null-terminated list.
