...
requirements. Properties can also contain operators, properties, and subscript
operators::
  
  properties RandomAccessContainer : Collection {
    var length : Int
...
that T be Comparable, e.g.,::
  ...
  func forEachKey<Key : Comparable, Value>(c : SortedDictionary<Key,
  Value>, f : (Key) -> Void) { /* ... */ }
...
function, simplifying the specification of forEachKey::
  ...
  func forEachKey<Key, Value>(c : SortedDictionary<Key, Value>, f :
  (Key) -> Void) { /* ... */ }
...
We're going to try a variant of #4. Essentially, we will need to tentatively
parse the expression when we see::

  identifier <

to classify it as either (a) definitely a specialization of a generic type, (b)
definitely a less-than operation, or (c) either a less-than or a generic type
specialization. For (c), we'll introduce a special expression kind to cover the
ambiguity. At name binding time, the result of the lookup for the identifier
will determine which way to resolve the ambiguity: if we find that the
identifier refers to a generic type, we treat the identifier < ... > as a
specialization of that generic type. Otherwise, we '<' is a less-than
operator. We then replace the ambiguous AST node for (c) with the appropriate
AST nodes(s) that would have been generated by (a) or (b). By the time we hit
semantic analysis, the ambiguity will be gone.

This approach makes '<' and '>' magic, and precludes writing an operator< that
operates on metatypes, e.g.,::

  func operator<(a : metatype<Int>, b : metatype<Int>)

We're comfortable with that implementation.

If this tentative parsing and disambiguation fails, we'll consider alternatives
