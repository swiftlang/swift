...
You can make this permanent by creating a text file with a name ending with `.REG` and with the following content.
Run the file to associate the `S:` drive to the `C:\toolchain` directory.
...
- Set the `SKIP_UPDATE_CHECKOUT` environment variable to `true` to avoid cloning the repositories again: `set SKIP_UPDATE_CHECKOUT=true`.
- Start the build: `S:\swift\utils\build.cmd`. This will build the toolchain, its installer and run all the test suite.
