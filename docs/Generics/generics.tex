...
\item \textbf{Query:} \IndexDefinition{#1()@\texttt{#1()}}$\texttt{#1}(#2)$
...
\subtitle{Implementation Reference Manual (DRAFT v2)}
\author{Slava Pestov}
...
\part{Syntax}\label{part fundamentals}
...
\part{Semantics}\label{part blocks}
...
\part{Processes}\label{part odds and ends}
...
\part[]{More Features}\label{part features}
