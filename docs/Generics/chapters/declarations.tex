...
\begin{itemize}
\item A class can inherit from a \index{superclass type}superclass type, and members of the superclass are also visible from the subclass (\SecRef{sec:subclassing}).
...
\begin{Verbatim}
class Polygon { static func callee1() {} }
...
\node (Shape) [class,anchor=mid] at (1,1) {\texttt{\vphantom{p}class Shape}};
\node (Polygon) [class,anchor=mid] at (2,1) {\texttt{protocol Polygon}};

\node (extShape) [class,anchor=mid] at (2,0) {\texttt{extension Polygon}};

\path [arrow] (caller) edge [above] node {\footnotesize{parent}} (Circle);
\path [arrow] (Circle) edge [above] node {\footnotesize{parent}} (Square);
\path [arrow] (Square) edge [right] node {\footnotesize{conforms to}} (Polygon);
\path [arrow] (Circle) edge [right] node {\footnotesize{superclass}} (Shape);
\draw [arrow] (Polygon) edge [right] node {\footnotesize{extension}} (extShape);
...
As with all type declarations, the interface type of a type alias declaration is the metatype of its declared interface type. In the above, the type of ``\texttt{Hands.self}'' is the metatype \texttt{Hands.Type}. This is a sugared type, canonically equal to \texttt{Int.Type}.
...
We've now seen the first two kinds of type declarations. In the next two sections, we expand upon this by looking at the declarations of generic parameters, protocols, and associated types. At this point, our foray into Swift generics can begin in earnest. Here are all the type declaration kinds and their declared interface types, with a representative specimen of each:
\begin{center}
\begin{tabular}{ll}
\toprule
...
Function, constructor, and subscript declarations can also declare \IndexDefinition{opaque parameter}\emph{opaque parameters} with the \texttt{some} keyword, possibly in conjunction with a generic parameter list:
\begin{Verbatim}
...
Before we introduce the trailing \texttt{where} clause syntax for stating requirements in a fully general way, let's look at the shorthand of stating a \emph{constraint type} in the \IndexDefinition{inheritance clause!generic parameter declaration}inheritance clause of a \index{generic parameter declaration}generic parameter declaration:
\begin{Verbatim}
func allEqual<E: Equatable>(_ elements: [E]) {...}
\end{Verbatim}
The generic parameter declaration~\texttt{E} declares the generic parameter type~\rT, and it also states the constraint type \IndexProto{Equatable}\texttt{Equatable}. This is a protocol declared in the standard library, so the stated requirement is the conformance requirement $\ConfReq{\rT}{Equatable}$.

\begin{definition}\label{def:constraint type}
A \IndexDefinition{constraint type}constraint type is one of the following:
\begin{enumerate}
...
\end{tikzpicture}
\end{center}
There are only two kinds of requirement representations, because the ``\texttt{:}'' form cannot distinguish between conformance, superclass, and layout requirements until we resolve the type representation on the right-hand side:
\begin{enumerate}
...
In \SecRef{derived req}, we will introduce a formal system for derived requirements, where we will assume that only conformance requirements to protocol types remain, and that the subject type of a requirement is always a type parameter, and not an arbitrary type. \SecRef{requirement desugaring} will show how we eliminate these unnecessary forms of generality.
...
In \ChapRef{chap:generic signatures}, we will see that the generic signature of a declaration records all of its requirements, regardless of how they were stated in source.
...
Protocols can also limit their conforming types to subclasses of some \index{superclass type}superclass. This was introduced in \IndexSwift{4.0}Swift~4 \cite{se0156}:
\begin{Verbatim}
class Plant {}
class Animal {}
...
A protocol can declare a list of \IndexDefinition{primary associated type}\emph{primary associated types} with a syntax resembling that of a generic parameter list. \IndexProto{IteratorProtocol}For example:
\begin{Verbatim}
...
\item An associated type declaration \texttt{A} can state an inheritance clause. Each entry defines a conformance, superclass, or layout requirement with a subject type of \texttt{Self.A}.
...
A protocol's associated requirements are collected in its \emph{requirement signature}, which we will see is dual to a generic signature in some sense (\SecRef{requirement sig}). How concrete types satisfy the requirement signature will be discussed in \ChapRef{chap:conformances}.
...
\end{Verbatim}
We derive the \index{interface type!self parameter}interface type and \index{ownership specifier!self parameter}ownership specifier for the \Index{self parameter@\texttt{self} parameter}\texttt{self} parameter as follows:
\begin{itemize}
\item We start with the \IndexDefinition{self interface type}\emph{self interface type} of the method's parent declaration context. In a struct, enum, or class, this is the same as the \index{declared interface type!self interface type}declared interface type. In a protocol, this is the protocol \IndexSelf\tSelf\ type (\SecRef{protocols}). The self interface type of an extension is the self interface type of the extended type.
...
\item If the method is \IndexDefinition{static method declaration}\texttt{static}, we further wrap the \texttt{self} type in a \index{metatype}metatype.

\item If the method is \texttt{mutating}, we pass the \texttt{self} parameter \Index{inout parameter@\texttt{inout} parameter}\texttt{inout}.

\end{itemize}

Here is the syntax tree for the method call ``\texttt{universe.wormhole(x:~1, y:~"hi")}'':
\begin{center}
\begin{forest}
for tree={folder, grow'=0, fit=band, s sep=2pt, draw=black, thick, rounded corners}
[\vphantom{p}call: \texttt{universe.wormhole(x:~1, y:~"hi")},
  [\vphantom{p}self apply: \texttt{universe.wormhole}
    [\vphantom{p}callee:
      [\vphantom{p}declaration reference: \texttt{Universe.wormhole(x:y:)}]]
    [\vphantom{p}self argument:
      [\vphantom{p}declaration reference: \texttt{universe}]]
  ]
  [\vphantom{p}argument list: \texttt{(1, "hi")}
    [\vphantom{p}literal: \texttt{1}]
    [\vphantom{p}literal: \texttt{"hi"}]
  ]
]
\end{forest}
\end{center}

Let's compare the interface type of a method declaration with the above structure:
\begin{itemize}
\item The outer expression's callee, \texttt{universe.wormhole}, is itself a special kind of \index{expression}call expression known as a \IndexDefinition{self apply expression}\emph{self apply expression}, so we must evaluate it first.

The self apply expression applies the argument \texttt{universe} to the \texttt{self} parameter of \texttt{Universe.wormhole()}. This represents the method lookup. The return type of the inner call expression is \verb|(Int, String) -> Bool|.
\item The outer call expression applies the argument list \verb|(1, "hi")| to the result of the self apply expression. This represents the method call itself, so this expression produces a value of \texttt{Bool} type.
\end{itemize}
The extra call to apply \texttt{self} disappears in \index{SILGen}SILGen, because a method is represented as a SIL function that receives all formal parameters and the \texttt{self} parameter at once.

A partially-applied method reference, such as \texttt{universe.wormhole}, can also be used as a value of type \verb|(Int, String) -> Bool|. This binds the \texttt{self} parameter but does not call the method. We implement this by wrapping the method reference in a closure that invokes the method; this closure has the same type as the method reference. In the \index{lambda calculus}lambda calculus formalism, this is what's known as \index{eta expansion}\emph{$\eta$-expansion}:
\begin{Verbatim}
...
\end{Verbatim}
This simplifies the SILGen implementation, because we only need to handle fully-applied method calls that pass all formal parameters and \texttt{self} at once.
...
\IndexDefinition{destructor declaration}Introduced with the \texttt{deinit} keyword, which is only valid inside a class. A destructor cannot have a generic parameter list or \texttt{where} clause. The interface type of a destructor in a class \tC\ is \texttt{(C) -> () -> ()}.
...
Local functions and closures can \IndexDefinition{captured value}\emph{capture} references to other local declarations from outer scopes. We use the standard technique of \IndexDefinition{closure conversion}\emph{closure conversion} to lower functions with captured values into ones without \cite{appel}. The basic idea is that we introduce an additional function parameter for each captured value, and then we walk the function body, to replace references to these captured values with references to the corresponding parameters. In Swift, this is part of \index{SILGen}SILGen's lowering process, and not a separate transformation on the abstract syntax tree.
...
\item (Nested closures) If $F$ contains a nested closure expression or local function $F'\!$, then any captures of $F'$ not declared by~$F$ are also captures of~$F$.

Recursively compute the captures of $F'\!$. For each $d$ captured by $F'$ such that $\texttt{PARENT}(d)\neq F$, set $C\leftarrow C\cup\{d\}$.
...
This completely explains captures of \texttt{let} variables, but mutable \texttt{var} variables and \Index{inout parameter@\texttt{inout} parameter}\texttt{inout} parameters merit further explanation.
...
\begin{forest}
for tree={folder, grow'=0, fit=band, s sep=2pt, draw=black, thick, rounded corners}
[\texttt{\vphantom{p}Decl}
  [\texttt{\vphantom{p}ValueDecl}
    [\texttt{\vphantom{p}TypeDecl}
      [\texttt{\vphantom{p}GenericTypeDecl}
        [\texttt{\vphantom{p}NominalTypeDecl}
          [\texttt{\vphantom{p}StructDecl}]
          [\texttt{\vphantom{p}EnumDecl}]
          [\texttt{\vphantom{p}ClassDecl}]
          [\texttt{\vphantom{p}ProtocolDecl}]
        ]
        [\texttt{\vphantom{p}TypeAliasDecl}]
      ]
      [\texttt{\vphantom{p}GenericTypeParamDecl}]
      [\texttt{\vphantom{p}AssociatedTypeDecl}]
      [\texttt{\vphantom{p}ModuleDecl}]
    ]
    [\texttt{\vphantom{p}AbstractFunctionDecl}
      [\texttt{\vphantom{p}FuncDecl}
        [\texttt{\vphantom{p}AccessorDecl}]
      ]
      [\texttt{\vphantom{p}ConstructorDecl}]
      [\texttt{\vphantom{p}DestructorDecl}]
    ]
    [\texttt{\vphantom{p}AbstractStorageDecl}
      [\texttt{\vphantom{p}VarDecl}
        [\texttt{\vphantom{p}ParamDecl}]
      ]
      [\texttt{\vphantom{p}SubscriptDecl}]
    ]
  ]
  [\texttt{\vphantom{p}ExtensionDecl}]
]
\end{forest}
\end{center}
\end{figure}

Instances are always allocated in the \IndexSource{permanent arena}permanent arena of the \IndexSource{AST context}\texttt{ASTContext}, either when the declaration is parsed, or \IndexSource{synthesized declaration}synthesized. The top-level \verb|isa<>|, \verb|cast<>| and \verb|dyn_cast<>| template functions support dynamic casting from \texttt{Decl *} to any of its subclasses.
\begin{itemize}
\item \texttt{getDeclContext()} returns the parent \texttt{DeclContext} of this declaration.
\item \texttt{getInnermostDeclContext()} returns the declaration as a \texttt{DeclContext} if this declaration is also a declaration context, otherwise returns the parent \texttt{DeclContext}.
...
\end{Verbatim}
However, just as with types, it can be more convenient to use the visitor pattern, by subclassing \texttt{ASTVisitor} and overriding various \texttt{visit\emph{Kind}Decl()} methods. The visitor's \texttt{visit()} method performs the switch and dynamic cast dance above, and calls each method:
\begin{Verbatim}
...
\begin{itemize}
\item \texttt{getAsDecl()} if this declaration context is also a declaration, returns the declaration, otherwise returns \texttt{nullptr}.
...
\item \texttt{getSelfStructDecl()} as above, but the result is a \texttt{StructDecl *} or \texttt{nullptr}.
\item \texttt{getSelfEnumDecl()} as above, but the result is a \texttt{EnumDecl *} or \texttt{nullptr}.
\item \texttt{getSelfClassDecl()} as above, but the result is a \texttt{ClassDecl *} or \texttt{nullptr}.
\item \texttt{getSelfProtocolDecl()} as above, but the result is a \texttt{ProtocolDecl *} or \texttt{nullptr}.
\item \texttt{getDeclaredInterfaceType()} delegates to the method with the same name on \texttt{NominalTypeDecl} or \texttt{ExtensionDecl} as appropriate.
...
\item \texttt{getGenericContextDepth()} returns the \IndexSource{depth}depth of this declaration's generic parameter list, or \texttt{(unsigned)-1} if neither this declaration nor any outer declaration is generic.
\item \texttt{getTrailingWhereClause()} returns this declaration's trailing \texttt{where} clause, or \texttt{nullptr}.
\end{itemize}
...
\item \texttt{getOuterParameters()} returns the outer generic parameter list, linking multiple generic parameter lists for the same generic context. Only used for extensions of nested generic types (\SecRef{chap:extensions}).
\end{itemize}
...
\begin{itemize}
\item \IndexSource{depth}\texttt{getDepth()} returns the depth of this generic parameter declaration.
\item \IndexSource{index}\texttt{getIndex()} returns the index of this generic parameter declaration.
\item \texttt{getName()} returns the name of this generic parameter declaration.
\item \texttt{getDeclaredInterfaceType()} returns the \IndexSource{sugared type}sugared generic parameter type for this declaration, which prints as this generic parameter's name.
...
\item \texttt{getInherited()} returns this generic parameter declaration's \IndexSource{inheritance clause!generic parameter declaration}inheritance clause.
\end{itemize}
...
\apiref{GenericTypeParamDecl}{class}
A \IndexSource{generic parameter declaration}generic parameter declaration.
\begin{itemize}
\item \texttt{getDepth()} returns the depth of this generic parameter declaration.
\item \texttt{getIndex()} returns the index of this generic parameter declaration.
\item \texttt{getName()} returns the name of this generic parameter declaration if this is the sugared form, otherwise returns a string of the form ``\ttgp{d}{i}''.
\end{itemize}
...
\item \texttt{getSuperclassDecl()} returns this protocol's \IndexSource{superclass declaration}superclass declaration.
\end{itemize}
