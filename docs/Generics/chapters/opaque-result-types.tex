...
\begin{enumerate}
\item Here we get a completely unconstrained opaque result type, because \Index{Any@\texttt{Any}!opaque result type}\texttt{Any} is the empty protocol composition; a conformance requirement to \texttt{Any} is a no-op:
\begin{Verbatim}
...
\section{Opaque Archetypes}\label{sec:opaque archetypes}
...
\index{$\Ot$!z@\igobble|seealso{opaque archetype}}\index{opaque archetype!notation}
...
In the final line of the below listing, the \index{call expression}call expression \texttt{someSequence()} has the type~$\Ox$. We will deduce the type of \texttt{pick(someSequence())}:
\begin{Verbatim}
...
\begin{algorithm}[Apply substitution map to opaque archetype]\label{alg:substitute opaque archetype}
Receives an opaque archetype~$\Opaque{T}_d \otimes \Sigma$ and a substitution map $\Sigma'$ as input. Outputs $(\Ot_d\otimes\Sigma)\otimes\Sigma'$.
\begin{enumerate}
...
\item Construct the substitution map composition $\Sigma \otimes \Sigma'$ (\DefRef{subst map composition}).
\item Construct an opaque archetype from the triple $(\tT, d, \Sigma\otimes\Sigma')$, and return it. (This opaque archetype is written as $\Ot_d \otimes (\Sigma\otimes\Sigma')$ in our notation, for reasons that are now hopefully clear.)
\end{enumerate}
\end{algorithm}
...
\]
Thus, an opaque archetype is an \index{interface type!opaque archetype}interface type for the \index{output generic signature!opaque archetype}output generic signature of its \index{substitution map!opaque archetype}substitution map. In particular, if $\Sigma$ is the identity substitution map~$1_G$, then:
\[
...
\[
\ConfReq{$(\Ot \otimes \Sigma)$}{P} \otimes \Sigma' := \ConfReq{$(\Ot \otimes (\Sigma \otimes \Sigma'))$}{P}
\]
...
\begin{enumerate}
\item If a type parameter is rooted in a generic parameter of the owner declaration, we say it is an \IndexDefinition{outer type parameter!opaque result type}\emph{outer} type parameter.
...
\begin{enumerate}
\item Let $\tT'$ and $\tU'$ denote the root generic parameters of \tT\ and \tU, respectively.
\item If $\tT'$ has weight~0 and $\tU'$ has weight~1, return ``$<$''.
\item If $\tT'$ has weight~1 and $\tU'$ has weight~0, return ``$>$''.
...
\end{Verbatim}
Notice how we refer to the opaque archetype with a substitution map. Let's use the \index{demangler}\texttt{swift-demangle} tool to print out the \index{mangling}mangled name of the owner declaration:
\begin{Verbatim}
...
\begin{gather*}
\tT \otimes \Sigma' \in \TypeObj{O}\\
\tT \otimes \Sigma' \otimes \Sigma \in \TypeObj{H}
\end{gather*}
...
\[
\rO \otimes \Sigma' \otimes \Sigma = \texttt{Array<\rT>} \otimes \Sigma = \texttt{Array<Int>}
\]
The underlying type of $\Opaque{\rO.Element} \otimes \Sigma$ is:
\[
\texttt{\rO.Element} \otimes \Sigma' \otimes \Sigma = \texttt{\rT} \otimes \Sigma = \texttt{Int}
\]
\end{example}
...
To determine the \index{size}size of the return value buffer, we generate a call to the appropriate Swift runtime entry point, handing it the opaque type descriptor for \texttt{someSequence2()}, and the type metadata for \rT. At run time, this outputs type metadata for the underlying type, which is \texttt{Array<Int>}. We recover the type's size from its metadata, and generate a dynamic stack allocation with the result.
...
When the above visibility condition holds, we can obtain the underlying type substitution map~$\Sigma'$ for~$d$, and safely assume that the underlying type of~$\Ot$ will always be equal to~$\tT \otimes \Sigma'$ inside~$f$. However, there is one more thing to check.
...
\begin{enumerate}
\item If its underlying type is visible, substitute in underlying type, and recursively transform the underlying type if it contains further opaque archetypes.
\item Otherwise, recursively transform~$\Sigma$ with \AlgRef{unwrap opaque substitution map algorithm} and form a new opaque archetype with the transformed substitution map.
\end{enumerate}

\item For an \index{existential archetype}\textbf{existential archetype} $\Et \otimes \Sigma$:  transform $\Sigma$ with \AlgRef{unwrap opaque substitution map algorithm}. (We will meet existential archetypes in \SecRef{sec:opening existentials}.)
...
\begin{itemize}
\item For a \index{specialized conformance!opaque archetype}\textbf{specialized conformance} $\XP \otimes \Sigma$: transform~$\Sigma$ with \AlgRef{unwrap opaque substitution map algorithm} and return a new specialized conformance.

\item For an \index{opaque abstract conformance}\textbf{opaque abstract conformance} $\ConfReq{$(\Ot \otimes \Sigma)$}{P}$:
  \begin{enumerate}
  \item If its underlying conformance is visible, substitute in the underlying conformance. Recurse if its conforming type again contains opaque archetypes.
  \item Otherwise, recursively transform~$\Sigma$ with \AlgRef{unwrap opaque substitution map algorithm}.
\end{enumerate}
\item For an \index{existential abstract conformance}\textbf{existential abstract conformance} $\ConfReq{$(\Et \otimes \Sigma)$}{P}$: transform~$\Sigma$ with \AlgRef{unwrap opaque substitution map algorithm} and form a new existential abstract conformance.
\end{itemize}
\end{algorithm}

\begin{algorithm}[Specialize opaque archetypes within a substitution map]\label{unwrap opaque substitution map algorithm}
Receives a type expansion context and substitution map $\Sigma$.
\begin{itemize}
\item Recursively transform each replacement type of $\Sigma$ with \AlgRef{unwrap opaque type algorithm}, and each conformance with \AlgRef{unwrap opaque conformance algorithm}, and form a new substitution map.
\end{itemize}
\end{algorithm}

\paragraph{Circularity.}
We encountered \index{termination}\index{non-terminating computation}non-terminating compile-time computation in our prior discussion of conditional and recursive conformances (\SecRef{sec:conditional conformances}, \SecRef{recursive conformances}). This issue can also arise with opaque result types, because nothing in our discussion so far precludes the possibility of an \index{opaque result type!circularity}opaque result type whose underlying type is \index{circular reference}defined in terms of itself. If this were to happen, our algorithm for replacing an opaque archetype with its underlying type would run forever, if implemented as described. However, in reality, we attempt to \index{termination check}detect this and \index{diagnostic!opaque result type}diagnose an error, as we will see now.
...
\begin{itemize}
\item \IndexFlag{max-substitution-depth}\texttt{-max-substitution-depth} is the maximum number of recursive calls to \AlgRef{unwrap opaque type algorithm}. The default value is 500.
...
\item \texttt{getKind()} returns \texttt{GenericEnvironment::Kind::Opaque} in this case.
...
\item \texttt{mapTypeIntoEnvironment()} implements \AlgRef{map into opaque alg}.
\end{itemize}
...
\item \texttt{getOpaqueReturnTypeDecls()} populates the per-source file lookup table with the mangled name of each owner declaration. This is Step 1~and~2 of \AlgRef{resolve opaque archetype algorithm}.
