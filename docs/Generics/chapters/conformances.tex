...
\lettrine{C}{onformances relate} types with the protocols they conform to. To be precise, a \index{protocol conformance|see{conformance}}\IndexDefinition{conformance}\emph{conformance} is a semantic object in the compiler for describing how its conforming type \emph{witnesses} the requirements of a protocol. We start by describing the representation of conformances, then move on to conformance lookup in \SecRef{conformance lookup}. We then expand the previous chapter's type substitution algebra to include conformances, which leads to substitution of dependent member types in \SecRef{abstract conformances}. Finally, we will study associated type inference, and its connection to Boolean satisfiability, in \SecRef{associated type inference}.

Conformances come in three distinct varieties:
\begin{enumerate}
\item A \IndexDefinition{concrete conformance}\textbf{concrete conformance} records how a nominal type conforms to a protocol, in which case we know where the conformance was declared, and all of its witnesses.
...
\item An \IndexDefinition{invalid conformance}\textbf{invalid conformance} records that some type does not conform.
\end{enumerate}
...
\item An \index{existential conformance}\textbf{existential conformance} represents the conformance of an existential type to a protocol. This is possible only in special circumstances (\SecRef{sec:existential conformances}).
\end{enumerate}
...
\item The \index{generic signature!normal conformance}\textbf{generic signature} of the conforming context. If the conformance context is a constrained extension, this generic signature has additional requirements not present in the extended type's generic signature; we say that we have a \emph{conditional conformance} (\SecRef{sec:conditional conformances}).
...
\end{Verbatim}
When building the conformance lookup table for a subclass, we wrap each superclass conformance in a new \index{subclassing}\IndexDefinition{inherited conformance}\emph{inherited conformance} structure, which consists of the subclass type, together with the superclass conformance:
\begin{center}
...
We use \IndexDefinition{global conformance lookup}\emph{global conformance lookup} to find the conformance of a given type to a protocol. In particular, we can answer the ``yes/no'' question of whether a type conforms by checking if the result is anything but an invalid conformance. (But for \emph{conditional conformances}, we also need to check conditional requirements; see \SecRef{sec:conditional conformances}.)
...
\item If \tX\ is an \index{archetype!global conformance lookup}archetype: proceed as described in \SecRef{sec:local requirements}.
\item If \tX\ is an \index{existential type!global conformance lookup}existential type: proceed as described in \SecRef{sec:existential conformances}.
\item If \tX\ is any other kind of type: return an invalid conformance.
\end{enumerate}
\end{algorithm}

We can pick any valid conformance $\XP$ apart, and get back the conforming type~\tX\ and the protocol~\tP. We relate this destructuring to global conformance lookup in the form of a \index{commutative diagram!global conformance lookup}commutative diagram:
\begin{center}
...
If we start with a type~\tX, look up the conformance to~\tP, and get the conforming type out of this conformance, we end up with a type canonically equivalent to \tX. (The two types might be \index{type pointer equality}distinct pointers, because conformance lookup does not always preserve \index{sugared type}type sugar.) The other direction is more subtle. If we start with a conformance~$\XP$, get the conforming type~\tX, and look up the conformance of \tX\ to \tP, we would like to find the \emph{same} conformance. We say that global conformance lookup is \emph{coherent} if this property holds. We follow the simplest approach, and only guarantee coherence if there is \emph{at most} one conformance for every possible pairing of type with protocol.

\paragraph{Coherence.} When a conformance is stated on an extension, we allow the extended type, the conformed protocol, and the extension itself to be in different \index{module declaration!conformance}modules. There are three possibilities. First, we can conform our type to a protocol from another module:
\begin{Verbatim}
struct MyType {...}
...
\end{Verbatim}
When used incorrectly, retroactive conformances can create a situation where we have two \emph{overlapping} conformances for the same conforming type and protocol. This violates coherence, which can lead to inconsistent behavior.
...
Avoiding the declaration of conformances which require the \verb|@retroactive| attribute mostly eliminates this possibility, but it does not foreclose it entirely, because \index{subclassing!coherence}subclassing, in conjunction with library evolution, can \index{limitation!conformance coherence and subclassing}introduce overlapping conformances. Suppose that a vendor publishes a \texttt{Farm} shared library which declares an open class and a protocol:
\begin{Verbatim}
...
\end{Verbatim}
Now, if the vendor subsequently conforms \texttt{Hay} to \texttt{Crop} in a future version of \texttt{Farm}, and the client runs their binary without recompiling, the client binary will now observe two overlapping conformances of \texttt{Alfalfa} to \texttt{Crop} at run time: the conformance declared in \texttt{Farm} that they inherited from \texttt{Hay}, and their local conformance of \texttt{Alfalfa} to \texttt{Crop}.
...
Global conformance lookup would need a disambiguation rule, perhaps by taking additional \index{source location}source location information into account. Parts of the compiler that require coherence would need to be redesigned to perform each lookup exactly once and save the result for later, instead of freely performing another lookup and assuming the same conformance will be found.

For example, after we resolve a generic nominal type written in source, we use global conformance lookup to construct its context substitution map, which we use to determine the types of its stored properties, for example. If we serialize this type in a \index{binary module}binary module and deserialize it from another compiler invocation, global conformance lookup might produce different results, giving us a different context substitution map. To ensure coherence, we would need to change the representation of a generic nominal type to store its context substitution map, avoiding the second lookup.

Dynamic casts at runtime pose similar challenges. One valid (but source-breaking) design would exclude retroactive conformances from dynamic cast behavior entirely. Another option is to introduce a new runtime entry point for dynamic casting that takes the reified lexical environment into account, allowing a shared library to use its own private conformances without changing the behavior of binaries that link against it.
...
\paragraph{Normal conformances.} Now, we suppose that \tX\ is a generic nominal type with declaration $d$, but with arbitrary generic arguments, so \tX\ is not necessarily equal to $\tXd$. We will use the type substitution algebra to figure out $\PP \otimes \tX$. From \SecRef{sec:nominal types}, we know we can factor \tX\ into the \index{declared interface type!nominal type declaration}declared interface type $\tXd$, and a substitution map~$\Sigma$, called the \index{context substitution map!conformance lookup}context substitution map of \tX. Thus, we have:
\[
\PP \otimes \tX = \PP \otimes (\tXd \otimes \Sigma)
\]
In order to reduce the right-hand side further, we will define it in a way that makes $\otimes$ into an \index{associative operation}associative operation. This is a reasonable assumption, because we already saw that $\otimes$ is associative when operating on types and substitution maps. Indeed, recall from \SecRef{sec:composition} that if \tX\ is a type, and $\Sigma_1$, $\Sigma_2$, $\Sigma_3$ are substitution maps, the order of operations does not matter in the expressions below:
\begin{gather*}
...
In the case where $\Sigma$ is the \index{identity substitution map}identity substitution map for the generic signature of the conforming context, say $G$, we don't form a specialized conformance. Instead, we just return the original normal conformance; that is, $\NormalConf \otimes 1_G := \NormalConf$.

\paragraph{Output generic signatures.} Before we proceed, we introduce a new concept. We associate an \index{output generic signature!conformance}\emph{output generic signature} with each conformance:
\begin{itemize}
...
\item The output generic signature of a \index{specialized conformance!output generic signature}specialized conformance is the \index{output generic signature!substitution map}output generic signature of its \index{conformance substitution map}conformance substitution map.
...
The standard library declares an \texttt{Array} nominal type, conforming to \IndexProto{Sequence}\tSequence.
If we look up the conformance of \texttt{Array<\rT>} to \tSequence, we get its normal conformance, because \texttt{Array<\rT>} is the declared interface type of \texttt{Array}:
\[ \PSequence\otimes\texttt{Array<\rT>} = \ArraySequence \]
Next, consider how \texttt{Array<Int>} conforms to \tSequence. Observe that:
...
\begin{enumerate}
\item With a \textbf{member type declaration}---that is, a nested nominal type or \index{type alias declaration}type alias declaration---having the same name as the associated type.
\item With \index{associated type inference}\textbf{associated type inference}, which deduces type witnesses by considering the candidate value witnesses for each \index{value requirement}value requirement in the protocol. (After a quick example below, we further examine this in \SecRef{associated type inference}.)
...
\item With a \IndexDefinition{default type witness}\textbf{default type witness} on the associated type declaration, if one is present. This is the fallback if all else fails.
\end{enumerate}
...
\end{Verbatim}
However, if \texttt{Dog} did \emph{not} state conformance to \texttt{Pet}, there would be no such type alias, and type resolution would diagnose an error while resolving \texttt{Dog<Ball>.Toy}!
...
The standard library's $\ArraySequence$ normal conformance witnesses the \texttt{Element} and \texttt{Iterator} associated types of \IndexProto{Sequence}\tSequence\ as follows:
\begin{gather*}
...
An important invariant is that any type parameters appearing in a type witness of a normal or specialized conformance are valid in the output generic signature of the conformance. In other words, if $\XP\in\ConfObj{G}$ and \nA\ is an associated type of~\tP, then $\APA\otimes\XP\in\TypeObj{G}$. (When we get to abstract conformances, we will see the property holds there as well.) We define \IndexSetDefinition{assoctype}{\AssocTypeObj{P}}$\AssocTypeObj{P}$ to mean the set of all associated type declarations of a fixed protocol~\tP, and $\ConfPObj{P}{G}$ to be the subset of $\ConfObj{G}$ containing only conformances to some given protocol~\tP. Thus, for each protocol~\tP, \IndexDefinition{type witness projection}type witness projection gives us this additional form of the \index{$\otimes$}$\otimes$ operation:
...
\begin{enumerate}
\item We can look up the conformance of $\tXd$ to \tP, project the type witness \texttt{$\tXd$.A} from the normal conformance $\NormalConf$, and apply $\Sigma$ to this type witness to get \texttt{X.A}.
\item We can look up the conformance of $\tXd$ to \tP, apply $\Sigma$ to this normal conformance to get a specialized conformance $\XP$, and project the type witness \texttt{X.A}.
\item We can apply $\Sigma$ to $\tXd$ to get \tX, look up the conformance of \tX\ to \tP\ to get the specialized conformance $\XP$, and project the type witness \texttt{X.A} from $\XP$.
\end{enumerate}
We can also exhibit all of this with a \index{commutative diagram!type witness projection}commutative diagram. Each of the three evaluation orders of our expression corresponds to one of the three unique paths from $\tXd$ to $\texttt{X.A}$:
\begin{center}
...
An \IndexDefinition{abstract conformance}\emph{abstract conformance} represents the conformance of a \index{type parameter}type parameter to a protocol:
\[\PP\otimes\tT := \TP\]

If \tT\ is a type parameter and \tP\ is a protocol, global conformance lookup forms the abstract conformance $\TP$ without knowledge of any fixed \index{generic signature}generic signature $G$. An abstract conformance $\TP$ is \IndexDefinition{valid abstract conformance}\emph{valid} for~$G$ if the \index{conformance requirement!type substitution}conformance requirement $\TP$ can be \index{derived requirement!abstract conformance}derived from~$G$ (\SecRef{derived req}). The client of the lookup establishes this validity separately with the appropriate generic signature~$G$, and we say that $\TP \in \ConfObj{G}$. The use of the same notation for conformance requirements and abstract conformances in the two respective formalisms will be mostly unambiguous.

We will use abstract conformances to fill in the part of \AlgRef{alg:substitute type} we have yet to explain, namely, how to apply a substitution map to a dependent member type. Before we can do this, we must define type witness projection and conformance substitution with an abstract conformance. Suppose that $G\vdash\TP$, and~\tP\ declares an associated type~\nA. In \SecRef{bound type params}, we saw that we can derive the \index{bound dependent member type!validity}\index{dependent member type!validity}bound dependent member type \texttt{T.[P]A} by applying the \IndexStep{AssocDecl}\textsc{AssocDecl} inference rule to a derivation of $\TP$:
\begin{gather*}
\AnyStep{\TP}{1}\\
...
Applying $\Sigma_{\XP}$ to \IndexSelf\tSelf\ outputs the conforming type, whereas applying $\Sigma_{\XP}$ to a \index{dependent member type}dependent member type \texttt{Self.[P]A}, where \nA\ is some associated type of \tP, outputs the type witness for \nA\ in the conformance:
\begin{gather*}
...
\end{enumerate}
Let's look at (1) first. The standard library's \texttt{IndexingIterator} type is interesting, because it implements the requirements of \IndexProto{IteratorProtocol}\texttt{IteratorProtocol} in terms of an abstract conformance to \tCollection; that is, it can serve as the \nIterator\ type witness for any \tCollection:
\begin{Verbatim}
...
\qquad {} = \rT
\end{gather*}
In terms of the input and output generic signature of $\Sigma$, this states that the \nElement\ associated type in the \nIterator\ associated conformance of \texttt{Array} is witnessed by the \nElement\ generic parameter of \texttt{Array}. Notice how the conformance substitution map of the $\ConfReq{IndexingIterator<Array<\rT>>}{IteratorProtocol}$ conformance refers to $\ArrayCollection$, which in turn refers to $\ArraySequence$ via associated conformance projection, closing the cycle:
\end{example}
...
The reader may wish to review \SecRef{type witnesses}, which listed four ways of declaring type witnesses. Associated type inference handles cases 2, 3, and 4 in that list. Therefore, we end up here if we are unable to resolve at least one type witness of a \index{normal conformance!associated type inference}normal conformance by case 1, that is, a qualified lookup into the conforming type. We proceed as follows:
\begin{enumerate}
...
\midrule
\verb|eat()|&\verb|(Self.Main, Self.Desert) -> ()|&\verb|(Int, Bool) -> ()|\\
&&\verb|(Bool, Float) -> ()|\\
...
\begin{example}
We will determine which of the three candidate value witnesses for \verb|P.f()| is the actual witness, based on the possible type witness assignments:
\begin{Verbatim}
protocol P {
  associatedtype A
  associatedtype B
...
\textbf{Name}&\textbf{Requirement}&\textbf{Witnesses}\\
\midrule
\verb|f()|&\verb|(Self.A, Self.B) -> ()|&\verb|(String, Float) -> ()|\\
&&\verb|(Int, Self.B) -> ()|\\
&&\verb|(Void, Array<Self.B>) -> ()|\\
\midrule
\verb|g()|&\verb|(Self.B) -> ()|&\verb|(Bool) -> ()|\\
\bottomrule
\end{tabular}
\end{center}

Consider the second candidate for \verb|f()|. If this were the witness, then \texttt{A} would have to be \texttt{Int}, but we cannot say anything about \texttt{B}. We omit the \emph{tautological} assignment $\SubstType{B}{B}$, so our partial solution is just $\{\SubstType{A}{Int}\}$. The third candidate appears similar, but unlike the second, it does not contribute a partial solution to the disjunction at all. We cannot represent the assignment $\SubstType{B}{Array<Self.[P]B>}$ because it really involves the infinite type \texttt{Array<Array<Array<...>>>}. Thus, we do not need to consider the possibility that $\SubstType{A}{Void}$, either. The disjunction for \verb|foo()| therefore only has two partial solutions. Here is our problem instance:
\begin{gather*}
\{ \{ \SubstType{A}{String},\, \SubstType{B}{Float} \},\, \{ \SubstType{A}{Int} \}\}\\
\{ \{ \SubstType{B}{Bool} \} \}
\end{gather*}
The only solution is $\{ \SubstType{A}{Int} \} \cup \{ \SubstType{B}{Bool} \} = \{ \SubstType{A}{Int},\, \SubstType{B}{Bool}\}$. Thus, (2) is the actual witness for \verb|P.f()| in this conformance.
\end{example}
...
\item Otherwise, let $D[i][j]$ be the $j$th partial solution in $D[i]$. If $S \cup D[i][j]$ is a consistent partial solution, recursively call Step~1 with $S\leftarrow S\cup D[i][j]$ and $i\leftarrow i+1$.
...
The above formula is more precisely in 3CNF, meaning it has the additional property that no clause contains more than three literals. We will not show this here, but in fact an arbitrary Boolean formula can always be converted first into CNF, and then 3CNF, in a way that preserves satisfiability (or lack thereof). Thus, for our purposes it suffices to solve 3SAT.

We will encode the above 3CNF formula as an instance of associated type inference, but it will be obvious that this encoding works for any 3CNF formula. The full listing is shown in \ListingRef{listing:sat}. First, we use two structs to represent true and false:
\begin{Verbatim}
struct T {}
struct F {}
\end{Verbatim}
...
\end{Verbatim}
We have two overloads of \verb|literal()|:
\begin{Verbatim}
  func literal(_: T, _: F) {}
  func literal(_: F, _: T) {}
\end{Verbatim}
We also have $2^3-1=7$ overloads of \verb|clause()|, one for each triple of true or false such that not all three are false:
\begin{Verbatim}
...
\end{Verbatim}
We add a series of methods named \verb|literal()| to our protocol, one for each variable~$x_i$, to encode the fact that $x_i$ and $\neg x_i$ are complementary literals:
\begin{Verbatim}
  func literal(_: X1P, _: X1N)
  func literal(_: X2P, _: X2N)
  func literal(_: X3P, _: X3N)
\end{Verbatim}
Finally, we add a series of methods named \verb|clause()|, one for each clause. To get the three parameter types, we translate each literal $x_i$ or $\neg x_i$ appearing in the clause into a parameter of type \texttt{XiP} or \texttt{XiN}. For example, the first clause in our formula is $(\neg x_1 \vee x_2 \vee x_3)$, which becomes \verb|(_: X1N, _: X2P, _: X3P)|, and so on. If a clause has fewer than three literals, we just repeat the last literal:
\begin{Verbatim}
...
\begin{enumerate}
\item Each \verb|Instance.literal()| has the two \verb|Solver.literal()| overloads as candidates, so each \texttt{XiP} must be \texttt{T} or \texttt{F}, and \texttt{XiN} must be the negation.
\item Each \verb|Instance.clause()| has seven \verb|Solver.clause()| overloads as candidates, so we get a disjunction where each partial solution is a satisfying assignment for the clause, because at least one literal in the clause is true.
\end{enumerate}
...
\begin{listing}\captionabove{Boolean satisfiability encoded with associated type inference}\label{listing:sat}
\VerbInput{conformances/sat.swift}
\end{listing}

\paragraph{Non-deterministic polynomial time.} We now attempt to summarize one of the most celebrated ideas in theoretical computer science. A classic text on this topic is~\cite{garey1979computers}, while \cite{maccormick2018can} has a gentler introduction. First, we imagine a \emph{non-deterministic} algorithm to solve satisfiability. We fix an encoding of truth assignments as strings of symbols; for example, we write down the 0~and~1 values assigned to each of our $n$ variables, in some fixed order. Our non-deterministic algorithm then simultaneously ``guesses'' all $2^n$ truth assignments, and ``checks'' each assignment in parallel with all others, by evaluating the formula to obtain a true or false result. The overall algorithm outputs success if at least one thread outputs success, otherwise it outputs failure.
...
\end{itemize}
The \texttt{ProtocolConformance} class is the root of a class hierarchy:
\begin{center}
\begin{forest}
for tree={folder, grow'=0, fit=band, s sep=2pt, draw=black, thick, rounded corners}
[\texttt{\vphantom{p}ProtocolConformance}
  [\texttt{\vphantom{p}RootProtocolConformance}
    [\texttt{\vphantom{p}NormalProtocolConformance}]
    [\texttt{\vphantom{p}ExistentialProtocolConformance}]
  ]
  [\texttt{\vphantom{p}InheritedProtocolConformance}]
  [\texttt{\vphantom{p}SpecializedProtocolConformance}]
]
\end{forest}
\end{center}
The \texttt{ExistentialProtocolConformance} subclass is documented in \SecRef{src:existential types}.

\apiref{RootProtocolConformance}{class}
Base class for \texttt{NormalProtocolConformance} and \texttt{ExistentialProtocolConformance}. Inherits from \texttt{ProtocolConformance}.
...
\begin{itemize}
\item \texttt{getDeclContext()} returns the declaration context where this conformance was declared, either a nominal type declaration or extension.
...
\begin{itemize}
\item \texttt{getInheritedConformance()} returns the base conformance, which is always normal or specialized.
\end{itemize}
...
\subsection*{Building Substitution Maps}
...
We discussed substitution maps in \SecRef{src:substitution maps}, but we postponed the entry points for constructing new substitution maps, because we hadn't met conformances yet. We introduce them now.

Recall that a \IndexSource{substitution map}substitution map stores a list of conformances, one for each \IndexSource{conformance requirement}conformance requirement in its \IndexSource{input generic signature}input generic signature. Three overloads of the \texttt{get()} static method construct substitution maps. They differ in how the substitution map's \IndexSource{replacement type}replacement types and conformances are specified:

\medskip
\noindent
\texttt{get(GenericSignature, ArrayRef<Type>, ArrayRef<ProtocolConformanceRef>)}:\newline
Builds a new substitution map from an input generic signature, an array of replacement types, and an array of conformances. The first array's elements are in one-to-one correspondence with the signature's generic parameters, and the second array's elements are in one-to-one correspondence with the signature's conformance requirements.

\medskip
\noindent
\texttt{get(GenericSignature, ArrayRef<Type>, LookupConformanceFn)}:\newline
Builds a new substitution map from an input generic signature, an array of replacement types, and a callback, which produces a conformance for each conformance requirement in the input generic signature.

\medskip
\noindent
\texttt{get(GenericSignature, TypeSubstitutionFn, LookupConformanceFn)}:\newline
Builds a new substitution map, applying the first callback to each generic parameter of the input generic signature to produce the replacement types, and the second callback to the conformance requirements of the input generic signature to produce the root conformances.

\medskip

Finally, \texttt{SubstitutionMap::getProtocolSubstitutions()} takes a conformance to some protocol, and builds a \IndexSource{protocol substitution map}substitution map for this \IndexSource{protocol generic signature}protocol's generic signature.
...
\end{verbatim}
The \texttt{SubstitutableType} class is the common superclass of \texttt{GenericTypeParamType} and \texttt{ArchetypeType}. The actual type is always a \texttt{GenericTypeParamType} when the callback is used with \texttt{SubstitutionMap::get()}.

\IndexSource{conformance lookup callback}
\apiref{LookupConformanceFn}{type alias}
The type signature of a conformance lookup callback for the second and third forms of \texttt{SubstitutionMap::get()}:
\begin{verbatim}
...
\apiref{InferredAssociatedTypesByWitnesses}{type alias}
The problem instance, represented as an \verb|llvm::SmallVector| of pairs, where the first element in each pair is a value requirement and the second is a disjunction.
...
\item \texttt{getPotentialTypeWitnessesFromRequirement()} builds a disjunction for a value requirement, by finding all \IndexSource{candidate value witness}candidate value witnesses for this requirement, and constructing a partial solution for each one.
