...
A top-level non-generic \index{nominal type}nominal type declaration declares a single type, which we referred to as the \index{declared interface type!nominal type declaration}declared interface type in \ChapRef{chap:decls}. Here, the type representation resolves to the \index{struct type}struct type \texttt{Int} declared by the \index{standard library}standard library:
\begin{Verbatim}
var x: Int = ...
\end{Verbatim}
...
\begin{Verbatim}
// This is okay; `Element' resolves to `Self.[Collection]Element'
...
\label{label:type parameter base}
\begin{enumerate}
...
\end{verbatim}
\end{quote}
At this point, \texttt{f()} has a generic signature, so type representations appearing inside the function can be resolved in the interface resolution stage. The rewriting of requirements that use unbound dependent member types into requirements that use bound dependent member types will be completely justified in \SecRef{minimal requirements}.
...
\paragraph{General principle.} Let's say that $H$ is the generic signature of the current context, and \tT\ is the resolved base type of our member type representation, obtained by a recursive call into type resolution. To resolve the member reference, we perform a \index{qualified lookup}qualified lookup into a set of type declarations, collected by considering the base type \tT:
\begin{itemize}
...
\end{itemize}
If qualified lookup fails to find any results at all, or finds more than one ambiguous result, we \index{diagnostic!type resolution}diagnose an error. Otherwise, let $d$ be the resolved type declaration with \index{declared interface type!type resolution}declared interface type $\tXd$ and generic signature $G$. We construct a substitution map $\Sigma\in\SubMapObj{G}{H}$, then compute $\tXd\otimes\Sigma$ to get the final resolved type. We construct $\Sigma$ from the base type \tT, and information about the parent context of $d$. There are three cases to handle:
\begin{itemize}
...
We want our resolved type to be an interface type for $H$, so $\tXd\otimes\Sigma\in\TypeObj{H}$. Since $\tXd\in\TypeObj{G}$, we wish to construct a $\Sigma\in\SubMapObj{G}{H}$. In the previous section, we dealt with the case where $d$ does not introduce any new generic parameters or requirements, but might be in a generic context. Back then, the resolved type was $\tXd\otimes\Sigma'$ where  $\Sigma'$ is the context substitution map for the base type~\tT. In the general case, $\Sigma'\in\SubMapObj{G'}{H}$ and not $\SubMapObj{G}{H}$, but the ``difference'' between the two is that a substitution map for $G$ must also fix $d$'s innermost generic parameters and witness any new conformance requirements; so we form $\Sigma\in\SubMapObj{G}{H}$ from $\Sigma'\in\SubMapObj{G'}{H}$, by adding the generic argument types from Step~2, and resolving their conformances.

That is all a fancy way of saying, the recursive nesting of type declarations gives rise to a recursive nesting of type representations. Type resolution forms a substitution map at each nesting level by adding new generic arguments to the previous substitution map.
...
\end{align*}
Note that our original requirements contained type parameters of~$G$. After substitution, they now contain the primary archetypes of $H$:
\begin{gather*}
...
\begin{itemize}
\item For a \index{conformance requirement!checking}\textbf{conformance requirement} $\XP$, we evaluate the \index{global conformance lookup!conformance requirement}global conformance lookup $\PP \otimes \tX$. There are three possible outcomes:
\begin{enumerate}
...
\item If \tX\ does not have a \index{superclass type}\index{subclassing}superclass type (\SecRef{sec:subclassing}), return false.
\item Otherwise, let $\tX'$ be the superclass type of \tX. Recursively apply the algorithm to the superclass requirement $\ConfReq{$\tX'$}{C}$.
\end{enumerate}
\item For a \index{layout requirement!checking}\textbf{layout requirement} $\ConfReq{X}{AnyObject}$, we check if \tX\ is a class type, an archetype satisfying the \Index{AnyObject@\texttt{AnyObject}}\texttt{AnyObject} \index{layout constraint}layout constraint, or an \index{Objective-C!existential type}\texttt{@objc} existential, and if so, we return true. Otherwise, we return false. (We'll discuss representation of existentials in \ChapRef{chap:existential types}.)
...
A type declaration with a trailing \Index{where clause@\texttt{where} clause!contextually-generic declaration}\texttt{where} clause but no generic parameter list was called a \index{contextually-generic declaration}contextually-generic declaration in \SecRef{sec:requirements}. A non-generic declaration inside a constrained extension is conceptually similar; we'll meet constrained extensions in \SecRef{constrained extensions}. In both cases, the generic signature~$G$ of the referenced declaration and the generic signature of the parent context~$G'$ share the same generic parameters, but $G$ has additional requirements not present in $G'$. While there are no generic arguments to apply, we still proceed to check that~$\Sigma$ satisfies the requirements of~$G$.
\begin{example}
...
\paragraph{A limitation.} An unbound generic type can refer to either a nominal type declaration, or a type alias declaration. However, an unbound generic type referring to a type alias declaration cannot be the parent type of a nominal type. It is an error to access a member type of a generic type alias without generic arguments, \index{limitation!generic type alias}even when an unbound generic type referencing a nominal type declaration can appear in the same position:
\begin{Verbatim}
...
_ = GenericType.Nested(value: 123)        // okay
_ = GenericType<Int>.Nested(value: 123)   // okay

_ = GenericAlias.Nested(value: 123)       // error
_ = GenericAlias<Int>.Nested(value: 123)  // okay
\end{Verbatim}

\paragraph{A future direction.} If we think of type representations as syntactic, and types as semantic, unbound generic types are somewhere in between. They are produced by type resolution and refer to type declarations, but they do not actually survive type checking. When all is said and done, generic arguments in expressions will either be resolved or replaced with error types. In fact, we could eliminate unbound generic types from the implementation, if we changed type resolution to fill in missing generic arguments with placeholder types.
...
The \IndexSource{type resolution context}type resolution context, which encodes the position of a type representation. The below list is not exhaustive; refer to the source code for a complete list.
\begin{itemize}
\item \texttt{TypeResolverContext::None}: no special type handling is required.
\item \texttt{TypeResolverContext::ScalarGenericArgument}: generic arguments of a \IndexSource{generic nominal type}generic nominal type or \IndexSource{parameterized protocol type}parameterized protocol type.
\item \texttt{TypeResolverContext::TupleElement}: elements of a \IndexSource{tuple type}tuple type.
\item \texttt{TypeResolverContext::AbstractFunctionDecl}: the base context of a \IndexSource{function declaration}function declaration's parameter list.
\item \texttt{TypeResolverContext::SubscriptDecl}: the base context of a \IndexSource{subscript declaration}subscript declaration's parameter list.
\item \texttt{TypeResolverContext::ClosureExpr}: the base context of a \IndexSource{closure expression}closure expression's parameter list. The generic arguments of a generic type may be omitted here, or the type of a closure parameter may be omitted entirely.
\item \texttt{TypeResolverContext::FunctionInput}: the type of a parameter in a function type or declaration. A function type appearing in this position becomes \IndexSource{non-escaping function type}non-escaping by default.
\item \texttt{TypeResolverContext::VariadicFunctionInput}: the type of a variadic parameter in a function type or declaration. This differs from the above in that nested function types remain \texttt{@escaping}. Recall from \SecRef{sec:more types} that a variadic parameter is lowered to a parameter of array type; non-escaping function types cannot be stored in arrays.
\item \texttt{TypeResolverContext::InoutFunctionInput}: the type of an \IndexSource{inout parameter@\texttt{inout} parameter}\texttt{inout} parameter in a function type or declaration.
...
\item \texttt{TypeResolverContext::ExtensionBinding}: the extended type of an \IndexSource{extension declaration}extension declaration.
\item \texttt{TypeResolverContext::InExpression}: a type appearing inside an expression.
\item \texttt{TypeResolverContext::ExplicitCastExpr}: the target type of an \texttt{as} cast expression.
\item \texttt{TypeResolverContext::EnumElementDecl}: the base context of an \IndexSource{enum element declaration}enum element parameter list.
\item \texttt{TypeResolverContext::EnumPatternPayload}: the payload type of an enum element pattern, in a \texttt{switch} or \texttt{if~case} statement. Tweaks the behavior of tuple element labels.
\item \texttt{TypeResolverContext::TypeAliasDecl}: the underlying type of a non-generic \IndexSource{type alias declaration}type alias. This can be an \IndexSource{unbound generi type}unbound generic type
\item \texttt{TypeResolverContext::GenericTypeAliasDecl}: the underlying type of a generic type alias. This cannot be an unbound generic type.
\item \texttt{TypeResolverContext::ExistentialConstraint}: the constraint type of an \IndexSource{existential type}existential type (\ChapRef{chap:existential types}).
\item \texttt{TypeResolverContext::GenericRequirement}: the constraint type of a \IndexSource{conformance requirement}conformance requirement in a \texttt{where} clause.
...
\item \texttt{TypeResolverContext::ProtocolMetatypeBase}: the instance type of a protocol \IndexSource{metatype}metatype, like \texttt{T.Protocol}.
...
\item \texttt{TypeResolverContext::Inherited}: the \IndexSource{inheritance clause}inheritance clause of a concrete type.
...
\item \texttt{TypeResolutionFlags::SILType}: used when resolving a position where \IndexSource{lowered type}lowered SIL types, such as \IndexSource{SIL}SIL function types, can appear. This only comes up when parsing SIL source files, which are only intended to be used when testing the compiler.
\item \texttt{TypeResolutionFlags::SILMode}: used when resolving a position where ordinary AST types appear, but in textual SIL, which enables special syntax, for example \verb|@dynamic_self| for the \IndexSource{dynamic Self type@dynamic \tSelf\ type}dynamic \tSelf\ type. This only comes up in SIL source files.
...
\item \texttt{TypeResolutionFlags::Direct}: set when resolving the top-level type representation, and cleared when type resolution recurses on a child. Allows certain type representations which cannot be nested inside other type representations, such as \IndexSource{inout parameter@\texttt{inout} parameter}\texttt{inout T} for an \texttt{inout} parameter type.
...
\item \texttt{TypeResolutionFlags::Preconcurrency}: when resolving a reference to a type alias marked with the \texttt{@preconcurrency} attribute, strip off \texttt{@Sendable} and global actor attributes from function types in the underlying type of the type alias~\cite{se0337}.
\end{itemize}
...
Determines if \tSelf\ in the given context refers to the innermost nominal type declaration, the \IndexSource{dynamic Self type@dynamic \tSelf\ type}dynamic \tSelf\ type of the innermost class declaration, or if it is invalid to utter \tSelf.
...
\apiref{TypeResolver::resolveDependentMemberType()}{function}
...
Determines if a member type access is invalid. This detects various oddities, like accessing a protocol type alias where the base type is the protocol itself (\SecRef{member type repr}), and accessing a member type of an unbound generic type referencing a type alias (\SecRef{unbound generic types}).
