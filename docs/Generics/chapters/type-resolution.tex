...
Speaking of \tSelf, inside a protocol or protocol extension this refers to the \IndexSelf implicit generic parameter named \tSelf, also known as $\rT$ (\SecRef{protocols}), which we resolve like a reference to any other named other generic parameter:
\begin{Verbatim}
protocol Proto {
...
\begin{itemize}
\item For a \index{conformance requirement}\textbf{conformance requirement} $\TP$, we perform the \index{global conformance lookup}global conformance lookup $\protosym{P}\otimes\texttt{T}$. There are three possible outcomes:
\begin{enumerate}
