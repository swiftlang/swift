...
\item Extensions cannot add new cases to \index{enum declaration!extension}enum declarations, for similar reasons as stored properties; doing so would complicate both the static exhaustiveness checking for \texttt{switch} statements and the in-memory layout computation of the enum.

\item When the extended type is a \index{class declaration!extension}\index{class type!extended type}class, the methods of the extension are implicitly \texttt{final}, and the extension's methods are not permitted to \index{override}override methods from the superclass. Calls to non-\texttt{final} class methods are dispatched through a \index{vtable}\emph{vtable}, or table of function pointers, attached to the runtime metadata of the class, and there is no mechanism for extensions to add new entries or replace existing entries in the vtable.

\item The rules for types \index{nested type declaration!extension}nested in extensions are the same as those for types nested in nominal types (\SecRef{nested nominal types}). An extension of a struct, enum, or class may contain a nested struct, enum, or class, while a protocol extension cannot, just as a protocol cannot. Extensions themselves must be at the \index{global declaration}top level of a \index{source file}source file, but the extended type can be nested inside of another nominal type (or extension).
\end{itemize}
...
\begin{example}\label{bad extension 1} An invalid extension of a synthesized type alias:\index{horse}
\begin{Verbatim}
protocol Animal {
...
\end{Verbatim}
Extension binding fails to resolve \texttt{Horse.FeedType}, because this type alias was not written in source, so it does not exist when extension binding runs. However, when the \Request{type-check primary file request} visits this extension later, type resolution will trigger associated type inference, which will synthesize the type alias from the witness \verb|Horse.eat(_:)|. We produce a diagnostic directing the user to state the correct extended type that can be found at extension binding time.
\end{example}
...
The other case where extension binding fails but \textbf{type-check primary file request} is able to resolve the extended type is when this type is not actually a nominal type. In this situation, the type checker emits the fallback ``non-nominal type cannot be extended'' diagnostic:
\begin{Verbatim}
typealias Fn = () -> ()
...
\index{limitation!conditional conformance}
...
\end{itemize}
We begin by parsing both Swift source files, skipping over the extension body with delayed parsing because it appears in a secondary file. Next, we perform extension binding, to associate the extension of \texttt{NSHorse} with the class declaration of \texttt{NSHorse}. Finally, we type check the body of the \texttt{ride()} function, since it appears in a primary file. Type checking the expressions in the body performs three direct lookups into \texttt{NSHorse}, first with the name \texttt{walk}, then \texttt{trot}, and finally \texttt{walk} again. Let's look at how each lookup plays out in detail.
...
An extension can impose its own requirements on the generic parameters of the extended type; we say this as a \IndexDefinition{constrained extension}\emph{constrained extension}. These requirements are always additive; the generic signature of a constrained extension is built from the generic signature of the extended type, together with the new requirements. The members of a constrained extension are then only available on specializations of the extended type that satisfy these requirements. There are three ways to declare a constrained extension:
\begin{enumerate}
\item using a \Index{where clause@\texttt{where} clause!extension declaration}\texttt{where} clause,
\item by writing a \index{generic nominal type!extended type}generic nominal type with generic arguments as the extended type,
...
extension Set<Int> {...}
\end{Verbatim}
A non-generic type alias type whose underlying type is a generic nominal type can also be used in the same manner:
\begin{Verbatim}
...
\begin{enumerate}
\item The underlying type its a generic nominal type.
\item The type alias has the same number of generic parameters as the underlying type declaration.
\item The type alias substitutes each generic parameter of the underlying type declaration with the corresponding generic parameter of the type alias (where the correspondence means they have the same depth and index).
\end{enumerate}
...
The \index{standard library}standard library defines a generic struct named \texttt{Range} with a single \texttt{Bound} generic parameter conforming to \texttt{Comparable}:
\begin{Verbatim}
...
\section{Conditional Conformances}\label{sec:conditional conformances}
...
For example, arrays have a natural notion of equality, defined in terms of an equality operation on the element type. However, we don't require that every array has an \texttt{Equatable} element type. Instead, we declare a conditional conformance of \texttt{Array} to \texttt{Equatable}, for those arrays whose element type is \texttt{Equatable}:
\begin{Verbatim}
...
\]
The $\ConfReq{Array<\rT>}{Equatable}$ conformance has a single conditional requirement, $\ConfReq{\rT}{Equatable}$. We apply $\Sigma$ to get the conditional requirement for the specialized conformance:
\[
...
We model protocol inheritance by introducing an \index{associated conformance requirement!protocol inheritance}associated conformance requirement with a subject type of \tSelf, so for instance \texttt{Derived} below has an associated conformance requirement $\ConfReq{Self}{Base}$:
\begin{Verbatim}
protocol Base {...}
protocol Derived: Base {...}
\end{Verbatim}
When checking a conformance to \texttt{Derived}, the \index{conformance checker}conformance checker ensures that the conforming type satisfies the $\ConfReq{Self}{Base}$ requirement. When the conformance to \texttt{Base} is unconditional, this always succeeds, because the conformance declaration on the conforming conforming type also \index{implied conformance}\emph{implies} an unconditional conformance to the base protocol:
\begin{Verbatim}
struct Pair<T, U> {}
extension Pair: Derived {...}
// implies `extension Pair: Base {}'
\end{Verbatim}
The nominal type's \index{conformance lookup table}conformance lookup table synthesizes these implied conformances and makes them available to global conformance lookup.

With a conditional conformance, we cannot synthesize implied conformances, because we do not know what conditional requirements they should have. Instead, the user must satisfy associated conformance requirements on \tSelf\ by explicitly declaring a conformance to each base protocol.

Suppose we wish for \texttt{Pair} to conform to \texttt{Derived}, but only when $\SameReq{\rT}{Int}$:
\begin{Verbatim}
extension Pair: Derived where T == Int {...}
\end{Verbatim}
The compiler will \index{diagnostic!conditional conformance}diagnose an error unless we explicitly declare a conformance of \texttt{Pair} to \texttt{Base}. There are several possible ways to declare a conformance of \texttt{Pair} to \texttt{Base}. The simplest way is to conform unconditionally:
\begin{Verbatim}
extension Pair: Base {...}
\end{Verbatim}
...
\begin{Verbatim}
extension Pair: Base where T == Int {...}  // okay
\end{Verbatim}
...
\begin{Verbatim}
extension Pair: Base where T: Equatable {...}  // okay
\end{Verbatim}
...
\begin{Verbatim}
extension Pair: Base where U: Equatable {...}  // error
\end{Verbatim}
...
Conditional conformances can express \index{termination}\index{non-terminating computation}non-terminating computation at compile time.

\begin{example}
The below code is taken from a bug report which remains \index{limitation!conditional conformance}unfixed for the time being \cite{sr6724}:
\VerbInput{extensions/non_terminating.swift}
...
Let $\Sigma$ be a substitution map with \index{input generic signature}input generic signature~$G$, and \index{fully-concrete type}fully concrete replacement types, meaning they do not contain any type parameters. We say that $\Sigma$ is \IndexDefinition{well-formed substitution map}\emph{well-formed}, if for each derived requirement~$R$ of~$G$, the \index{substituted requirement}substituted requirement $R\otimes\Sigma$ is satisfied according to \AlgRef{reqissatisfied}.
\end{definition}

The restriction on the replacement types above is not a real limitation, by the way; as we saw in \SecRef{checking generic arguments}, we can first compose $\Sigma$ with the \index{forwarding substitution map}forwarding substitution map~$1_{\EquivClass{H}}$ for some generic signature~$H$, to replace type parameters with archetypes.)

The difficulty with this definition is that whenever $G$ has an \index{generic signature!infinite}infinite theory of derived requirements, we cannot directly check if~$\Sigma$ is well-formed. Recall that \AlgRef{check generic arguments algorithm} can only verify that $\Sigma$ satisfies the \emph{explicit} requirements of~$G$, and this by itself is not sufficient. Indeed, it is easy to write down a substitution map that satisfies all explicit requirements of its generic signature, but not all derived requirements, because one of the conformances stored in the substitution map does not satisfy the \index{associated requirement}associated requirements of its protocol.

\begin{example}
The \texttt{Bad} type conforms to \texttt{Sequence}, but the \texttt{Iterator} \index{type witness}type witness in this conformance does not conform to \texttt{IteratorProtocol}:
\begin{Verbatim}
struct Bad: Sequence {
...
struct BrewPub<T> {}
\end{Verbatim}
If \texttt{BrewPub} conforms to these protocols unconditionally, Swift's \index{re-declaration checking}re-declaration checking rules prevent us from declaring the two conformances with distinct type witnesses:
\begin{Verbatim}
...
extension BrewPub: Pub {
  typealias Beer = Float  // error: invalid re-declaration
  func pour() -> Float { return 1.0 }
}
\end{Verbatim}
If the conformances are conditional on the other hand, neither type alias is visible from the other \index{constrained extension}constrained extension, so the second one is not rejected as a re-declaration:
\begin{Verbatim}
...
extension BrewPub: Pub where T: Numeric {
  typealias Beer = Float  // okay
  func pour() -> Float { return 1.0 }
}
\end{Verbatim}
Next, we declare a function generic over types that conform to both \texttt{Bar} and \texttt{Pub}:
\begin{Verbatim}
func both<T: Bar & Pub>(_ t: T) -> (T.Beer, T.Beer) {
  return (t.brew(), t.pour())
}
\end{Verbatim}
Here is the generic signature of \texttt{both()}:
\begin{quote}
\texttt{<\rT\ where \rT:~Bar, \rT:~Pub>}:
\end{quote}

All that remains is to choose a generic argument type for \texttt{BrewPub} that satisfies the conditional requirements of both conformances. One candidate type is \texttt{Int}, which conforms to \texttt{Equatable} and \texttt{Numeric}. We pass an instance of \texttt{BrewPub<Int>} to \texttt{both()}:
\begin{Verbatim}
let result = both(BrewPub<Int>())
\end{Verbatim}
This call is valid, because \texttt{BrewPub<Int>} conforms to both \texttt{Bar} and \texttt{Pub}. Here is the \index{substitution map}substitution map for the call:
\begin{align*}
\Sigma := \SubstMapC{
...
\begin{gather*}
\texttt{\rT.[Bar]Beer}\otimes\Sigma = \AssocType{Bar}{Beer}\otimes\ConfReq{BrewPub<Int>}{Bar} = \texttt{Float}\\
\texttt{\rT.[Pub]Beer}\otimes\Sigma = \AssocType{Pub}{Beer}\otimes\ConfReq{BrewPub<Int>}{Pub} = \texttt{String}
\end{gather*}

In the generic signature of \texttt{both()}, we can derive a same-type requirement between the \index{bound dependent member type!equivalence}bound dependent member types \texttt{\rT.[Bar]Beer} and \texttt{\rT.[Pub]Beer}, because both are equivalent to the \index{unbound dependent member type!equivalence}unbound dependent type \texttt{\rT.Beer}. This means that within the body of \texttt{both()}, we can freely assume the calls to \texttt{brew()} and \texttt{pour()} return the same type. However, in reality, the first call returns a \texttt{Float}, while the second call returns a \texttt{String}. This results in undefined behavior. The complete example is shown in \ListingRef{listing:unsound conditional conformance}, and the reader can verify it produces a segmentation fault at runtime.
\end{example}

\begin{listing}\captionabove{Unsound behavior with conditional conformances}\label{listing:unsound conditional conformance}
\VerbInput{extensions/unsound.swift}
\end{listing}

There are two potential solutions to our soundness problem in the implementation conditional conformances:
\begin{enumerate}
\item We could tighten the rules for \index{re-declaration checking}re-declaration checking of type aliases in constrained extensions, requiring that all such type aliases have canonically equal underlying types. This would \textsl{reject the conditional conformances} above as invalid.
\item We could extend \AlgRef{check generic arguments algorithm} to detect conformances having incompatible type witnesses in the given substitution map. This would instead \textsl{reject the call} to \texttt{both()} written above.
\end{enumerate}
The second approach is more desirable, and we will revisit this problem in \ChapRef{chap:minimization}.

\section{Source Code Reference}\label{src:extensions}
...
The request evaluator request implementing direct lookup. The entry point is the \texttt{NominalTypeDecl::lookupDirect()} method, which was introduced in \SecRef{src:compilation model}. This request is uncached, because the member lookup table effectively implements caching outside of the request evaluator.
...
\end{itemize}
The \texttt{GenericSignatureRequest} was previously introduced in \SecRef{src:building generic signatures}. It delegates to a pair of utility functions to implement special behaviors of extensions.

\apiref{collectAdditionalExtensionRequirements()}{function}
Collects an extension's requirements from the extended type, which handles extensions of pass-through type aliases (\verb|extension CountableRange {...}|) and extensions of generic nominal types (\verb|extension Array<Int> {...}|).
...
A utility function that first calls \IndexSource{global conformance lookup}\texttt{lookupConformance()} (\SecRef{src:conformances}), and then checks any conditional requirements using \texttt{checkRequirements()}, described in \SecRef{src:type resolution}.

The \verb|NormalProtocolConformance| and \verb|SpecializedProtocolConformance| classes were previously introduced in \SecRef{src:conformances}.
