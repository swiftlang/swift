...
The conditional requirements of the specialized conformance $\ConfReq{T}{P}$ are the \index{substituted requirement}substituted requirements obtained by applying $\Sigma$ to each conditional requirement of \ConfReq{$\texttt{T}_d$}{P}. This makes the following \index{commutative diagram}diagram commute for each conditional requirement~$R$:
\begin{quote}
...
\ConfReq{$\texttt{T}_d$}{P} \arrow[d, "\GetConditionalRequirements"{left}] \arrow[r, "\text{apply $\Sigma$}"] &\ConfReq{T}{P} \arrow[d, "\GetConditionalRequirements"] \\
...
Protocol inheritance is modeled as an \index{associated conformance requirement}associated conformance requirement on \texttt{Self}, so for instance \verb|Derived| has an associated conformance requirement $\ConfReq{Self}{Base}$:
\begin{Verbatim}
protocol Base {...}
