...
\chapter{Building Generic Signatures}\label{chap:building generic signatures}
...
\item A \index{source location}source location for diagnostics.
\end{enumerate}
...
\begin{enumerate}
\item \index{requirement resolution}\textbf{Requirement resolution} builds user-written requirements, from constraint types stated in generic parameter \index{inheritance clause!generic parameter declaration}inheritance clauses and requirement representations in the trailing \Index{where clause@\texttt{where} clause}\texttt{where} clause. This happens in \index{structural resolution stage}structural resolution stage  (\ChapRef{chap:type resolution}), so the resolved requirements may contain \index{unbound dependent member type!in requirements}unbound dependent member types, which will reduce to bound dependent member types in requirement minimization.
...
\begin{itemize}
\item Computing the generic signature of an opaque type declaration (\ChapRef{chap:opaque result types}).
\item Computing the generic signature for an opened existential type (\SecRef{sec:opening existentials}).
...
\item The \IndexDefinition{type alias requirements request}\Request{type alias requirements request} implements some source compatibility behaviors around \index{protocol type alias}protocol type aliases, discussed in \SecRef{protocol type aliases}.
\end{itemize}
...
A mistake of this sort is baked into the \index{standard library}standard library's \IndexProto{SIMDScalar}\texttt{SIMDScalar} protocol. The \tSelf\ type of \texttt{SIMDScalar} must conform to all of \IndexProto{Equatable}\texttt{Equatable}, \IndexProto{Hashable}\texttt{Hashable}, \IndexProto{Encodable}\texttt{Encodable}, and \IndexProto{Decodable}\texttt{Decodable}, via an associated same-type requirement:
\begin{Verbatim}
...
\end{Verbatim}
When \index{type resolution}resolving the function's return type, we must establish that the \index{generic argument}generic argument \texttt{\rT.Element} satisfies the requirements in the generic signature of the \texttt{Set} type declared in the \index{standard library}standard library:
\begin{Verbatim}
...
\begin{enumerate}
\item The parameter and return types of \index{function declaration}function, \index{constructor declaration}constructor, and \index{subscript declaration}subscript declarations, if the generic declaration is one of those. In our \texttt{uniqueElements()} example, we infer requirements from the function's return type.
...
\item The \index{typed throws}typed throws feature, introduced with \IndexSwift{6.0}Swift~6 \cite{se0413}, allows specifying the type of error thrown by a function. For function, constructor, and subscript declarations, we infer a conformance requirement to \texttt{Error} with the declaration's thrown error type as the subject type:
\begin{Verbatim}
...
\begin{itemize}
\item For a \index{generic nominal type}generic nominal type, we use the context substitution map, constructed from the type's list of generic arguments (\SecRef{sec:nominal types}).
\item For a generic \index{type alias type}type alias type, we similarly construct a substitution map from the generic arguments of the type alias type.
\end{itemize}
Let's call this substitution map $\Sigma$, and let $G$ be the \index{input generic signature}input generic signature of $\Sigma$. This is the generic signature of the referenced nominal type or type alias declaration. The \index{output generic signature}output generic signature of $\Sigma$ is $H$, so $\Sigma\in\SubMapObj{G}{H}$. We proceed as if we were checking generic arguments, applying $\Sigma$ to each requirement of $G$. This gives us a substituted requirement in $\ReqObj{H}$. (However, when checking generic arguments, we work with archetypes, so we get something in $\ReqObj{\EquivClass{H}}$; here, we instead want the substituted requirement to talk about type parameters).

In all examples we've seen so far, the inferred requirement's subject type was a type parameter of~$H$, and the inferred requirement was just added to~$H$ unchanged. We might also get a \index{fully-concrete type}fully concrete type after substitution. In the next example, we infer the requirement $\ConfReq{Int}{Hashable}$. This is not a statement about the type parameters of \texttt{f()} at all; it is ``always true,'' and it does not affect the generic signature of \texttt{f()}:
\begin{Verbatim}
func f<T>(_: T, _: Set<Int>) {}
\end{Verbatim}
A more complex scenario is possible with \index{conditional conformance}conditional conformances. In the next example, the inferred requirement is $\ConfReq{\rT}{Hashable}$:
\begin{Verbatim}
func f<T>(_: Set<Array<T>>) /* where T: Hashable */ {}
\end{Verbatim}
To understand why, first note the generic nominal type \texttt{Set<Array<\rT>>} gives us the inferred requirement $\ConfReq{Array<\rT>}{Hashable}$. The \index{standard library}standard library declares a conditional conformance of \texttt{Array} to \texttt{Hashable} when the element type is \texttt{Hashable}. We replace our original requirement with the conditional requirements of this conformance, which gives us $\ConfReq{\rT}{Hashable}$.
...
\]
To get our two inferred requirements, we apply $\Sigma$ to each requirement in the generic signature of \texttt{SequenceOf}:
\begin{gather*}
...
\paragraph{Decomposition.} This \IndexDefinition{requirement decomposition}formalizes the syntax sugar from \SecRef{sec:requirements}~and~\SecRef{protocols}. For example, the \index{standard library}standard library defines the \texttt{Codable} \index{type alias type}type alias, whose underlying type is a \index{protocol composition type!decomposition}composition of two protocols, \texttt{Decodable} and \texttt{Encodable}:
\begin{Verbatim}
...
\begin{itemize}
\item If the requirement is satisfied, we can delete it, that is, replace it with the \index{empty set}empty set of requirements, without violating our invariant. The requirement doesn't contribute anything new.
\item On the other hand, if the requirement is unsatisfied, the only way to proceed is to replace this requirement with something else unsatisfiable, so we must diagnose an error and give up.
\end{itemize}
...
\item If \tT\ does not have a \index{superclass type}superclass type (\SecRef{sec:subclassing}), then \tT\ cannot be a subclass of~\tC; add $\TC$ to the conflict list.
...
The protocol generic signature describes the structure generated by the protocol's requirement signature. These are the \index{valid type parameter!protocol generic signature}valid type parameters and derived requirements inside the declaration of the protocol and its \index{unconstrained extension}unconstrained extensions. These type parameters are all rooted in the protocol \tSelf\ type, and the derived requirements talk about these \tSelf-rooted type parameters. Informally, anything we can say about the protocol \tSelf\ type in $\GP$, should also be true of an arbitrary type parameter \tT\ in some other generic signature~$G$ where $G\vdash\TP$. We will now make this precise.
...
\item If $\GP\vdash R$, then $G\vdash R'$, where $R'$ is the substituted requirement obtained by replacing \tSelf\ with \tT\ in $R$.
\end{itemize}
\end{lemma}
...
Because of the central role that generic signatures play in the Swift \index{ABI}ABI, it is worth describing the requirement minimization problem in the abstract, which is our goal in the present section. The implementation itself will be revealed in \PartRef{part:rqm}, when we build a \emph{finite complete rewriting system} from desugared requirements (\SecRef{building rules}) and then perform the minimization procedure (\ChapRef{chap:minimization}). The material in this section was based on~\cite{gensig}, an earlier write-up about the \Index{GenericSignatureBuilder@\texttt{GenericSignatureBuilder}}\texttt{GenericSignatureBuilder}.
...
What if \emph{both} same-type requirements are explicit? Let's declare this protocol:
\begin{Verbatim}
protocol Foe {
  associatedtype X
  associatedtype Y
...
Now, consider this function:
\begin{Verbatim}
func f3<T: Foe>(_: T) where T.X == Set<T.Y>, T.X == Set<Int> {}
\end{Verbatim}
...
\begin{enumerate}
\item For two concrete \index{same-type requirement!conflicts}same-type requirements $\SameReq{T}{$\tX_1$}$ and $\SameReq{T}{$\tX_2$}$, we desugar the ``combined'' requirement $\SameReq{$\tX_1$}{$\tX_2$}$ using \AlgRef{desugar same type algo}. Desugaring will either detect a conflict, or produce a simpler list of requirements to replace one of the two original requirements, in which case we can look for conflicts again.
\item For a concrete same-type requirement $\TX$ and superclass requirement $\TC$, we desugar $\ConfReq{X}{C}$. Desugaring will succeed if and only if \tX~is a class type that is also a subclass of~\tC, otherwise it will detect a conflict.
\item For a same-type requirement $\TX$ and a \index{layout requirement!conflicts}layout requirement $\TAnyObject$, we desugar $\ConfReq{X}{AnyObject}$. This will succeed if and only if \tX~is any \index{class type}class type.
\item For a same-type requirement $\TX$ and a \index{conformance requirement!conflicts}conformance requirement $\TP$, we desugar $\ConfReq{X}{P}$. This will succeed if and only if \tX\ conforms to \tP.
...
\item If the two declarations are unrelated, we have a conflict.
\end{enumerate}
...
\item For a superclass requirement $\TC$ and a conformance requirement $\TP$, we desugar $\ConfReq{C}{P}$. If \tC\ conforms to \tP, the conformance requirement $\TP$ becomes redundant. However, if \tC\ does not conform to \tP, there is no conflict; the generic signature just requires that \tT\ is a subclass of \tC\ that \emph{also} conforms to \tP.
\end{enumerate}
We say $G$ is \emph{conflict-free} if it does not have any pairs of conflicting requirements.
\end{definition}

We will investigate the compiler's implementation of superclass, layout, and concrete same-type requirements in \ChapRef{chap:property map}, but in this section, we're just going to look at a couple of examples.
...
Our next example involves superclass requirements. While a description of generic classes awaits us in \ChapRef{sec:subclassing}, we only need non-generic classes to demonstrate the key ideas in requirement minimization. Since it is such a clich\'e at this point, we're going to follow the trend and go with the classic object oriented ``shape hierarchy'', shown on the right. We also introduce a \texttt{Canvas} protocol, which subjects it associated type to an \index{associated superclass requirement}associated superclass requirement $\AssocConfReq{Self.Boundary}{Polygon}{Canvas}$:
...
\section{Overriding Class Methods}\label{sec:overriding class methods}
...
\index{abstract generic signature request!method override check}
...
\begin{algorithm}[Compute override generic signature] As input, takes the superclass method's generic signature \texttt{G}, the \index{superclass declaration}superclass declaration \texttt{B}, and some subclass declaration \texttt{D}. Outputs a new generic signature.
\begin{enumerate}
...
\section{Source Code Reference}\label{src:building generic signatures}
...
The header file declares the requests; the evaluation functions are implemented by the Requirement Machine (\SecRef{src:basic operation}).
...
The \texttt{GenericContext::getGenericSignature()} method (\SecRef{src:generic signatures}) evaluates this request, which either returns the parent declaration's generic signature, or evaluates \texttt{InferredGenericSignatureRequest} with the appropriate arguments.
...
\index{primary file}
\index{type-check primary file request}
The \texttt{TypeCheckPrimaryFileRequest} then visits all \texttt{where} clauses in every primary file again, this time with \texttt{TypeResolutionStage::Interface}. This is how we diagnose any invalid dependent member types in \texttt{where} clauses. Recall that the structural resolution stage builds unbound dependent member types, without any knowledge of what associated type declarations are visible.
...
The \texttt{ProtocolDecl::getRequirementSignature()} method (\SecRef{src:generic signatures}) evaluates this request, which computes the protocol's requirement signature if the protocol is in the main module, or deserializes it if the protocol is from a serialized module.
...
The \texttt{ProtocolDecl::getTypeAliasRequirements()} method evaluates this request to collect same-type requirements implied by a type aliases in protocol extensions that have the same name as an associated type of an inherited protocol. This implements a source compatibility behavior, because normally, protocol extension type alises do not participate in rewriting, unlike type aliases declared in the protocol itself (\ExRef{ex:gsb legacy type alias}).
...
\end{itemize}
Just as \SecRef{minimal requirements} only describes the invariants around minimization, here we only call out the code related to checking those invariants. For the actual implementation of minimization, see \SecRef{src:minimization}.
