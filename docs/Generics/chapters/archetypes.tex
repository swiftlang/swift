...
\end{Verbatim}
The \texttt{take()} and \texttt{compare()} methods have different generic signatures. Let's say that $G$ is \texttt{<T>}, the generic signature of \texttt{take()}, and $H$ is \texttt{<T where T: Equatable>}, the generic signature of \texttt{compare()}. The \index{type representation}type representation ``\tT'' resolves to the \emph{same} generic parameter type~\tT\ (or \rT) in all source locations where it appears. However, this generic parameter maps to different archetypes in \texttt{take()} and \texttt{compare()}. Thus, the type of the expression ``\texttt{self.contents}'' is actually different in \texttt{take()} and \texttt{compare()}. Let's use the notation \index{$\archetype{T}$}\index{$\archetype{T}$!z@\igobble|seealso{archetype}}$\archetype{T}_G$ for the archetype obtained by mapping the type parameter \tT\ into the generic environment of a generic signature~$G$. Here, we have two archetypes:
\begin{enumerate}
\item $\archetype{T}_G$, which does not conform to any protocols.
\item $\archetype{T}_H$, which conforms to \texttt{Equatable}.
\end{enumerate}
...
\node [genericenvpart] (c) {\strut unique identifier};\\
};
\begin{scope}[on background layer]
...
\section{Local Requirements}\label{sec:local requirements}
...
\begin{itemize}
\item \textbf{Required protocols:} A list of all protocols \tP\ such that $G\vdash\TP$. This is the result of the \IndexQuery{getRequiredProtocols}$\Query{getRequiredProtocols}{}$ generic signature query.
...
\begin{enumerate}
\item If $G\vdash\TC$ for some class type~\tC, and \tC\ conforms to \tP, then the archetype \emph{conforms concretely} to \tP. The class type \tC\ may contain type parameters, so global conformance lookup recursively calls itself on~$\MapIn(\tC)$:
...
\item Otherwise, $\archetype{T}$ does not conform to \tP, and global conformance lookup returns an invalid conformance.
\end{enumerate}
...
We can use this to convert between substitution maps with contextual and interface replacement types, by composing the substitution map with the \index{identity substitution map}identity or forwarding substitution map on the right. If $\Sigma\in\SubMapObj{G}{H}$ and $\Sigma' \in \SubMapObjCtx{G}{H}$, we have:
\begin{gather*}
\Sigma \otimes \FwdMap{H}\in\SubMapObjCtx{G}{H}\\
\Sigma' \otimes 1_H\in\SubMapObj{G}{H}
\end{gather*}
Furthermore, if $\tX\in\TypeObj{G}$,
\begin{gather*}
\tX\otimes (\Sigma\otimes \FwdMap{H}) = \mathsf{in}_{H}(\tX \otimes \Sigma)\\
\tX\otimes (\Sigma' \otimes 1_H) = \mathsf{out}_{H}(\tX \otimes \Sigma')
\end{gather*}
...
Recall \IndexProto{N}protocol \texttt{N} from \ExRef{protocol n example}:
\begin{Verbatim}
protocol N {
  associatedtype A: N
}
...
\begin{example}\label{protocol z4 graph}  Now let's \IndexProto{Z4}change \tN\ by adding an associated same-type requirement:
\begin{Verbatim}
protocol Z4 {
...
\begin{example}\label{protocol collection graph} Recall the simplified \IndexProto{Collection}\tCollection\ protocol from \ExRef{protocol collection example}:
\begin{Verbatim}
...
\end{Verbatim}
When we studied the protocol generic signature $G_\texttt{Collection}$, we saw that it defines an infinite theory, but only a finite set of equivalence classes. The type parameter graph looks like this. Note the cycle of length~1 (sometimes known as a \index{loop|see{cycle}}\emph{loop}):
\begin{center}
...
\end{tikzpicture}
\end{center}
By repeatedly traversing this cycle, we see the equivalence class of $\archetype{\rT.SubSequence}$ contains an infinite set of type parameters:
\begin{quote}
\begin{verbatim}
τ_0_0.SubSequence
...
\end{verbatim}
\end{quote}
If we instead take the cycle zero or more times followed by some other edge, we get the other two infinite equivalence classes in $G_\texttt{Collection}$. In fact, if a generic signature has an infinite set of type parameters but a finite set of equivalence classes, the generic signature's type parameter graph must contain a cycle. However, the next example shows this is not true in general.
\end{example}
...
Takes a potential archetype~$t$ and a protocol \tP\ as input. Has side effects.
\begin{enumerate}
\item If $\PAForward(t)$ is non-null, load $t\leftarrow\PAForward(t)$, and repeat if necessary.
\item If $\tP\in\PAConforms(t)$, return.
\item Otherwise, set $\PAConforms(t)\leftarrow \PAConforms(t)\cup\{\tP\}$.
\item For each protocol~\tQ\ appearing in the inheritance clause of the declaration of \tP, recursively apply this algorithm to $t$ and \tQ.
\item Let $\tT:=\PAType(t)$.
\item For each associated type declaration \nA\ of \tP:
\begin{enumerate}
\item If $\PAMembers(t)$ does not have an entry with name \nA, create a new potential archetype~$v$ with $\PAType(v)=\texttt{T.A}$. Add the ordered pair $(\nA, v)$ to $\PAMembers(t)$.

\item Otherwise, $\PAMembers(t)$ contains an existing entry~$(\nA, v)$ with $\PAType(v)=\texttt{T.A}$.

For each protocol \tQ\ listed in the inheritance clause of the declaration of \nA, first check if \tQ\ already appears among the set of all protocols we've visited in prior recursive calls; if so, emit a diagnostic. Otherwise, add \tQ\ to the visited set, and recursively apply this algorithm to $v$ and \tQ.
\end{enumerate}
\end{enumerate}
\end{algorithm}
...
\item For each entry $(\nA,w)\in\PAMembers(u)$,
\begin{enumerate}
\item If $\PAMembers(t)$ does not contain a potential archetype for \nA, create a new potential archetype~$v$ such that $\PAType(v)=\texttt{T.A}$. Add $(\nA,v)$ to $\PAMembers(t)$.
\item Otherwise, $\PAMembers(t)$ contains an existing entry $(\nA, v)$ with $\PAType(v)=\texttt{T.A}$.
...
\item Otherwise, \tT\ must be a dependent member type \texttt{U.A} with some base type~\tU\ and identifier~\nA. First, invoke the algorithm recursively to resolve \tU\ to a potential archetype~$u$. If $u$ is null, go to Step~4.
\item Otherwise, if $\PAMembers(u)$ contains an entry $(\nA, t)$, consider the potential archetype $t$. If $\PAForward(t)$ is non-null, load $t\leftarrow\PAForward(t)$, and repeat if necessary. Return~$t$.
...
\item (Conformance) For a conformance requirement $\TP$, invoke \AlgRef{archetype builder lookup} to resolve \tT\ to a potential archetype. If this potential archetype does not exist, add $\TP$ to the delayed list. Otherwise, invoke \AlgRef{archetype builder expand} to expand the conformance requirement, and set the flag.
\item (Same-type) For a same-type requirement $\TU$, invoke \AlgRef{archetype builder lookup} to resolve each one of \tT\ and \tU\ to an potential archetype. If either potential archetype does not exist, add $\TU$ to the delayed list. Otherwise, invoke \AlgRef{archetype builder merge} to merge the two potential archetypes, and set the flag.
...
\item \IndexQuery{requiresProtocol}$\Query{requiresProtocol}{G,\,\tT,\,\tP}$: Resolve \tT\ to a potential archetype~$t$, and check if $\tP\in\PAConforms(t)$.
...
These problems motivated the search for a sound and decidable foundation on top of which Swift generics should be built, which became the Requirement Machine. Instead of incrementally constructing finite subgraphs of the type parameter graph, the correct approach is to construct a \emph{finite complete rewriting system}. While more abstract, this is actually much \emph{simpler} than the ``lazy expansion'' of the type parameter graph. Just like the original eager expansion design, a rewriting system is a finite data structure that is built once and then remains immutable after construction. Unlike eager expansion, a rewriting system can describe an infinite set of equivalence classes, and in many cases, it can even encode a \emph{finite} set without exhaustive enumeration. We will investigate the contemporary approach and study a correctness proof in \PartRef{part:rqm}.
...
\item \texttt{mapTypeIntoEnvironment()} returns the contextual type obtained by mapping an interface type into this generic environment.
...
\begin{itemize}
\item \texttt{mapTypeOutOfEnvironment()} returns the interface type obtained by \IndexSource{map type out of environment}mapping this contextual type out of its generic environment.
\end{itemize}

\apiref{SubstitutionMap}{class}
See also \SecRef{src:substitution maps}.
\begin{itemize}
\item \texttt{mapReplacementTypesOutOfEnvironment()} maps this substitution map's \IndexSource{map replacement types out of environment}replacement types and conformances out of their generic environment.
\end{itemize}

\apiref{ProtocolConformanceRef}{class}
See also \SecRef{src:conformances}.
\begin{itemize}
\item \texttt{mapConformanceOutOfEnvironment()} returns the protocol conformance obtained by mapping this protocol conformance out of its generic environment.
\end{itemize}
...
\item \texttt{mapTypeIntoEnvironment()} maps an interface type into the primary generic environment for the innermost generic declaration. If at least one outer declaration context is generic, this is equivalent to:
\begin{Verbatim}
dc->getGenericEnvironmentOfContext()->mapTypeIntoEnvironment(type);
\end{Verbatim}
For convenience, the \texttt{DeclContext} version of \texttt{mapTypeIntoEnvironment()} also handles the case where no outer declaration is generic. In this case, it returns the input type unchanged, after asserting that it does not contain any type parameters (since type parameters appearing outside of a generic declaration are nonsensical).
\end{itemize}
...
\item \texttt{getFullName()} returns the printed form of this archetype's type parameter.
\end{itemize}
...
\item \texttt{getGenericEnvironment()} returns this archetype's generic environment.
\end{itemize}
Local requirements (\SecRef{sec:local requirements}):
\begin{itemize}
\item \texttt{getConformsTo()} returns this archetype's required protocols. This does not include inherited protocols. To check if an archetype conforms to a specific protocol, use \IndexSource{global conformance lookup}global conformance lookup (\SecRef{src:conformances}) instead of looking through this array.
\item \texttt{getSuperclass()} returns this archetype's superclass bound, or the empty \texttt{Type} if there isn't one.
\item \texttt{requiresClass()} answers if this archetype's requires class flag.
