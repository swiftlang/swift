...
\end{enumerate}
Each path also has a \IndexDefinition{source!of path}\emph{source} and \IndexDefinition{destination!of path}\emph{destination}. If $p := (v,\,(e_1,\,\ldots,\,e_n))$ as above, its source is~$v$, and the destination is either the source vertex $v$ again if the sequence of edges is empty, or the destination of the final edge otherwise:
\begin{gather*}
\Src(p) := v\\
\Dst(p) := \begin{cases}
v&\text{(if $n=0$)}\\
...
\begin{itemize}
\item If the generic signature has more than one generic parameter, the \index{vertex!of type parameter graph}vertex set contains a distinguished \index{root vertex!of type parameter graph}root vertex.
\item The vertex set also contains a vertex for each \index{equivalence class!of type parameters}equivalence class of type parameters of~$G$. Each such vertex is labeled with the \index{reduced type!type parameter graph}reduced type of the equivalence class (which might be a concrete type).
\item If there is a root node, a series of edges join the root node with each equivalence class containing a generic parameter type $\ttgp{d}{i}$ of $G$. Each such edge is labeled with this generic parameter type.
\item A second series of edges relates pairs of equivalence classes where the destination contains some dependent member type \texttt{U.A}, and the source contains its base type~\tU. Each such edge is labeled with the name of the associated type declaration ``\texttt{.A}''.
\end{itemize}
\end{definition}
...
Now, further suppose that $G\vdash\SameReq{U}{$\tU^\prime$}$ and $G\vdash\SameReq{V}{$\texttt{V}^\prime$}$. We can then derive both $G\vdash\tU^\prime\texttt{.A}$ (7) and $G\vdash\SameReq{$\tU^\prime$\texttt{.A}}{$\texttt{V}^\prime$}$ (11):
\begin{gather*}
\AnyStep{\ConfReq{U}{P}}{1}\\
\AnyStep{\SameReq{U.A}{V}}{2}\\
\AnyStep{\SameReq{U}{$\tU^\prime$}}{3}\\
\AnyStep{\SameReq{V}{$\texttt{V}^\prime$}}{4}\\
\SymStep{3}{$\tU^\prime$}{U}{5}\\
\SameConfStep{1}{5}{$\tU^\prime$}{P}{6}\\
\AssocNameStep{6}{$\tU^\prime$.A}{7}\\
\SameNameStep{1}{5}{$\tU^\prime$.A}{U.A}{8}\\
\TransStep{8}{2}{$\tU^\prime$.A}{V}{9}\\
\TransStep{10}{4}{$\tU^\prime$.A}{$\texttt{V}^\prime$}{11}
\end{gather*}
