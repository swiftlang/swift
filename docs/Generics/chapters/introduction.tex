...
More accurately, the type witness here is the \texttt{Element} \index{type alias type}\emph{type alias type}, whose \index{underlying type}underlying type is \texttt{Int}. A type alias type is an example of a \index{sugared type}\emph{sugared type}. The \index{optional sugared type}\emph{optional type} \texttt{Int?} is another sugared type, equivalent to \texttt{Optional<Int>}. Finally, we can actually omit the declaration of \texttt{Element} here, and instead rely on \index{associated type inference}\emph{associated type inference}.
