...
\item Find a conformance path $s_n\otimes \cdots \otimes s_1\otimes s_0$ for $\ConfReq{T}{P}$ using \AlgRef{find conformance path algorithm}.
...
\item (Lookup) Construct the abstract conformance $\ConfReq{T}{P}$ and invoke \AlgRef{local conformance lookup algorithm} to perform the local conformance lookup $\ConfReq{T}{P}\otimes\Sigma$.
...
\end{theorem}
\begin{proof}
The proof relies on a few results developed in \SecRef{generic signature validity}. Let's call the two requisite derivations $D_1$ and $D_2$. We do a \index{structural induction}structural induction on the derivation for $\ConfReq{T}{P}$, building up $D_1$ and $D_2$ step by step. We only need to consider the derivation steps which produce new conformance requirements:
\begin{enumerate}
\item \IndexStep{Conf}\textsc{Conf} steps: $\vdash\ConfReq{T}{P}$.
\item \IndexStep{SameConf}\textsc{SameConf} steps: $\SameReq{T}{U},\,\ConfReq{U}{P}\vdash\ConfReq{T}{P}$.
...
\end{gather*}
To construct $D_2$, we recall that $G$ is well-formed, thus $G\vDash\texttt{T}$, since \texttt{T} appears in $\ConfReq{T}{P}$. We then derive $\SameReq{T}{T}$ via an \IndexStep{Ident}\textsc{Ident} derivation step:
\begin{gather*}
