...
> Note: Possible implementations include generating a hidden symbol into a
...
> Note: If an enum value has a known case, or can be proven to belong to a set
...
> Note: Were a "closed" enum allowed to have non-public cases, clients of the
...
Even for default "open" enums, adding new cases should not be done
lightly. Any clients attempting to do an exhaustive switch over all enum
cases will likely not handle new cases well.

> Note: One possibility would be a way to map new cases to older ones on older
...
> Note: It is legal to change the implementation of an inlineable function in
...
> Note: Why isn't this a special form of `public`? Because we don't want it to
...
> Note: Types are not allowed to be pinned because that would have many more
...
> Note: It would be possible to allow `@fixed` on non-final properties, and
...
-   `trivial`: Promises that the type is [trivial](#trivial). Note that this is not a
...
> Note: This may feel like a regression from Objective-C, where [duck
