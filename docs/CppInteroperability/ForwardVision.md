...
More generally: many languages, such as Rust, leverage powerful and complex memory ownership models. These ownership models and borrow-checkers are powerful and highly expressive. But, they lack simplicity, and are sometimes difficult to use. Swift is able to achieve much of the benefit of these ownership models using much simpler constructs, by understanding the semantics of an idiom at a higher level, and providing a specific language feature to support that idiom. Generalized accessors are an example of this. 

By understanding the semantics of the C++ APIs that are being imported, Swift can apply this same philosophy to imported C++ APIs. The compiler doesn’t need such a powerful, feature-rich ownership model to express most C++ APIs correctly, rendering imported APIs simple to use in Swift. In other words, for most APIs, Swift doesn’t need to model C++‘s complicated and often ad-hoc memory ownership and lifetimes, instead, Swift can map most C++ APIs to specific Swift idioms. The semantics and high-level understanding of these idioms allows both the programmer and compiler to reason about them simply, and without the need for a complex ownership model or the user-input and annotations that such an ownership model requires. 
