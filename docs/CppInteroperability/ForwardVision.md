...
As described above, using reference types as an example, there is often no C++ language feature or programming pattern that directly expresses the semantics of an API. This lack of strong idioms extends beyond the distinction between type categories; C++ is fond of defining complex APIs with specific, sometimes unique, semantics that impose implicit restrictions on their use. Some of the most common and most problematic API patterns concern the use of memory projections, pointers, and C++ references. Taking, for example, three different values: a value of type `std::vector`, a value returned from that vector's subscript operator, and a value returned from that vector's `begin()` method, it becomes obvious both how diverse and pervasive of these APIs patterns are.

In the above example, the type of each value looks simliar; each type is either a pointer or a type containing a pointer. Unfortuantly, this similarity does not extend beyond the lose structure of type. Each value has its own, implicit lifetime properties and semantics which users must take into account. The vector is a value type that will be copied, users need to be awair that copies might be expensive and mutating a copy will not change the source value. The result of the subscript operator is a mutable projection with exactly the opposite semantics, however, it can be implicitly converted to a value type as well, and this is how it is often used. Finally, the `begin` method returns an iterator, which is also a projection, but with completely different semantics. With both projections, there is an implicit lifetime dependency that C++ represents lexically and programmers must take care to enforce manually.

C++'s types are often either views into managed memory, or managed memory themselves. The line between these two categories is not clear, and neither are the categories themselves. Unlike Objective-C where pointers are almost exclusively managed with ARC, C++ often deals in un-onwed views into managed memory. In the rare case that Objective-C projects it's memory in an un-owned view, the source is to an object with a long and stable lifetime. The fact that C++ leans so heavily on managed pointers and views into these pointers, and that C++'s safety model is so manual, makes C++ interop a much more difficult task that Objective-C interop. 

The detailed approach for importing C++ APIs into Swift follows from the goals above. Much like the example above picked off vairous API patterns from the mess of APIs that `std::vector` provides, C++ interoperability will need to pick off specific API patterns from the mess of all C++ APIs. For example, `std::vector` itself maps well to a Swift value type. The subscript operator can be bridged to a Swift subscript with a getter and a setter. And with some bridging, the `begin` method can be mapped to a safe Swift iterator protocol. The Swift compiler will know about many possible C++ API patterns. If a C++ API has semantic annotations telling Swift that it follows a certain pattern, Swift will try to create a Swift interface for it following the rules of that pattern. In the absence of those annotations, Swift will try to use huristics to recognize an appropriate pattern. If this fails, Swift will make the API unavailable.
...
The "API patterns" model picks off various C++ APIs that Swift can handle in a specific, idomatic way such as self contained value types, reference types, singletons, iterators, generalized accessors, and so on.
