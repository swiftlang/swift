...
#### Base class: `SwiftExistentialType`

All non-marker protocol existential wrappers inherit from `SwiftExistentialType`,
defined in `_SwiftCxxInteroperability.h`. The base class owns the existential
container's value buffer and type metadata, and provides VWT-delegated lifecycle
operations (copy, move, destroy).

```c++
class SwiftExistentialType {
  const ValueWitnessTable *_getVWT() const noexcept;
  void _destroyValue() noexcept;

public:
  SwiftExistentialType(const SwiftExistentialType &other) noexcept;
  SwiftExistentialType(SwiftExistentialType &&other) noexcept;
  SwiftExistentialType &operator=(const SwiftExistentialType &other) noexcept;
  SwiftExistentialType &operator=(SwiftExistentialType &&other) noexcept;
  ~SwiftExistentialType() noexcept;

protected:
  struct uninit_t {};
  SwiftExistentialType(uninit_t) noexcept {}
  void _initializeWithCopy(const SwiftExistentialType &src) noexcept;
  void _initializeWithValue(const void *src) noexcept;
  ...
  void *_buffer[3];  // 24-byte inline value buffer (or HeapObject* at [0] for outline)
  void *_type;       // Swift type metadata pointer
};
```
