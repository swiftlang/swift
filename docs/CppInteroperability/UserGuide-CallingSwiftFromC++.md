...
Swift error handling in C++ is not supported yet on Windows and on arm64e.
In Embedded Swift, `what()` returns the fixed string `"swift::Error"`, and
`as<T>()` is not supported.
