...
Safety is a top priority for the Swift programming language. C++ often puts safety in the hands of the programmer. Nullability, memory safety, lifetimes, and mutability are all the programmers responsibility in C++. Swift has the opposite default behavior in many of these cases. Simply using C++ APIs should not completely destroy the memory safety, nullability check, etc. that Swift tries to offer. At a minimum, imported C++ APIs should generally not be less safe to use from Swift than they would be in C++, and C++ interoperability should strive to make imported APIs *safer* in Swift than they are in C++ by providing safe API interfaces for common, unsafe C++ API patterns (such as iterators). When it is possible for the Swift compiler to statically derive safety properties and API semantics (i.e., how to safely use an API in Swift) from the C++ API interface, C++ interoperability should take advantage of this information. When that is not possible, C++ interoperability should provide annotations to communicate the nessisary information to use these APIs safely in Swift. When APIs cannot be used safely or need careful management, Swift should make that clear to the programmer, and even make those APIs unavailable if there's no reasonable path to a sufficently safe interface.

C++ interoperability should strive to have good diagnostics. When an API cannot be imported, this needs to be communicated to the programmer. The compiler should produce a clear error any time an API that could not be imported is used, and should strive to guide the programmer towards an API interface that can be imported (for example, by suggesting annotations). The compiler should warn users about incorrect API usage, and potentially slow or unsafe patterns. C++ often has complex type names, especially for template specializations, this is an area where the Swift compiler can do better than C++ by showing shorter and more helpful diagnostics, right at the call-site. When the Swift compiler can present better diagnostics than Clang, it absolutely should. 

C++ provides tools to create high-performance APIs. C++ interoperability should embrace this. Interop should not be a significant source of overhead and performance concerns should not be a reason to continue using C++ to call C++ APIs rather than Swift.
...
This type is not intended to be used directly as the type of a local variable or a `std::vector` element; it is allocated on the heap by the `create` method and passed around as a pointer. This is weakly enforced by the way it hides its constructors, but mostly it's communicated in the documentation and by the overall shape of the API; there is no language feature or programming pattern that expresses the specific semantics of this type. 

In Swift, `StatefulObject` would be explicitly defined as a reference type using a specific language construct (defining it as a `class`). Swift defines clear patterns for naming, generic programming, value categories, error handling, and so on, and codebases are encouraged to use these standard practices. These well-defined programming patterns make using Swift APIs a cohesive experience; an experience that C++ interoperability should stive to maintain for Swift programmers using C++ APIs.

To maintain this cohesive experience, the compiler should map C++ APIs to one of these specific Swift programming patterns. To be able to map *any* C++ APIs to *some* specific Swift pattern, the Swift compiler must have some semantic information about the API. This semantic information may be provided through annotations or heuristics in the compiler. In the above example, the user should annotate `StatefulObject` as a reference type so that the Swift compiler can import it as a Swift `class`. 

The Swift compiler should not import APIs for which it does not have sufficent semantic information. It is not a goal to import every C++ API into Swift, especially without additional, required information to present the API in an idiomatic way that promotes a cohesive Swift expirence. The Swift compiler will have a default path for importing APIs where C++ interoperability makes various assumptions about the API that is being imported. Sometimes, as is the case above with `StatefulObject`, an API will fall outside of this path, and will not be imported by default. Other times, the default assumptions will be incorrect for a given API. In both cases, Swift must provide ways for the user to communicate the correct semantics of an API, overriding the default behavior.
