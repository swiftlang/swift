...
sil-instruction ::= 'unconditional_checked_cast_addr'
                    sil-prohibit-isolated-conformances?
                    ('take_always' | 'copy_on_success')?
                    sil-type 'in' sil-operand 'to'
                    sil-type 'in' sil-operand

unconditional_checked_cast_addr A in %0 : $*A to B in %1 : $*B
// %0 and %1 must both be addresses.
// The default consumption kind is take_always.
// With take_always, %0 is consumed during the conversion.
// With copy_on_success, %0 remains initialized and is not consumed.
```
...
uninitialized. `take_always` consumes the source value; `copy_on_success`
preserves it and produces an independently owned destination value. Both forms
