...
| Type of param (`a` or `self`) | default                        | 
| ---------------------------   | -------                        | 
...
### Implicit initializers and setters
...
## Same-type lifetime inference (unimplemented)

Given a function declaration:

`func foo(..., a: A, ...) -> R { ... }`

Where `R: ~Escapable` and `A == R`, Infer `@_lifetime(copy a)`.
...
### Generic same-type lifetime inference (unimplemented)

The same-type lifetime inference rule described above is convenient for nominal types but essential for generic type parameters.

Given a generic function declaration:

`func foo<A, R>(..., a: A, ...) -> R { ... }`

The same-type lifetime inference rule applies to the types in the function declaration's generic context just as it did for nominal types in the previous example. So, again, if type resolution determines `R: ~Escapable` and `A == R`, then `@_lifetime(copy a)` will be inferred.
...
Here we see how same-type lifetime inference applies to type substitution and associated types:
...
struct S<A: P, B: Q> {
  /* OK: @_lifetime(copy a) is valid and infered */
