...
``type*`` contain parameter and return types (including the error result),
 in that order.
...
::
  ...
  identifier ::= '0_' 'a-z'* [A-Z]           // identifier containing only word substitutions
...
letters and digits ``[A-Za-z0-9]``. Words are separated by is underscores
