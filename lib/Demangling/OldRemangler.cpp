...
#include "swift/Demangling/ValueWitnessMangling.def"
  default:
    return ManglingError(ManglingError::BadValueWitnessKind, node);
  }
  Buffer << 'w' << Code;
...
ManglingError Remangler::mangleAccessor(Node *storageNode,
  ...
  default:
    return ManglingError(ManglingError::NotAStorageNode, storageNode);
  }
...
#define CONTEXT_NODE(ID)
#include "swift/Demangling/DemangleNodes.def"
    return ManglingError(ManglingError::NotAContextNode, node);
...
ManglingError Remangler::mangleImplFunctionType(Node *node, unsigned depth) {
  ...
  if (i != e && (*i)->getKind() == Node::Kind::ImplConvention) {
    ...
    if (text == "@callee_unowned") {
      ...
      return ManglingError(ManglingError::InvalidImplCalleeConvention, *i);
    }
  } else {
    Buffer << 't';
  }
...
ManglingError Remangler::mangleImplFunctionAttribute(Node *node,
  ...
  if (text == "@yield_once") {
    ...
    return ManglingError(ManglingError::InvalidImplFunctionAttribute, node);
  }
  return ManglingError::Success;
}
...
ManglingError Remangler::mangleImplConvention(Node *node, unsigned depth) {
  ...
  if (text == "@autoreleased") {
    ...
    return ManglingError(ManglingError::InvalidImplParameterConvention, node);
  }
  return ManglingError::Success;
}
