...
bool ProjectionPath::hasUncoveredNonTrivials(SILType B, const SILFunction &F,
  ...
  do {
    ...
    // Get the current type to process.
    SILType Ty = PP.getMostDerivedType(Mod, TypeExpansionContext(F));

    // Get the first level projection of the current type.
    llvm::SmallVector<Projection, 4> Projections;
    Projection::getFirstLevelProjections(Ty, Mod, TypeExpansionContext(F),
                                         Projections);
    ...
    for (auto &P : Projections) {
      ProjectionPath X(B);
      X.append(PP);
      assert(PP.getMostDerivedType(Mod, TypeExpansionContext(F)) ==
             X.getMostDerivedType(Mod, TypeExpansionContext(F)));
      X.append(P);
      Worklist.push_back(X);
    }
    // Keep iterating if the worklist is not empty.
  ...
  for (auto &X : Paths) {
    if (!X.getMostDerivedType(Mod, TypeExpansionContext(F)).isTrivial(F))
      return true;
  }   
  return false;
}
