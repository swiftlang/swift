...
static bool canCreateDebugifyDebugValue(swift::SILValue value,
                                        swift::SILFunction &function) {
  swift::SILType type = value->getType();

  // Tokens are not real values.
  ...
  // No declaration can have a polymorphic type or a method convention, so
  // IRGen cannot reconstruct debug info for one.
  if (auto fnType = type.getAs<swift::SILFunctionType>())
    if (fnType->isPolymorphic() || fnType->hasSelfParam())
      return false;

  return true;
}

// See the Debugify pass.
OptionalBridgedInstruction
BridgedBuilder::createDebugifyDebugValue(BridgedValue src) const {
  swift::SILBuilder builder = unbridged();
  swift::SILValue value = src.getSILValue();

  if (!canCreateDebugifyDebugValue(value, builder.getFunction()))
    return {nullptr};
  ...
  swift::SILDebugVariable var(name, /*Constant=*/true, /*ArgNo=*/0,
                              ...
                              swift::RegularLocation::getAutoGeneratedLocation());
