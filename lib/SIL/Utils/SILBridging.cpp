...
// of class types.
static void setUnimplementedRange(SwiftMetatype metatype, SILNodeKind from,
                                  SILNodeKind to) {
...
#define VALUE_RANGE(ID) SILNodeKind::First_##ID, SILNodeKind::Last_##ID
  ...
  if (clName == "UnimplementedMultiValueInst")
    return setUnimplementedRange(metatype,
                                 VALUE_RANGE(MultipleValueInstruction));
...
#define BRIDGED_VALUE(ID, PARENT) valueNamesToKind[#ID] = SILNodeKind::ID;
#define BRIDGED_NON_VALUE_INST(ID, NAME, PARENT, MEMBEHAVIOR, MAYRELEASE)      \
  BRIDGED_VALUE(ID, NAME)
#define BRIDGED_ARGUMENT(ID, PARENT) BRIDGED_VALUE(ID, NAME)
...
BridgedArgument SILBasicBlock_getArgument(BridgedBasicBlock block,
                                          SwiftInt index) {
...
BridgedInstruction
MultiValueInstResult_getParent(BridgedMultiValueResult result) {
  return {
      static_cast<MultipleValueInstructionResult *>(result.obj)->getParent()};
}
