...
InitTupleAddrInst *InitTupleAddrInst::create(
  ...
  auto Buffer = M.allocateInst(Size, alignof(InitTupleAddrInst));
  llvm::SmallVector<SILValue, 16> Data;
  Data.push_back(DestAddr);
  copy(Elements, std::back_inserter(Data));
  return ::new (Buffer) InitTupleAddrInst(Loc, Data, IsInitOfDest);
}
