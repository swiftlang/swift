...
namespace {
  ...
  public:
    ...
    bool parseSILDebugVar(SILDebugVariable::Builder &VarBUilder);
...
bool SILParser::parseSILDebugVar(SILDebugVariable::Builder &b) {
  ...
  while (P.Tok.is(tok::comma) && !peekSILDebugLocation(P)) {
    ...
    if (P.consumeIf(tok::l_paren)) {
      ...
      SILDebugInfoExpression DbgExpr;
      if (parseSILDebugInfoExpression(DbgExpr))
        return true;
      b.withDIExpr(DbgExpr);
      NoConsume = true;
    } else if (Key == "type") {
...
bool SILParser::parseSpecificSILInstruction(SILBuilder &B,
  ...
  case SILInstructionKind::AllocBoxInst: {
    ...
    if (parseSILType(Ty))
      return true;
    SILDebugVariable::Builder VarInfoBuilder(SILMod);
    if (parseSILDebugVar(VarInfoBuilder))
      return true;
    if (parseSILDebugLocation(InstLoc, B))
      return true;
    ResultVal = B.createAllocBox(InstLoc, Ty.castTo<SILBoxType>(), std::move(VarInfoBuilder).finalize(),
                                 hasDynamicLifetime);
    break;
  }
...
#undef REFCOUNTING_INSTRUCTION
  ...
  case SILInstructionKind::DebugValueInst: {
    bool poisonRefs = false;
    SILDebugVariable::Builder VarInfoBuilder(B.getModule());
    if (parseSILOptional(poisonRefs, *this, "poison")
        || parseTypedValueRef(Val, B) || parseSILDebugVar(VarInfoBuilder) ||
        parseSILDebugLocation(InstLoc, B))
      return true;
    if (Val->getType().isAddress())
      assert(!poisonRefs && "debug_value w/ address value does not support poison");
    ResultVal = B.createDebugValue(InstLoc, Val, std::move(VarInfoBuilder).finalize(), poisonRefs);
    break;
  }
...
#include "swift/AST/ReferenceStorage.def"
  ...
  case SILInstructionKind::AllocStackInst: {
    ...
    SILDebugVariable::Builder VarInfoBuilder(B.getModule());
    if (parseSILDebugVar(VarInfoBuilder) || parseSILDebugLocation(InstLoc, B))
      return true;
    // It doesn't make sense to attach a debug var info if the name is empty
    auto VarInfo = std::move(VarInfoBuilder).finalize();
