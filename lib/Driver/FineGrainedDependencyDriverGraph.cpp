...
ModuleDepGraph::loadFromBuffer(const Job *job, llvm::MemoryBuffer &buffer,
                               DiagnosticEngine &diags) {
  std::optional<SourceFileDepGraph> sourceFileDepGraph =
...
ModuleDepGraph::Changes ModuleDepGraph::integrate(const SourceFileDepGraph &g,
  ...
  std::optional<std::unordered_set<ModuleDepGraphNode *>> changedNodes =
  ...
  g.forEachNode([&](const SourceFileDepGraphNode *integrand) {
    ...
    std::optional<NullablePtr<ModuleDepGraphNode>> newNodeOrChangedNode =
        integrateSourceFileDepGraphNode(g, integrand, preexistingMatch,
                                        swiftDepsOfJob);

    if (!newNodeOrChangedNode)
      changedNodes = std::nullopt;
...
ModuleDepGraphNode *ModuleDepGraph::integrateByCreatingANewNode(
    ...
    const std::optional<std::string> swiftDepsForNewNode) {
...
void ModuleDepGraph::verifyNodeIsInRightEntryInNodeMap(
  ...
  const std::optional<std::string> swiftDeps =
      swiftDepsString.empty() ? std::nullopt
                              : std::optional<std::string>(swiftDepsString);
  (void)nodeKey;
  (void)swiftDeps;
...
StringRef ModuleDepGraph::getProvidingFilename(
    const std::optional<std::string> &swiftDeps) const {
  if (!swiftDeps)
    return "<unknown";
