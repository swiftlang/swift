...
bool ModuleDepGraph::integrateSourceFileDepGraphNode(
  ...
  return integrand->getSwiftDeps().hasValue()
             ...
             : integrateFrontendExpatNode(
                   integrand, swiftDepsOfSourceFileDepGraph,
                   preexistingNodeInPlace, preexistingExpat,
...
ModuleDepGraphNode *ModuleDepGraph::integrateByCreatingANewNode(
    const SourceFileDepGraphNode *integrand) {
  const auto &key = integrand->getKey();
  ModuleDepGraphNode *newNode =
      new ModuleDepGraphNode(key, integrand->getFingerprint());
