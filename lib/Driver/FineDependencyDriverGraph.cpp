...
// Definitions for the portion of the fine-grained dependency system used by the
// driver.

using namespace swift;

using namespace swift::fine_dependencies;
...
using LoadResult = fine_dependencies::DependencyGraphImpl::LoadResult;

LoadResult ModuleDepGraph::loadFromPath(const Job *Cmd, StringRef path,
                                        DiagnosticEngine &diags) {
  FrontendStatsTracer tracer(stats, "fine-dependencies-loadFromPath");
  ...
  if (emitFineDependencyDotFileAfterEveryImport)
    emitDotFileForJob(diags, Cmd);
  if (verifyFineDependencyGraphAfterEveryImport)
    verify();
  return r;
}
...
void ModuleDepGraph::markTransitive(
  ...
  FrontendStatsTracer tracer(stats, "fine-dependencies-markTransitive");
...
void ModuleDepGraph::markExternal(SmallVectorImpl<const Job *> &uses,
  ...
  FrontendStatsTracer tracer(stats, "fine-dependencies-markExternal");
...
LoadResult ModuleDepGraph::integrate(const SourceFileDepGraph &g) {
  FrontendStatsTracer tracer(stats, "fine-dependencies-integrate");
...
void ModuleDepGraph::emitDotFile(llvm::raw_ostream &out) {
  FrontendStatsTracer tracer(stats, "fine-dependencies-emitDotFile");
...
bool ModuleDepGraph::verify() const {
  FrontendStatsTracer tracer(stats, "fine-dependencies-verify");
