...
/// Creates a diagnostic consumer that serializes diagnostics in \p format,
/// based on the supplementary output paths specified by \p inputsAndOutputs.
///
  ...
  return createDispatchingDiagnosticConsumerIfNeeded(
      ...
      [format, emitMacroExpansionFiles](
        ...
        case DiagnosticOptions::SerializedFormat::LLVMBitcode:
          path = input.getLLVMBitcodeDiagnosticsPath();
        ...
        if (path.empty())
          return nullptr;
        return createConsumer(path, emitMacroExpansionFiles);
      });
}
