...
/// Determine whether position-independent code (PIC) should be generated by
/// default for the given target. Swift generates PIC by default, except on
/// Windows, which is implicitly position-independent and does not support
/// `-fPIC`.
static bool getDefaultUsePositionIndependentCode(const llvm::Triple &Triple) {
...
static bool ParseClangImporterArgs(ClangImporterOptions &Opts, ArgList &Args,
  ...
  Opts.UsePositionIndependentCode =
      Args.hasFlag(OPT_enable_position_independent_code,
                   OPT_disable_position_independent_code,
                   getDefaultUsePositionIndependentCode(LangOpts.Target));
...
static bool ParseIRGenArgs(IRGenOptions &Opts, ArgList &Args,
  ...
  Opts.UsePositionIndependentCode =
      Args.hasFlag(OPT_enable_position_independent_code,
                   OPT_disable_position_independent_code,
                   getDefaultUsePositionIndependentCode(Triple));
  // Non-PIC code generation is only meaningful for Embedded Swift, which does
  // not rely on the position-independent Swift runtime. Diagnose an explicit
  // request to disable PIC outside of Embedded Swift (the default may already
  // be non-PIC on some targets, e.g. Windows).
  if (Args.hasArg(OPT_disable_position_independent_code) &&
      !LangOpts.hasFeature(Feature::Embedded)) {
    Diags.diagnose(SourceLoc(), diag::disable_pic_without_embedded);
    return true;
  }
