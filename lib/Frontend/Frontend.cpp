...
bool CompilerInstance::setupInputs(Optional<unsigned> codeCompletionBufferID) {
  ...
  for (const InputFileOrBuffer &input: Invocation.getFrontendOptions().Inputs.getInputs())
...
void CompilerInstance::createREPLFile(
  ...
  auto *SingleInputFile = createSourceFileForMainModule(Invocation.getSourceFileKind(), implicitImports.kind, None);
...
void CompilerInstance::addMainFileToModule(
  ...
  auto *MainFile = createSourceFileForMainModule(Invocation.getSourceFileKind(), implicitImports.kind, MainBufferID);
...
void CompilerInstance::parseLibraryFile(
  ...
  auto *NextInput = createSourceFileForMainModule(SourceFileKind::Library, implicitImports.kind, BufferID);
...
SourceFile *CompilerInstance::createSourceFileForMainModule(SourceFileKind FileKind,
  ...
  SourceFile *InputFile = new (*Context)SourceFile(*MainModule, FileKind, BufferID,
             ImportKind, KeepSyntaxInfo);
...
void CompilerInstance::performParseOnly(bool EvaluateConditionals) {
  ...
  if (MainBufferID != NO_SUCH_BUFFER) {
    assert(Kind == InputFileKind::IFK_Swift);
    SourceMgr.setHashbangBufferID(MainBufferID);
    createSourceFileForMainModule(Invocation.getSourceFileKind(), SourceFile::ImplicitModuleImportKind::None, MainBufferID);
  }
  ...
  for (auto BufferID : InputSourceCodeBufferIDs) {
    ...
    SourceFile *NextInput = createSourceFileForMainModule(SourceFileKind::Library, SourceFile::ImplicitModuleImportKind::None, BufferID);
