...
Parser::parseDeclPrecedenceGroup(ParseDeclOptions flags,
  ...
  auto checkCodeCompletion = [&](PrecedenceGroupCompletionKind PK) -> bool {
    if (Tok.is(tok::code_complete)) {
      if (CodeCompletion)
        CodeCompletion->completeInPrecedenceGroup(PK);
      consumeToken();
      return true;
    }
    return false;
  ...
  while (Tok.isNot(tok::r_brace)) {
    if (checkCodeCompletion(PrecedenceGroupCompletionKind::AttributeList)) {
    ...
    if (attrName == "associativity") {
      ...
      if (checkCodeCompletion(PrecedenceGroupCompletionKind::Associativity))
    ...
    if (attrName == "assignment") {
      ...
      TokReceiver->registerTokenKindChange(assignmentKeywordLoc,
                                           tok::contextual_keyword);
      if (checkCodeCompletion(PrecedenceGroupCompletionKind::Assignment))
        return abortBody(/*hasCodeCompletion*/true);
    ...
    if (attrName == "higherThan" ||
      ...
      do {
        if (checkCodeCompletion(PrecedenceGroupCompletionKind::Relation)) {
          return abortBody(/*hasCodeCompletion*/true);
        }
