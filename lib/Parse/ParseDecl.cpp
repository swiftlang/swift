...
#include "swift/AST/DeclAttr.def"
  ...
  case DeclAttrKind::ObjCImplementation: {
    ...
    // `@implementation(safe)` is a contextual modifier (not a category name)
    // that marks an `@c` function as a safe-typed implementation of a
    // C-imported function. The corresponding C entry point is generated by an
    // implicit `@_Unswiftify` peer macro invocation. This spelling is only
    // accepted on functions because functions can't have category names, so
    // there's no ambiguity with the existing category-name argument on
    // extensions.
    bool isSafe = false;
    Identifier categoryName = *name;
    if (!categoryName.empty() && categoryName.str() == "safe") {
      isSafe = true;
      categoryName = Identifier();
    }
    Attributes.add(new (Context) ObjCImplementationAttr(
        categoryName, AtLoc, range, isEarlyAdopter,
        /*Implicit=*/false, /*isCategoryNameInvalid=*/false, isSafe));
    break;
  }
