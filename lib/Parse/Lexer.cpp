...
Lexer::Lexer(const Lexer &Parent, State BeginState, State EndState,
    ...
    : Lexer(PrincipalTag(), Parent.LangOpts, Parent.SourceMgr, Parent.BufferID,
            ...
            modeForChildLexer(Parent.LexMode),
...
bool Lexer::previousLineWasEmpty() {
    ...
    if (ch == '`' || ch == '~') {
      if (ch == MarkdownFence) {
        if (CurPtr - FenceStart == 3) {
          ...
          if (Language.size() && Language != "swift")
            skip = true;
  ...
  while (CurPtr < BufferEnd) {
    ...
    case IndentState::GotBlankLine:
      if (ch == ' ')
        ...
        if (Rest.starts_with("code-block::")
            || Rest.starts_with("sourcecode::")) {
          CurPtr += 12;
...
bool Lexer::atEndOfCodeBlock() {
  ...
  case LexerMode::LaTeX: {
    StringRef Rest(CurPtr, BufferEnd - CurPtr);
    return Rest.starts_with("\\end{swift}");
  }
  default:
    return false;
  }
