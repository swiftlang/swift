...
/// doesn't exist if the experimental feature \c ModuleSelectors is disabled.
static bool isColon(Parser &P, Token Tok) {
  ...
  // when the experimental feature \c ModuleSelectors is true. We will need to
  // address this before the feature can be enabled.
  if (P.Context.LangOpts.hasFeature(Feature::ModuleSelectors))
