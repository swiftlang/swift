...
static bool generateReproducer(CompilerInstance &Instance,
  ...
  if (!mapOpts.empty()) {
    ...
    // Write new explicit module map.
    std::string newModuleMap;
    llvm::raw_string_ostream mapStream(newModuleMap);
    mapStream << *map;

    auto newMapRef = db->first->storeFromString({}, newModuleMap);
    if (!newMapRef) {
