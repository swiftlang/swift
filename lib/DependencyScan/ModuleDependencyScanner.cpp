...
ModuleDependencyScanner::resolveImportedModuleDependencies(
  ...
  resolveHeaderDependencies(rootModuleID, discoveredSwiftModules.getArrayRef(),
                            cache, discoveredHeaderDependencyClangModules);
  ...
  for (const auto &moduleID : allSwiftModules) {
    ...
    if (!moduleDependencyInfo.getHeaderClangDependencies().empty()) {
      allDiscoveredHeaderDependencyClangModules.insert(
          moduleDependencyInfo.getImportedSwiftDependencies().begin(),
...
llvm::Error ModuleDependencyScanner::performBridgingHeaderChaining(
  ...
  // If `-scanner-output-dir` is set, materialize the output file.
  bool emitOnDiskFile =
      !ScanCompilerInvocation.getFrontendOptions().ScannerOutputDir.empty();
  if (emitOnDiskFile) {
