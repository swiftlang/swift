...
private:
  ...
  StringRef getMangledName(DebugTypeInfo DbgTy) {
    ...
    // Returns true iff the given type comes from an implicit declaration
    // generated by the differentiation transform: a linear map struct or
    // branching trace enum.
    auto isAutoDiffGeneratedDeclarationType = [](Type type) -> bool {
      // Peer through metatype types.
      type = type->getMetatypeInstanceType();
      // Check for linear map structs.
      if (auto *structDecl = type->getStructOrBoundGenericStruct())
        return structDecl->isImplicit() &&
               structDecl->getNameStr().contains("__PB__");
      // Check for branching trace enums.
      if (auto *enumDecl = type->getEnumOrBoundGenericEnum())
        return enumDecl->isImplicit() &&
               enumDecl->getNameStr().contains("__Pred__");
      return false;
    };
    ...
    if (!Opts.DisableRoundTripDebugTypes
        ...
        // FIXME(TF-1232): Add type reconstruction for AutoDiff-generated decls.
        && !isAutoDiffGeneratedDeclarationType(Ty)) {
