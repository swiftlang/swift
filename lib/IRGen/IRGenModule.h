...
public:
  /// Attempt to configure this IRGenerator.  This might not succeed,
  /// in which case isValid() will return false.
  explicit IRGenerator(IRGenOptions &opts, SILModule &module);

  /// Did configuration succeed?
  bool isValid() const { return TargetMachine != nullptr; }
...
public:
  ...
  IRGenModule(IRGenerator &irgen, SourceFile *SF,
              ASTContext &Context,
              llvm::LLVMContext &LLVMContext,
