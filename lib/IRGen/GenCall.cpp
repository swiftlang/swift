...
void irgen::emitAsyncLetEnd(IRGenFunction &IGF, llvm::Value *asyncLet) {
//  if (task->getType() != IGF.IGM.SwiftTaskPtrTy) {
//    task = IGF.Builder.CreateBitCast(task, IGF.IGM.SwiftTaskPtrTy); // TODO: need to cast to pointer to AsyncLet or RawPointer?
//  }

  auto *call = IGF.Builder.CreateCall(IGF.IGM.getAsyncLetEndFn(), {asyncLet});
  call->setDoesNotThrow();
  call->setCallingConv(IGF.IGM.SwiftCC);
}
