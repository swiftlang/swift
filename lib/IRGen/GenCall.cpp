...
/// (maybeAddIndirectThrowingResult) to decide whether to emit the ind_error
/// slot.
static bool
...
public:
  ...
  void setFromCallee() override {
    ...
    // write the context before ind_error, or at the tail below if the callee
    // has no indirect typed error.
    ...
    // branch above already placed it ahead of ind_error.)
    if (contextPtr) {
      assert(LastArgWritten > 0);
      Args[--LastArgWritten] = contextPtr;
      IGF.IGM.addSwiftSelfAttributes(CurCallee.getMutableAttributes(),
