...
protected:
  ...
  template<typename Body>
  void eachElementAddr(IRGenFunction &IGF, Address addr, Body &&body) const {
    for (unsigned i = 0; i < ArraySize; ++i) {
  ...
  std::optional<unsigned> getFixedArraySize(SILType T) const override {
    return ArraySize;
  }
...
{
public:
  FixedArrayTypeInfo(unsigned arraySize,
                     const FixedTypeInfo &elementTI)
    : FixedArrayTypeInfoBase(arraySize, elementTI,
                       getArrayType(arraySize, elementTI),
                       getArraySize(arraySize, elementTI),
