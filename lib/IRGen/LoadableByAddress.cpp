...
void LargeValueVisitor::visitApply(ApplySite applySite) {
  ...
  for (Operand &operand : applySite.getArgumentOperands()) {
    ...
    if (std::find(pass.largeLoadableArgs.begin(), pass.largeLoadableArgs.end(),
        ...
        isLargeLoadableType(genEnv, silType, pass.Mod) ||
...
public:
  ...
  void replaceLoadWithCopyAddrForApplies(LoadInst *unoptimizableLoad);
...
void LoadableStorageAllocation::replaceLoadWithCopyAddr(
    LoadInst *optimizableLoad, bool replaceProjection) {
  ...
  for (auto *user : optimizableLoad->getUses()) {
    ...
    case ValueKind::StructExtractInst: {
      ...
      assert(replaceProjection && "Unexpected cast failure");
      if (!replaceProjection) {
        llvm_unreachable("Unexpected usage: Can't replace StructExtractInst");
      }
    ...
    case ValueKind::SwitchEnumInst: {
      ...
      assert(replaceProjection && "Unexpected cast failure");
      if (!replaceProjection) {
        llvm_unreachable("Unexpected usage: Can't replace SwitchEnumInst");
      }
...
void LoadableStorageAllocation::replaceLoadWithCopyAddrForApplies(
...
static void setInstrUsers(StructLoweringState &pass, AllocStackInst *allocInstr,
  ...
  for (Operand *userOp : uses) {
    ...
    if (std::find(pass.applies.begin(), pass.applies.end(), user) !=
        pass.applies.end()) {
      // We can only modify pre-filtered (non-objc) apply sites
...
static void allocateAndSetForNonInstrOperand(StructLoweringState &pass,
                                             SILValue value,
...
static bool allUsesAreReplaceable(SILInstruction *instr, bool replaceProjection,
                                  irgen::IRGenModule &Mod) {
...
static void rewriteFunction(StructLoweringState &pass,
    ...
    while (!pass.switchEnumInstsToMod.empty()) {
      ...
      for (unsigned i = 0; i < numOfCases; ++i) {
        ...
        for (SILArgument *arg : currBB->getArguments()) {
          ...
          if (allUsesAreReplaceable(loadArg, true, pass.Mod)) {
            allocator.replaceLoadWithCopyAddr(loadArg, true);
          } else {
            allocator.replaceLoadWithCopyAddrForApplies(loadArg);
          }
        }
  ...
  for (SILInstruction *applyInst : pass.applies) {
    ...
    for (Operand &operand : applySite.getArgumentOperands()) {
      ...
      if (isLargeLoadableType(genEnv, silType, pass.Mod)) {
        ...
        if (!currOperandInstr) {
          allocateAndSetForNonInstrOperand(pass, currOperand, applyInst);
        } else if (LoadInst *load = dyn_cast<LoadInst>(currOperandInstr)) {
          if (allUsesAreReplaceable(load, false, pass.Mod)) {
            allocator.replaceLoadWithCopyAddr(load, false);
          } else {
            allocator.replaceLoadWithCopyAddrForApplies(load);
          }
        } else {
...
void LoadableByAddress::recreateApplies() {
  for (auto *applyInst : modApplies) {
    ...
    auto subs = applySite.getSubstitutions();
    ...
    case ValueKind::ApplyInst: {
      ...
      newApply = applyBuilder.createApply(
          castedApply->getLoc(), callee, newSubType, resultType, subs, callArgs,
          castedApply->isNonThrowing());
    ...
    case ValueKind::TryApplyInst: {
      ...
      newApply = applyBuilder.createTryApply(
          castedApply->getLoc(), callee, newSubType, subs, callArgs,
