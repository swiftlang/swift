...
/// Moves \p AS out of its inlined debug scope before it is hoisted into the
/// entry block and rewrite any variables to use debug_value.  Otherwise the
/// instruction would become part of the inlined subroutine's address ranges
/// which would create an unintended breakpoint for that function.
static void rescopeInlinedAllocStack(AllocStackInst *AS) {
  const SILDebugScope *DS = AS->getDebugScope();
  if (!DS || !DS->InlinedCallSite)
    return;
  if (auto VarInfo = AS->getVarInfo()) {
    SILBuilder Builder(AS, DS);
    Builder.createDebugValue(AS->getLoc(), AS, *VarInfo,
                             AS->usesMoveableValueDebugInfo());
    AS->invalidateVarInfo();
  }
  AS->setDebugLocation(
      SILDebugLocation(RegularLocation::getAutoGeneratedLocation(),
                       AS->getFunction()->getDebugScope()));
}
...
public:
  ...
  bool mayOverlap(AllocStackInst *A, AllocStackInst *B) {
    ...
    if (singleDeallocB->strictlyDominatesInBlock(A))
      return false;
    if (singleDeallocA->strictlyDominatesInBlock(B))
      return false;
...
void HoistAllocStack::hoist() {
  ...
  for (auto *AllocStack : AllocStackToHoist) {
    rescopeInlinedAllocStack(AllocStack);
