...
/// FIXME: A direct reference to an internal witness table generated by the old
/// WitnessBuilder path.
class OldNBustedConformanceInfo : public ConformanceInfo {
  friend class ProtocolInfo;
  
  llvm::Constant *Table;
public:
  OldNBustedConformanceInfo(llvm::Constant *Table)
    : Table(Table) {}
...
ProtocolInfo::getConformance(IRGenModule &IGM, CanType concreteType,
  ...
  if (IGM.Context.LangOpts.EmitSILProtocolWitnessTables) {
    ...
    auto table = buildWitnessTable(IGM, realConcreteType, protocol, witnesses);

    info = new OldNBustedConformanceInfo(table);
  }
