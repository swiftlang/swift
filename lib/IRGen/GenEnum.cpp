...
namespace {
  ...
  public:
    ...
    void loadAsCopy(IRGenFunction &IGF, Address addr,
      ...
      copy(IGF, tmp, e, getAtomicity(IGF));
    }
    ...
    void assign(IRGenFunction &IGF, Explosion &e, Address addr) const override {
      ...
      if (!isPOD(ResilienceExpansion::Maximal))
        consume(IGF, old, getAtomicity(IGF));
    }
    ...
    : public PayloadEnumImplStrategyBase
  {
    ...
    llvm::Function *emitCopyEnumFunction(IRGenModule &IGM, EnumDecl *theEnum) {
      ...
      if (PayloadBitCount > 0) {
        ...
        loadableTI.copy(IGF, payloadValue, payloadCopy, getAtomicity(IGF));
    ...
    llvm::Function *emitConsumeEnumFunction(IRGenModule &IGM,
      ...
      if (PayloadBitCount > 0) {
        ...
        auto &loadableTI = getLoadablePayloadTypeInfo();
        loadableTI.unpackFromEnumPayload(IGF, payload, payloadValue, 0);
        loadableTI.consume(IGF, payloadValue, getAtomicity(IGF));
      }
  ...
  private:
    ...
    void retainRefcountedPayload(IRGenFunction &IGF,
      ...
      switch (CopyDestroyKind) {
      case NullableRefcounted:
        IGF.emitStrongRetain(ptr, Refcounting, getAtomicity(IGF));
        return;
      case POD:
      case Normal:
        llvm_unreachable("not a refcounted payload");
    ...
    void releaseRefcountedPayload(IRGenFunction &IGF,
      ...
      switch (CopyDestroyKind) {
      case NullableRefcounted:
        IGF.emitStrongRelease(ptr, Refcounting, getAtomicity(IGF));
        return;
      case POD:
      case Normal:
        llvm_unreachable("not a refcounted payload");
    ...
    : public PayloadEnumImplStrategyBase
  {
    ...
    llvm::Function *emitCopyEnumFunction(IRGenModule &IGM, EnumDecl *theEnum) {
      ...
      forNontrivialPayloads(IGF, parts.tag, [&](unsigned tagIndex,
        ...
        Explosion tmp;
        lti.copy(IGF, value, tmp, getAtomicity(IGF));
    ...
    llvm::Function *emitConsumeEnumFunction(IRGenModule &IGM,
      ...
      forNontrivialPayloads(IGF, parts.tag, [&](unsigned tagIndex,
        ...
        lti.consume(IGF, value, getAtomicity(IGF));
      });
    ...
    void emitIndirectAssign(IRGenFunction &IGF,
      ...
      case Normal: {
        ...
        if (TI->isLoadable()) {
          ...
          consume(IGF, tmpOld, getAtomicity(IGF));
          return;
        }
    ...
    void destroy(IRGenFunction &IGF, Address addr, SILType T) const override {
      ...
      case TaggedRefcounted: {
        ...
        if (TI->isLoadable()) {
          Explosion tmp;
          loadAsTake(IGF, addr, tmp);
          consume(IGF, tmp, getAtomicity(IGF));
          return;
        }
