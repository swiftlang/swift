...
public:
  ...
  Address emitShadowCopy(llvm::Value *Storage, const SILDebugScope *Scope,
    ...
    if (!Alloca.isValid())
      Alloca =
          createAlloca(Storage->getType(), Align, VarInfo.getName() + ".debug");
  ...
  void emitShadowCopyIfNeeded(SILValue &SILVal, const SILDebugScope *Scope,
    ...
    if (Alloca.isValid()) {
      ...
      Alloca = LTI.allocateStack(*this, Type, VarInfo.getName() + ".debug")
                   .getAddress();
      zeroInit(cast<llvm::AllocaInst>(Alloca.getAddress()));
      ArtificialLocation AutoRestore(Scope, IGM.DebugInfo.get(), Builder);
      ...
      runtimeTy.visit([&](CanType t) {
        if (auto archetype = dyn_cast<ArchetypeType>(t))
          ...
          if (PackShapeExpressions.insert(Shape).second) {
              ...
              auto &ctx = CurSILFn->getASTContext();
              SILDebugVariable Var(CurSILFn->getModule(),
                                   ctx.getIdentifier(Name.str()), true, 0);
...
void IRGenSILFunction::emitPoisonDebugValueInst(DebugValueInst *i) {
  ...
  varInfo->Name =
      CurSILFn->getASTContext().getIdentifier(getVarName(i, isAnonymous));
  ...
  if (singleValueExplosion) {
    ...
    if (!alloca.isValid()) {
      ...
      alloca = lti.allocateStack(*this, silTy, varInfo->getName() + ".debug")
                   .getAddress();
    }
