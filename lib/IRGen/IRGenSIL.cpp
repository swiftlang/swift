...
public:
  ...
  StringRef getVarName(DebugVarCarryingInst *i, bool &IsAnonymous) {
    ...
    StringRef Name = i->getVarInfo()->getName();
  ...
  bool canAllocaStoreValue(llvm::Value *Alloca, llvm::Value *Val,
    ...
    llvm::errs() << "Invalid shadow copy:\n"
                 ...
                 << "  Alloca: " << *Alloca << "\n"
                 << "---\n"
                 << "Previous shadow copy into " << VarInfo.getName()
                 << " in the same scope!\n"
                 << "Scope:\n";
  ...
  llvm::Value *emitShadowCopy(llvm::Value *Storage, const SILDebugScope *Scope,
    ...
    unsigned ArgNo = VarInfo.getArgNo();
    auto &Alloca = ShadowStackSlots[{ArgNo, {Scope, VarInfo.getName()}}];
    if (!Alloca.isValid())
      Alloca = createAlloca(Storage->getType(), Align, VarInfo.getName() + ".debug");
  ...
  void emitShadowCopyIfNeeded(SILValue &SILVal, const SILDebugScope *Scope,
    ...
    if (Alloca.isValid()) {
      ...
      Alloca =
        LTI.allocateStack(*this, Type, VarInfo.getName() + ".debug").getAddress();
      zeroInit(cast<llvm::AllocaInst>(Alloca.getAddress()));
      ArtificialLocation AutoRestore(Scope, IGM.DebugInfo.get(), Builder);
...
void IRGenSILFunction::emitPoisonDebugValueInst(DebugValueInst *i) {
  ...
  varInfo = varInfo->withName(CurSILFn->getModule(),
                              getVarName(i, isAnonymous));
  ...
  if (singleValueExplosion) {
    ...
    if (!alloca.isValid()) {
      ...
      alloca =
        lti.allocateStack(*this, silTy, varInfo->getName() + ".debug").getAddress();
    }
    shadowAddress =
...
void IRGenSILFunction::visitDebugValueInst(DebugValueInst *i) {
  ...
  if (isa<SILUndef>(SILVal)) {
    ...
    if (!IsAddrVal &&
        !i->getDebugScope()->InlinedCallSite && VarInfo->getName() == "$error") {
      auto funcTy = CurSILFn->getLoweredFunctionType();
  ...
  bool IsAnonymous = false;
  VarInfo = VarInfo->withName(CurSILFn->getModule(), getVarName(i, IsAnonymous));
  DebugTypeInfo DbgTy;
  SILType SILTy;
  bool IsFragmentType = false;
  if (auto MaybeSILTy = VarInfo->getType()) {
    // If there is auxiliary type info, use it
    SILTy = *MaybeSILTy;
  } else {
    SILTy = SILVal->getType();
    if (VarInfo->getDIExpr())
      IsFragmentType = VarInfo->getDIExpr().hasFragment();
  }
  ...
  if (VarDecl *Decl = i->getDecl()) {
    DbgTy = DebugTypeInfo::getLocalVariable(Decl, RealTy, getTypeInfo(SILTy),
                                            IsFragmentType);
  } else if (!SILTy.hasArchetype() && !VarInfo->getName().empty()) {
...
void IRGenSILFunction::emitDebugInfoForAllocStack(AllocStackInst *i,
  ...
  bool IsAnonymous = false;
  VarInfo = VarInfo->withName(i->getModule(),
                              getVarName(i, IsAnonymous));
