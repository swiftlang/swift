...
/// Returns true if there exists an instruction after `I` without a
/// CleanupLocation. Terminator instructions are ignored.
static bool NonCleanupLocationExistsAfter(SILInstruction &I) {
  auto *BB = I.getParent();
  for (auto &Next : make_range(std::next(I.getIterator()), BB->end()))
    if (!isa<TermInst>(Next) && !Next.getLoc().is<CleanupLocation>())
      return true;
  return false;
}

bool IRGenSILFunction::setupDebugLocationFor(SILInstruction &I,
                                             bool OldKeepCurrentLocation) {
  auto DS = I.getDebugScope();
  SILLocation ILoc = I.getLoc();

  SILInstruction *PreviousInst = I.getPreviousInstruction();
  const bool PreviouslyInCleanupBlock =
      PreviousInst ? PreviousInst->getLoc().is<CleanupLocation>() : false;

  const bool InCleanupBlock = ILoc.is<CleanupLocation>();
  bool KeepCurrentLocation = OldKeepCurrentLocation;

  if (InCleanupBlock) {
    // Cleanup locations point to the decl of the value that is being destroyed
    // (for diagnostic generation). For the linetable, cleanups at the end of a
    // lexical scope should point to the cleanup location, which is the location
    // of the last instruction in the basic block.
    if (!PreviouslyInCleanupBlock)
      KeepCurrentLocation = NonCleanupLocationExistsAfter(I);
  ...
  } else if (PreviouslyInCleanupBlock) {
  ...
  else if (Builder.getCurrentDebugLocation() == llvm::DebugLoc())
    IGM.DebugInfo->setCurrentLoc(Builder, DS,
                                 RegularLocation::getAutoGeneratedLocation());

  return KeepCurrentLocation;
}
...
void IRGenSILFunction::visitSILBasicBlock(SILBasicBlock *BB) {
  ...
  for (auto &I : *BB) {
    if (IGM.DebugInfo) {
      KeepCurrentLocation = setupDebugLocationFor(I, KeepCurrentLocation);
      if (isa<TermInst>(&I))
