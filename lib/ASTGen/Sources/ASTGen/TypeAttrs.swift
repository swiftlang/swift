...
extension ASTGenVisitor {
  ...
  ///   ```
  ///   @convention(c)
  ///   @convention(c, cType: "size_t (*)(size_t)")
  ///   ```
  func generateConventionTypeAttr(attribute node: AttributeSyntax) -> BridgedConventionTypeAttr? {
    self.generateWithLabeledExprListArguments(attribute: node) { args in
      let nameAndLoc: (name: BridgedStringRef, loc: BridgedSourceLoc)? =  self.generateConsumingPlainIdentifierAttrOption(attribute: node, args: &args, example: "c") {
        ($0.rawText.bridged, self.generateSourceLoc($0))
      }
      ...
      if !args.isEmpty {
        ...
        cTypeName = self.generateConsumingSimpleStringLiteralAttrOption(attribute: node, args: &args, label: "cType")
      ...
      return .createParsed(
        ...
        name: ctx.allocateCopy(string: nameAndLoc.name),
  ...
  func generateOpaqueReturnTypeOfTypeAttr(attribute node: AttributeSyntax) -> BridgedOpaqueReturnTypeOfTypeAttr? {
    self.generateWithLabeledExprListArguments(attribute: node) { args in
      ...
      let mangledName = self.generateConsumingSimpleStringLiteralAttrOption(attribute: node, args: &args)
      guard let mangledName else {
        return nil
      }

      let indexLoc = self.generateSourceLoc(args.first?.expression)
      let index: Int? = self.generateConsumingAttrOption(attribute: node, args: &args, label: nil) { expr in
