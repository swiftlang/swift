  ...
  clang::Decl *readDeclRef() {
    ...
    if (refKind == 1) {
      ...
      return const_cast<clang::Decl*>(
        ClangLoader.lookupStableSerializationPath(swiftDecl));
    }
    ...
    if (refKind == 2) {
      ...
      ExternalPath path;
      path.M = cast<ModuleDecl>(MF.getDecl(DeclID(readUInt64())));
      uint64_t length = readUInt64();
      path.Path.reserve(length);
      for (uint64_t i = 0; i != length; ++i) {
        auto idns = ExternalPath::IdentifierNamespace(readUInt64());
        auto name = readSwiftIdentifier();
        path.Path.push_back({idns, name});
      }
      return const_cast<clang::Decl*>(
        ClangLoader.lookupStableSerializationPath(std::move(path)));
    }
