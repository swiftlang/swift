...
SILDeserializer::readDebugScopes(SILFunction *F,
  ...
  // llvm::errs() << "Reading debug scopes for function " << F->getName() <<
  // "\n";
  do {
    ...
    // llvm::errs() << "parent, inlined, row, column, Fname: " << (unsigned)
    // ParentID << " " << (unsigned) InlinedID << " " << Row << " " << Column <<
    // " " << FName << "\n" ;
    if (isFuncParent) {
      // llvm::errs() << "parent is func:  " << MF->getIdentifierText(ParentID)
      // << "\n";
    }
    ...
    // llvm::errs() << "Deserialized Loc: " << Loc.value() << " \n";
    if (isFuncParent) {
      // Parent = getFuncForReference(MF->getIdentifierText(ParentID),
      // getSILType(MF->getType(FuncType), (SILValueCategory)FuncCategory,F),
    ...
    // Scope->Loc.dump();
    // llvm::errs() << "Parent: " << Scope->Parent.getOpaqueValue() << "\n";
    // if (Scope->Parent.is<const SILDebugScope *>())
    // Scope->Parent.get<const SILDebugScope *>()->Loc.dump();
...
SILFunction *SILDeserializer::getFuncForReference(StringRef name,
  ...
  /*
  if (isDebug) {
    auto iter = DebugFuncTable->find(name);
    assert(iter != DebugFuncTable->end());
    auto maybeFn = readSILFunctionChecked(*iter, nullptr, name, true, true,
  true); assert(maybeFn); return maybeFn.get();
  }*/
  // Check to see if we have a function by this name already.
  SILFunction *fn = SILMod.lookUpFunction(name);
  if (fn)
  ...
  auto maybeFn =
      readSILFunctionChecked(*iter, nullptr, name,
                             /*declarationOnly*/ true, true, isDebug);
  if (!maybeFn) {
...
SILDeserializer::readSILFunctionChecked(DeclID FID, SILFunction *existingFn,
                                        ...
                                        bool errorIfEmptyBody, bool isDebug) {
  ...
  if (FID == 0)
    return nullptr;
  // if (isDebug) {
  // assert(FID <= FuncsDebug.size() && "invalid SILFunction ID");
  //} else {
  ...
  // if (isDebug) {
  // cacheEntryOpt = &FuncsDebug[FID-1];
  //} else {
  ...
  llvm::errs() << "Cache fully deserialized "
               << cacheEntry.isFullyDeserialized() << " FID: " << FID << "\n";
  if (cacheEntry.isFullyDeserialized() ||
      (cacheEntry.isDeserialized() && (declarationOnly || isDebug))) {
    auto fn = cacheEntry.get();
    // SILSerializationFunctionBuilder builder(SILMod);

    if (fn->isZombie() && !isDebug) {
      // TODO: We don't need to do anything here?
      // fn = builder.resurrectFunction(fn->getName());
    } else {
      return fn;
    }
  }
  ...
  unsigned rawLinkage, isTransparent, serializedKind, isThunk,
      ...
      isDistributed, isRuntimeAccessible, forceEnableLexicalLifetimes, isZombie;
  ...
  SILFunctionLayout::readRecord(
      ...
      LIST_VER_TUPLE_PIECES(available), isDynamic, isExactSelfClass,
      isDistributed, isRuntimeAccessible, forceEnableLexicalLifetimes, isZombie,
      funcTyID, replacedFunctionID, usedAdHocWitnessFunctionID, genericSigID,
  ...
  if (fn) {
    ...
    if (fn->isZombie()) {
      // deserialized earlier as zombie -> resurrect function
      fn = builder.resurrectFunction(fn->getName());
      assert(!fn->isZombie());
      fn->setLinkage(
  ...
  assert((!isEmptyFunction || !genericEnv || isDebug) &&
  ...
  bool isFullyDeserialized =
      (isEmptyFunction || !declarationOnly /*|| isDebug*/);
  ...
  llvm::errs() << "Fulyl des :" << cacheEntry.isFullyDeserialized()
               << " FID: " << FID << "\n";
...
bool SILDeserializer::readSILInstruction(SILFunction *Fn,
  ...
  case SILInstructionKind::DebugValueInst: {
    ...
    if (HaveDebugVar) {
      ...
      if (HasScope) {
        SmallVector<uint64_t, 64> scratch;
        unsigned kind = readNextRecord(scratch);
        Scope = readDebugScopes(Fn, scratch, Builder, kind);
      }
