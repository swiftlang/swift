...
#define BLOCK_RECORD(K, X) emitRecordID(K::X, #X, nameBuffer)
  ...
  BLOCK(RECORD_BLOCK);
  BLOCK_RECORD(record_block, FORMAT_METADATA);
...
void Serializer::emitFormatMetadata() {
  using namespace record_block;

  FormatMetadataLayout::emitRecord(
      Out, ScratchRecord, AbbrCodes[FormatMetadataLayout::Code],
      MODULE_SUMMARY_FORMAT_VERSION_MAJOR, MODULE_SUMMARY_FORMAT_VERSION_MINOR,
...
void Serializer::emitModuleSummary(const ModuleSummaryIndex &index) {
  ...
  registerRecordAbbr<FormatMetadataLayout>();
...
bool Deserializer::readFormatMetadata() {
  using namespace record_block;
  ...
  if (*recordID != FORMAT_METADATA)
    return true;

  unsigned majorVersion, minorVersion;

  FormatMetadataLayout::readRecord(Scratch, majorVersion, minorVersion);
  if (majorVersion != MODULE_SUMMARY_FORMAT_VERSION_MAJOR ||
      minorVersion != MODULE_SUMMARY_FORMAT_VERSION_MINOR) {
    return true;
  }
...
bool Deserializer::readModuleSummary() {
  ...
  while (!Cursor.AtEndOfStream()) {
    ...
    case FUNC_METADATA: {
      ...
      // Overwrite iff flags are true for merging function summaries of same
      // functions.
    ...
    case CALL_GRAPH_EDGE: {
      ...
      FunctionSummary::Call call(calleeGUID, BlobData.str(),
                                 callKind.getValue());
