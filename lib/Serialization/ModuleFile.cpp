...
static void readPosition(SourcePosition &Pos, const char *&Data,
                         StringRef StringData) {
  Pos.Offset = endian::readNext<uint32_t, little, unaligned>(Data);
  Pos.Line = endian::readNext<uint32_t, little, unaligned>(Data);
  Pos.Column = endian::readNext<uint32_t, little, unaligned>(Data);

  Pos.Directive.Offset = endian::readNext<uint32_t, little, unaligned>(Data);
  Pos.Directive.LineOffset = endian::readNext<int32_t, little, unaligned>(Data);
  Pos.Directive.Length = endian::readNext<uint32_t, little, unaligned>(Data);
  Pos.Directive.Name = readLocString(Data, StringData);
}

Optional<BasicDeclPositions>
ModuleFile::getBasicPositionsForDecl(const Decl *D) const {
  assert(D);
  // Keep these as assertions instead of early exits to ensure that we are not
  // doing extra work.  These cases should be handled by clients of this API.
  assert(!D->hasClangNode() &&
  ...
  if (DocRangesOffset) {
    ...
    for (uint32_t I = 0; I < NumLocs; ++I) {
      auto &Range = Result.DocRanges.emplace_back(SourcePosition(), 0);
      readPosition(Range.first, Data, Core->SourceLocsTextData);
      Range.second = endian::readNext<uint32_t, little, unaligned>(Data);
    }
  }

  readPosition(Result.Loc, Record, Core->SourceLocsTextData);
  readPosition(Result.StartLoc, Record, Core->SourceLocsTextData);
  readPosition(Result.EndLoc, Record, Core->SourceLocsTextData);
  return Result;
}
