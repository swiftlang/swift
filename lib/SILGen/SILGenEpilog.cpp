...
emitEpilog(SILLocation TopLevel, bool UsesCustomEpilog) {
  ...
  if (!maybeReturnValue) {
    ...
    // A @c/@_extern(c) function is emitted directly as its C entry point, so
    // bridge a native thick metatype result (e.g. AnyClass) to the ObjC 'Class'
    // representation the foreign result type expects. No-op for other functions.
    auto fnResultTy =
        F.getConventions().getSILResultType(getTypeExpansionContext());
    if (returnValue->getType() != fnResultTy) {
      auto fromMeta =
          dyn_cast<AnyMetatypeType>(returnValue->getType().getASTType());
      auto toMeta = dyn_cast<AnyMetatypeType>(fnResultTy.getASTType());
      if (fromMeta && toMeta && fromMeta->hasRepresentation() &&
          toMeta->hasRepresentation() &&
          fromMeta->getRepresentation() == MetatypeRepresentation::Thick &&
          toMeta->getRepresentation() == MetatypeRepresentation::ObjC) {
        returnValue = B.emitThickToObjCMetatype(
            RegularLocation::getAutoGeneratedLocation(), returnValue,
            fnResultTy);
      }
    }
