...
emitBridgeNativeToObjectiveC(SILGenFunction &SGF, SILLocation loc,
  ...
  // protocols.
  SILFunctionConventions witnessConv(
      witnessFnTy.castTo<SILFunctionType>(),
...
static void buildFuncToBlockInvokeBody(SILGenFunction &SGF,
  ...
  SILAddressConventions silConv =
      SILAddressConventions::forFunction(SGF.F);
  SILFunctionConventions blockConv(blockTy, silConv);
  SILFunctionConventions funcConv(funcTy, silConv);
...
#include "swift/SIL/BridgedTypes.def"
  ...
  if (auto bridgeAnything =
    ...
    // bridgeAnything always takes an indirect argument as @in. Lacking the SIL
    // type here, consult the function's lowered-addresses state.
    if (v.getType().isObject() && SGF.useLoweredAddresses()) {
...
static void buildBlockToFuncThunkBody(SILGenFunction &SGF,
  ...
  SILFunctionConventions fnConv(
      funcTy, SILAddressConventions::forFunction(SGF.F));
...
emitObjCThunkArguments(SILGenFunction &SGF, SILLocation loc, SILDeclRef thunk,
  ...
  SILFunctionConventions swiftConv(
      swiftFnTy, SILAddressConventions::forFunction(SGF.F));
...
void SILGenFunction::emitNativeToForeignThunk(SILDeclRef thunk) {
  ...
  auto substTy = nativeInfo.SILFnType->substGenericArgs(
      SGM.M, subs, getTypeExpansionContext());
  SILFunctionConventions substConv(substTy,
  ...
  SILFunctionConventions objcConv(CanSILFunctionType(objcFnTy), silConv);
...
/// Strips the options describing arguments that the C calling convention passes
/// implicitly from every parameter of \p fnTy, producing the type of a plain
/// function pointer that can hold it.
static CanSILFunctionType
getTypeWithoutForeignImplicitArguments(SILModule &M, CanSILFunctionType fnTy) {
  SmallVector<SILParameterInfo, 4> params;
  ...
  return SILFunctionType::get(
      fnTy->getInvocationGenericSignature(), fnTy->getExtInfo(),
      fnTy->getCoroutineKind(), fnTy->getCalleeConvention(), params,
      fnTy->getYields(), fnTy->getResults(), fnTy->getOptionalErrorResult(),
      fnTy->getPatternSubstitutions(), fnTy->getInvocationSubstitutions(),
      M.getASTContext(), fnTy->getWitnessMethodConformanceOrInvalid());
}

SILFunction *
SILGenModule::getOrCreateForeignImplicitArgumentThunk(SILDeclRef foreignRef,
                                                      SILLocation loc) {
  auto foreignTy = Types.getConstantFunctionType(
      TypeExpansionContext::minimal(), foreignRef);
  auto thunkTy = getTypeWithoutForeignImplicitArguments(M, foreignTy);
  assert(thunkTy != foreignTy && "no implicit arguments to supply?");

  Mangle::ASTMangler mangler(getASTContext());
  auto name = mangler.mangleEntity(
      foreignRef.getDecl(),
      Mangle::ASTMangler::SymbolKind::ForeignImplicitArgumentThunk);

  SILGenFunctionBuilder builder(*this);
  auto *thunk = builder.getOrCreateSharedFunction(
      loc, name, thunkTy, ActorIsolation::forUnspecified(), IsBare,
      IsNotTransparent, IsSerialized, ProfileCounter(), IsThunk, IsNotDynamic,
      IsNotDistributed, IsNotRuntimeAccessible);

  if (thunk->empty()) {
    auto *dc = foreignRef.getDecl()->getInnermostDeclContext();
    SILGenFunction(*this, *thunk, dc)
        .emitForeignImplicitArgumentThunk(foreignRef);
  }

  return thunk;
}

void SILGenFunction::emitForeignImplicitArgumentThunk(SILDeclRef foreignRef) {
  auto thunkTy = F.getLoweredFunctionType();
  SILLocation loc = F.getLocation();
  loc.markAutoGenerated();

  // The thunk differs from its callee only in parameter flags, so every
  // argument is forwarded unchanged and the result needs no adjustment. The
  // implicit arguments are not represented in SIL at all; IRGen derives them
  // from the callee's parameter flags.
  SILFunctionConventions thunkConv(thunkTy,
                                   SILAddressConventions::forFunction(F));
  SmallVector<SILValue, 4> args;
  for (auto resultTy :
       thunkConv.getIndirectSILResultTypes(getTypeExpansionContext()))
    args.push_back(F.begin()->createFunctionArgument(resultTy));
  for (auto param : thunkTy->getParameters()) {
    args.push_back(F.begin()->createFunctionArgument(
        thunkConv.getSILType(param, getTypeExpansionContext())));
  }

  auto callee = emitGlobalFunctionRef(loc, foreignRef);
  auto result = B.createApply(loc, callee, SubstitutionMap(), args);
  B.createReturn(loc, result);
}
...
void SILGenFunction::emitForeignToNativeThunk(SILDeclRef thunk) {
  ...
  SILValue indirectResult;
  SILFunctionConventions nativeConv(
      nativeFnTy, SILAddressConventions::forFunction(F));
