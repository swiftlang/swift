...
void SILGenFunction::emitEpilog(SILLocation TopLevel, bool AutoGen) {
  ...
  TopLevel = returnLoc ? *returnLoc : TopLevel;

  // Return () if no return value was given.
  if (!returnValue)
    returnValue = emitEmptyTuple(TopLevel);
  
  B.createReturn(TopLevel, returnValue)->setDebugScope(F.getDebugScope());
}
...
void SILGenFunction::emitDestructor(ClassDecl *cd, DestructorDecl *dd) {
  ...
  // For implicit destructor, use class destructor location.
  SILLocation EpilogLoc = dd ? (SILLocation)RegularLocation(dd) :
                               (SILLocation)RegularLocation(/* AutoGen */true);
  if (!emitEpilogBB(EpilogLoc).first)
    return;
...
static void emitImplicitValueConstructor(SILGenFunction &gen,
                                         ConstructorDecl *ctor) {
  // FIXME: all locations here should be marked as auto generated.
  ...
  for (RValue &rv : elements) {
    eltValues.push_back(std::move(rv).forwardAsSingleValue(gen, ctor));
  }
