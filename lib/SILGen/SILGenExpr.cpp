...
void SILGenFunction::emitIVarInitializer(SILDeclRef ivarInitializer) {
  // Always emit physical property accesses in ivar initializers.
  AlwaysDirectStoredPropertyAccess = true;
  ...
  // It's not a destructor, but this is handy to avoid having to build
  // patterns or retain/release 'self'.
  emitDestructorProlog(cd, cd->getDestructor());  
...
void SILGenFunction::emitIVarDestroyer(SILDeclRef ivarDestroyer) {
  // Always emit physical property accesses in ivar destroyers.
  AlwaysDirectStoredPropertyAccess = true;
  ...
  SILValue selfValue = emitDestructorProlog(cd, cd->getDestructor());  
