...
static
SILFunction *getOrCreateKeyPathPayloadCaseGetter(SILGenModule &SGM,
  ...
  auto signature = [&]() {
    CanType loweredBaseTy, loweredCaseTy;
    ...
    loweredCaseTy = SGM.Types.getLoweredRValueType(
        TypeExpansionContext::minimal(), opaque, resultType);
    ...
    SILResultInfo result(loweredCaseTy, ResultConvention::Indirect);
  ...
  auto name = Mangle::ASTMangler()
    .mangleKeyPathGetterThunkHelper(enumElement, genericSig, baseType,
                                    subs, expansion);
  ...
  if (genericEnv) {
    ...
    resultType = genericEnv->mapTypeIntoContext(resultType)->getCanonicalType();
  ...
  auto baseOrigValue =
    ManagedValue::forBorrowedRValue(baseArg).copy(subSGF, loc);
  auto baseSubstValue = subSGF.emitOrigToSubstValue(loc, baseOrigValue,
                                             AbstractionPattern::getOpaque(),
                                             baseType);

  SwitchEnumBuilder switchBuilder(subSGF.B, loc, baseSubstValue);

  auto returnBlock = subSGF.createBasicBlock();
  auto caseBlock = subSGF.createBasicBlock();
  auto defaultBlock = subSGF.createBasicBlock();

  auto caseHandler = [&](ManagedValue mv, SwitchCaseFullExpr &&expr) {
    auto &tl = subSGF.getTypeLowering(resultArg->getType());

    // If we don't have any associated values, then just emit an empty tuple
    // into the some value of the optional.
    if (!enumElement->hasAssociatedValues()) {
      auto tupleRValue = subSGF.emitEmptyTupleRValue(loc, SGFContext());
      subSGF.emitInjectOptionalValueInto(loc, {loc, std::move(tupleRValue)},
                                         resultArg, tl);
    ...
    auto eltTy = baseSubstValue.getType().getEnumElementType(enumElement,
        subSGF.SGM.M, subSGF.getTypeExpansionContext());
    auto &eltTL = subSGF.getTypeLowering(eltTy);

    // If our enum value is an address, then take the payload out of it.
    if (mv.getType().isAddress()) {
      mv = subSGF.B.createUncheckedTakeEnumDataAddr(loc, mv, enumElement,
                                                    mv.getType());

      if (eltTL.isLoadable()) {
        mv = subSGF.B.createLoadTake(loc, mv);
      }
    }

    if (enumElement->isIndirect() || enumElement->getParentEnum()->isIndirect()) {
      auto boxedValue = subSGF.B.createProjectBox(loc, mv, 0);
      auto &boxedTL = subSGF.getTypeLowering(boxedValue.getType());

      if (boxedTL.isLoadable() || !subSGF.silConv.useLoweredAddresses()) {
        UnenforcedAccess access;
        auto accessAddress = access.beginAccess(subSGF, loc,
                                                boxedValue.getValue(),
                                                SILAccessKind::Read);
        auto mvAccessAddress = ManagedValue::forUnmanaged(accessAddress);

        {
          Scope loadScope(subSGF, loc);
          auto borrowedVal = subSGF.B.createLoadBorrow(loc, mvAccessAddress);
          mv = loadScope.popPreservingValue(borrowedVal.copyUnmanaged(subSGF,
                                                                      loc));
        }

        access.endAccess(subSGF);
      } else {
        // If we do not have a loadable value, just do a copy of the
        // boxedValue.
        mv = boxedValue.copyUnmanaged(subSGF, loc);
      }
    }

    // Reabstract to the substituted type, if needed.
    if (mv.getType().getAddressType() != resultArg->getType()) {
      auto substEltTy = baseSubstValue.getType().getASTType()
          ->getTypeOfMember(subSGF.SGM.M.getSwiftModule(), enumElement,
                            enumElement->getArgumentInterfaceType())
          ->getCanonicalType();

      mv = subSGF.emitSubstToOrigValue(loc, mv, AbstractionPattern::getOpaque(),
                                       substEltTy);
    }

    auto rvalue = RValue(subSGF, loc, mv.getType().getASTType(), mv);
    subSGF.emitInjectOptionalValueInto(loc, {loc, std::move(rvalue)}, resultArg,
                                       tl);

    expr.exitAndBranch(loc);
  };

  switchBuilder.addCase(enumElement, caseBlock, returnBlock, caseHandler);

  auto defaultHandler = [&subSGF, &loc, &resultArg, &resultType](ManagedValue mv,
                                        SwitchCaseFullExpr &&expr) {
    auto &tl = subSGF.getTypeLowering(resultType);

    subSGF.emitInjectOptionalNothingInto(loc, resultArg, tl);
    expr.exitAndBranch(loc);
  };

  switchBuilder.addDefaultCase(defaultBlock, returnBlock, defaultHandler);
  std::move(switchBuilder).emit();

  subSGF.B.setInsertionPoint(returnBlock);
...
RValue RValueEmitter::visitKeyPathExpr(KeyPathExpr *E, SGFContext C) {
  ...
  for (auto &component : E->getComponents()) {
    ...
    case KeyPathExpr::Component::Kind::EnumCase: {
      ...
      auto componentTy = component.getComponentType()->getCanonicalType();

      if (needsGenericContext) {
        componentTy = componentTy->mapTypeOutOfContext()->getCanonicalType();
      }

      // If our deployment target is less than Swift 5.*, emit a computed
      // get only keypath component that will achieve the same thing for older
      // stdlib runtimes.
      if (component.getEnumCaseRequiresComputedGetter()) {
        auto thunk = getOrCreateKeyPathPayloadCaseGetter(SGF.SGM, enumElement,
                                      component.getDeclRef().getSubstitutions(),
                                      SGF.F.getGenericEnvironment(),
                                      SGF.F.getResilienceExpansion(),
                                      baseTy,
        ...
        auto id = KeyPathPatternComponent::ComputedPropertyId(enumElement);
        loweredComponents.push_back(
            KeyPathPatternComponent::forComputedEnumCase(id, thunk,
                                                         componentTy));
      ...
      // Otherwise, use the new payload case pattern representation.

      auto loweredComponent =
          KeyPathPatternComponent::forEnumCase(enumElement, componentTy);
