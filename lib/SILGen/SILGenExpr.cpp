...
void SILGenFunction::emitEpilog(SILLocation TopLevel, bool AutoGen) {
  ...
  SILLocation Loc = AutoGen ?
                      AutoGeneratedLocation::getAutoGeneratedLoc(TopLevel) :
                      TopLevel;
  ...
  Loc = returnLoc ? *returnLoc : Loc;

  // Return () if no return value was given.
  if (!returnValue)
    returnValue = emitEmptyTuple(Loc);
  
  B.createReturn(Loc, returnValue)->setDebugScope(F.getDebugScope());
}
...
void SILGenFunction::emitDestructor(ClassDecl *cd, DestructorDecl *dd) {
  ...
  SILLocation EpilogLoc = dd ? (SILLocation)RegularLocation(dd) :
                               (SILLocation)AutoGeneratedLocation(cd);
