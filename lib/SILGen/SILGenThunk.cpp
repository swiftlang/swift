...
SILLinkage getDispatchThunkLinkage(SILDeclRef constant,
  ...
  case AccessLevel::FilePrivate:
    llvm_unreachable("Should not be emitting a dispatch thunk for a "
                     "private method");
...
SILFunction *SILGenModule::getDispatchThunk(SILDeclRef constant,
  ...
  auto linkage = getDispatchThunkLinkage(constant, forDefinition, M);
...
static bool shouldUseDispatchThunk(SILDeclRef constant,
  ...
  auto *dc = classDecl->getModuleScopeContext();

  // Resilient classes in other resilience domains use dispatch thunks.
  if (!classDecl->hasFixedLayout(M.getSwiftModule(),
  ...
  // resilient, public method's thunks are not going to be public.
...
SILValue SILGenFunction::emitClassMethodRef(SILLocation loc,
  ...
  if (shouldUseDispatchThunk(constant, &F, SGM.M)) {
    auto *thunkFn = SGM.getDispatchThunk(constant, NotForDefinition);
    return B.createFunctionRef(loc, thunkFn);
  }
