...
//  // FIXME(distributed): enable this again; Assertion failed: (!hasValidInsertionPoint()), function emitBlock, file SILBuilder.h, line 334.
...
void SILGenFunction::injectDistributedActorDestructorLifecycleCall(
  ...
  if (cd->lookupDirect(ctx.Id_id).size() > 0)
    return;
