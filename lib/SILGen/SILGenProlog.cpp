...
void SILGenFunction::emitProlog(CaptureInfo captureInfo,
  ...
  if (ExpectedExecutor) {
    if (F.isAsync()) {
      ...
      auto loc = RegularLocation::getAutoGeneratedLocation(F.getLocation());
      emitPreconditionCheckExpectedExecutor(loc,
                            ExpectedExecutor.emitGetCurrentExecutor(loc, B));
    }
  }
}
...
SILValue SILGenFunction::emitGetCurrentExecutor(SILLocation loc) {
  return ExpectedExecutor.emitGetCurrentExecutor(loc, B);
}

SILValue ExecutorValue::emitGetCurrentExecutor(SILLocation loc, SILGenBuilder B) {
...
void ExecutorValue::emitHopToExecutor(SILLocation loc, SILGenBuilder B, bool isMandatory) {
  B.createHopToExecutor(loc, emitGetCurrentExecutor(loc, B), isMandatory);
}
