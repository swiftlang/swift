...
void SILGenFunction::emitProlog(
  ...
  // In nonisolated async functions, the executor is either the tasks'
  // preferred executor, or the default generic executor.
  if (!ExpectedExecutor && F.isAsync() && !unsafelyInheritsExecutor()) {
    ExpectedExecutor = emitGetNonisolatedFuncExecutor(
...
SILValue SILGenFunction::emitGetNonisolatedFuncExecutor(SILLocation loc) {
  auto ty = SILType::getOptionalType(
      SILType::getPrimitiveObjectType(
          getASTContext().TheExecutorType));
  ...
  return B.createOptionalSome(loc, executor, ty);
}
