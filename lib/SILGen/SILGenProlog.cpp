...
void SILGenFunction::emitProlog(CaptureInfo captureInfo,
  ...
  if (ExpectedExecutor) {
    if (F.isAsync()) {
      ...
      ExpectedExecutor.emitHopToExecutor(
      ...
      auto loc = RegularLocation::getAutoGeneratedLocation(F.getLocation());
      emitPreconditionCheckExpectedExecutor(loc, emitGetCurrentExecutor(loc));
    }
  }
}
...
void SILGenFunction::emitPrologGlobalActorHop(SILLocation loc,
  ...
  ExpectedExecutor.emitHopToExecutor(
                      RegularLocation::getDebugOnlyLocation(loc, getModule()),
                      B, /*mandatory*/ false);
}
...
void ExecutorBreadcrumb::emit(SILGenFunction &SGF, SILLocation loc) {
  if (mustReturnToExecutor) {
    ...
    if (auto executor = SGF.ExpectedExecutor)
      executor.emitHopToExecutor(
          RegularLocation::getDebugOnlyLocation(loc, SGF.getModule()), SGF.B,
          /*mandatory*/ false);
  }
}

SILValue SILGenFunction::emitGetCurrentExecutor(SILLocation loc) {
  return ExpectedExecutor.emitLoadExecutor(loc, B);
}

SILValue ExecutorValue::emitLoadExecutor(SILLocation loc, SILGenBuilder &B) {
  assert(rawValue && "unspecified executor?");

  SILValue result = rawValue;

  // Load the executor out of the box on-demand.
  if (isa<ProjectBoxInst>(result)) {
    CleanupLocation cleanupLoc(loc);
    auto managedExe = ManagedValue::forUnmanaged(result);
    result = B.createLoadBorrow(cleanupLoc, managedExe).getValue();
  }

  // TODO: assert that it returns a value of type Builtin.Executor
  return result;
}

void ExecutorValue::emitHopToExecutor(SILLocation loc, SILGenBuilder &B, bool isMandatory) {
  B.createHopToExecutor(loc, emitLoadExecutor(loc, B), isMandatory);
}
