...
void SILGenFunction::emitPropertyWrappedFieldInitAccessor(SILDeclRef function, 
                                                          Expr* value, bool EmitProfilerIncrement) {
  auto vd = cast<VarDecl>(function.getDecl());
  auto *declContext = function.getDecl()->getInnermostDeclContext();
  ...
  auto createArgument = [&](VarDecl *property, SILType type,
    ...
    auto *arg = ParamDecl::createImplicit(
        getASTContext(), property->getBaseIdentifier(),
        property->getBaseIdentifier(), property->getInterfaceType(), declContext,
        ParamSpecifier::InOut);
  ...
  auto backingStorageTy =
        getSILTypeInContext(loweredFuncDeclTy->getResults()[0], loweredFuncDeclTy);
  createArgument(backingStorage, backingStorageTy, /*markUninitialized=*/true);

  // Emit `newValue` argument
  ParameterList *params = nullptr;
  auto newValueParam = new (ctx) ParamDecl(SourceLoc(), SourceLoc(),
                                     ctx.getIdentifier("$input_value"),
                                     SourceLoc(),
                                     ctx.getIdentifier("$input_value"),
                                     declContext);
  newValueParam->setSpecifier(ParamSpecifier::LegacyOwned);
  newValueParam->setImplicit();
  newValueParam->setInterfaceType(vd->getPropertyWrapperInitValueInterfaceType());
  params = ParameterList::create(ctx, SourceLoc(), {newValueParam}, SourceLoc());
