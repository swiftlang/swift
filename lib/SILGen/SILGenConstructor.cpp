...
void SILGenFunction::emitClassConstructorInitializer(ConstructorDecl *ctor) {
  ...
  if (isDesignatedDistActorInit) {
    // If we're using a box for self, reload the value at the end of the init
    // method.
    ManagedValue selfValue = selfArg;
//    if (NeedsBoxForSelf) {
//      auto storedSelf =
//          ManagedValue::forBorrowedAddressRValue(VarLocs[selfDecl].value);
//      selfValue =
//          B.createLoadCopy(VarLocs[selfDecl].value.getLoc(), storedSelf);
//      fprintf(stderr, "[%s:%d](%s) the copy \n", __FILE_NAME__, __LINE__, __FUNCTION__);
//      selfValue.dump();
//      SelfCleanupSave.pushCleanupState(selfValue.getCleanup(),
//                                       CleanupState::Dormant);
//    } else {
//      selfValue = selfArg;
//    }
    emitDistributedActorImplicitPropertyInits(ctor, selfValue);
  }
...
//      selfValue = selfArg;
//    }
    emitDistributedActorReady(loc, ctor, selfValue);
  }
