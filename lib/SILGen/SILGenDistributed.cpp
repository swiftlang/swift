...
void SILGenFunction::emitDistributedActorImplicitPropertyInits(
  ...
  if (ctor->isImplicit()) {
    ...
    if (!actorSystem && ctor->isObjC() && F.getArguments().size() == 1) {
      // FIXME: it seems we're sometimes trying to synthesize an init() for objective-c,
      //   but that can't work for a distributed actor as we need a system parameter.
      return;
    }
