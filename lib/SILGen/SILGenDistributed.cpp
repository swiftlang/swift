...
emitDistributedActorTransportInit(SILGenFunction &SGF, VarDecl *selfDecl, ConstructorDecl *ctor,
  ...
  // FIXME: something like that? SGF.emitExprInto(Init, initialization.get(), SILLocation(PBD));

  SGF.emitExprInto(var->getParentInitializer(), initialization.get());
}
...
/// \endverbatim
static InitializationPtr
emitDistributedActorIdentityInit(SILGenFunction &SGF, VarDecl *selfDecl, ConstructorDecl *ctor,
                                 Pattern *pattern, VarDecl *var) {
  auto &C = selfDecl->getASTContext();
  auto &B = SGF.B;
  auto &F = SGF.F;
...
//  ManagedValue transportArgManaged = ManagedValue::forUnmanaged(transportArgValue);
  ...
  for (auto member : classDecl->getMembers()) {
    ...
    if (var->getName() == C.Id_actorTransport &&
      ...
      auto transportInit = emitDistributedActorTransportInit(*this, selfDecl, ctor, pattern, var);
      ...
      auto identityInit = emitDistributedActorIdentityInit(*this, selfDecl, ctor, pattern, var);
    }
