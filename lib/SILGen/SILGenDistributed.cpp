...
static void createDistributedActorFactory_resolve(
  ...
  Optional<SILValue> temporaryIdentityBuffer;
  if (!identityValue->getType().isAddress()) {
    auto buf = B.createAllocStack(loc, identityValue->getType(), None);
    identityValue = B.emitCopyValueOperation(loc, identityValue);
    B.emitStoreValueOperation(
        loc, identityValue, buf, StoreOwnershipQualifier::Init);
    temporaryIdentityBuffer = SILValue(buf);
  }

  // // ---- actually call transport.resolve(id, as: Self.self)
  auto apply = emitActorTransportWitnessCall(
      B, loc, C.Id_resolve, transportValue, SGF.getLoweredType(selfTy),
      /*args=*/{
        temporaryIdentityBuffer ? *temporaryIdentityBuffer : identityValue,
        selfMetatypeValue
      },
  ...
  auto emitCleanup = [&](llvm::function_ref<void(SILBuilder &builder)> fn) {
    {
      SILBuilderWithScope normalBuilder(normalBB, /*inheritScopeFrom=*/apply);
      fn(normalBuilder);
    }
    {
      SILBuilderWithScope errorBuilder(errorBB, /*inheritScopeFrom=*/apply);
      fn(errorBuilder);
    }
  };

  // If we had to create a buffer to pass the call, deallocate it
  if (temporaryIdentityBuffer) {
    emitCleanup([&](SILBuilder & builder) {
      auto value = builder.emitLoadValueOperation(
          loc, *temporaryIdentityBuffer, LoadOwnershipQualifier::Take);
      builder.emitDestroyValueOperation(loc, value);
      builder.createDeallocStack(loc, *temporaryIdentityBuffer);
    });
  }
}
