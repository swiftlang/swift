...
void SILGenFunction::emitPatternBinding(PatternBindingDecl *PBD,
  ...
  if (PBD->isAsyncLet()) {
    ...
    // TODO: should we instead stack alloc the AsyncLet and pass it to start to initialize?
    //       today we have the start allocate it using task-local allocation.
    ...
    // TODO: force in typesystem that we always await; then end async let does not have to be async
    // the local let variable is actually owning the result
    // - but since throwing we can't know; maybe we didn't await on a thing yet
...
//    enterDestroyCleanup(alet);
//
//    // Push a cleanup that will cancel the child task at the end of the scope.
//    enterCancelAsyncTaskCleanup(alet); // TODO: this is "went out scope" rather than just a cancel
...
public:
  ...
  void dump(SILGenFunction &) const override {
    ...
    llvm::errs() << "AsyncLetCleanup\n"
                 << "AsyncLet:" << alet << "\n";
                 // << "Task:" << alet.Task << "\n"; // TODO: we could/should print the task here too?
#endif
  }
};
} // end anonymous namespace
