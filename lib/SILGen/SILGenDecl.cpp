...
substSelfTypeIntoProtocolRequirementType(SILModule &M,
  ...
  if (auto *outerSig = conformance->getGenericSignature()) {
    ...
    allReqts.append(outerSig->getRequirements().begin(),
                    outerSig->getRequirements().end());
  }
  ...
  allParams.append(unappliedParams.begin(), unappliedParams.end());
  ...
  for (auto &reqt : reqtTy->getRequirements()) {
    ...
    case RequirementKind::SameType: {
      // We should never have same-type constraints between associated types,
      // because there's no way to write that in the language.
      assert(rootedInSelf(reqt.getFirstType()) ||
  ...
  if (!allParams.empty() && !allReqts.empty()) {
    auto *sig = GenericSignature::get(allParams, allReqts);
