...
void SILGenFunction::emitObjCDestructor(SILDeclRef dtor, DestructorDecl *dd) {
  // Always emit physical property accesses in destructors.
  AlwaysDirectStoredPropertyAccess = true;

  auto cd = cast<ClassDecl>(dtor.getDecl());
  ...
  SILDeclRef superclassDtor(superclass, SILDeclRef::Kind::Destroyer,
