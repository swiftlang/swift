...
void SILGenFunction::emitObjCMethodThunk(SILDeclRef thunk) {
  ...
  auto swiftResultTy
    = SGM.getConstantType(native).getFunctionTypeInfo(SGM.M)->getResult();
  auto objcResultTy
    = SGM.getConstantType(thunk).getFunctionTypeInfo(SGM.M)->getResult();
  ...
  SILValue result = B.createApply(loc, nativeFn, nativeFn.getType(),
                                  ...
                                  thunk.isTransparent());
  emitObjCReturnValue(*this, loc, result, objcResultTy.getSILType(), ownership);
}
...
void SILGenFunction::emitObjCPropertyGetter(SILDeclRef getter) {
  ...
  SILType swiftResultTy
    = SGM.getConstantType(native).getFunctionTypeInfo(SGM.M)->getResult().getSILType();

  auto objcFnTy = SGM.getConstantType(getter).getFunctionTypeInfo(SGM.M);
  SILType objcResultTy = objcFnTy->getResult().getSILType();
...
void SILGenFunction::emitObjCSubscriptGetter(SILDeclRef getter) {
  ...
  auto swiftResultTy
    = SGM.getConstantType(native).getFunctionTypeInfo(SGM.M)->getResult();
  auto objcResultTy
    = SGM.getConstantType(getter).getFunctionTypeInfo(SGM.M)->getResult();
