...
/// Lifetimes feature.
static bool findClosureLifetimes(Decl *decl, bool onlyAttributes) {
  // Search for any Decl that uses a type with lifetime dependencies, possibly
  // restricting this to explicit @_lifetime attributes.
  ...
  auto const hasLifetimeDependencies = onlyAttributes ? [](Type type) -> bool {
    if (auto *aft = type->getAs<AnyFunctionType>()) {
      return aft->hasLifetimeDependencies() &&
             llvm::any_of(aft->getLifetimeDependencies(),
                          [](const LifetimeDependenceInfo &info) {
                            return info.getEntry() != nullptr;
                          });
    }
    return false;
  }
...
static bool usesFeatureClosureLifetimes(Decl *decl) {
  ...
  if (findClosureLifetimes(decl, /*onlyAttributes*/ true)) {
    return true;
  }
