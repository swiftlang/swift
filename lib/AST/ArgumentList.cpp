...
/// Whether the given argument expression is either a trailing closure, or is
/// a variadic expansion containing a trailing closure.
static bool isOrContainsTrailingClosure(const Expr *argExpr,
                                        SourceLoc rparenLoc) {
  // If the argument has no source info, we can't count it as a trailing
  // closure.
  if (argExpr->getSourceRange().isInvalid())
    return false;
...
computeTrailingClosureInfo(ArrayRef<Argument> args, SourceLoc rparenLoc,
  ...
  auto iter = llvm::find_if(args, [&](const auto &arg) {
    return isOrContainsTrailingClosure(arg.getExpr(), rparenLoc);
  });
  if (iter == args.end()) {
  ...
  if (auto *array = getVariadicArrayFrom(args[argIdx].getExpr())) {
    auto numTrailing = llvm::count_if(array->getElements(), [&](auto *expr) {
      return isOrContainsTrailingClosure(expr, rparenLoc);
    });
...
ArgumentList *ArgumentList::create(ASTContext &ctx, SourceLoc lParenLoc,
  ...
  auto bytesToAllocate = totalSizeToAlloc<Expr *, Identifier, SourceLoc>(
      exprs.size(), labels.size(), labelLocs.size());
  auto *mem = ctx.Allocate(bytesToAllocate, alignof(ArgumentList),
  ...
  auto *argList = ::new (mem) ArgumentList(
      lParenLoc, rParenLoc, args.size(), trailingClosureInfo.ArgIndex,
      trailingClosureInfo.HasMultiple, isImplicit, hasLabels, hasLabelLocs);
...
SourceRange ArgumentList::getSourceRange() const {
  ...
  // If we have trailing closures, we may need to extend the end loc.
  auto end = RParenLoc;
  if (hasAnyTrailingClosures() || end.isInvalid()) {
    for (auto arg : llvm::reverse(*this)) {
      if (!isOrContainsTrailingClosure(arg.getExpr(), RParenLoc))
        continue;
...
OriginalArguments ArgumentList::getOriginalArguments() const {
  // We need to sort out the trailing closures seperately to handle cases like:
  //
  ...
  auto addArg = [&](Argument arg) {
    if (isOrContainsTrailingClosure(arg.getExpr(), getRParenLoc())) {
  ...
  return OriginalArguments(this, std::move(newArgs));
}

OriginalArguments::OriginalArguments(const ArgumentList *fromArgList,
                                     Storage args)
    : Args(std::move(args)) {
  auto trailingInfo = computeTrailingClosureInfo(
      Args, fromArgList->getRParenLoc(), fromArgList->isImplicit());
  FirstTrailingClosureIndex = trailingInfo.ArgIndex;
  assert(hasMultipleTrailingClosures() == trailingInfo.HasMultiple);
}
