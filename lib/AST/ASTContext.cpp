...
void ASTContext::setModuleAliases(const llvm::StringMap<StringRef> &aliasMap) {
  ...
  for (auto k: aliasMap.keys()) {
    ...
    if (!v.empty()) {
      ...
      // key is a module alias, val is its corresponding binary name
      ModuleAliasMap[key] = std::make_pair(val, true);
      // add an entry with a syntax name as key for an easier lookup later
...
Identifier ASTContext::getBinaryModuleName(Identifier key, ModuleAliasLookupOption option) const {
  ...
  // With the binaryName option, look up the binary name by treating
  // the given key as a syntax name; if the key's not a syntax name,
  // return the key itself since that's the binary name.
  if (option == ModuleAliasLookupOption::binaryName) {
     return value.second ? value.first : key;
  }

  // With binaryNameWithSyntaxNameKey or syntaxNameWithBinaryNameKey option, only return the
  // value if the given key matches the description (whether it's an alias or binary name)
  // by looking up the value.second (true if keyed by a syntax name). If not matched,
  // return an empty Identifier.
  if ((option == ModuleAliasLookupOption::binaryNameWithSyntaxNameKey && !value.second) ||
      (option == ModuleAliasLookupOption::syntaxNameWithBinaryNameKey && value.second))
      return Identifier();
...
ModuleDecl *ASTContext::getLoadedModule(Identifier ModuleName) const {
  ...
  auto realName = getBinaryModuleName(ModuleName);
