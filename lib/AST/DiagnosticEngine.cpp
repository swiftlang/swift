...
bool DiagnosticState::isIgnoredDiagnosticGroupTree(DiagGroupID id) const {
  ...
  getDiagGroupInfoByID(id).traverseDepthFirst([&](auto group) {
    anyEnabled |=
         warningGroupBehaviorMap.at(group.id).getBehavior() !=
  ...
  for (const auto &rule : rules) {
    if (rule.hasGroup()) {
      ...
      for (const auto &groupInfo : diagnosticGroupsInfo) {
        ...
        if (warningGroupBehaviorMap.count(groupInfo.id) &&
            warningGroupBehaviorMap.at(groupInfo.id).getBehavior() == WarningGroupBehavior::Ignored)
          continue;
...
DiagnosticState::determineUserControlledWarningBehavior(
  ...
  if (warningGroupBehaviorMap.count(diag.getGroupID())) {
    switch (warningGroupBehaviorMap.at(diag.getGroupID()).getBehavior()) {
