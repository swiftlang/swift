...
// CxxIterableIterator or an extension of one of these.
static bool isCxxIterableOrIterator(Decl *decl) {
  ...
  if (auto *sd = dyn_cast<StructDecl>(decl))
    return sd->getNameStr() == "CxxIterableIterator";
  return false;
}
...
void swift::printWithCompatibilityFeatureChecks(ASTPrinter &printer,
  ...
  // CxxIterable and CxxIterableIterator, defined in the Cxx overlay,
  // conform to Iterable and IterableIteratorProtocol. When a newer
