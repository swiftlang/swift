...
public:
  ...
  LifetimeDependenceChecker(FunctionTypeRepr *funcRepr,
      ...
      : afd(nullptr), ctx(funcType->getASTContext()),
        ...
        depBuilder(resultIndex, /*shouldInferDependenceOnClosureContext=*/true),
  ...
  checkEnumElementDecl(EnumElementDecl *eed) {
    ...
    LifetimeDependenceBuilder depBuilder(resultIndex, /*shouldInferDependenceOnClosureContext=*/false);
...
bool LifetimeDependenceInfo::satisfies(
    const LifetimeDependenceInfo &other) const {
  return this->hasImmortalSpecifier() == other.hasImmortalSpecifier() &&
         // If the other dependence includes the closure context, so must this.
         (not other.dependsOnClosureContext() or
          this->dependsOnClosureContext()) &&
