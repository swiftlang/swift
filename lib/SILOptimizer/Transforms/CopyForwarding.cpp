...
///
/// @foo : (@inout NonLoadable) -> ()
/// bb(%0 : $*NonLoadable):
///   %2 = struct_element_addr %0 : $*S<B>, #S._value
///   %3 = load %2 : $*Builtin.Int64
///   %4 = alloc_stack $S<B>
///   copy_addr %1 to [initialization] %4 : $*S<B>
///   %6 = struct_element_addr %4 : $*S<B>, #S._value
///   %7 = load %6 : $*Builtin.Int64
///   %8 = builtin "cmp_slt_Int64"(%3 : $Builtin.Int64, %7 : $Builtin.Int64)
///        : $Builtin.Int1
///   destroy_addr %4 : $*S<B>
///   dealloc_stack %4 : $*S<B>
///
/// This differs from the copy forwarding algorithm below because it handles
...
///
/// The optimization is identified in two parts.
///
/// 1. The copy's destination is an AllocStack in which all all users are of the
/// copied value. i.e. They all access the value initialized at the copy and
/// destroyed by the subsequent destroy. This is sufficient to prove that the
/// copy can be safely removed via replace-all-uses. i.e. there are no
/// undetected aliases into the copy's destination memory.
///
...
/// optimizable temporary.
static Optional<TempRValue> canOptimizeTempRValue(CopyAddrInst *copyInst) {
  if (copyInst->isTakeOfSrc() || !copyInst->isInitializationOfDest())
    return None;

  auto *copyDest = dyn_cast<AllocStackInst>(copyInst->getDest());
  if (!copyDest)
    return None;

  // Scan all users to verify they refer to the copy's destination value. It is
  // sufficient to check that the only users that modify memory are the
  ...
  for (auto *useOper : copyDest->getUses()) {
    ...
    if (isa<DestroyAddrInst>(user)) {
      if (rvalue.destroy != nullptr)
        return None;
      rvalue.destroy = cast<DestroyAddrInst>(user);
      continue;
    }
    if (isa<DeallocStackInst>(user)) {
      if (rvalue.dealloc != nullptr)
        return None;
  ...
  if (!rvalue.destroy || !rvalue.dealloc)
    return None;

  // If this copied value is destroyed before memory is written, then it
  // is an optimizable temp rvalue.
  auto II = copyInst->getIterator(), IE = copyInst->getParent()->end();
  for (++II; II != IE; ++II) {
    auto *inst = &*II;
    if (inst == rvalue.destroy)
      return rvalue;
    if (inst->mayWriteToMemory())
      return None;
  }
  // The copied value is not destroyed in this block.
  return None;
}

/// Precondition: canOptimizeTempRValue(copyInst) == true.
///
/// Replace all uses of the copy's alloc_stack destination with the copy's
/// source.
static void removeTempRValue(TempRValue rvalue) {
  DEBUG(llvm::dbgs() << "Eliminating temp RValue copy" << *rvalue.copy);

  SILValue copySrc = rvalue.copy->getSrc();
  auto *copyDest = cast<AllocStackInst>(rvalue.copy->getDest());
  rvalue.copy->eraseFromParent();
  rvalue.destroy->eraseFromParent();
  rvalue.dealloc->eraseFromParent();
...
class CopyForwardingPass : public SILFunctionTransform
{
  void run() override {
    ...
    llvm::SmallVector<TempRValue, 4> RValueCopies;
    llvm::SmallVector<CopyAddrInst*, 4> NRVOCopies;
    for (auto &BB : *getFunction())
      for (auto II = BB.begin(), IE = BB.end(); II != IE; ++II) {
        if (auto *CopyInst = dyn_cast<CopyAddrInst>(&*II)) {
          auto rvalue = canOptimizeTempRValue(CopyInst);
          if (rvalue) {
            RValueCopies.push_back(rvalue.getValue());
            continue;
          }
