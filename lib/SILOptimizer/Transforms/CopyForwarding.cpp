...
//
// 1. Read-Only Temp Copy: The copy's destination is an AllocStack that is
// only written by the initializing copy. i.e. all users of the alloc stack are
// using the copied value. This is sufficient to prove that the copy can be
// safely removed via replace-all-uses. i.e. there are no undetected aliases
// into the copy's destination memory.
//
...
// Return the value that fully identifies the memory which may be
// modified by the specified instruction. If a valid value is returned then any
// modified memory must be reachable via that value. The returned value may or
// may not be an address.
//
// FIXME: attempt to factor this logic with MemBehavior.
static SILValue getSingleModifiedSource(SILInstruction *inst) {
  switch (inst->getKind()) {
  default:
    return SILValue();
  ...
  case ValueKind::StoreInst:
    return cast<StoreInst>(inst)->getDest();
  case ValueKind::StoreWeakInst:
    return cast<StoreWeakInst>(inst)->getDest();
...
// Return true if this copy initializes a stack variable and does not destroy
// its source (if the copy destroys its source, it should be handled via
// normal copy propagation).
static bool isCopyIntoTemp(CopyAddrInst *copyInst) {
  if (!copyInst->isTakeOfSrc() && copyInst->isInitializationOfDest())
    return isa<AllocStackInst>(copyInst->getDest());
...
bool TempRValueOptimization::collectUses() {
  for (auto *useOper : copyDest->getUses()) {
    ...
    if (auto destroy = dyn_cast<DestroyAddrInst>(user)) {
      destroyInsts.push_back(destroy);
      continue;
    }
    if (auto dealloc = dyn_cast<DeallocStackInst>(user)) {
      deallocInsts.push_back(dealloc);
      continue;
    }
...
class CopyForwardingPass : public SILFunctionTransform
{
  void run() override {
    ...
    llvm::SmallVector<CopyAddrInst *, 4> TempCopies;
    llvm::SmallVector<CopyAddrInst*, 4> NRVOCopies;
    llvm::SmallSetVector<SILValue, 16> CopiedDefs;
    for (auto &BB : *getFunction())
      for (auto II = BB.begin(), IE = BB.end(); II != IE; ++II) {
        if (auto *CopyInst = dyn_cast<CopyAddrInst>(&*II)) {
          if (isCopyIntoTemp(CopyInst)) {
            TempCopies.push_back(CopyInst);
            // This copy is not necessarily rvalue-optimizable, so allow the
            // other optimizations to see it too.
          }
    ...
    // Eliminate short-lived temporaries.  Note that the copies in
