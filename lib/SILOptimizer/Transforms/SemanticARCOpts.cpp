...
void OwnershipLiveRange::insertEndBorrowsAtDestroys(
  ...
  while (!scratch.empty()) {
    ...
    builder.createEndBorrow(insertPoint->getLoc(), newGuaranteedValue);
  }
}
  ...
  for (auto pair : joinedOwnedIntroducerToConsumedOperands.getRange()) {
    ...
    while (!incomingValueOperandList.empty()) {
      ...
      if (incomingValueOperand.isGuaranteedConsuming() &&
        ...
        auto *incomingValueInst = incomingValueOperand.getInst();
        SILBuilderWithScope builder(incomingValueInst);
        originalValue = builder.createBeginBorrow(incomingValueInst->getLoc(),
                                                  originalValue);
      }
