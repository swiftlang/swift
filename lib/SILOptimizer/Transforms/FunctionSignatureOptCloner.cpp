...
/// Return true if there exists at least one direct caller.
///
/// TODO: This needs to be put into an analysis !. This is introducing
/// a quadratic complexity.
static bool existCaller(SILFunction *Fn) {
        ...
        if (auto Apply = FullApplySite::isa(&II)) {
         ...
         //  Strip ThinToThickFunctionInst.
          if (auto TTTF = dyn_cast<ThinToThickFunctionInst>(Callee)) {
            ...
            if (Fn == FRI->getReferencedFunction())
              return true;
        }
      }
    }
...
static SILFunction *createOptimizedFunctionBody(SILFunction *F,
                                                const std::string &NewFName,
                                                SignatureOptimizer &Optimizer) {
...
static SILFunction* createOptimizedFunction(llvm::BumpPtrAllocator &BPA,
...
public:
  void run() override {
    ...
    SILFunction *NewF = createOptimizedFunction(Allocator, RCIA->get(F),
                                                FSA->get(F), AA, F);
    if (NewF) { 
