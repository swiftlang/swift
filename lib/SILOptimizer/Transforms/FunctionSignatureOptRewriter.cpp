...
#include "swift/SILOptimizer/Utils/FunctionSignatureUtils.h"
...
static bool optimizeCallSite(SILModule *Mod, llvm::BumpPtrAllocator &BPA,
  ...
  SILFunction *OFn = Mod->lookUpFunction(NewFName);
  if (!OFn)
    return false;
  ...
  rewriteApplyInstToCallNewFunction(FSI, OFn, CallSite, DeadInsts);
  return true;
}
...
public:
  ...
  void run() override {
    ...
    for (auto &Apply : Sites) {
      ...
      Changed |= optimizeCallSite(&F->getModule(), Allocator, RCIA->get(F),
                                  FSA->get(F), AA, Apply, DeadInsts);
    }
  ...
  StringRef getName() override { return "Function Signature Rewriter"; }
};
