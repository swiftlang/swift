    ...
    for (unsigned arg2Idx = arg1Idx + 1; arg2Idx < block->getNumArguments();) {
      ...
      if (valuesAreEqual(arg1, arg2)) {
        if (block->getParent()->hasOwnership()) {
          ...
          // When both phis are @owned, collapsing them requires inserting a
          // copy_value to maintain ownership invariants. These copies may not
          // be eliminated by later copy-propagation passes, causing performance
          // regressions. Bail out and leave both phis in place.
