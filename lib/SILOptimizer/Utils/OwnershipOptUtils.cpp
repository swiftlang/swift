...
bool swift::completeOSSALifetime(SILValue value) {
  bool isOwned = value.getOwnershipKind() == OwnershipKind::Owned;
  if (!isOwned) {
  ...
  auto endLifetime = [&](SILBuilder &builder){
    auto loc = RegularLocation::getAutoGeneratedLocation(
      builder.getInsertionPointLoc());
    if (isOwned) {
      builder.createDestroyValue(loc, value);
    } else {
      builder.createEndBorrow(loc, value);
    }
    changed = true;
  };

  if (boundary.empty()) {
    SILBuilderWithScope builder(value->getNextInstruction());
    endLifetime(builder);
    return changed;
  }
  ...
  for (SILInstruction *lastUser : boundary.lastUsers) {
    if (liveness.isInterestingUser(lastUser)
      ...
      SILBuilderWithScope::insertAfter(lastUser, endLifetime);
    }
  }
  return changed;
}
