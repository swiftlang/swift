...
SILInlineCloner::cloneInline(ArrayRef<SILValue> AppliedArgs) {
  ...
  case FullApplySiteKind::BeginApplyInst: {
    ReturnToBB =
        callerBlock->split(std::next(Apply.getInstruction()->getIterator()));
    // For begin_apply, we insert the end_borrow in the end apply
    // block to ensure that our borrowed values live over both the
    // body and resume block of our coroutine.
