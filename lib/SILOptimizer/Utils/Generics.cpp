...
void swift::trySpecializeApplyOfGeneric(
  ...
  if (isMandatory) {
    if (F->isSerialized() && !RefF->canBeSerializedIntoCaller(F->getSerializedKind()))
      needSetLinkage = true;
  } else {
    if (F->isSerialized() && !RefF->canBeSerializedIntoCaller(F->getSerializedKind()))
        return;
  ...
  if (needSetLinkage) {
    assert(F->isSerialized() && !RefF->canBeSerializedIntoCaller(F->getSerializedKind()));
    ...
    SpecializedF->setSerializedKind(IsNotSerialized);
  } else if (F->isSerialized() && !SpecializedF->canBeSerializedIntoCaller(F->getSerializedKind())) {
    ...
    assert(SpecializedF->canBeSerializedIntoCaller(F->getSerializedKind()));
