...
void CanonicalizeOSSALifetime::findOrInsertDestroyInBlock(SILBasicBlock *bb) {
  auto *defInst = getCurrentDef()->getDefiningInstruction();
  ...
  while (true) {
    ...
    case PrunedLiveness::NonLifetimeEndingUse:
      ...
      if (isa<TermInst>(inst)) {
        ...
        insertDestroyAtInst(std::next(instIter), existingDestroy,
                            getCurrentDef(), consumes, getCallbacks());
      }
      return;
    ...
    if (!ignoredByDestroyHoisting(inst->getKind())) {
      ...
      if (auto *destroy = dyn_cast<DestroyValueInst>(inst)) {
        auto destroyDef = CanonicalizeOSSALifetime::getCanonicalCopiedDef(
            destroy->getOperand());
        if (destroyDef == getCurrentDef()) {
          existingDestroy = destroy;
        }
      }
    }
    if (instIter == bb->begin()) {
      assert(cast<SILArgument>(getCurrentDef())->getParent() == bb);
      insertDestroyAtInst(instIter, existingDestroy, getCurrentDef(),
    ...
    if (&*instIter == defInst) {
      insertDestroyAtInst(std::next(instIter), existingDestroy, getCurrentDef(),
                          consumes, getCallbacks());
      return;
    }
  }
