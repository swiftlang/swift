...
bool SemanticARCOptVisitor::visitSILPhiArgument(SILPhiArgument *arg) {
  ...
  for (auto *use : incomingValueOperands) {
    // First before we do anything, see if our value has OwnershipKind::None
    // ownership. In that case, it may be trivially dead.
    ...
    use->set(SILUndef::get(value->getType(), *value->getFunction()));

    // Now that we have disconnected the def-use in between user and value, see
    // if value has become dead. In such a case, delete it.
    if (auto *inst = value->getDefiningInstruction()) {
      eliminateDeadInstruction(inst, [&](SILInstruction *i) {
