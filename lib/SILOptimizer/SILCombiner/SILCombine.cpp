...
void SILCombiner::addReachableCodeToWorklist(SILBasicBlock *BB) {
  ...
  // callbacks before the worklist is initialized.
  //
  // WARNING: Any iterators generated by this->deleter will be invalidated here.
...
void SILCombiner::canonicalizeOSSALifetimes(SILInstruction *currentInst) {
  ...
  // Chain the SILCombiner's onDelete callback to ensure iterator updates, but
  // drop all callbacks related to the SILInstructionWorklist.
  auto localCallbacks =
      InstModCallbacks().onDelete(deleter.getCallbacks().deleteInstFunc);
  InstructionDeleter localDeleter(std::move(localCallbacks));

  CanonicalizeOSSALifetime canonicalizer(
      /*prune debug*/ false, /*poison refs*/ false, NLABA, domTree,
      localDeleter);
...
bool SILCombiner::doOneIteration(SILFunction &F, unsigned Iteration) {
  ...
  CanonicalizeInstruction scCanonicalize(DEBUG_TYPE, deadEndBlocks, deleter);
  ...
  while (!Worklist.isEmpty()) {
    ...
    //
    // TODO: When all modifications are done through the deleter's callbacks,
    // this DCE check should no longer trigger.
    ...
    if (EnableSILCombineCanonicalize) {
      if (scCanonicalize.canonicalize(I)) {
        continue;
      }
    }
