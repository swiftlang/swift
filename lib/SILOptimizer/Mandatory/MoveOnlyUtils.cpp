...
//===----------------------------------------------------------------------===//
///
/// Transformation utilities used by MoveOnlyObjectChecker,
/// MoveOnlyAddressChecker, and MoveOnlyBorrowToDestructureTransform.
///
/// The reason that this is being done is so that the borrow to destructure
/// transform can be tested separately from testing the relevant checkers.
///
/// TODO: This should eventually be in SILOptimizer/Utils.
///
//===----------------------------------------------------------------------===//
      ...
      for (auto &operand : inst->getAllOperands()) {
        ...
        if (!operand.isConsuming()) {
          ...
          if (auto *ti = dyn_cast<TermInst>(inst)) {
            if (ti->isFunctionExiting() && operand.getOperandOwnership() == OperandOwnership::TrivialUse) {
