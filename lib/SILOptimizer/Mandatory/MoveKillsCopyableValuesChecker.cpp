  ...
  while (SILValue value = defUseWorklist.pop()) {
    ...
    for (Operand *use : value->getUses()) {
      ...
      case OperandOwnership::InteriorPointer: {
        ...
        auto operand = InteriorPointerOperand(use);
        assert(bool(operand) && "Unhandled case?!");
        auto addrUseKind =
            operand.findTransitiveUses(&interiorPointerTransitiveUses);
        assert(addrUseKind == AddressUseKind::NonEscaping &&
               "Found an interior pointer that escapes?!");
