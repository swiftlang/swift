...
static void diagnoseCaptureLoc(ASTContext &Context, DeclContext *DC,
  ...
  while (!uses.empty()) {
    ...
    // Look through indirect pack references. A function with a pack parameter will get
    // the address, set the pack, and later get and load the pack. We want to follow
    // this indirection to diagnose where the pack value is actually used.
    ...
    // Follow the uses of the pack that is set, to discover get/load instructions
