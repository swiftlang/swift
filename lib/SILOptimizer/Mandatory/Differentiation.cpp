...
/// associated with a function with linkage `originalLinkage`. Flag `isExported`
/// specifies whther this function
...
DifferentiationTask::DifferentiationTask(ADContext &context,
  ...
  auto isAssocFnExported = invoker.getKind()
...
void DifferentiationTask::createEmptyPrimal() {
  ...
  // We set generated primal linkage to Private because generated primals are
  // never called cross-file: all cross-module calls to associated functions
  // call the VJP.
...
void DifferentiationTask::createEmptyAdjoint() {
  ...
  SILOptFunctionBuilder fb(context.getTransform());
  // We set generated adjoint linkage to Private because generated adjoints are
  // never called cross-file: all cross-module calls to associated functions
  // call the VJP.
  auto linkage = SILLinkage::Private;
...
void DifferentiationTask::createJVP(bool isExported) {
  ...
  SILOptFunctionBuilder fb(context.getTransform());
  auto linkage = getAutoDiffFunctionLinkage(
      original->getEffectiveSymbolLinkage(), isExported);
    ...
    if (auto *sourceFn = dyn_cast<FunctionRefInst>(ai->getCallee())) {
      ...
      if (auto resultFnTy = curriedTy.getType()->getAs<SILFunctionType>()) {
        ...
        SILFunction *newF = functionBuilder.createFunction(
            ...
            orig->isTransparent(), IsNotSerialized,
