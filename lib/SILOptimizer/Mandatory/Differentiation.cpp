...
static void emitFatalError(ADContext &context, SILFunction *f,
  ...
  // Create a `StringValue` from the fatal error string message argument.
  ...
  auto stringInitRef =
      SILDeclRef(stringInitDecl.getDecl(), SILDeclRef::Kind::Allocator);
  ...
  auto stringParamInfo =
      SILParameterInfo(stringType, ParameterConvention::Direct_Guaranteed);
  auto neverResultInfo =
      SILResultInfo(astCtx.getNeverType(), ResultConvention::Unowned);
