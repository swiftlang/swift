...
public:
  LowerHopToActor(SILFunction *f, DominanceInfo *dominance,
                  SILOptFunctionBuilder &funcBuilder)
      : F(f), Dominance(dominance), FuncBuilder(funcBuilder) {}
...
static SILValue
getExecutorForImplicitIsolationActor(SILOptFunctionBuilder &funcBuilder,
  ...
  // @leading Builtin.ImplicitIsolationActor.
  auto implicitIsolatedActorType =
      SILType::getBuiltinImplicitIsolationActorType(ctx);
  ...
  if (newFunc->empty()) {
    ...
    auto value = clearImplicitIsolationActorBits(
        builder, autoGenLoc, fArg, SILType::getOpaqueIsolationType(ctx));
...
SILValue LowerHopToActor::emitGetExecutor(SILBuilderWithScope &B,
  ...
  if (actorType == F->getASTContext().TheImplicitIsolationActorType) {
    unmarkedExecutor =
        getExecutorForImplicitIsolationActor(FuncBuilder, B, loc, actor);
...
class LowerHopToActorPass : public SILFunctionTransform {
  ...
  void run() override {
    ...
    LowerHopToActor pass(getFunction(), domTree, funcBuilder);
    if (pass.run())
