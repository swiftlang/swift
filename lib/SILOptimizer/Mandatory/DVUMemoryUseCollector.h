//===- DVUMemoryUseCollector.h - Memory use information for DVU -*- C++ -*-===//
//
...
///
/// NOTE: DVU is an abbreviation for "Predictable Memory Optimizations".
///
/// Historical Note
/// ---------------
///
...
#ifndef SWIFT_SILOPTIMIZER_MANDATORY_DVUMEMORYUSECOLLECTOR_H
#define SWIFT_SILOPTIMIZER_MANDATORY_DVUMEMORYUSECOLLECTOR_H
...
/// DVUMemoryObjectInfo - This struct holds information about the memory object
...
class DVUMemoryObjectInfo {
public:
  /// This is the instruction that represents the memory. It is either an
  /// alloc_box or alloc_stack. TODO: Fill in other options
  SILInstruction *Inst;

public:
  DVUMemoryObjectInfo(SILInstruction *Inst);
  ...
  SingleValueInstruction *getAddress() const {
    if (isa<AllocStackInst>(Inst))
      return cast<AllocationInst>(Inst);
    assert(false);
    return nullptr;
  }

  AllocBoxInst *getContainer() const {
    return dyn_cast<AllocBoxInst>(Inst);
  }
};
...
struct DVUMemoryUse {
  ...
  DVUMemoryUse(SILInstruction *Inst, DVUUseKind Kind)
      : Inst(Inst), Kind(Kind) {}

  DVUMemoryUse() : Inst(nullptr) {}
...
LLVM_NODISCARD bool
collectDVUElementUsesFrom(const DVUMemoryObjectInfo &MemoryInfo,
