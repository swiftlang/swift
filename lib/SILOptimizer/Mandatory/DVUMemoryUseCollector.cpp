//===--- DVUMemoryUseCollector.cpp - Memory use analysis for DVU ----------===//
//
...
//===----------------------------------------------------------------------===//
//                     DVUMemoryObjectInfo Implementation
//===----------------------------------------------------------------------===//

DVUMemoryObjectInfo::DVUMemoryObjectInfo(SILInstruction *Inst)
    : Inst(Inst) {
...
class ElementUseCollector {
  SILModule &Module;
  const DVUMemoryObjectInfo &TheMemory;
  SmallVectorImpl<DVUMemoryUse> &Uses;
...
public:
  ElementUseCollector(const DVUMemoryObjectInfo &TheMemory,
                      SmallVectorImpl<DVUMemoryUse> &Uses,
                      SmallVectorImpl<SILInstruction *> &Releases)
      : Module(TheMemory.Inst->getModule()), TheMemory(TheMemory),
...
bool ElementUseCollector::collectFrom() {
  if (auto *abi = dyn_cast<AllocBoxInst>(TheMemory.Inst)) {
    return collectContainerUses(cast<AllocationInst>(TheMemory.Inst));
  }else if (auto *asi = dyn_cast<AllocStackInst>(TheMemory.Inst)) {
    return collectUses(asi);
  } else if (auto *si = dyn_cast<StructInst>(TheMemory.Inst)) {
    return collectUses(si->getUniqueNonTrivialFieldValue());
  }
  auto *ti = cast<TupleInst>(TheMemory.Inst);
  return collectUses(ti->getUniqueNonTrivialElt());
}
...
bool ElementUseCollector::collectContainerUses(SILValue boxValue) {
  ...
  for (auto *ui : boxValue->getUses()) {
    ...
    // use block.
    Uses.emplace_back(user, DVUUseKind::Escape);
  }
  ...
  for (auto *UI : Pointer->getUses()) {
    ...
    if (isa<LoadInst>(User) || isa<LoadBorrowInst>(User)) {
      if (PointeeType.is<TupleType>())
        ...
        Uses.emplace_back(User, DVUUseKind::Load);
      continue;
    }
        ...
        auto kind = ([&]() -> DVUUseKind {
          ...
          case StoreOwnershipQualifier::Unqualified:
            if (PointeeType.isTrivial(*User->getFunction()))
              return DVUUseKind::InitOrAssign;
            return DVUUseKind::Initialization;

          case StoreOwnershipQualifier::Init:
            return DVUUseKind::Initialization;

          case StoreOwnershipQualifier::Assign:
            return DVUUseKind::Assign;
    ...
    if (auto *Apply = dyn_cast<ApplyInst>(User)) {
      ...
      if (ArgumentNumber < NumIndirectResults) {
        ...
        Uses.emplace_back(User, DVUUseKind::Initialization);
        continue;
      ...
      case ParameterConvention::Indirect_In_Guaranteed:
        Uses.emplace_back(User, DVUUseKind::IndirectIn);
        continue;
      ...
      case ParameterConvention::Indirect_InoutAliasable: {
        ...
        // inout use.
        Uses.emplace_back(User, DVUUseKind::InOutUse);
        continue;
      }
      }
...
/// collectDVUElementUsesFrom - Analyze all uses of the specified allocation
/// instruction (alloc_box, alloc_stack or mark_uninitialized), classifying them
/// and storing the information found into the Uses and Releases lists.
bool swift::collectDVUElementUsesFrom(
    const DVUMemoryObjectInfo &MemoryInfo, SmallVectorImpl<DVUMemoryUse> &Uses,
    SmallVectorImpl<SILInstruction *> &Releases) {
