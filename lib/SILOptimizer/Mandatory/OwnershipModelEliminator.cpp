...
static bool stripOwnership(SILFunction &func) {
  ...
  for (auto &block : func) {
    for (auto &ii : block) {
      ...
      visitLifetimeEndsOfOnStackPartialApplyAndCopies(pai, [&](Operand *op) {
  ...
  // Now that we are in non-ossa, create dealloc_stack at the lifetime boundary.
  for (auto &it : lifetimeEnds) {
    auto *pai = it.first;
    SmallVector<SILBasicBlock *, 4> discoveredBlocks;
    SSAPrunedLiveness liveness(&func, &discoveredBlocks);
    liveness.initializeDef(pai);
    PrunedLivenessBoundary boundary;
    for (auto *lifetimeEnd : it.second) {
      liveness.updateForUse(lifetimeEnd, /* lifetimeEnding */ false);
    }
    liveness.computeBoundary(boundary);
    boundary.visitInsertionPoints([&](SILBasicBlock::iterator insertPt) {
      SILBuilderWithScope(insertPt).createDeallocStack(
          RegularLocation::getAutoGeneratedLocation(), pai);
    });
  }
