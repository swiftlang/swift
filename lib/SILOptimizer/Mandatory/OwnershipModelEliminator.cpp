...
bool OwnershipModelEliminatorVisitor::visitReturnInst(ReturnInst *ri) {
  // An @unowned result is returned at +0. visitApplyInst inserts the caller's
  // retain, so release the value before returning it.
  auto *fn = ri->getFunction();
  auto fnConv = fn->getConventions();
  auto operand = ri->getOperand();
  bool isTuple = fnConv.getNumDirectSILResults() > 1;
  bool changed = false;
  for (auto indexed : llvm::enumerate(fnConv.getDirectSILResults())) {
    if (indexed.value().getConvention() != ResultConvention::Unowned)
      continue;
    auto type = isTuple
                    ? operand->getType().getTupleElementType(indexed.index())
                    : operand->getType();
    if (type.isTrivial(*fn))
      continue;
    // Only a return may carry a return location.
    auto loc =
        RegularLocation::getAutoGeneratedLocation(ri->getLoc().getSourceLoc());
    SILBuilderWithScope builder(ri, builderCtx);
    SILValue value = operand;
    if (isTuple)
      value = builder.createTupleExtract(loc, operand, indexed.index());
