...
static void extendLifetimeToEndOfFunction(SILFunction &fn,
  ...
  if (cvt->getParent() == cvt->getFunction()->getEntryBlock()) {
    auto *innerCVI = SILBuilderWithScope(cvt).createCopyValue(cvt->getLoc(),
                                                              escapingClosure);
    ...
    for (auto *block : exitingBlocks) {
      ...
      SILBuilderWithScope(safeDestructPoint)
          .createDestroyValue(safeDestructPoint->getLoc(), innerCVI);
    }
    return;
  }
  ...
  auto *cvi = [&]() -> CopyValueInst * {
    ...
    cvt->setLifetimeGuaranteed();
    cvt->setOperand(innerCVI);
    auto next = std::next(cvt->getIterator());
    SILBuilderWithScope b(next);
    updater.AddAvailableValue(cvt->getParent(),
                              b.createOptionalSome(next->getLoc(), innerCVI,
...

  {
    ...
    SILBuilderWithScope(cvi).createDestroyValue(cvi->getLoc(), v);
  }

  for (auto *block : exitingBlocks) {
    auto *safeDestructionPt = getDeinitSafeClosureDestructionPoint(block);
    SILValue v = updater.GetValueAtEndOfBlock(block);
    SILBuilderWithScope(safeDestructionPt)
        .createDestroyValue(safeDestructionPt->getLoc(), v);
  }
...
static bool trySwitchEnumPeephole(ConvertEscapeToNoEscapeInst *cvt) {
...

  {
    ...
    auto value =
        builder.emitExtractOptionalPayloadOperation(cvt->getLoc(), borrow);
...

  {
    ...
    builder.emitEndBorrowOperation(onlyDestroy->getLoc(), borrow);
    builder.emitDestroyValueOperation(onlyDestroy->getLoc(), copy);
  }
