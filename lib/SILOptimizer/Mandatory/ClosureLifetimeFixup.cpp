...
static void extendLifetimeToEndOfFunction(SILFunction &Fn,
  ...
  SILBuilderForCodeExpansion B(Cvt);
  ...
  {
    SILBuilderForCodeExpansion B(Fn.getEntryBlock()->begin());
  ...
  for (auto *Exit : ExitingBlocks) {
    ...
    SILBuilderForCodeExpansion B(Term);
    B.createDestroyAddr(loc, Slot);
...
static bool tryExtendLifetimeToLastUse(
  ...
  if (auto SingleApplyUser = FullApplySite::isa(SingleUser)) {
    ...
    SILBuilderForCodeExpansion B2(Cvt);
    ...
    if (auto *Apply = dyn_cast<ApplyInst>(SingleApplyUser.getInstruction())) {
      ...
      SILBuilderForCodeExpansion B3(InsertPt);
      ...
      for (auto *SuccBB : Try->getSuccessorBlocks()) {
        SILBuilderForCodeExpansion B3(SuccBB->begin());
        B3.createDestroyValue(loc, ClosureCopy);
      }
    } else {
...
static bool trySwitchEnumPeephole(ConvertEscapeToNoEscapeInst *Cvt) {
  ...
  // Extend the lifetime.
  SILBuilderForCodeExpansion copyB(SwitchEnum1);
  auto loc = RegularLocation::getAutoGeneratedLocation();
  auto copy = copyB.createCopyValue(loc, SwitchEnum1->getOperand());
  SILBuilderForCodeExpansion destroyB(onlyDestroy);
  destroyB.createDestroyValue(loc, copy);
  return true;
}
...
static bool fixupCopyBlockWithoutEscaping(CopyBlockWithoutEscapingInst *CB) {
  ...
  SILBuilderForCodeExpansion B(CB);
  ...
  {
    SILBuilderForCodeExpansion B(NewCB);
  ...
  for (auto LifetimeEndPoint : LifetimeEndPoints) {
    SILBuilderForCodeExpansion B(LifetimeEndPoint);
    auto IsEscaping =
  ...
  for (auto *ExitBlock: ExitingBlocks) {
    ...
    SILBuilderForCodeExpansion B(Terminator);
