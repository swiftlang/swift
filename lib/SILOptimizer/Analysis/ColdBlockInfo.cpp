...
static StringRef toString(ColdBlockInfo::Energy e) {
  if (e.all())
    ...
    return "<<NONE>>";
  else if (e.test(ColdBlockInfo::State::Warm))
    return "warm";
  else if (e.test(ColdBlockInfo::State::Cold))
    return "cold";
  else
...
/// Check ProfileCounter information to determine if the edge is cold.
/// \returns none if there is no edge FromBB -> ToBB, or if a successor
///          edge of FromBB is missing profile data.
static std::optional<ColdBlockInfo::State::Temperature>
  getEdgeProfile(const SILBasicBlock *FromBB, const SILBasicBlock *ToBB) {
  ProfileCounter total{0};
  ProfileCounter toBBCount;

  for (auto const &succ : FromBB->getSuccessors()) {
    // Can't determine coldness via profiling if there is missing data.
    ...
    if (ToBB == succ.getBB()) {
      ASSERT(!toBBCount && "double edge?");
      toBBCount = counter;
    }

    auto didSaturate = total.add_saturating(counter);

    ASSERT(!didSaturate && "should rescale the profile data first");
    (void)didSaturate;
  }

  // If missing, then there is no direct edge FromBB -> ToBB.
  ASSERT(toBBCount && "no edge?");

  double takenProbability = toBBCount.getValue() / (double)total.getValue();

  // It's a cold edge if the profiling-based probability is below the threshold.
  return takenProbability < WARM_EDGE_MINIMUM ? ColdBlockInfo::State::Cold
...
void ColdBlockInfo::analyze(SILFunction *fn) {
  ...
  for (auto &BB : *fn) {
    ...
    if (hasColdTerminator(&BB)) {
      LLVM_DEBUG(llvm::dbgs()
        << "ColdBlockInfo: setting to cold (hasColdTerminator): "
        << toString(&BB) << "\n");

      resetToCold(&BB);

      // Treat program terminating blocks as strong sources of coldness
      // by ignoring its predecessors.
      if (BB.getTerminator()->isProgramTerminating()) {
        LLVM_DEBUG(llvm::dbgs() << "ColdBlockInfo: "
          << "program terminating " << toString(&BB) << "\n");
        continue;
      }
    }
    ...
    for (auto *predBB : BB.getPredecessorBlocks()) {
      ...
      // Check profile data.
      if (auto profile = getEdgeProfile(predBB, &BB)) {
        LLVM_DEBUG(llvm::dbgs()
          << "ColdBlockInfo: setting to " << toString(*profile)
          << " (getEdgeProfile): " << toString(&BB) << "\n");

        set(&BB, *profile);

      // Check for _fastPath/_slowPath hints.
      } else if (auto *CBI = dyn_cast<CondBranchInst>(term)) {
        auto cond = getCondition(CBI->getCondition());
        setExpectedCondition(CBI, getExpectedValue(cond));
      }
    }
  }
    ...
    for (auto &BB : llvm::reverse(*fn)) {
      ...
      if (!completedIters) {
        if (auto *CBI = dyn_cast<CondBranchInst>(BB.getTerminator())) {
          auto cond = getCondition(CBI->getCondition());
          if (auto expectedValue = searchForExpectedValue(cond)) {
            setExpectedCondition(CBI, expectedValue);
            changed = true;
          }
        }
      }
...
inline bool isColdEnergy(ColdBlockInfo::Energy e) {
  return e.test(ColdBlockInfo::State::Cold)
     && !e.test(ColdBlockInfo::State::Warm);
}
...
void ColdBlockInfo::resetToCold(const SILBasicBlock *BB) {
  ...
  entry.reset();
  entry.set(State::Cold);
}

void ColdBlockInfo::set(const SILBasicBlock *BB, State::Temperature temp) {
  auto &entry = EnergyMap.getOrInsertDefault(BB);
  LLVM_DEBUG(entry.test(temp) ? false : changedMap = true);
  entry.set(temp);
}
