...
getPartialApplyArgMutationsAndEscapes(PartialApplyInst *pai,
  ...
  if (incrementalMutatingUses.empty() && incrementalEscapes.empty())
    return false;
  auto &accMutations = state.accumulatedMutations;
  while (!incrementalMutatingUses.empty())
    accMutations.push_back(incrementalMutatingUses.pop_back_val());
  auto &accEscapes = state.accumulatedEscapes;
  while (!incrementalEscapes.empty())
    accEscapes.push_back(incrementalEscapes.pop_back_val());
  return true;
}
