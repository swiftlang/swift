...
bool
isBindOverloadDisjunction(ConstraintSystem &CS, Constraint *disjunction) {
...
bool ConstraintSystem::collectNeighboringBindOverloadDisjunctions(llvm::SetVector<Constraint *> &neighbors) {
  bool failed = false;
  ...
  while (!ActiveConstraints.empty()) {
    ...
    if (constraint->getKind() == ConstraintKind::Disjunction) {
      ...
      if (auto *bindDisjunction = getBindOverloadDisjunction(*this, constraint)) {
...
void ConstraintSystem::reviseBindOverloadDisjunction(Constraint *disjunction,
                                                     llvm::SetVector<Constraint *> &workList,
