...
TypeChecker::handleSILGenericParams(GenericParamList *genericParams,
  ...
  for (unsigned i = 0, e = nestedList.size(); i < e; i++) {
    ...
    checkGenericParamList(&builder, genericParams, parentSig, true, nullptr);
  ...
  void visitFuncDecl(FuncDecl *FD) {
    ...
    if (auto gp = FD->getGenericParams()) {
      ...
      if (TC.validateGenericFuncSignature(FD)) {
        ...
        TC.checkGenericParamList(&builder, gp, parentSig, true, nullptr);
...
static Type checkExtensionGenericParams(
  ...
  tc.checkGenericParamList(&builder, genericParams, parentSig, true, nullptr);
