    ...
    Expr *buildSubscriptHelper(Expr *base, ArgumentList *args,
      ...
      if (choice.getKind() == OverloadChoiceKind::KeyPathApplication) {
        ...
        auto keyPathAp = new (cs.getASTContext()) KeyPathApplicationExpr(
            ...
            base->isImplicit() && base->isImplicit());
...
#include "swift/AST/MagicIdentifierKinds.def"
  ...
  public:
    ...
    Expr *visitEditorPlaceholderExpr(EditorPlaceholderExpr *E) {
      ...
      // FIXME: We should never be calling typeCheckExpression from CSApply.
...
ArgumentList *ExprRewriter::coerceCallArguments(
  ...
  auto *result = ArgumentList::create(ctx, args->getLParenLoc(), newArgs,
                                      args->getRParenLoc(), args->isImplicit());
  assert(result->hasAnyTrailingClosures() == args->hasAnyTrailingClosures());
  assert(result->hasMultipleTrailingClosures() ==
         args->hasMultipleTrailingClosures());
  assert(result->getOriginalArguments().getFirstTrailingClosureIndex() ==
         args->getFirstTrailingClosureArgOnlyIndex());
  assert(result->getOriginalArguments().size() == args->size());
  return result;
}
