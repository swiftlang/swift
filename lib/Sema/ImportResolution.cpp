...
CheckInconsistentAccessLevelOnImport::evaluate(
  ...
  // Error once per import decl at most.
  llvm::SmallSet<const ImportDecl *, 4> diagnosedImports;

  auto diagnose = [&](const ImportDecl *otherImport,
                      const ImportDecl *implicitImport) {
    ...
    auto &diags = mod->getDiags();
    if (diagnosedImports.insert(implicitImport).second) {
  ...
  // Grab onto the implicit one as this is where we raise the error.
  auto predicate = [](ImportDecl *decl) {
