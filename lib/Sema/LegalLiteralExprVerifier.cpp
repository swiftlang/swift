  ...
  while (!expressionsToCheck.empty()) {
    ...
    if (isa<BinaryExpr>(expr)) {
      return std::make_pair(expr,
                            IllegalConstError::UnsupportedBinaryOperator);
    }
    if (isa<PrefixUnaryExpr>(expr) || isa<PostfixUnaryExpr>(expr)) {
      return std::make_pair(expr,
                            IllegalConstError::UnsupportedUnaryOperator);
    }
...
void swift::diagnoseInvalidConstExpressions(const Expr *expr,
  ...
  if (isConstInitExpr) {
    auto exprToCheck = expr;
    if (ctx.LangOpts.hasFeature(Feature::LiteralExpressions))
      exprToCheck = evaluateOrDefault(ctx.evaluator,
    ...
    if (declContext->getASTContext().LangOpts.hasFeature(
      ...
      if (auto error = checkSupportedInConst(exprToCheck, declContext))
        diagnoseError(error->first->getLoc(), error->second, ctx.Diags);
    } else {
      // Syntactic checking for SE-0492
      if (auto error =
              checkSupportedWithSectionAttribute(exprToCheck, declContext))
        diagnoseError(error->first->getLoc(), error->second, ctx.Diags);
    }
