...
public:
  ...
  ~ArgumentFailureTracker() override {
    if (!MissingArguments.empty()) {
      ...
      (void)CS.recordFix(fix,
                         FixImpact::TypeMismatch * MissingArguments.size());
    }
  }
  ...
  bool relabelArguments(ArrayRef<Identifier> newLabels) override {
    ...
    auto impact = FixImpact::Mismatch + numOutOfOrder +
                  FixImpact::TypeMismatch * numExtraneous +
                  (FixImpact::TypeMismatch + 1) * numRenames +
                  FixImpact::TypeMismatch * MissingArguments.size();
    ...
    if (argTuple && argTuple->getNumElements() == params.size() &&
      ...
      (void)cs.recordFix(
          AddMissingArguments::create(cs, synthesizedArgs,
                                      cs.getConstraintLocator(locator)),
          FixImpact::TypeMismatch * synthesizedArgs.size());
    }
  }
...
static bool fixExtraneousArguments(ConstraintSystem &cs,
  ...
  return cs.recordFix(
      RemoveExtraneousArguments::create(cs, contextualType, extraneous,
                                        cs.getConstraintLocator(locator)),
      /*impact=*/FixImpact::TypeMismatch * numExtraneous);
}
...
ConstraintSystem::SolutionKind ConstraintSystem::matchFunctionTypes(
    FunctionType *func1, FunctionType *func2, ConstraintKind kind,
    TypeMatchOptions flags, Type origType1, Type origType2,
    ConstraintLocatorBuilder locator) {
    ...
    if (hasLabelingFailures && !hasFixFor(loc)) {
      ConstraintFix *fix =
          loc->isLastElement<LocatorPathElt::ApplyArgToParam>()
              ? AllowArgumentMismatch::create(*this, func1, func2, origType1,
                                              origType2, loc)
              : ContextualMismatch::create(*this, func1, func2, origType1,
                                           origType2, loc);
...
bool ConstraintSystem::repairFailures(
    Type lhs, Type rhs, Type rawLhs, Type rawRhs, ConstraintKind matchKind,
    TypeMatchOptions flags,
    SmallVectorImpl<RestrictionOrFix> &conversionsOrFixes,
    ConstraintLocatorBuilder locator) {
  SmallVector<LocatorPathElt, 4> path;
    ...
    if (!path.empty()) {
      ...
      if (auto argConv = path.back().getAs<LocatorPathElt::ApplyArgToParam>()) {
        ...
        if (result == SolutionKind::Error) {
          ...
          fix = AllowArgumentMismatch::create(*this, lhs, rhs, rawLhs, rawRhs,
                                              loc);
        } else {
          fix = AllowInOutConversion::create(*this, lhs, rhs, rawLhs, rawRhs,
                                             loc);
        }
      ...
      if (path.back().is<LocatorPathElt::FunctionResult>()) {
        return repairFailures(lhs, rhs, rawLhs, rawRhs, matchKind, flags,
                              conversionsOrFixes,
                              getConstraintLocator(anchor, path));
      }
  ...
  case ConstraintLocator::ApplyArgToParam: {
    ...
    conversionsOrFixes.push_back(
        AllowArgumentMismatch::create(*this, lhs, rhs, rawLhs, rawRhs, loc));
    break;
  }
  ...
  case ConstraintLocator::FunctionArgument: {
    ...
    if (lhs->is<InOutType>() != rhs->is<InOutType>()) {
      ...
      if (result == SolutionKind::Solved) {
        conversionsOrFixes.push_back(AllowInOutConversion::create(
            *this, lhs, rhs, rawLhs, rawRhs, getConstraintLocator(locator)));
        break;
      }
    }
    ...
    if (parentLoc->isForContextualType() ||
      ...
      conversionsOrFixes.push_back(AllowFunctionTypeMismatch::create(
          *this, lhs, rhs, rawLhs, rawRhs, parentLoc, /*index=*/0));
      break;
    }
  ...
  case ConstraintLocator::ExistentialConstraintType: {
    ...
    return repairFailures(ExistentialType::get(lhs), ExistentialType::get(rhs),
                          rawLhs, rawRhs, matchKind, flags, conversionsOrFixes,
                          getConstraintLocator(anchor, path));
  }
  ...
  case ConstraintLocator::TupleElement: {
    ...
    if (tupleLocator->isForContextualType()) {
      if (auto contextualTy = isSingleUnlabeledElementTuple(rhs)) {
        return repairFailures(lhs, contextualTy, rawLhs, rawRhs, matchKind,
    ...
    if (tupleLocator->isLastElement<LocatorPathElt::FunctionArgument>()) {
      fix = AllowFunctionTypeMismatch::create(*this, lhs, rhs, rawLhs, rawRhs,
                                              tupleLocator, index);
    } else if (tupleLocator->isLastElement<LocatorPathElt::ApplyArgToParam>()) {
      fix = AllowArgumentMismatch::create(*this, lhs, rhs, rawLhs, rawRhs,
                                          tupleLocator);
    } else {
      fix = AllowTupleTypeMismatch::create(*this, lhs, rhs, rawLhs, rawRhs,
                                           tupleLocator, index);
    }
    conversionsOrFixes.push_back(fix);
    break;
    ...
    if (!path.empty()) {
      ...
      if (auto argConv = path.back().getAs<LocatorPathElt::ApplyArgToParam>()) {
        ...
        if (auto overload = findSelectedOverloadFor(getCalleeLocator(argLoc))) {
          ...
          auto rawOverload = overload->adjustedOpenedType;
          auto rawParmType = rawOverload->is<FunctionType>()
                                 ? rawOverload->castTo<FunctionType>()
                                       ->getParams()[paramIdx]
                                       .getParameterType()
                                 : rawRhs;

          conversionsOrFixes.push_back(AllowArgumentMismatch::create(
              *this, getType(argList->getExpr(argIdx)), paramType, rawLhs,
              rawParmType, argLoc));
          return true;
        }
      }
...
#include "swift/AST/TypeNodes.def"
    ...
    case TypeKind::Function: {
      ...
      auto result = matchFunctionTypes(func1, func2, kind, flags, origType1,
                                       origType2, locator);
  ...
  if (shouldAttemptFixes() && !flags.contains(TMF_ApplyingFix)) {
    if (repairFailures(type1, type2, origType1, origType2, kind, flags,
                       conversionsOrFixes, locator)) {
...
ConstraintSystem::SolutionKind ConstraintSystem::simplifyConformsToConstraint(
  ...
  if (shouldAttemptFixes() && result == SolutionKind::Error) {
    ...
    ConstraintFix *fix = nullptr;
    if (loc->isLastElement<LocatorPathElt::ApplyArgToParam>()) {
      fix = AllowArgumentMismatch::create(*this, type, protocol, origType,
                                          protocol, loc);
...
ConstraintSystem::SolutionKind ConstraintSystem::simplifyConformsToConstraint(
  ...
  // makes it much easier to diagnose problems like that.
  {
    ...
    if ((kind == ConstraintKind::ConformsTo ||
      ...
      auto *fix = AllowArgumentMismatch::create(*this, type, protocolTy,
                                                rawType, protocolTy, loc);
      return recordFix(fix, FixImpact::TypeMismatch) ? SolutionKind::Error
                                                     : SolutionKind::Solved;
    }
...
ConstraintSystem::simplifyBindTupleOfFunctionParamsConstraint(
  ...
  if (unwrapCount > 0) {
    auto *fix = ForceOptional::create(*this, simplifiedCopy, second,
                                      getConstraintLocator(locator));
    if (recordFix(fix, FixImpact::Mismatch * unwrapCount))
      return SolutionKind::Error;
  }
  return SolutionKind::Solved;
}
...
ConstraintSystem::SolutionKind ConstraintSystem::simplifyMemberConstraint(
  ...
  if (shouldAttemptFixes()) {
    auto fixMissingMember = [&](Type baseTy, Type memberTy,
      ...
      auto impact = FixImpact::InvalidReference;
...
bool ConstraintSystem::resolveClosure(TypeVariableType *typeVar,
  ...
  auto origContextualType = contextualType;
...
ConstraintSystem::SolutionKind ConstraintSystem::simplifyApplicableFnConstraint(
  ...
  if (shouldAttemptFixes() && result == SolutionKind::Error) {
    ...
    // Let's make this fix a high impact so if there is a function or member
    // overload with e.g. argument-to-parameter type mismatches or missing/extra
    // arguments it would take a higher priority.
    return recordFix(fix, FixImpact::InvalidApplication) ? SolutionKind::Error
                                                         : SolutionKind::Solved;
  }
      ...
      if (path.size() > 0 &&
                ...
                DestructureTupleToMatchPackExpansionParameter::create(
                    *this,
                    (type2->is<PackType>()
                         ...
                         : PackType::getSingletonPackExpansion(type2)),
                    loc),
                FixImpact::TypeMismatch * numParams);
          }
        }
        ...
        if (numParams > numArgs) {
          ...
          return recordShapeFix(
              ...
              FixImpact::TypeMismatch * extraneousArgs.size());
        }
      }
    }
...
ConstraintSystem::simplifyRestrictedConstraintImpl(
  ...
  auto matchPointerBaseTypes =
    ...
    auto baseImpact = FixImpact::TypeMismatch;
...
bool ConstraintSystem::recordFix(ConstraintFix *fix, FixImpact impact,
  ...
  if (auto impactScoreKind = fix->impact())
    increaseScore(*impactScoreKind, fix->getLocator(),
                  static_cast<unsigned>(impact));
...
ConstraintSystem::SolutionKind ConstraintSystem::simplifyFixConstraint(
  ...
  case FixKind::AllowTupleTypeMismatch: {
    if (fix->getAs<AllowTupleTypeMismatch>()->isElementMismatch()) {
      ...
      if (recordFix(fix, FixImpact::Mismatch *
                             (locator->isForContextualType() ? 5 : 1)))
        return SolutionKind::Error;
      return SolutionKind::Solved;
    }
  ...
  case FixKind::TreatEphemeralAsNonEphemeral: {
    ...
    case ConversionEphemeralness::Ephemeral:
      ...
      if (recordFix(fix, FixImpact::None))
        return SolutionKind::Error;
  ...
  case FixKind::IgnoreInvalidASTNode: {
    return recordFix(fix, FixImpact::InvalidAST) ? SolutionKind::Error
                                                 : SolutionKind::Solved;
  }
  case FixKind::IgnoreUnresolvedPatternVar: {
    return recordFix(fix, FixImpact::InvalidAST * 10) ? SolutionKind::Error
                                                      : SolutionKind::Solved;
  }

  case FixKind::AllowInvalidMemberReferenceInInitAccessor: {
    return recordFix(fix, FixImpact::InvalidReference) ? SolutionKind::Error
                                                       : SolutionKind::Solved;
  }
  ...
  case FixKind::ExplicitlyConstructRawRepresentable: {
    ...
    return recordFix(fix,
                     /*impact=*/isExpr<BinaryExpr>(locator.getAnchor())
                         ? FixImpact::TypeMismatch
                         : FixImpact::Mismatch)
               ? SolutionKind::Error
               : SolutionKind::Solved;
  ...
  case FixKind::TreatRValueAsLValue: {
    auto impact = TreatRValueAsLValue::assessImpact(*this, fix->getLocator());
    return recordFix(fix, impact) ? SolutionKind::Error : SolutionKind::Solved;
  }
  ...
  case FixKind::IgnoreDefaultExprTypeMismatch: {
    ...
    if (llvm::any_of(getFixes(), [&](const ConstraintFix *fix) {
      ...
      impact += FixImpact::TypeMismatch + 1;
  ...
  case FixKind::TreatArrayLiteralAsDictionary: {
    ...
    if (recordFix(fix, FixImpact::Mismatch + optionals.size()))
      return SolutionKind::Error;
  ...
  case FixKind::IgnoreCollectionElementContextualMismatch: {
    ...
    if (auto branchElt =
      ...
      if (branchElt->forElse()) {
        impact = FixImpact::TypeMismatch * 5;
      } else {
    ...
    if (auto branchElt = locator->getLastElementAs<SingleValueStmtResult>()) {
      ...
      if (branchElt->getIndex() > 0)
        impact = FixImpact::TypeMismatch * 5 + (branchElt->getIndex() - 1);
    }
    ...
    if (auto *fnType1 = type1->getAs<FunctionType>()) {
      ...
      if (type2->is<FunctionType>()) {
        increaseScore(
            SK_Fix, locator,
            static_cast<unsigned>(FixImpact::FunctionTypeMismatch * 2));
      } else {
...
ConstraintSystem::simplifyConstraint(const Constraint &constraint) {
  ...
  case ConstraintKind::BindOverload: {
    if (auto *fix = constraint.getFix()) {
      ...
      auto impact = fix->getKind() == FixKind::AddQualifierToAccessTopLevelName
