    ...
    for (auto argIdx : parameterBindings[paramIdx]) {
      ...
      if (auto typeVarAndBinding = shouldOpenExistentialCallArgument(
        ...
        std::tie(bindingTy, openedTy) =
            cs.openExistentialType(bindingTy, cs.getConstraintLocator(loc));
        ...
        argTy = argTy.transformRec([&](TypeBase *t) -> std::optional<Type> {
          if (t->isAnyExistentialType()) {
            return OpenedArchetypeType::getAny(t, openedTy);
          }
...
ConstraintSystem::SolutionKind ConstraintSystem::simplifyApplicableFnConstraint(
  ...
  if (auto func2 = dyn_cast<FunctionType>(desugar2)) {
    ...
    if (result2->hasTypeVariable() && !openedExistentials.empty()) {
      ...
      for (auto *opened : openedExistentials) {
        result2 = typeEraseCovariantOpenedArchetypesFromEnvironment(
            result2, opened->getGenericEnvironment(),
