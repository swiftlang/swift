...
lookupDynamicCallableMethods(NominalTypeDecl *decl, ConstraintSystem &CS,
  ...
  // set). We map in context here to avoid an OverloadChoice assertion for an
  // interface type base.
  auto type = decl->getDeclaredTypeInContext();

  // TODO: Can this be requestified? It's not clear if we actually need to go
  // through ConstraintSystem::performMemberLookup vs just
  // TypeChecker::lookupMember. This would avoid needing to map into context
  // above, and would let us cache independently of the constraint system.
