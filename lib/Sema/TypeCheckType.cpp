...
Type TypeResolution::applyUnboundGenericArguments(
  ...
  if (hasTypeVariables && getStage() == TypeResolutionStage::Interface) {
    ASSERT(requirementOpener);
    requirementOpener(decl, substitutions);
  }

  // Check the generic arguments against the requirements of the declaration's
  // generic signature.
  if (!skipRequirementsCheck && !hasTypeVariables &&
      getStage() == TypeResolutionStage::Interface) {
    // Check the generic arguments against the requirements of the declaration's
    // generic signature.
