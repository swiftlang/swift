...
AddToEnvF ConflictMerger::TypeUnifier::addToStorageEnv(StorageEnv &env) {
  return [&](TypeVariableType *typeVar, Type nv) {
    ...
    env[typeVar] = ConflictedType{nv->getCanonicalType(), nv, (ConstraintLocator*) 0, (ConflictReason*) 0};
  };
}
...
// Find a locator, work out the reason, which should be ambiguity
AddToEnvF ConflictMerger::TypeUnifier::addToMergeable(MergeableTypes &env) {
  return [&](TypeVariableType *typeVar, Type nv) {
    env.map[typeVar].insert(ConflictedType{nv->getCanonicalType(), nv, (ConstraintLocator*) 0, (ConflictReason*) 0});
  };
}
...
void ConflictMerger::mergeTypes(ConflictEnv baseMap, const Solution *s1,
  ...
  for (auto tv : baseMap) {
    ...
    unifiedTypes1.insert(ConflictedType(tv.second.first->getCanonicalType(), tv.second.first, nullptr, nullptr));
    unifiedTypes2.insert(
                         ConflictedType(tv.second.second->getCanonicalType(),
                           tv.second.second, nullptr, nullptr));
