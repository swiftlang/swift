...
// #if 0
  if (diffAttr) {
    ...
    auto hasSameDerivativeGenericSignatures = [&] {
      ...
      auto second = diffAttr->getDerivativeGenericSignature();
      if (first && second)
    ...
    case AutoDiffDerivativeFunctionKind::JVP:
      ...
      if ((diffAttr->getJVP() &&
           diffAttr->getJVP()->Name.getBaseName() != derivative->getBaseName()) ||
          (diffAttr->getJVPFunction() && diffAttr->getJVPFunction() != derivative)) {
        diagnoseAndRemoveAttr(
            attr, diag::derivative_attr_original_already_has_derivative,
            originalAFD->getFullName());
        return;
    ...
    case AutoDiffDerivativeFunctionKind::VJP:
      // If there's a different registered derivative, emit an error.
      if ((diffAttr->getVJP() &&
           diffAttr->getVJP()->Name.getBaseName() != derivative->getBaseName()) ||
          (diffAttr->getVJPFunction() && diffAttr->getVJPFunction() != derivative)) {
        diagnoseAndRemoveAttr(
            attr, diag::derivative_attr_original_already_has_derivative,
            originalAFD->getFullName());
        return;
