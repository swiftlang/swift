...
#define CLONED_ATTR(NAME) \
  ...
  UNCONSTRAINED_ATTR(Prefix)
  UNCONSTRAINED_ATTR(Postfix)
  NONABI_ATTR(Infix)

  // Checked against each other separately
  UNCONSTRAINED_ATTR(Borrowing)
  UNCONSTRAINED_ATTR(Consuming)
  UNCONSTRAINED_ATTR(LegacyConsuming)
  UNCONSTRAINED_ATTR(Mutating)
  UNCONSTRAINED_ATTR(NonMutating)

  UNCONSTRAINED_ATTR(AddressableSelf)
