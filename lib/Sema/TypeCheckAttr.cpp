...
static bool typeCheckDerivativeAttr(ASTContext &Ctx, Decl *D,
  ...
  if (!ctx.LangOpts.EnableExperimentalDifferentiableProgramming) {
    diags.diagnose(
        attr->getLocation(), diag::experimental_differentiable_programming_disabled);
    return true;
  }
  ...
  if (!derivativeResultTupleType ||
    ...
    diags.diagnose(attr->getLocation(), diag::derivative_attr_expected_result_tuple);
    return true;
  }
  ...
  if (valueResultElt.getName().str() != "value") {
    diags.diagnose(
        attr->getLocation(), diag::derivative_attr_invalid_result_tuple_value_label);
    return true;
  }
  if (funcResultElt.getName().str() == "differential") {
    ...
    diags.diagnose(
        attr->getLocation(), diag::derivative_attr_invalid_result_tuple_func_label);
    return true;
  }
  ...
  auto notFunctionDiagnostic = [&]() {
    diags.diagnose(originalName.Loc, diag::autodiff_attr_original_decl_invalid_kind,
             originalName.Name);
  };
  ...
  if (!actualFuncEltType->isEqual(expectedFuncEltType)) {
    ...
    diags.diagnose(attr->getLocation(), diag::derivative_attr_result_func_type_mismatch,
    ...
    diags.diagnose(funcEltTypeRepr->getStartLoc(),
  ...
  auto &derivativeAttrs =
      Ctx.DerivativeAttrs[{originalAFD, resolvedWrtParamIndices, kind}];
