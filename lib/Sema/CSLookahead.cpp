...
static ConflictReason canPossiblyConvertTo(
  ...
  auto lhsKind = inference::getConversionBehavior(argType);
  auto rhsKind = inference::getConversionBehavior(paramType);

  // Conversion between two types with the same conversion behavior.
  if (lhsKind == rhsKind) {
    switch (lhsKind) {
    case inference::ConversionBehavior::None:
      if (!argType->hasTypeVariable() && !argType->hasTypeParameter() &&
          !paramType->hasTypeVariable() && !paramType->hasTypeParameter()) {
        if (!argType->isEqual(paramType))
          return ConflictFlag::Exact;
      }

      if (argType->getAnyNominal() != paramType->getAnyNominal())
    ...
    case inference::ConversionBehavior::Array: {
      auto subResult = canPossiblyConvertTo(
          cs,
          argType->getArrayElementType(),
          paramType->getArrayElementType(), sig);
      if (subResult)
    ...
    case inference::ConversionBehavior::Optional: {
      ...
      auto argObjectType = argType->getOptionalObjectType();
      auto objectType = paramType->getOptionalObjectType();
    ...
    case inference::ConversionBehavior::Structural:
      if (argType->getCanonicalType()->getKind()
          != paramType->getCanonicalType()->getKind())
  ...
  // from an unknown type.
    ...
    switch (rhsKind) {
    case inference::ConversionBehavior::AnyHashable:
      // FIXME: Check if argType definitely not Hashable
      break;
  ...
  auto isConformanceTransitiveOnLHS = [lhsKind, argType]() -> bool {
  ...
  if (sig &&
    ...
    bool failed = llvm::any_of(
        sig->getRequiredProtocols(argType),
        [&](ProtocolDecl *proto) {
          if (shouldBeConservativeWithProto(proto))
            return false;
          return !lookupConformance(paramType, proto);
        });
    if (failed)
      return ConflictFlag::Conformance;
  }
