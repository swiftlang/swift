...
void CalleeCandidateInfo::collectCalleeCandidates(Expr *fn,
  ...
  if (auto declRefExpr = dyn_cast<DeclRefExpr>(fn)) {
    ...
    declName = decl->getBaseName().userFacingName().str();
    return;
  }
  ...
  if (auto overloadedDRE = dyn_cast<OverloadedDeclRefExpr>(fn)) {
    ...
    if (!candidates.empty())
      declName = candidates[0].getDecl()->getBaseName().userFacingName().str();
    return;
  }
  ...
  if (auto UDE = dyn_cast<UnresolvedDotExpr>(fn)) {
    declName = UDE->getName().getBaseName().userFacingName().str();
    hasCurriedSelf = true;
