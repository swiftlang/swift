...
deriveRawValueInit(AbstractFunctionDecl *initDecl, void *) {
  ...
  ParamDecl *rawValueDecl =
      ParamDecl::createImplicit(C, C.Id_rawValue, C.Id_rawValue, C.getIntType(),
                                parentDC, ParamSpecifier::Default);
...
static ValueDecl *deriveInitDecl(DerivedConformance &derived, Type paramType,
  ...
  // rawValue
  ParamDecl *rawDecl =
      ParamDecl::createImplicit(C, paramName, paramName, paramType, parentDC,
...
static ValueDecl *deriveCodingKeyViaMacros(DerivedConformance &derived,
  ...
  os << "#_deriveCodingKey(" << QuotedString(getNominalTypeInfoString(derived))
     ...
     << QuotedString(isString ? "string" : "int") << ", " << (usingRaw ? "true" : "false") << ")";
  os.flush();
