...
private:
  ...
  static Type getMemberType(ValueDecl *decl) {
    ...
    // A C++ references must be implemented as pointers (e.g., `const T &` in
    // C++ should be `UnsafePointer<T>` in Swift). Map references to pointers
    // in order to match against that spelling.
    const auto *clangFD =
  ...
  MatchOutcome matchesImpl(ValueDecl *req, ValueDecl *cand,
    ...
    if (auto reqAFD = dyn_cast<AbstractFunctionDecl>(req)) {
      ...
      if (clangFD && clangFD->getNumParams() != reqAFD->getParameters()->size())
        clangFD = nullptr;
      ...
      for (auto [reqParam, candParam] :
        ...
        unsigned i = paramIdx++;
        // Skip the ownership check for C++ references.
        if (clangFD &&
