...
public:
  ...
  T *operator->() { return Value.template dyn_cast<T>(); }
  FoldingError getError() { return *Value.template dyn_cast<FoldingError>(); }
  bool isError() { return Value.template dyn_cast<FoldingError>() != nullptr; }

  /// Return false if there is an error.
  explicit operator bool() { return !isError(); }
};
...
public:
  ...
  Expr *fold(const Expr *exprToFold) {
    auto resultOrError = tryFoldExpression(exprToFold);
    if (resultOrError)
      return createIntegerLiteralExpr(exprToFold, resultOrError->get());
    else {
      emitFoldingErrorDiagnostic(resultOrError.getError());
      return nullptr;
    }
  }
...
private:
  FoldingErrorOr<ConstantValuePtr> tryFoldExpression(const Expr *expr) {
    if (auto *binaryExpr = dyn_cast<BinaryExpr>(expr))
      ...
      return tryFoldExpression(parenExpr->getSubExpr());
    else if (auto *intLiteralExpr = dyn_cast<IntegerLiteralExpr>(expr))
      return foldIntegerLiteralExpr(intLiteralExpr);
    else
      return FoldingError(IllegalConstError::Default);
  }
  ...
  FoldingErrorOr<ConstantValuePtr> tryFoldBinaryExpr(const BinaryExpr *expr) {
    ...
    if (auto lhsOrErr = tryFoldExpression(expr->getLHS())) {
      if (auto rhsOrErr = tryFoldExpression(expr->getRHS())) {
        auto lhsIntPtr = cast<IntegerValue>(lhsOrErr->get());
        auto rhsIntPtr = cast<IntegerValue>(rhsOrErr->get());
        const auto operatorDeclRefExpr = expr->getFn()->getMemberOperatorRef();
        ...
        if (operatorIdentifier.isArithmeticOperator())
          return tryFoldIntegerBinaryArithmeticOperator(
              operatorIdentifier, expr->getStartLoc(), lhsIntPtr, rhsIntPtr);
  ...
  tryFoldUnaryExpr(const PrefixUnaryExpr *expr) {
    ...
    auto operandOrErr = tryFoldExpression(expr->getOperand());
    if (operandOrErr) {
      auto operatorIdentifier = expr->getFn()
                                    ->getMemberOperatorRef()
                                    ->getDecl()
                                    ->getBaseName()
                                    .getIdentifier();
      return tryFoldIntegerUnaryArithmeticOperator(
          operatorIdentifier, expr->getStartLoc(),
  ...
  Expr *createIntegerLiteralExpr(const Expr *foldedExpr,
    ...
    assert(isIntegerType(resultType));
...
Expr *ConstantFoldExpression::evaluate(Evaluator &evaluator,
                                       const Expr *expr,
