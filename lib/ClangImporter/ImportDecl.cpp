...
namespace {
  ...
  public:
    ...
    Decl *importSwift2TypeAlias(const clang::NamedDecl *decl,
      ...
      if (auto generic = dyn_cast<GenericTypeDecl>(typeDecl)) {
        if (generic->getGenericSignature() && !isa<ProtocolDecl>(typeDecl)) {
          ...
          archetypes = generic->getArchetypes();
    ...
    Decl *importGlobalAsMethod(const clang::FunctionDecl *decl, DeclName name,
      ...
      result->setArchetypes(dc->getArchetypesOfContext());
  ...
  public:
    ...
    FuncDecl *buildSubscriptGetterDecl(const FuncDecl *getter, Type elementTy,
      ...
      thunk->setArchetypes(dc->getArchetypesOfContext());
  ...
  public:
    ...
    std::pair<GenericSignature *, ArchetypeMapping *>
    calculateGenericSignature(GenericParamList *genericParams,
      ...
      return std::make_pair(sig, archetypes);
    }
    ...
    Decl *VisitObjCCategoryDecl(const clang::ObjCCategoryDecl *decl) {
      ...
      if (objcClass->getGenericParams()) {
        ...
        std::tie(sig, archetypes) =
            calculateGenericSignature(genericParams, result);

        result->setGenericSignature(sig);
        result->setArchetypes(archetypes);
    ...
    Decl *VisitObjCProtocolDecl(const clang::ObjCProtocolDecl *decl) {
      ...
      auto *archetypes =
          ArchetypeMapping::get(Impl.SwiftContext, interfaceToArchetypeMap);
      result->setArchetypes(archetypes);
  ...
  EffectiveClangContext context)
{
  ...
  if (auto protoDecl = ext->getAsProtocolExtensionContext()) {
    ...
    auto *archetypes =
        ArchetypeMapping::get(SwiftContext, interfaceToArchetypeMap);
    ext->setArchetypes(archetypes);
  }
