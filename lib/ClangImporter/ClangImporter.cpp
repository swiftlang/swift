...
bool hasDiamondInheritance(const clang::CXXRecordDecl *decl) {
  if (!decl->hasDefinition() || decl->isDependentType())
    return false;

  llvm::DenseMap<const clang::CXXRecordDecl *, int> BaseCount;
  bool hasDiamond = false;

  decl->forallBases([&](const clang::CXXRecordDecl *Base) {
    BaseCount[Base]++;
    if (BaseCount[Base] > 1 && !decl->isVirtuallyDerivedFrom(Base)) {
      hasDiamond = true;
    }
    return true;
  });
...
getRefParentDecls(const clang::RecordDecl *decl) {
  ...
  if (const auto *cxxRecordDecl = llvm::dyn_cast<clang::CXXRecordDecl>(decl)) {
    if (!cxxRecordDecl->hasDefinition() || hasDiamondInheritance(cxxRecordDecl))
...
getRefParentOrDiag(const clang::RecordDecl *decl, ASTContext &ctx,
  ...
  if (refParentDecls.empty()) {
    if (const auto *cxxRecordDecl =
      ...
      if (hasDiamondInheritance(cxxRecordDecl)) {
        if (importerImpl) {
          if (importerImpl->DiagnosedCxxRefDecls.find(decl) ==
              importerImpl->DiagnosedCxxRefDecls.end()) {
  ...
  // Ensure exactly one retain and one release function
...
// Is this a pointer to a foreign reference type.
// DOUBT: Do we need to change other palces in the ClangImpoter codebase like
// this when checking for c++ foreign ref types for inheritance purposes
