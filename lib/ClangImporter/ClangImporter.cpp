...
ClangImporter::create(ASTContext &ctx, const IRGenOptions *IRGenOpts,
                      StringRef swiftPCHHash, DependencyTracker *tracker,
...
isPotentiallyConflictingSetter(const clang::ObjCProtocolDecl *proto,
  ...
  return llvm::any_of(proto->properties(),
                      [sel](clang::ObjCPropertyDecl *prop) {
                        return prop->getSetterName() == sel;
                      });
}
...
bool ClangImporter::Implementation::isVisibleClangEntry(
  ...
  return llvm::any_of(clangDecl->redecls(), [&clangSema](clang::Decl *d) {
    return clangSema.isVisible(cast<clang::NamedDecl>(d));
  });
}
...
bool ClangModuleUnit::hasClangModule(ModuleDecl *M) {
  return llvm::any_of(M->getFiles(),
                      [](auto F) { return isa<ClangModuleUnit>(F); });
}
...
bool ClangImporter::Implementation::forEachLookupTable(
  ...
  return llvm::any_of(moduleNames, [&](StringRef moduleName) {
    return fn(*LookupTables[moduleName]);
  });
}
...
ExplicitSafety ClangDeclExplicitSafety::evaluate(
  ...
  auto isUnsafe = [&](clang::QualType type) -> bool {
    ...
    if (!pointeeType.isNull()) {
      ...
      return !(recordDecl && hasImportAsRefAttr(recordDecl));
    }
