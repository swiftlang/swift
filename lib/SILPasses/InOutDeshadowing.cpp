...
static AllocStackInst *findAutogeneratedStoreToAllocStack(SILInstruction *Inst){
  for (auto UI : SILValue(Inst, 0).getUses()) {
    // Look through copy_value instructions if present.
    if (auto *CV = dyn_cast<CopyValueInst>(UI->getUser()))
      if (auto ASI = findAutogeneratedStoreToAllocStack(CV))
        return ASI;
    
    auto *SI = dyn_cast<StoreInst>(UI->getUser());
...
static AllocStackInst *isCopyToOrFromStack(Operand *UI) {
  ...
  if (auto SI = dyn_cast<StoreInst>(ArgUser)) {
    ...
    // Look through a copy_value (which would be maintained by this pass).
    SILValue Src = SI->getSrc();
    if (auto *CV = dyn_cast<CopyValueInst>(Src))
      Src = CV->getOperand();
  ...
  if (auto LI = dyn_cast<LoadInst>(ArgUser))
    if (auto ASI = findAutogeneratedStoreToAllocStack(LI))
      return ASI;
