...
static unsigned basiBlockInlineCost(SILBasicBlock *BB, unsigned Cutoff) {
  unsigned Cost = 0;
  for (auto &I : *BB) {
...
/// DomCondition.
static
SILValue trackBackBBArgValue(SILValue Value,
                             SILBasicBlock *DomBB,
                             SILValue DomValue,
                             DominanceInfo *DT) {
  llvm::DenseSet<ValueBase *> SeenValues;
...
/// Classify a predecessor as being reachable via success or failure branches
/// of a dominating checked_cast_br or as unknown if it can be reached via
/// success or failure branches at the same time.
static void classifyPredecessor(SILBasicBlock *Pred,
  ...
  else if (SuccessDominates)
    SuccessPreds.push_back(Pred);
  else if (FailureDominates)
...
SimplifyCFG::trySimplifyCheckedCastBr(TermInst *Term, DominanceInfo *DT) {
  ...
  auto *BB = Term->getParent();
  auto Condition = stripClassCasts(Term->getOperand(0));
  ...
  for (auto *Node = DT->getNode(BB)->getIDom(); Node; Node = Node->getIDom()) {
    ...
    auto *PredSuccessBB = DomCCBI->getSuccessBB();
    auto *PredFailureBB = DomCCBI->getFailureBB();
    ...
    SILBasicBlock *ArgBB = nullptr;

    auto PredCondition = stripClassCasts(DomTerm->getOperand(0));

    // Are conditions equivalent along all paths?
    if (!areEquivalentConditions(PredCondition, Condition)) {
      ...
      // backwards and see if it is actually equivalent to PredCondition.
      ...
      // is equivalent to the PredCondition. If this is the case, it is
      // possible to try jump-threading along this path.
      size_t idx = 0;
      for (auto *PredBB : ArgBB->getPreds()) {
        auto IncomingValue = IncomingValues[idx];
        SILValue ReachingValue = trackBackBBArgValue(
            IncomingValue, DomBB, PredCondition, DT);
        ...
        if (ReachingValue == SILValue()) {
          ...
          bool SuccessDominates = DT->dominates(PredSuccessBB, PredBB) ||
                                  ...
                                  PredSuccessBB == BB;
          bool FailureDominates = DT->dominates(PredFailureBB, PredBB) ||
                                  DT->dominates(PredFailureBB, BB) ||
                                  PredFailureBB == BB;
      ...
      if (ArgBB->getPreds().empty()) {
        ...
        // See if it is reached over Success or Failure path.
        bool SuccessDominates = PredSuccessBB == BB;
        bool FailureDominates = PredFailureBB == BB;
      ...
      for (auto *PredBB : BB->getPreds()) {
        ...
        bool SuccessDominates =
            DT->dominates(PredSuccessBB, PredBB) || PredSuccessBB == BB;
        bool FailureDominates =
            DT->dominates(PredFailureBB, PredBB) || PredFailureBB == BB;
    ...
    if (TotalPreds != SuccessPreds.size() &&
      ...
      if (basiBlockInlineCost(BB, InlineCostCutoff) >= InlineCostCutoff)
        return false;
    }
    ...
    for (auto &B: BB->getSuccs()) {
      addToWorklist(B.getBB());
    }

    auto *SuccessBB = CCBI->getSuccessBB();
    auto *FailureBB = CCBI->getFailureBB();

    SILBasicBlock *TargetFailureBB = nullptr;
    SILBasicBlock *TargetSuccessBB = nullptr;

    BasicBlockCloner *FailureBBCloner = nullptr;
    BasicBlockCloner *SuccessBBCloner = nullptr;

    // Create a copy of the BB as a landing BB
    // for all FailurePreds.
    if (!FailurePreds.empty()) {
      FailureBBCloner = new BasicBlockCloner(BB);
    ...
    if (!UnknownPreds.empty()) {
      if (!SuccessPreds.empty()) {
        ...
        SuccessBBCloner = new BasicBlockCloner(BB);
        ...
        SuccessBBArgs.push_back(PredSuccessBB->getBBArg(0));
    ...
    if (Children.size() > 1) {
      std::vector<DominanceInfoNode *> ChildrenCopy;
    ...
    assert(!(SuccessBBCloner  && FailureBBCloner) &&
           "Both cloners cannot be used at the same time yet");

    // Now update the SSA form.
    if (!FailurePreds.empty() && FailureBBCloner && !SuccessBBCloner)
      updateSSAAfterCloning(*FailureBBCloner, TargetFailureBB, BB);

    if (SuccessBBCloner && !FailureBBCloner) {
      updateSSAAfterCloning(*SuccessBBCloner, TargetSuccessBB, BB);
    }

    // Since a few BBs were changed now, add them for re-processing.
    if (TargetFailureBB) {
      if (!TargetFailureBB->pred_empty())
        addToWorklist(TargetFailureBB);
    }

    if (TargetSuccessBB) {
      if (!TargetSuccessBB->pred_empty())
        addToWorklist(TargetSuccessBB);
    }
    ...
    // Clean-up.
    if (FailureBBCloner)
      delete FailureBBCloner;

    if (SuccessBBCloner)
      delete SuccessBBCloner;
