...
extern "C" _impl::swift_interop_stub_Swift_String $sSS10describingSSx_tclufC(
    const void *_Nonnull value,
    const void *_Nonnull metadata) SWIFT_NOEXCEPT SWIFT_CALL;
#endif
...
public:
  ...
  SWIFT_INLINE_THUNK Error &operator=(Error &&other) noexcept {
    if (this == &other)
      return *this;
    // Leave `other` empty, as the move constructor does.
    if (opaqueValue)
      swift_errorRelease(opaqueValue);
    if (whatBuffer)
      _impl::opaqueFree(static_cast<char *_Nonnull>(whatBuffer));
  ...
  /// this object. In Embedded Swift, which has no `String(describing:)` for
  /// arbitrary values, this returns a fixed placeholder.
  ...
  template <class T>
  SWIFT_INLINE_THUNK swift::Optional<T> as() const {
