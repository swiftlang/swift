...
This code is also diagnosed under complete concurrency checking. Even though the `shared` variable is a `let` constant, the `MyModel` type is not `Sendable`, so it could have mutable stored properties. The most common fix in this case is to isolate the variable to the main actor:
...
Alternatively, you can isolate the `MyModel` class itself to the main actor, which will also make the type `Sendable` because the main actor will protect access to all mutable state:
