...
functions to run in the isolation domain of the caller by default, and provides
an explicit way to express the execution semantics for these functions:
...
* The `@execution(caller)` attribute specifies that a function must always 
  run in the isolation domain of its caller.
