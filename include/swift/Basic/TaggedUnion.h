...
public:
  ...
  TaggedUnionBase(T &&value,
                  typename std::enable_if<
                    Members::template contains<
                      typename std::remove_reference<T>::type>(),
                    Empty>::type = {}) {
    using TargetType = typename std::remove_reference<T>::type;
    TheKind = StorageType::template kindForMember<TargetType>();
    Storage.template emplace<TargetType>(TheKind, std::forward<T>(value));
  }

  template <class T>
  typename std::enable_if<Members::template contains<
                            typename std::remove_reference<T>::type>(),
