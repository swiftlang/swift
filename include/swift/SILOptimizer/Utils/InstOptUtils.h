...
private:
  ...
  // A set of non-trivial types found to have value semantics. This is populated
  // while checking whether an instruction is dead. This set helps identify more
  // instructions as dead, particularly instructions used only in
  // destroy_values.
  SmallPtrSet<CanType, 4> knownPureValueTypes;
...
public:
  ...
  /// Delete the instruction \p inst and record instructions that may become
  /// dead because of the removal of \c inst. This function will not fix the
  ...
  /// of the contaning function is made consistent. If in ownership SIL, use the
  /// \c forceDelete function unless the lifetimes of the deleted operands
  /// should not be fixed.
  ///
  /// \pre the instruction to be deleted must not have any use other than
  /// incidental uses.
