...
public:
  ...
  struct Effect final {
    ...
    Effect compose(Effect other) const {
  ...
  public:
    ...
    /// Set the specified block's initial state to be ::Unknown.
  ...
  IterativeBackwardReachability(SILFunction *function, SILBasicBlock *defBlock,
      ...
      : function(function), defBlock(defBlock), effects(effects),
        result(result), worklist(function) {}
...
private:
  ...
  SmallVector<SILInstruction *, 32> localGens;
  ...
  /// Summarize the effect of the block starting above the indicated
  /// instruction.
  Effect summarizeLocalEffectBefore(SILInstruction *from);
...
IterativeBackwardReachability<Effects>::summarizeLocalEffect(
    SILBasicBlock *block) {
  auto *end = &block->back();
  auto endEffect = effects.effectForInstruction(end);
  auto bodyEffect = summarizeLocalEffectBefore(end);
  auto effect = endEffect.compose(bodyEffect);
  return effect;
}

/// Summarize the effect of the block starting above the indicated instruction.
///
/// Form the composition of each instruction's effect and the phi's effect.
template <typename Effects>
typename IterativeBackwardReachability<Effects>::Effect
IterativeBackwardReachability<Effects>::summarizeLocalEffectBefore(
    SILInstruction *from) {
  ...
  for (auto *instruction = from->getPreviousInstruction(); instruction;
    ...
    runningEffect = runningEffect.compose(effect);
  }
...
void IterativeBackwardReachability<Effects>::addKill(
  ...
  // If the kill isn't even in a discovered block, we don't care.
  ...
  result.template setEffectForBlock</*SkipOverwrite=*/false>(block, effect);
}
...
/// unreachable-at-end-ness of its predecessors.
///
/// If that new unreachable-at-end-ness of any predecessor results in new
/// unreachable-at-begin-ness of _its_ predecessors, those newly
/// unreachable-at-begin blocks are added to the worklist.
///
...
void IterativeBackwardReachability<Effects>::findBarriers(Visitor &visitor) {
  ...
  for (auto *gen : effects.gens()) {
    ...
    assert((foundBarrier || (llvm::find(localGens, gen) == localGens.end())) &&
...
///
/// Return whether a barrier was bound.
///
/// interface Visitor {
///     /// Invoked with an instruction which is a barrier.
///     void visitBarrierInstruction(SILInstruction *)
...
//===----------------------------------------------------------------------===//
// MARK: IterativeBackwardReachability - Helpers
//===----------------------------------------------------------------------===//

/// Whether the specified block is the effective def block.  If no def block
/// is specified, the effective def block is the function's entry block.
template <typename Effects>
bool IterativeBackwardReachability<Effects>::isEffectiveDefBlock(
    SILBasicBlock *block) {
  if (!defBlock)
    return block == &*function->begin();
...
/// The begin state of the specified block, obtained by transferring the
/// previously recorded end state to the begin.
template <typename Effects>
typename IterativeBackwardReachability<Effects>::State
IterativeBackwardReachability<Effects>::Result::getBeginStateForBlock(
    SILBasicBlock *block) {
  return transferEndToBegin(block, getEndStateForBlock(block));
}
...
template <typename Effects>
typename IterativeBackwardReachability<Effects>::State
IterativeBackwardReachability<Effects>::Result::transferEndToBegin(
...
/// Blocks that are in neither are in ::Reachable.
template <typename Effects>
