...
private:
  ...
  /// if the module names appearing in source files are aliases or binary names.
  /// \see ASTContext::getBinaryModuleName.
  ///
  /// The boolean in the value indicates whether or not the entry is keyed by the syntax name vs binary name,
  /// i.e. keyed by syntax name: [key: syntax_name, value: (binary_name, true)],
  /// keyed by binary name: [key: binary_name, value: (syntax_name, false)]
...
public:
  ...
  enum class ModuleAliasLookupOption {
    binaryName,
    binaryNameWithSyntaxNameKey,
    syntaxNameWithBinaryNameKey
  };

  /// Look up the module alias map by the given \p key and a lookup \p option.
  ///
  /// \param key A module alias or binary name to look up the map by.
  /// \param option A look up option \c ModuleAliasLookupOption. Defaults to binaryName.
  ///
  /// \return The binary name or alias mapped to the key.
  ///         If no aliasing is used, return \p key regardless of \p option.
  ///         If \p option is binaryName, return the binary module name whether the \p key is a syntax name
  ///         or a binary name.
  ///         If \p option is binaryNameWithSyntaxNameKey, only return a binary name if \p key is a syntax name.
  ///         If \p option is syntaxNameWithBinaryNameKey, only return an alias if \p key is a binary name.
  ///         Else return a binary name or a syntax name mapped to the \p key.
  Identifier getBinaryModuleName(Identifier key,
                               ModuleAliasLookupOption option = ModuleAliasLookupOption::binaryName) const;
