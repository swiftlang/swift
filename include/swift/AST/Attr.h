...
/// Sema entry point that runs `@c @implementation(safe)` processing; declared
/// here so `ObjCImplementationAttr` can grant it sole write access to the
/// transient `isSafeProcessed` guard bit. Defined in the Sema layer.
...
protected:
  ...
  union {
    ...
    SWIFT_INLINE_BITFIELD(ObjCImplementationAttr, DeclAttribute, 5,
...
public:
  ...
  ObjCImplementationAttr(Identifier CategoryName, SourceLoc AtLoc,
    ...
    Bits.ObjCImplementationAttr.isSafe = isSafe;
    Bits.ObjCImplementationAttr.isSafeProcessed = false;
  }
  ...
  /// Is this the \c \@c \@implementation(safe) form, which marks the Swift
  /// function as a safe-typed implementation of a C function whose unsafe
  /// counterpart will be generated by an \c \@_Unswiftify peer macro?
  ///
  /// Only valid on \c AbstractFunctionDecl s that also carry a non-underscored
  /// \c \@c (\c CDeclAttr) attribute.
  bool isSafeInteropImplementation() const {
    return Bits.ObjCImplementationAttr.isSafe;
  }

  /// For an \c isSafeInteropImplementation() attribute, has the compiler
  /// already attempted the safe-mode processing (matching the imported C
  /// declaration, attaching the \c \@_Unswiftify peer macro, and disabling the
  /// corresponding \c \@_SwiftifyImport peer overload)?
  ///
  /// The processing is triggered both by attribute checking and by peer-macro
  /// expansion on the same decl, so it can run in either order; this bit
  /// guarantees it happens exactly once.
  bool isSafeProcessed() const {
    return Bits.ObjCImplementationAttr.isSafeProcessed;
  }

private:
  // The `isSafeProcessed` guard bit is transient sema state. Only
  // `processSafeImplementationIfNeeded` (which owns the run-once discipline)
  // and `clone()` may write it; it is deliberately not part of the general
  // public attribute API.
  friend void processSafeImplementationIfNeeded(Decl *decl);

  void setSafeProcessed(bool newValue = true) {
    Bits.ObjCImplementationAttr.isSafeProcessed = newValue;
  }
...
public:
  ...
  ObjCImplementationAttr *clone(ASTContext &ctx) const {
    auto *copy = new (ctx) ObjCImplementationAttr(
        CategoryName, AtLoc, Range, isEarlyAdopter(), isImplicit(),
        isCategoryNameInvalid(), isSafeInteropImplementation());
    copy->setSafeProcessed(isSafeProcessed());
    return copy;
  }
  ...
  bool isEquivalent(const ObjCImplementationAttr *other,
    ...
    return isEarlyAdopter() == other->isEarlyAdopter()
        && isSafeInteropImplementation() == other->isSafeInteropImplementation();
  }
};
