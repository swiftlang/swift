...
class alignas(Argument) ArgumentList final
  ...
  /// trailing closure. In type-checked AST, this may be an index to a variadic
  /// argument that contains the first trailing closure.
  unsigned FirstTrailingClosureIndex : 16;
  ...
  ArgumentList(SourceLoc lParenLoc, SourceLoc rParenLoc, unsigned numArgs,
    ...
    if (auto idx = firstTrailingClosureIndex) {
      assert(*idx < numArgs);
      FirstTrailingClosureIndex = *idx;
    } else {
      FirstTrailingClosureIndex = numArgs;
...
public:
  ...
  Optional<unsigned> getFirstTrailingClosureArgOnlyIndex() const {
    if (FirstTrailingClosureIndex == NumArgs)
      return None;
  ...
  bool hasAnyTrailingClosures() const {
    return getFirstTrailingClosureArgOnlyIndex().hasValue();
  }
  ...
  bool isTrailingClosureIndex(unsigned i) const {
    if (!hasAnyTrailingClosures())
      return false;
    return i >= getFirstTrailingClosureArgOnlyIndex() && i < size();
  }
  ...
  // backward argument matching cases.
  ...
  /// transformation. This has no effect on un-typechecked argument lists.
...
class OriginalArguments final {
  ...
  explicit OriginalArguments(const ArgumentList *fromArgList, Storage args);
