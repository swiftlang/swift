...
public:
  ...
  RemoteAddress getAddressData() const { return Address; }
...
private:
  ...
  StoredPointer LastIndexedClassesCount;
...
public:
  ...
  RemoteAddress stripSignedPointer(RemoteAddress P) {
    return Reader->stripSignedPointer(P);
  }

  RemoteAddress stripSignedPointer(StoredSignedPointer P) {
    auto address =
        RemoteAddress(P.SignedValue, RemoteAddress::DefaultAddressSpace);
    return Reader->stripSignedPointer(address);
  }
      ...
      case Demangle::SymbolicReferenceKind::Context: {
        ...
        if (useOpaqueTypeSymbolicReferences
          ...
          return dem.createNode(
              ...
              context.getResolved().getAddressData().getAddressData());
        }
  ...
  readMetadataAndValueOpaqueExistential(RemoteAddress ExistentialAddress) {
    ...
    if (!Reader->readRemoteAddress(ExistentialAddress, BoxAddress))
      return std::nullopt;
  ...
  bool isTaggedPointer(RemoteAddress objectAddress) {
    ...
    auto address =
        (objectAddress ^ TaggedPointerObfuscator) & TaggedPointerMask;
    return (bool)address;
  }
  ...
  readMetadataFromTaggedPointer(RemoteAddress objectAddress) {
    auto readArrayElement =
      ...
      RemoteAddress isa;
      if (!Reader->readRemoteAddress(addr, isa))
        return std::nullopt;
      return isa;
    };
    ...
    if (TaggedPointerExtendedMask &&
        (((objectAddress.getAddressData() ^ TaggedPointerObfuscator) &
  ...
  readMetadataFromInstance(RemoteAddress objectAddress) {
    ...
    RemoteAddress isa;
    if (!Reader->readRemoteAddress(objectAddress, isa))
      return std::nullopt;
    ...
    case IsaEncodingKind::Indexed: {
      ...
      if ((isa & IsaMagicMask).getAddressData() != IsaMagicValue)
        return isa;
      ...
      if (!Reader->readRemoteAddress(eltPointer, metadataPointer))
        return std::nullopt;
  ...
  std::optional<Result> forTypeReference(TypeReferenceKind refKind,
    ...
    case TypeReferenceKind::IndirectObjCClass: {
      ...
      if (!Reader->readRemoteAddress(ref, classRef))
        return std::nullopt;
  ...
  readGenericArgFromMetadata(RemoteAddress metadata, unsigned index) {
    ...
    if (!Reader->readRemoteAddress(addressOfGenericArgAddress,
                                   genericArgAddress))
      return std::nullopt;
  ...
  std::optional<RemoteAddress> readResolvedPointerValue(RemoteAddress address) {
    if (auto pointer = readPointer(address)) {
      if (!pointer->getResolvedAddress())
        return std::nullopt;
      return (RemoteAddress)pointer->getResolvedAddress();
    }
    return std::nullopt;
  }
...
protected:
  ...
  bool readObjCClassName(RemoteAddress classAddress, std::string &className) {
    ...
    if (!Reader->readRemoteAddress(roDataPtr + OffsetToName, namePtr))
      return false;
  ...
  MetadataRef readMetadata(RemoteAddress address) {
    ...
    switch (getEnumeratedMetadataKind(KindValue)) {
      ...
      case MetadataKind::ExtendedExistential: {
        ...
        if (!Reader->readRemoteAddress(shapeAddress, signedShapePtr))
          return nullptr;
...
private:
  ...
  RemoteAddress resolveRelativeIndirectProtocol(
    ...
    // address in memory.
    if (indirect) {
      if (!Reader->readRemoteAddress(resultAddress, resultAddress))
        return RemoteAddress();
    }
  ...
  buildContextDescriptorMangling(ContextDescriptorRef descriptor,
    ...
    case ContextDescriptorKind::Anonymous: {
      ...
      snprintf(addressBuf, sizeof(addressBuf), "$%" PRIx64,
               (uint64_t)descriptor.getAddressData().getAddressData());
    ...
    for (auto param : generics->getGenericParams()) {
      ...
      case GenericParamKind::Type:
        ...
        if (param.hasKeyArgument()) {
          ...
          RemoteAddress arg;
          if (!Reader->readRemoteAddress(genericArgsAddr, arg)) {
            return {};
          }
