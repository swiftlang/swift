...
void SILCloner<ImplClass>::commonFixUp(SILFunction *F) {
  ...
  // A whole-function clone is in the same lowered-address form and stage as
  // its source. Copy both so the clone's conventions and verification see the
  // right form.
  ...
  if (asImpl().isWholeFunctionClone() &&
      !getBuilder().isInsertingIntoGlobal()) {
    SILFunction &clone = getBuilder().getFunction();
    clone.setHasLoweredAddresses(F->hasLoweredAddresses());
    // The clone was born at the floor, which can be ahead of a source that
    // predates a commit, such as a snapshot. Only move the clone forward.
    if (F->getFunctionStage() > clone.getFunctionStage())
      clone.setFunctionStage(F->getFunctionStage());
  } else {
