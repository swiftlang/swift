...
protected:
  ...
  llvm::DenseMap<SILInstruction *, SmallVector<PartialApplyInst *>>
      lifetimeBoundaryToPartialApplyMap;
...
public:
  ...
  void recordClonedInstruction(SILInstruction *Orig, SILInstruction *Cloned) {
    tryInsertDeallocForNoEscapePartialApply(Orig, Cloned);
  ...
  void recordDeletedInstruction(SILInstruction *Orig) {
    assert(getBuilder().hasValidInsertionPoint());
    tryInsertDeallocForNoEscapePartialApply(Orig, nullptr);
  }
  ...
  void recordFoldedValue(SILValue origValue, SILValue mappedValue) {
    tryInsertDeallocForNoEscapePartialApply(
        origValue->getDefiningInstruction(),
        isa<SILArgument>(mappedValue) ? nullptr
                                      : mappedValue->getDefiningInstruction());
    asImpl().mapValue(origValue, mappedValue);
  }

  void tryInsertDeallocForNoEscapePartialApply(SILInstruction *inst,
                                               SILInstruction *clonedInst) {
    if (!inst->getFunction() || getBuilder().hasOwnership() ||
        !inst->getFunction()->hasOwnership()) {
      return;
    }
    auto it = lifetimeBoundaryToPartialApplyMap.find(inst);
    if (it != lifetimeBoundaryToPartialApplyMap.end()) {
      for (auto *pai : it->second) {
        if (clonedInst) {
          SILBuilderWithScope(clonedInst)
              .createDeallocStack(RegularLocation::getAutoGeneratedLocation(),
                                  getOpValue(pai));
        } else {
          getBuilder().createDeallocStack(
              RegularLocation::getAutoGeneratedLocation(), getOpValue(pai));
        }
      }
    }
  }
...
#include "swift/SIL/SILNodes.def"
  ...
  /// SIL before cloning.
  ///
  /// Common fix-ups are handled first in `postCloningFixUp` and may not be
  /// overridden.
...
private:
  ...
  /// Perform analysis/fixup before cloning.
  void preCloningFixUp(SILFunction *F);
  /// Also perform fundamental cleanup first, then call the CRTP extension,
  /// `postFixUp`.
  void postCloningFixUp(SILFunction *F);
};
...
template <typename ImplClass>
void SILCloner<ImplClass>::preCloningFixUp(SILFunction *F) {
  SmallVector<SILInstruction *> lifetimeEnds;
  SmallVector<SILBasicBlock *> discoveredBlocks;

  if (!F->hasOwnership() || getBuilder().hasOwnership()) {
    return;
  }
  ...
  for (auto &block : *F) {
    for (auto &inst : block) {
      ...
      // Collect lifetime ends of the partial_apply [on_stack]
      visitLifetimeEndsOfOnStackPartialApplyAndCopies(pai, [&](Operand *op) {
        lifetimeEnds.push_back(op->getUser());
        return true;
      });
      SSAPrunedLiveness liveness(F, &discoveredBlocks);
      ...
      boundary.visitInsertionPoints([&](SILBasicBlock::iterator insertPt) {
        lifetimeBoundaryToPartialApplyMap[&*insertPt].push_back(pai);
      });
    }
  }
}
...
template <typename ImplClass>
void SILCloner<ImplClass>::postCloningFixUp(SILFunction *F) {
  ...
  if (!getBuilder().hasOwnership()) {
    ...
    if (auto fnTy = Inst->getType().getAs<SILFunctionType>()) {
      if (fnTy->isTrivialNoEscape()) {
        recordFoldedValue(Inst, getOpValue(Inst->getOperand()));
        return;
      }
    }
