...
protected:
  ...
  enum { BaseBits = 4, BaseMask = 0xF, AutoGeneratedBit = 5, PositionBit = 6 };
...
public:
  ...
  void setAutoGenerated() { KindData |= (1 << AutoGeneratedBit); }
  ...
  /// \brief Check if the corresponding source code location points to the
  /// start of the AST node.
  bool alwaysPointsToStart() const { return false; }
...
public:
  RegularLocation(bool AutoGen = false) : SILLocation() {
    if (AutoGen)
      setAutoGenerated();
  }
...
public:
  ...
  static SILLocation getImplicitReturnLoc(SILLocation L) {
    ...
    // Autogenerated returns might wrap an arbitrary expression.
    if (L.isAutoGenerated())
      if (Expr *E = L.getAsASTNode<Expr>())
        return getAutoGeneratedImplicitReturnLocation(E);
    if (L.isNull() || L.isAutoGenerated())
      return ImplicitReturnLocation();
...
private:
  ...
  ImplicitReturnLocation(Expr *E)
    : SILLocation(E, ImplicitReturnKind) {}

  /// \brief Construct auto generated implicit return location.
  ///
  /// Auto-generated returns might wrap arbitrary expressions.
  static ImplicitReturnLocation getAutoGeneratedImplicitReturnLocation(Expr *E){
    ImplicitReturnLocation L = ImplicitReturnLocation(E);
    L.setAutoGenerated();
    return L;
  }
};
...
public:
  ...
  /// the AST node.
  void pointToStart() { KindData |= (1 << PositionBit); }
...
private:
  ...
  bool alwaysPointsToStart() const { return KindData & (1 << PositionBit); }
};
